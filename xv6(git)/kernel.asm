
kernel:     file format elf32-i386


Disassembly of section .text:

80100000 <multiboot_header>:
80100000:	02 b0 ad 1b 00 00    	add    0x1bad(%eax),%dh
80100006:	00 00                	add    %al,(%eax)
80100008:	fe 4f 52             	decb   0x52(%edi)
8010000b:	e4 0f                	in     $0xf,%al

8010000c <entry>:
8010000c:	0f 20 e0             	mov    %cr4,%eax
8010000f:	83 c8 10             	or     $0x10,%eax
80100012:	0f 22 e0             	mov    %eax,%cr4
80100015:	b8 00 a0 10 00       	mov    $0x10a000,%eax
8010001a:	0f 22 d8             	mov    %eax,%cr3
8010001d:	0f 20 c0             	mov    %cr0,%eax
80100020:	0d 00 00 01 80       	or     $0x80010000,%eax
80100025:	0f 22 c0             	mov    %eax,%cr0
80100028:	bc 50 c6 10 80       	mov    $0x8010c650,%esp
8010002d:	b8 ff 33 10 80       	mov    $0x801033ff,%eax
80100032:	ff e0                	jmp    *%eax

80100034 <binit>:
  struct buf head;
} bcache;

void
binit(void)
{
80100034:	55                   	push   %ebp
80100035:	89 e5                	mov    %esp,%ebp
80100037:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  initlock(&bcache.lock, "bcache");
8010003a:	c7 44 24 04 ac 86 10 	movl   $0x801086ac,0x4(%esp)
80100041:	80 
80100042:	c7 04 24 60 c6 10 80 	movl   $0x8010c660,(%esp)
80100049:	e8 e4 4f 00 00       	call   80105032 <initlock>

//PAGEBREAK!
  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
8010004e:	c7 05 90 db 10 80 84 	movl   $0x8010db84,0x8010db90
80100055:	db 10 80 
  bcache.head.next = &bcache.head;
80100058:	c7 05 94 db 10 80 84 	movl   $0x8010db84,0x8010db94
8010005f:	db 10 80 
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
80100062:	c7 45 f4 94 c6 10 80 	movl   $0x8010c694,-0xc(%ebp)
80100069:	eb 3a                	jmp    801000a5 <binit+0x71>
    b->next = bcache.head.next;
8010006b:	8b 15 94 db 10 80    	mov    0x8010db94,%edx
80100071:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100074:	89 50 10             	mov    %edx,0x10(%eax)
    b->prev = &bcache.head;
80100077:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010007a:	c7 40 0c 84 db 10 80 	movl   $0x8010db84,0xc(%eax)
    b->dev = -1;
80100081:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100084:	c7 40 04 ff ff ff ff 	movl   $0xffffffff,0x4(%eax)
    bcache.head.next->prev = b;
8010008b:	a1 94 db 10 80       	mov    0x8010db94,%eax
80100090:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100093:	89 50 0c             	mov    %edx,0xc(%eax)
    bcache.head.next = b;
80100096:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100099:	a3 94 db 10 80       	mov    %eax,0x8010db94

//PAGEBREAK!
  // Create linked list of buffers
  bcache.head.prev = &bcache.head;
  bcache.head.next = &bcache.head;
  for(b = bcache.buf; b < bcache.buf+NBUF; b++){
8010009e:	81 45 f4 18 02 00 00 	addl   $0x218,-0xc(%ebp)
801000a5:	81 7d f4 84 db 10 80 	cmpl   $0x8010db84,-0xc(%ebp)
801000ac:	72 bd                	jb     8010006b <binit+0x37>
    b->prev = &bcache.head;
    b->dev = -1;
    bcache.head.next->prev = b;
    bcache.head.next = b;
  }
}
801000ae:	c9                   	leave  
801000af:	c3                   	ret    

801000b0 <bget>:
// Look through buffer cache for sector on device dev.
// If not found, allocate fresh block.
// In either case, return B_BUSY buffer.
static struct buf*
bget(uint dev, uint sector)
{
801000b0:	55                   	push   %ebp
801000b1:	89 e5                	mov    %esp,%ebp
801000b3:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  acquire(&bcache.lock);
801000b6:	c7 04 24 60 c6 10 80 	movl   $0x8010c660,(%esp)
801000bd:	e8 91 4f 00 00       	call   80105053 <acquire>

 loop:
  // Is the sector already cached?
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
801000c2:	a1 94 db 10 80       	mov    0x8010db94,%eax
801000c7:	89 45 f4             	mov    %eax,-0xc(%ebp)
801000ca:	eb 63                	jmp    8010012f <bget+0x7f>
    if(b->dev == dev && b->sector == sector){
801000cc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000cf:	8b 40 04             	mov    0x4(%eax),%eax
801000d2:	3b 45 08             	cmp    0x8(%ebp),%eax
801000d5:	75 4f                	jne    80100126 <bget+0x76>
801000d7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000da:	8b 40 08             	mov    0x8(%eax),%eax
801000dd:	3b 45 0c             	cmp    0xc(%ebp),%eax
801000e0:	75 44                	jne    80100126 <bget+0x76>
      if(!(b->flags & B_BUSY)){
801000e2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000e5:	8b 00                	mov    (%eax),%eax
801000e7:	83 e0 01             	and    $0x1,%eax
801000ea:	85 c0                	test   %eax,%eax
801000ec:	75 23                	jne    80100111 <bget+0x61>
        b->flags |= B_BUSY;
801000ee:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000f1:	8b 00                	mov    (%eax),%eax
801000f3:	89 c2                	mov    %eax,%edx
801000f5:	83 ca 01             	or     $0x1,%edx
801000f8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801000fb:	89 10                	mov    %edx,(%eax)
        release(&bcache.lock);
801000fd:	c7 04 24 60 c6 10 80 	movl   $0x8010c660,(%esp)
80100104:	e8 ac 4f 00 00       	call   801050b5 <release>
        return b;
80100109:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010010c:	e9 93 00 00 00       	jmp    801001a4 <bget+0xf4>
      }
      sleep(b, &bcache.lock);
80100111:	c7 44 24 04 60 c6 10 	movl   $0x8010c660,0x4(%esp)
80100118:	80 
80100119:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010011c:	89 04 24             	mov    %eax,(%esp)
8010011f:	e8 ee 46 00 00       	call   80104812 <sleep>
      goto loop;
80100124:	eb 9c                	jmp    801000c2 <bget+0x12>

  acquire(&bcache.lock);

 loop:
  // Is the sector already cached?
  for(b = bcache.head.next; b != &bcache.head; b = b->next){
80100126:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100129:	8b 40 10             	mov    0x10(%eax),%eax
8010012c:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010012f:	81 7d f4 84 db 10 80 	cmpl   $0x8010db84,-0xc(%ebp)
80100136:	75 94                	jne    801000cc <bget+0x1c>
      goto loop;
    }
  }

  // Not cached; recycle some non-busy and clean buffer.
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
80100138:	a1 90 db 10 80       	mov    0x8010db90,%eax
8010013d:	89 45 f4             	mov    %eax,-0xc(%ebp)
80100140:	eb 4d                	jmp    8010018f <bget+0xdf>
    if((b->flags & B_BUSY) == 0 && (b->flags & B_DIRTY) == 0){
80100142:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100145:	8b 00                	mov    (%eax),%eax
80100147:	83 e0 01             	and    $0x1,%eax
8010014a:	85 c0                	test   %eax,%eax
8010014c:	75 38                	jne    80100186 <bget+0xd6>
8010014e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100151:	8b 00                	mov    (%eax),%eax
80100153:	83 e0 04             	and    $0x4,%eax
80100156:	85 c0                	test   %eax,%eax
80100158:	75 2c                	jne    80100186 <bget+0xd6>
      b->dev = dev;
8010015a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010015d:	8b 55 08             	mov    0x8(%ebp),%edx
80100160:	89 50 04             	mov    %edx,0x4(%eax)
      b->sector = sector;
80100163:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100166:	8b 55 0c             	mov    0xc(%ebp),%edx
80100169:	89 50 08             	mov    %edx,0x8(%eax)
      b->flags = B_BUSY;
8010016c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010016f:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
      release(&bcache.lock);
80100175:	c7 04 24 60 c6 10 80 	movl   $0x8010c660,(%esp)
8010017c:	e8 34 4f 00 00       	call   801050b5 <release>
      return b;
80100181:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100184:	eb 1e                	jmp    801001a4 <bget+0xf4>
      goto loop;
    }
  }

  // Not cached; recycle some non-busy and clean buffer.
  for(b = bcache.head.prev; b != &bcache.head; b = b->prev){
80100186:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100189:	8b 40 0c             	mov    0xc(%eax),%eax
8010018c:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010018f:	81 7d f4 84 db 10 80 	cmpl   $0x8010db84,-0xc(%ebp)
80100196:	75 aa                	jne    80100142 <bget+0x92>
      b->flags = B_BUSY;
      release(&bcache.lock);
      return b;
    }
  }
  panic("bget: no buffers");
80100198:	c7 04 24 b3 86 10 80 	movl   $0x801086b3,(%esp)
8010019f:	e8 99 03 00 00       	call   8010053d <panic>
}
801001a4:	c9                   	leave  
801001a5:	c3                   	ret    

801001a6 <bread>:

// Return a B_BUSY buf with the contents of the indicated disk sector.
struct buf*
bread(uint dev, uint sector)
{
801001a6:	55                   	push   %ebp
801001a7:	89 e5                	mov    %esp,%ebp
801001a9:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  b = bget(dev, sector);
801001ac:	8b 45 0c             	mov    0xc(%ebp),%eax
801001af:	89 44 24 04          	mov    %eax,0x4(%esp)
801001b3:	8b 45 08             	mov    0x8(%ebp),%eax
801001b6:	89 04 24             	mov    %eax,(%esp)
801001b9:	e8 f2 fe ff ff       	call   801000b0 <bget>
801001be:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(!(b->flags & B_VALID))
801001c1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801001c4:	8b 00                	mov    (%eax),%eax
801001c6:	83 e0 02             	and    $0x2,%eax
801001c9:	85 c0                	test   %eax,%eax
801001cb:	75 0b                	jne    801001d8 <bread+0x32>
    iderw(b);
801001cd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801001d0:	89 04 24             	mov    %eax,(%esp)
801001d3:	e8 d4 25 00 00       	call   801027ac <iderw>
  return b;
801001d8:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
801001db:	c9                   	leave  
801001dc:	c3                   	ret    

801001dd <bwrite>:

// Write b's contents to disk.  Must be B_BUSY.
void
bwrite(struct buf *b)
{
801001dd:	55                   	push   %ebp
801001de:	89 e5                	mov    %esp,%ebp
801001e0:	83 ec 18             	sub    $0x18,%esp
  if((b->flags & B_BUSY) == 0)
801001e3:	8b 45 08             	mov    0x8(%ebp),%eax
801001e6:	8b 00                	mov    (%eax),%eax
801001e8:	83 e0 01             	and    $0x1,%eax
801001eb:	85 c0                	test   %eax,%eax
801001ed:	75 0c                	jne    801001fb <bwrite+0x1e>
    panic("bwrite");
801001ef:	c7 04 24 c4 86 10 80 	movl   $0x801086c4,(%esp)
801001f6:	e8 42 03 00 00       	call   8010053d <panic>
  b->flags |= B_DIRTY;
801001fb:	8b 45 08             	mov    0x8(%ebp),%eax
801001fe:	8b 00                	mov    (%eax),%eax
80100200:	89 c2                	mov    %eax,%edx
80100202:	83 ca 04             	or     $0x4,%edx
80100205:	8b 45 08             	mov    0x8(%ebp),%eax
80100208:	89 10                	mov    %edx,(%eax)
  iderw(b);
8010020a:	8b 45 08             	mov    0x8(%ebp),%eax
8010020d:	89 04 24             	mov    %eax,(%esp)
80100210:	e8 97 25 00 00       	call   801027ac <iderw>
}
80100215:	c9                   	leave  
80100216:	c3                   	ret    

80100217 <brelse>:

// Release a B_BUSY buffer.
// Move to the head of the MRU list.
void
brelse(struct buf *b)
{
80100217:	55                   	push   %ebp
80100218:	89 e5                	mov    %esp,%ebp
8010021a:	83 ec 18             	sub    $0x18,%esp
  if((b->flags & B_BUSY) == 0)
8010021d:	8b 45 08             	mov    0x8(%ebp),%eax
80100220:	8b 00                	mov    (%eax),%eax
80100222:	83 e0 01             	and    $0x1,%eax
80100225:	85 c0                	test   %eax,%eax
80100227:	75 0c                	jne    80100235 <brelse+0x1e>
    panic("brelse");
80100229:	c7 04 24 cb 86 10 80 	movl   $0x801086cb,(%esp)
80100230:	e8 08 03 00 00       	call   8010053d <panic>

  acquire(&bcache.lock);
80100235:	c7 04 24 60 c6 10 80 	movl   $0x8010c660,(%esp)
8010023c:	e8 12 4e 00 00       	call   80105053 <acquire>

  b->next->prev = b->prev;
80100241:	8b 45 08             	mov    0x8(%ebp),%eax
80100244:	8b 40 10             	mov    0x10(%eax),%eax
80100247:	8b 55 08             	mov    0x8(%ebp),%edx
8010024a:	8b 52 0c             	mov    0xc(%edx),%edx
8010024d:	89 50 0c             	mov    %edx,0xc(%eax)
  b->prev->next = b->next;
80100250:	8b 45 08             	mov    0x8(%ebp),%eax
80100253:	8b 40 0c             	mov    0xc(%eax),%eax
80100256:	8b 55 08             	mov    0x8(%ebp),%edx
80100259:	8b 52 10             	mov    0x10(%edx),%edx
8010025c:	89 50 10             	mov    %edx,0x10(%eax)
  b->next = bcache.head.next;
8010025f:	8b 15 94 db 10 80    	mov    0x8010db94,%edx
80100265:	8b 45 08             	mov    0x8(%ebp),%eax
80100268:	89 50 10             	mov    %edx,0x10(%eax)
  b->prev = &bcache.head;
8010026b:	8b 45 08             	mov    0x8(%ebp),%eax
8010026e:	c7 40 0c 84 db 10 80 	movl   $0x8010db84,0xc(%eax)
  bcache.head.next->prev = b;
80100275:	a1 94 db 10 80       	mov    0x8010db94,%eax
8010027a:	8b 55 08             	mov    0x8(%ebp),%edx
8010027d:	89 50 0c             	mov    %edx,0xc(%eax)
  bcache.head.next = b;
80100280:	8b 45 08             	mov    0x8(%ebp),%eax
80100283:	a3 94 db 10 80       	mov    %eax,0x8010db94

  b->flags &= ~B_BUSY;
80100288:	8b 45 08             	mov    0x8(%ebp),%eax
8010028b:	8b 00                	mov    (%eax),%eax
8010028d:	89 c2                	mov    %eax,%edx
8010028f:	83 e2 fe             	and    $0xfffffffe,%edx
80100292:	8b 45 08             	mov    0x8(%ebp),%eax
80100295:	89 10                	mov    %edx,(%eax)
  wakeup(b);
80100297:	8b 45 08             	mov    0x8(%ebp),%eax
8010029a:	89 04 24             	mov    %eax,(%esp)
8010029d:	e8 4a 46 00 00       	call   801048ec <wakeup>

  release(&bcache.lock);
801002a2:	c7 04 24 60 c6 10 80 	movl   $0x8010c660,(%esp)
801002a9:	e8 07 4e 00 00       	call   801050b5 <release>
}
801002ae:	c9                   	leave  
801002af:	c3                   	ret    

801002b0 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
801002b0:	55                   	push   %ebp
801002b1:	89 e5                	mov    %esp,%ebp
801002b3:	53                   	push   %ebx
801002b4:	83 ec 14             	sub    $0x14,%esp
801002b7:	8b 45 08             	mov    0x8(%ebp),%eax
801002ba:	66 89 45 e8          	mov    %ax,-0x18(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
801002be:	0f b7 55 e8          	movzwl -0x18(%ebp),%edx
801002c2:	66 89 55 ea          	mov    %dx,-0x16(%ebp)
801002c6:	0f b7 55 ea          	movzwl -0x16(%ebp),%edx
801002ca:	ec                   	in     (%dx),%al
801002cb:	89 c3                	mov    %eax,%ebx
801002cd:	88 5d fb             	mov    %bl,-0x5(%ebp)
  return data;
801002d0:	0f b6 45 fb          	movzbl -0x5(%ebp),%eax
}
801002d4:	83 c4 14             	add    $0x14,%esp
801002d7:	5b                   	pop    %ebx
801002d8:	5d                   	pop    %ebp
801002d9:	c3                   	ret    

801002da <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
801002da:	55                   	push   %ebp
801002db:	89 e5                	mov    %esp,%ebp
801002dd:	83 ec 08             	sub    $0x8,%esp
801002e0:	8b 55 08             	mov    0x8(%ebp),%edx
801002e3:	8b 45 0c             	mov    0xc(%ebp),%eax
801002e6:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
801002ea:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801002ed:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
801002f1:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
801002f5:	ee                   	out    %al,(%dx)
}
801002f6:	c9                   	leave  
801002f7:	c3                   	ret    

801002f8 <cli>:
  asm volatile("movw %0, %%gs" : : "r" (v));
}

static inline void
cli(void)
{
801002f8:	55                   	push   %ebp
801002f9:	89 e5                	mov    %esp,%ebp
  asm volatile("cli");
801002fb:	fa                   	cli    
}
801002fc:	5d                   	pop    %ebp
801002fd:	c3                   	ret    

801002fe <printint>:
  int locking;
} cons;

static void
printint(int xx, int base, int sign)
{
801002fe:	55                   	push   %ebp
801002ff:	89 e5                	mov    %esp,%ebp
80100301:	83 ec 48             	sub    $0x48,%esp
  static char digits[] = "0123456789abcdef";
  char buf[16];
  int i;
  uint x;

  if(sign && (sign = xx < 0))
80100304:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80100308:	74 19                	je     80100323 <printint+0x25>
8010030a:	8b 45 08             	mov    0x8(%ebp),%eax
8010030d:	c1 e8 1f             	shr    $0x1f,%eax
80100310:	89 45 10             	mov    %eax,0x10(%ebp)
80100313:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80100317:	74 0a                	je     80100323 <printint+0x25>
    x = -xx;
80100319:	8b 45 08             	mov    0x8(%ebp),%eax
8010031c:	f7 d8                	neg    %eax
8010031e:	89 45 f0             	mov    %eax,-0x10(%ebp)
80100321:	eb 06                	jmp    80100329 <printint+0x2b>
  else
    x = xx;
80100323:	8b 45 08             	mov    0x8(%ebp),%eax
80100326:	89 45 f0             	mov    %eax,-0x10(%ebp)

  i = 0;
80100329:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
  do{
    buf[i++] = digits[x % base];
80100330:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80100333:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100336:	ba 00 00 00 00       	mov    $0x0,%edx
8010033b:	f7 f1                	div    %ecx
8010033d:	89 d0                	mov    %edx,%eax
8010033f:	0f b6 90 04 90 10 80 	movzbl -0x7fef6ffc(%eax),%edx
80100346:	8d 45 e0             	lea    -0x20(%ebp),%eax
80100349:	03 45 f4             	add    -0xc(%ebp),%eax
8010034c:	88 10                	mov    %dl,(%eax)
8010034e:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
  }while((x /= base) != 0);
80100352:	8b 55 0c             	mov    0xc(%ebp),%edx
80100355:	89 55 d4             	mov    %edx,-0x2c(%ebp)
80100358:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010035b:	ba 00 00 00 00       	mov    $0x0,%edx
80100360:	f7 75 d4             	divl   -0x2c(%ebp)
80100363:	89 45 f0             	mov    %eax,-0x10(%ebp)
80100366:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
8010036a:	75 c4                	jne    80100330 <printint+0x32>

  if(sign)
8010036c:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80100370:	74 23                	je     80100395 <printint+0x97>
    buf[i++] = '-';
80100372:	8d 45 e0             	lea    -0x20(%ebp),%eax
80100375:	03 45 f4             	add    -0xc(%ebp),%eax
80100378:	c6 00 2d             	movb   $0x2d,(%eax)
8010037b:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)

  while(--i >= 0)
8010037f:	eb 14                	jmp    80100395 <printint+0x97>
    consputc(buf[i]);
80100381:	8d 45 e0             	lea    -0x20(%ebp),%eax
80100384:	03 45 f4             	add    -0xc(%ebp),%eax
80100387:	0f b6 00             	movzbl (%eax),%eax
8010038a:	0f be c0             	movsbl %al,%eax
8010038d:	89 04 24             	mov    %eax,(%esp)
80100390:	e8 bb 03 00 00       	call   80100750 <consputc>
  }while((x /= base) != 0);

  if(sign)
    buf[i++] = '-';

  while(--i >= 0)
80100395:	83 6d f4 01          	subl   $0x1,-0xc(%ebp)
80100399:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010039d:	79 e2                	jns    80100381 <printint+0x83>
    consputc(buf[i]);
}
8010039f:	c9                   	leave  
801003a0:	c3                   	ret    

801003a1 <cprintf>:
//PAGEBREAK: 50

// Print to the console. only understands %d, %x, %p, %s.
void
cprintf(char *fmt, ...)
{
801003a1:	55                   	push   %ebp
801003a2:	89 e5                	mov    %esp,%ebp
801003a4:	83 ec 38             	sub    $0x38,%esp
  int i, c, locking;
  uint *argp;
  char *s;

  locking = cons.locking;
801003a7:	a1 f4 b5 10 80       	mov    0x8010b5f4,%eax
801003ac:	89 45 e8             	mov    %eax,-0x18(%ebp)
  if(locking)
801003af:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
801003b3:	74 0c                	je     801003c1 <cprintf+0x20>
    acquire(&cons.lock);
801003b5:	c7 04 24 c0 b5 10 80 	movl   $0x8010b5c0,(%esp)
801003bc:	e8 92 4c 00 00       	call   80105053 <acquire>

  if (fmt == 0)
801003c1:	8b 45 08             	mov    0x8(%ebp),%eax
801003c4:	85 c0                	test   %eax,%eax
801003c6:	75 0c                	jne    801003d4 <cprintf+0x33>
    panic("null fmt");
801003c8:	c7 04 24 d2 86 10 80 	movl   $0x801086d2,(%esp)
801003cf:	e8 69 01 00 00       	call   8010053d <panic>

  argp = (uint*)(void*)(&fmt + 1);
801003d4:	8d 45 0c             	lea    0xc(%ebp),%eax
801003d7:	89 45 f0             	mov    %eax,-0x10(%ebp)
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
801003da:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801003e1:	e9 20 01 00 00       	jmp    80100506 <cprintf+0x165>
    if(c != '%'){
801003e6:	83 7d e4 25          	cmpl   $0x25,-0x1c(%ebp)
801003ea:	74 10                	je     801003fc <cprintf+0x5b>
      consputc(c);
801003ec:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801003ef:	89 04 24             	mov    %eax,(%esp)
801003f2:	e8 59 03 00 00       	call   80100750 <consputc>
      continue;
801003f7:	e9 06 01 00 00       	jmp    80100502 <cprintf+0x161>
    }
    c = fmt[++i] & 0xff;
801003fc:	8b 55 08             	mov    0x8(%ebp),%edx
801003ff:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100403:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100406:	01 d0                	add    %edx,%eax
80100408:	0f b6 00             	movzbl (%eax),%eax
8010040b:	0f be c0             	movsbl %al,%eax
8010040e:	25 ff 00 00 00       	and    $0xff,%eax
80100413:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    if(c == 0)
80100416:	83 7d e4 00          	cmpl   $0x0,-0x1c(%ebp)
8010041a:	0f 84 08 01 00 00    	je     80100528 <cprintf+0x187>
      break;
    switch(c){
80100420:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100423:	83 f8 70             	cmp    $0x70,%eax
80100426:	74 4d                	je     80100475 <cprintf+0xd4>
80100428:	83 f8 70             	cmp    $0x70,%eax
8010042b:	7f 13                	jg     80100440 <cprintf+0x9f>
8010042d:	83 f8 25             	cmp    $0x25,%eax
80100430:	0f 84 a6 00 00 00    	je     801004dc <cprintf+0x13b>
80100436:	83 f8 64             	cmp    $0x64,%eax
80100439:	74 14                	je     8010044f <cprintf+0xae>
8010043b:	e9 aa 00 00 00       	jmp    801004ea <cprintf+0x149>
80100440:	83 f8 73             	cmp    $0x73,%eax
80100443:	74 53                	je     80100498 <cprintf+0xf7>
80100445:	83 f8 78             	cmp    $0x78,%eax
80100448:	74 2b                	je     80100475 <cprintf+0xd4>
8010044a:	e9 9b 00 00 00       	jmp    801004ea <cprintf+0x149>
    case 'd':
      printint(*argp++, 10, 1);
8010044f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100452:	8b 00                	mov    (%eax),%eax
80100454:	83 45 f0 04          	addl   $0x4,-0x10(%ebp)
80100458:	c7 44 24 08 01 00 00 	movl   $0x1,0x8(%esp)
8010045f:	00 
80100460:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80100467:	00 
80100468:	89 04 24             	mov    %eax,(%esp)
8010046b:	e8 8e fe ff ff       	call   801002fe <printint>
      break;
80100470:	e9 8d 00 00 00       	jmp    80100502 <cprintf+0x161>
    case 'x':
    case 'p':
      printint(*argp++, 16, 0);
80100475:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100478:	8b 00                	mov    (%eax),%eax
8010047a:	83 45 f0 04          	addl   $0x4,-0x10(%ebp)
8010047e:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80100485:	00 
80100486:	c7 44 24 04 10 00 00 	movl   $0x10,0x4(%esp)
8010048d:	00 
8010048e:	89 04 24             	mov    %eax,(%esp)
80100491:	e8 68 fe ff ff       	call   801002fe <printint>
      break;
80100496:	eb 6a                	jmp    80100502 <cprintf+0x161>
    case 's':
      if((s = (char*)*argp++) == 0)
80100498:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010049b:	8b 00                	mov    (%eax),%eax
8010049d:	89 45 ec             	mov    %eax,-0x14(%ebp)
801004a0:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
801004a4:	0f 94 c0             	sete   %al
801004a7:	83 45 f0 04          	addl   $0x4,-0x10(%ebp)
801004ab:	84 c0                	test   %al,%al
801004ad:	74 20                	je     801004cf <cprintf+0x12e>
        s = "(null)";
801004af:	c7 45 ec db 86 10 80 	movl   $0x801086db,-0x14(%ebp)
      for(; *s; s++)
801004b6:	eb 17                	jmp    801004cf <cprintf+0x12e>
        consputc(*s);
801004b8:	8b 45 ec             	mov    -0x14(%ebp),%eax
801004bb:	0f b6 00             	movzbl (%eax),%eax
801004be:	0f be c0             	movsbl %al,%eax
801004c1:	89 04 24             	mov    %eax,(%esp)
801004c4:	e8 87 02 00 00       	call   80100750 <consputc>
      printint(*argp++, 16, 0);
      break;
    case 's':
      if((s = (char*)*argp++) == 0)
        s = "(null)";
      for(; *s; s++)
801004c9:	83 45 ec 01          	addl   $0x1,-0x14(%ebp)
801004cd:	eb 01                	jmp    801004d0 <cprintf+0x12f>
801004cf:	90                   	nop
801004d0:	8b 45 ec             	mov    -0x14(%ebp),%eax
801004d3:	0f b6 00             	movzbl (%eax),%eax
801004d6:	84 c0                	test   %al,%al
801004d8:	75 de                	jne    801004b8 <cprintf+0x117>
        consputc(*s);
      break;
801004da:	eb 26                	jmp    80100502 <cprintf+0x161>
    case '%':
      consputc('%');
801004dc:	c7 04 24 25 00 00 00 	movl   $0x25,(%esp)
801004e3:	e8 68 02 00 00       	call   80100750 <consputc>
      break;
801004e8:	eb 18                	jmp    80100502 <cprintf+0x161>
    default:
      // Print unknown % sequence to draw attention.
      consputc('%');
801004ea:	c7 04 24 25 00 00 00 	movl   $0x25,(%esp)
801004f1:	e8 5a 02 00 00       	call   80100750 <consputc>
      consputc(c);
801004f6:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801004f9:	89 04 24             	mov    %eax,(%esp)
801004fc:	e8 4f 02 00 00       	call   80100750 <consputc>
      break;
80100501:	90                   	nop

  if (fmt == 0)
    panic("null fmt");

  argp = (uint*)(void*)(&fmt + 1);
  for(i = 0; (c = fmt[i] & 0xff) != 0; i++){
80100502:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100506:	8b 55 08             	mov    0x8(%ebp),%edx
80100509:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010050c:	01 d0                	add    %edx,%eax
8010050e:	0f b6 00             	movzbl (%eax),%eax
80100511:	0f be c0             	movsbl %al,%eax
80100514:	25 ff 00 00 00       	and    $0xff,%eax
80100519:	89 45 e4             	mov    %eax,-0x1c(%ebp)
8010051c:	83 7d e4 00          	cmpl   $0x0,-0x1c(%ebp)
80100520:	0f 85 c0 fe ff ff    	jne    801003e6 <cprintf+0x45>
80100526:	eb 01                	jmp    80100529 <cprintf+0x188>
      consputc(c);
      continue;
    }
    c = fmt[++i] & 0xff;
    if(c == 0)
      break;
80100528:	90                   	nop
      consputc(c);
      break;
    }
  }

  if(locking)
80100529:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
8010052d:	74 0c                	je     8010053b <cprintf+0x19a>
    release(&cons.lock);
8010052f:	c7 04 24 c0 b5 10 80 	movl   $0x8010b5c0,(%esp)
80100536:	e8 7a 4b 00 00       	call   801050b5 <release>
}
8010053b:	c9                   	leave  
8010053c:	c3                   	ret    

8010053d <panic>:

void
panic(char *s)
{
8010053d:	55                   	push   %ebp
8010053e:	89 e5                	mov    %esp,%ebp
80100540:	83 ec 48             	sub    $0x48,%esp
  int i;
  uint pcs[10];
  
  cli();
80100543:	e8 b0 fd ff ff       	call   801002f8 <cli>
  cons.locking = 0;
80100548:	c7 05 f4 b5 10 80 00 	movl   $0x0,0x8010b5f4
8010054f:	00 00 00 
  cprintf("cpu%d: panic: ", cpu->id);
80100552:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80100558:	0f b6 00             	movzbl (%eax),%eax
8010055b:	0f b6 c0             	movzbl %al,%eax
8010055e:	89 44 24 04          	mov    %eax,0x4(%esp)
80100562:	c7 04 24 e2 86 10 80 	movl   $0x801086e2,(%esp)
80100569:	e8 33 fe ff ff       	call   801003a1 <cprintf>
  cprintf(s);
8010056e:	8b 45 08             	mov    0x8(%ebp),%eax
80100571:	89 04 24             	mov    %eax,(%esp)
80100574:	e8 28 fe ff ff       	call   801003a1 <cprintf>
  cprintf("\n");
80100579:	c7 04 24 f1 86 10 80 	movl   $0x801086f1,(%esp)
80100580:	e8 1c fe ff ff       	call   801003a1 <cprintf>
  getcallerpcs(&s, pcs);
80100585:	8d 45 cc             	lea    -0x34(%ebp),%eax
80100588:	89 44 24 04          	mov    %eax,0x4(%esp)
8010058c:	8d 45 08             	lea    0x8(%ebp),%eax
8010058f:	89 04 24             	mov    %eax,(%esp)
80100592:	e8 6d 4b 00 00       	call   80105104 <getcallerpcs>
  for(i=0; i<10; i++)
80100597:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010059e:	eb 1b                	jmp    801005bb <panic+0x7e>
    cprintf(" %p", pcs[i]);
801005a0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801005a3:	8b 44 85 cc          	mov    -0x34(%ebp,%eax,4),%eax
801005a7:	89 44 24 04          	mov    %eax,0x4(%esp)
801005ab:	c7 04 24 f3 86 10 80 	movl   $0x801086f3,(%esp)
801005b2:	e8 ea fd ff ff       	call   801003a1 <cprintf>
  cons.locking = 0;
  cprintf("cpu%d: panic: ", cpu->id);
  cprintf(s);
  cprintf("\n");
  getcallerpcs(&s, pcs);
  for(i=0; i<10; i++)
801005b7:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801005bb:	83 7d f4 09          	cmpl   $0x9,-0xc(%ebp)
801005bf:	7e df                	jle    801005a0 <panic+0x63>
    cprintf(" %p", pcs[i]);
  panicked = 1; // freeze other CPU
801005c1:	c7 05 a0 b5 10 80 01 	movl   $0x1,0x8010b5a0
801005c8:	00 00 00 
  for(;;)
    ;
801005cb:	eb fe                	jmp    801005cb <panic+0x8e>

801005cd <cgaputc>:
#define CRTPORT 0x3d4
static ushort *crt = (ushort*)P2V(0xb8000);  // CGA memory

static void
cgaputc(int c)
{
801005cd:	55                   	push   %ebp
801005ce:	89 e5                	mov    %esp,%ebp
801005d0:	83 ec 28             	sub    $0x28,%esp
  int pos;
  
  // Cursor position: col + 80*row.
  outb(CRTPORT, 14);
801005d3:	c7 44 24 04 0e 00 00 	movl   $0xe,0x4(%esp)
801005da:	00 
801005db:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
801005e2:	e8 f3 fc ff ff       	call   801002da <outb>
  pos = inb(CRTPORT+1) << 8;
801005e7:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
801005ee:	e8 bd fc ff ff       	call   801002b0 <inb>
801005f3:	0f b6 c0             	movzbl %al,%eax
801005f6:	c1 e0 08             	shl    $0x8,%eax
801005f9:	89 45 f4             	mov    %eax,-0xc(%ebp)
  outb(CRTPORT, 15);
801005fc:	c7 44 24 04 0f 00 00 	movl   $0xf,0x4(%esp)
80100603:	00 
80100604:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
8010060b:	e8 ca fc ff ff       	call   801002da <outb>
  pos |= inb(CRTPORT+1);
80100610:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
80100617:	e8 94 fc ff ff       	call   801002b0 <inb>
8010061c:	0f b6 c0             	movzbl %al,%eax
8010061f:	09 45 f4             	or     %eax,-0xc(%ebp)

  if(c == '\n')
80100622:	83 7d 08 0a          	cmpl   $0xa,0x8(%ebp)
80100626:	75 30                	jne    80100658 <cgaputc+0x8b>
    pos += 80 - pos%80;
80100628:	8b 4d f4             	mov    -0xc(%ebp),%ecx
8010062b:	ba 67 66 66 66       	mov    $0x66666667,%edx
80100630:	89 c8                	mov    %ecx,%eax
80100632:	f7 ea                	imul   %edx
80100634:	c1 fa 05             	sar    $0x5,%edx
80100637:	89 c8                	mov    %ecx,%eax
80100639:	c1 f8 1f             	sar    $0x1f,%eax
8010063c:	29 c2                	sub    %eax,%edx
8010063e:	89 d0                	mov    %edx,%eax
80100640:	c1 e0 02             	shl    $0x2,%eax
80100643:	01 d0                	add    %edx,%eax
80100645:	c1 e0 04             	shl    $0x4,%eax
80100648:	89 ca                	mov    %ecx,%edx
8010064a:	29 c2                	sub    %eax,%edx
8010064c:	b8 50 00 00 00       	mov    $0x50,%eax
80100651:	29 d0                	sub    %edx,%eax
80100653:	01 45 f4             	add    %eax,-0xc(%ebp)
80100656:	eb 32                	jmp    8010068a <cgaputc+0xbd>
  else if(c == BACKSPACE){
80100658:	81 7d 08 00 01 00 00 	cmpl   $0x100,0x8(%ebp)
8010065f:	75 0c                	jne    8010066d <cgaputc+0xa0>
    if(pos > 0) --pos;
80100661:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80100665:	7e 23                	jle    8010068a <cgaputc+0xbd>
80100667:	83 6d f4 01          	subl   $0x1,-0xc(%ebp)
8010066b:	eb 1d                	jmp    8010068a <cgaputc+0xbd>
  } else
    crt[pos++] = (c&0xff) | 0x0700;  // black on white
8010066d:	a1 00 90 10 80       	mov    0x80109000,%eax
80100672:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100675:	01 d2                	add    %edx,%edx
80100677:	01 c2                	add    %eax,%edx
80100679:	8b 45 08             	mov    0x8(%ebp),%eax
8010067c:	66 25 ff 00          	and    $0xff,%ax
80100680:	80 cc 07             	or     $0x7,%ah
80100683:	66 89 02             	mov    %ax,(%edx)
80100686:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
  
  if((pos/80) >= 24){  // Scroll up.
8010068a:	81 7d f4 7f 07 00 00 	cmpl   $0x77f,-0xc(%ebp)
80100691:	7e 53                	jle    801006e6 <cgaputc+0x119>
    memmove(crt, crt+80, sizeof(crt[0])*23*80);
80100693:	a1 00 90 10 80       	mov    0x80109000,%eax
80100698:	8d 90 a0 00 00 00    	lea    0xa0(%eax),%edx
8010069e:	a1 00 90 10 80       	mov    0x80109000,%eax
801006a3:	c7 44 24 08 60 0e 00 	movl   $0xe60,0x8(%esp)
801006aa:	00 
801006ab:	89 54 24 04          	mov    %edx,0x4(%esp)
801006af:	89 04 24             	mov    %eax,(%esp)
801006b2:	e8 be 4c 00 00       	call   80105375 <memmove>
    pos -= 80;
801006b7:	83 6d f4 50          	subl   $0x50,-0xc(%ebp)
    memset(crt+pos, 0, sizeof(crt[0])*(24*80 - pos));
801006bb:	b8 80 07 00 00       	mov    $0x780,%eax
801006c0:	2b 45 f4             	sub    -0xc(%ebp),%eax
801006c3:	01 c0                	add    %eax,%eax
801006c5:	8b 15 00 90 10 80    	mov    0x80109000,%edx
801006cb:	8b 4d f4             	mov    -0xc(%ebp),%ecx
801006ce:	01 c9                	add    %ecx,%ecx
801006d0:	01 ca                	add    %ecx,%edx
801006d2:	89 44 24 08          	mov    %eax,0x8(%esp)
801006d6:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801006dd:	00 
801006de:	89 14 24             	mov    %edx,(%esp)
801006e1:	e8 bc 4b 00 00       	call   801052a2 <memset>
  }
  
  outb(CRTPORT, 14);
801006e6:	c7 44 24 04 0e 00 00 	movl   $0xe,0x4(%esp)
801006ed:	00 
801006ee:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
801006f5:	e8 e0 fb ff ff       	call   801002da <outb>
  outb(CRTPORT+1, pos>>8);
801006fa:	8b 45 f4             	mov    -0xc(%ebp),%eax
801006fd:	c1 f8 08             	sar    $0x8,%eax
80100700:	0f b6 c0             	movzbl %al,%eax
80100703:	89 44 24 04          	mov    %eax,0x4(%esp)
80100707:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
8010070e:	e8 c7 fb ff ff       	call   801002da <outb>
  outb(CRTPORT, 15);
80100713:	c7 44 24 04 0f 00 00 	movl   $0xf,0x4(%esp)
8010071a:	00 
8010071b:	c7 04 24 d4 03 00 00 	movl   $0x3d4,(%esp)
80100722:	e8 b3 fb ff ff       	call   801002da <outb>
  outb(CRTPORT+1, pos);
80100727:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010072a:	0f b6 c0             	movzbl %al,%eax
8010072d:	89 44 24 04          	mov    %eax,0x4(%esp)
80100731:	c7 04 24 d5 03 00 00 	movl   $0x3d5,(%esp)
80100738:	e8 9d fb ff ff       	call   801002da <outb>
  crt[pos] = ' ' | 0x0700;
8010073d:	a1 00 90 10 80       	mov    0x80109000,%eax
80100742:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100745:	01 d2                	add    %edx,%edx
80100747:	01 d0                	add    %edx,%eax
80100749:	66 c7 00 20 07       	movw   $0x720,(%eax)
}
8010074e:	c9                   	leave  
8010074f:	c3                   	ret    

80100750 <consputc>:

void
consputc(int c)
{
80100750:	55                   	push   %ebp
80100751:	89 e5                	mov    %esp,%ebp
80100753:	83 ec 18             	sub    $0x18,%esp
  if(panicked){
80100756:	a1 a0 b5 10 80       	mov    0x8010b5a0,%eax
8010075b:	85 c0                	test   %eax,%eax
8010075d:	74 07                	je     80100766 <consputc+0x16>
    cli();
8010075f:	e8 94 fb ff ff       	call   801002f8 <cli>
    for(;;)
      ;
80100764:	eb fe                	jmp    80100764 <consputc+0x14>
  }

  if(c == BACKSPACE){
80100766:	81 7d 08 00 01 00 00 	cmpl   $0x100,0x8(%ebp)
8010076d:	75 26                	jne    80100795 <consputc+0x45>
    uartputc('\b'); uartputc(' '); uartputc('\b');
8010076f:	c7 04 24 08 00 00 00 	movl   $0x8,(%esp)
80100776:	e8 82 65 00 00       	call   80106cfd <uartputc>
8010077b:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80100782:	e8 76 65 00 00       	call   80106cfd <uartputc>
80100787:	c7 04 24 08 00 00 00 	movl   $0x8,(%esp)
8010078e:	e8 6a 65 00 00       	call   80106cfd <uartputc>
80100793:	eb 0b                	jmp    801007a0 <consputc+0x50>
  } else
    uartputc(c);
80100795:	8b 45 08             	mov    0x8(%ebp),%eax
80100798:	89 04 24             	mov    %eax,(%esp)
8010079b:	e8 5d 65 00 00       	call   80106cfd <uartputc>
  cgaputc(c);
801007a0:	8b 45 08             	mov    0x8(%ebp),%eax
801007a3:	89 04 24             	mov    %eax,(%esp)
801007a6:	e8 22 fe ff ff       	call   801005cd <cgaputc>
}
801007ab:	c9                   	leave  
801007ac:	c3                   	ret    

801007ad <consoleintr>:

#define C(x)  ((x)-'@')  // Control-x

void
consoleintr(int (*getc)(void))
{
801007ad:	55                   	push   %ebp
801007ae:	89 e5                	mov    %esp,%ebp
801007b0:	83 ec 28             	sub    $0x28,%esp
  int c;

  acquire(&input.lock);
801007b3:	c7 04 24 a0 dd 10 80 	movl   $0x8010dda0,(%esp)
801007ba:	e8 94 48 00 00       	call   80105053 <acquire>
  while((c = getc()) >= 0){
801007bf:	e9 41 01 00 00       	jmp    80100905 <consoleintr+0x158>
    switch(c){
801007c4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801007c7:	83 f8 10             	cmp    $0x10,%eax
801007ca:	74 1e                	je     801007ea <consoleintr+0x3d>
801007cc:	83 f8 10             	cmp    $0x10,%eax
801007cf:	7f 0a                	jg     801007db <consoleintr+0x2e>
801007d1:	83 f8 08             	cmp    $0x8,%eax
801007d4:	74 68                	je     8010083e <consoleintr+0x91>
801007d6:	e9 94 00 00 00       	jmp    8010086f <consoleintr+0xc2>
801007db:	83 f8 15             	cmp    $0x15,%eax
801007de:	74 2f                	je     8010080f <consoleintr+0x62>
801007e0:	83 f8 7f             	cmp    $0x7f,%eax
801007e3:	74 59                	je     8010083e <consoleintr+0x91>
801007e5:	e9 85 00 00 00       	jmp    8010086f <consoleintr+0xc2>
    case C('P'):  // Process listing.
      procdump();
801007ea:	e8 a1 41 00 00       	call   80104990 <procdump>
      break;
801007ef:	e9 11 01 00 00       	jmp    80100905 <consoleintr+0x158>
    case C('U'):  // Kill line.
      while(input.e != input.w &&
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
        input.e--;
801007f4:	a1 5c de 10 80       	mov    0x8010de5c,%eax
801007f9:	83 e8 01             	sub    $0x1,%eax
801007fc:	a3 5c de 10 80       	mov    %eax,0x8010de5c
        consputc(BACKSPACE);
80100801:	c7 04 24 00 01 00 00 	movl   $0x100,(%esp)
80100808:	e8 43 ff ff ff       	call   80100750 <consputc>
8010080d:	eb 01                	jmp    80100810 <consoleintr+0x63>
    switch(c){
    case C('P'):  // Process listing.
      procdump();
      break;
    case C('U'):  // Kill line.
      while(input.e != input.w &&
8010080f:	90                   	nop
80100810:	8b 15 5c de 10 80    	mov    0x8010de5c,%edx
80100816:	a1 58 de 10 80       	mov    0x8010de58,%eax
8010081b:	39 c2                	cmp    %eax,%edx
8010081d:	0f 84 db 00 00 00    	je     801008fe <consoleintr+0x151>
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
80100823:	a1 5c de 10 80       	mov    0x8010de5c,%eax
80100828:	83 e8 01             	sub    $0x1,%eax
8010082b:	83 e0 7f             	and    $0x7f,%eax
8010082e:	0f b6 80 d4 dd 10 80 	movzbl -0x7fef222c(%eax),%eax
    switch(c){
    case C('P'):  // Process listing.
      procdump();
      break;
    case C('U'):  // Kill line.
      while(input.e != input.w &&
80100835:	3c 0a                	cmp    $0xa,%al
80100837:	75 bb                	jne    801007f4 <consoleintr+0x47>
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
        input.e--;
        consputc(BACKSPACE);
      }
      break;
80100839:	e9 c0 00 00 00       	jmp    801008fe <consoleintr+0x151>
    case C('H'): case '\x7f':  // Backspace
      if(input.e != input.w){
8010083e:	8b 15 5c de 10 80    	mov    0x8010de5c,%edx
80100844:	a1 58 de 10 80       	mov    0x8010de58,%eax
80100849:	39 c2                	cmp    %eax,%edx
8010084b:	0f 84 b0 00 00 00    	je     80100901 <consoleintr+0x154>
        input.e--;
80100851:	a1 5c de 10 80       	mov    0x8010de5c,%eax
80100856:	83 e8 01             	sub    $0x1,%eax
80100859:	a3 5c de 10 80       	mov    %eax,0x8010de5c
        consputc(BACKSPACE);
8010085e:	c7 04 24 00 01 00 00 	movl   $0x100,(%esp)
80100865:	e8 e6 fe ff ff       	call   80100750 <consputc>
      }
      break;
8010086a:	e9 92 00 00 00       	jmp    80100901 <consoleintr+0x154>
    default:
      if(c != 0 && input.e-input.r < INPUT_BUF){
8010086f:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80100873:	0f 84 8b 00 00 00    	je     80100904 <consoleintr+0x157>
80100879:	8b 15 5c de 10 80    	mov    0x8010de5c,%edx
8010087f:	a1 54 de 10 80       	mov    0x8010de54,%eax
80100884:	89 d1                	mov    %edx,%ecx
80100886:	29 c1                	sub    %eax,%ecx
80100888:	89 c8                	mov    %ecx,%eax
8010088a:	83 f8 7f             	cmp    $0x7f,%eax
8010088d:	77 75                	ja     80100904 <consoleintr+0x157>
        c = (c == '\r') ? '\n' : c;
8010088f:	83 7d f4 0d          	cmpl   $0xd,-0xc(%ebp)
80100893:	74 05                	je     8010089a <consoleintr+0xed>
80100895:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100898:	eb 05                	jmp    8010089f <consoleintr+0xf2>
8010089a:	b8 0a 00 00 00       	mov    $0xa,%eax
8010089f:	89 45 f4             	mov    %eax,-0xc(%ebp)
        input.buf[input.e++ % INPUT_BUF] = c;
801008a2:	a1 5c de 10 80       	mov    0x8010de5c,%eax
801008a7:	89 c1                	mov    %eax,%ecx
801008a9:	83 e1 7f             	and    $0x7f,%ecx
801008ac:	8b 55 f4             	mov    -0xc(%ebp),%edx
801008af:	88 91 d4 dd 10 80    	mov    %dl,-0x7fef222c(%ecx)
801008b5:	83 c0 01             	add    $0x1,%eax
801008b8:	a3 5c de 10 80       	mov    %eax,0x8010de5c
        consputc(c);
801008bd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801008c0:	89 04 24             	mov    %eax,(%esp)
801008c3:	e8 88 fe ff ff       	call   80100750 <consputc>
        if(c == '\n' || c == C('D') || input.e == input.r+INPUT_BUF){
801008c8:	83 7d f4 0a          	cmpl   $0xa,-0xc(%ebp)
801008cc:	74 18                	je     801008e6 <consoleintr+0x139>
801008ce:	83 7d f4 04          	cmpl   $0x4,-0xc(%ebp)
801008d2:	74 12                	je     801008e6 <consoleintr+0x139>
801008d4:	a1 5c de 10 80       	mov    0x8010de5c,%eax
801008d9:	8b 15 54 de 10 80    	mov    0x8010de54,%edx
801008df:	83 ea 80             	sub    $0xffffff80,%edx
801008e2:	39 d0                	cmp    %edx,%eax
801008e4:	75 1e                	jne    80100904 <consoleintr+0x157>
          input.w = input.e;
801008e6:	a1 5c de 10 80       	mov    0x8010de5c,%eax
801008eb:	a3 58 de 10 80       	mov    %eax,0x8010de58
          wakeup(&input.r);
801008f0:	c7 04 24 54 de 10 80 	movl   $0x8010de54,(%esp)
801008f7:	e8 f0 3f 00 00       	call   801048ec <wakeup>
        }
      }
      break;
801008fc:	eb 06                	jmp    80100904 <consoleintr+0x157>
      while(input.e != input.w &&
            input.buf[(input.e-1) % INPUT_BUF] != '\n'){
        input.e--;
        consputc(BACKSPACE);
      }
      break;
801008fe:	90                   	nop
801008ff:	eb 04                	jmp    80100905 <consoleintr+0x158>
    case C('H'): case '\x7f':  // Backspace
      if(input.e != input.w){
        input.e--;
        consputc(BACKSPACE);
      }
      break;
80100901:	90                   	nop
80100902:	eb 01                	jmp    80100905 <consoleintr+0x158>
        if(c == '\n' || c == C('D') || input.e == input.r+INPUT_BUF){
          input.w = input.e;
          wakeup(&input.r);
        }
      }
      break;
80100904:	90                   	nop
consoleintr(int (*getc)(void))
{
  int c;

  acquire(&input.lock);
  while((c = getc()) >= 0){
80100905:	8b 45 08             	mov    0x8(%ebp),%eax
80100908:	ff d0                	call   *%eax
8010090a:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010090d:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80100911:	0f 89 ad fe ff ff    	jns    801007c4 <consoleintr+0x17>
        }
      }
      break;
    }
  }
  release(&input.lock);
80100917:	c7 04 24 a0 dd 10 80 	movl   $0x8010dda0,(%esp)
8010091e:	e8 92 47 00 00       	call   801050b5 <release>
}
80100923:	c9                   	leave  
80100924:	c3                   	ret    

80100925 <consoleread>:

int
consoleread(struct inode *ip, char *dst, int n)
{
80100925:	55                   	push   %ebp
80100926:	89 e5                	mov    %esp,%ebp
80100928:	83 ec 28             	sub    $0x28,%esp
  uint target;
  int c;

  iunlock(ip);
8010092b:	8b 45 08             	mov    0x8(%ebp),%eax
8010092e:	89 04 24             	mov    %eax,(%esp)
80100931:	e8 78 10 00 00       	call   801019ae <iunlock>
  target = n;
80100936:	8b 45 10             	mov    0x10(%ebp),%eax
80100939:	89 45 f4             	mov    %eax,-0xc(%ebp)
  acquire(&input.lock);
8010093c:	c7 04 24 a0 dd 10 80 	movl   $0x8010dda0,(%esp)
80100943:	e8 0b 47 00 00       	call   80105053 <acquire>
  while(n > 0){
80100948:	e9 a8 00 00 00       	jmp    801009f5 <consoleread+0xd0>
    while(input.r == input.w){
      if(proc->killed){
8010094d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100953:	8b 40 24             	mov    0x24(%eax),%eax
80100956:	85 c0                	test   %eax,%eax
80100958:	74 21                	je     8010097b <consoleread+0x56>
        release(&input.lock);
8010095a:	c7 04 24 a0 dd 10 80 	movl   $0x8010dda0,(%esp)
80100961:	e8 4f 47 00 00       	call   801050b5 <release>
        ilock(ip);
80100966:	8b 45 08             	mov    0x8(%ebp),%eax
80100969:	89 04 24             	mov    %eax,(%esp)
8010096c:	e8 ef 0e 00 00       	call   80101860 <ilock>
        return -1;
80100971:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80100976:	e9 a9 00 00 00       	jmp    80100a24 <consoleread+0xff>
      }
      sleep(&input.r, &input.lock);
8010097b:	c7 44 24 04 a0 dd 10 	movl   $0x8010dda0,0x4(%esp)
80100982:	80 
80100983:	c7 04 24 54 de 10 80 	movl   $0x8010de54,(%esp)
8010098a:	e8 83 3e 00 00       	call   80104812 <sleep>
8010098f:	eb 01                	jmp    80100992 <consoleread+0x6d>

  iunlock(ip);
  target = n;
  acquire(&input.lock);
  while(n > 0){
    while(input.r == input.w){
80100991:	90                   	nop
80100992:	8b 15 54 de 10 80    	mov    0x8010de54,%edx
80100998:	a1 58 de 10 80       	mov    0x8010de58,%eax
8010099d:	39 c2                	cmp    %eax,%edx
8010099f:	74 ac                	je     8010094d <consoleread+0x28>
        ilock(ip);
        return -1;
      }
      sleep(&input.r, &input.lock);
    }
    c = input.buf[input.r++ % INPUT_BUF];
801009a1:	a1 54 de 10 80       	mov    0x8010de54,%eax
801009a6:	89 c2                	mov    %eax,%edx
801009a8:	83 e2 7f             	and    $0x7f,%edx
801009ab:	0f b6 92 d4 dd 10 80 	movzbl -0x7fef222c(%edx),%edx
801009b2:	0f be d2             	movsbl %dl,%edx
801009b5:	89 55 f0             	mov    %edx,-0x10(%ebp)
801009b8:	83 c0 01             	add    $0x1,%eax
801009bb:	a3 54 de 10 80       	mov    %eax,0x8010de54
    if(c == C('D')){  // EOF
801009c0:	83 7d f0 04          	cmpl   $0x4,-0x10(%ebp)
801009c4:	75 17                	jne    801009dd <consoleread+0xb8>
      if(n < target){
801009c6:	8b 45 10             	mov    0x10(%ebp),%eax
801009c9:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801009cc:	73 2f                	jae    801009fd <consoleread+0xd8>
        // Save ^D for next time, to make sure
        // caller gets a 0-byte result.
        input.r--;
801009ce:	a1 54 de 10 80       	mov    0x8010de54,%eax
801009d3:	83 e8 01             	sub    $0x1,%eax
801009d6:	a3 54 de 10 80       	mov    %eax,0x8010de54
      }
      break;
801009db:	eb 20                	jmp    801009fd <consoleread+0xd8>
    }
    *dst++ = c;
801009dd:	8b 45 f0             	mov    -0x10(%ebp),%eax
801009e0:	89 c2                	mov    %eax,%edx
801009e2:	8b 45 0c             	mov    0xc(%ebp),%eax
801009e5:	88 10                	mov    %dl,(%eax)
801009e7:	83 45 0c 01          	addl   $0x1,0xc(%ebp)
    --n;
801009eb:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
    if(c == '\n')
801009ef:	83 7d f0 0a          	cmpl   $0xa,-0x10(%ebp)
801009f3:	74 0b                	je     80100a00 <consoleread+0xdb>
  int c;

  iunlock(ip);
  target = n;
  acquire(&input.lock);
  while(n > 0){
801009f5:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801009f9:	7f 96                	jg     80100991 <consoleread+0x6c>
801009fb:	eb 04                	jmp    80100a01 <consoleread+0xdc>
      if(n < target){
        // Save ^D for next time, to make sure
        // caller gets a 0-byte result.
        input.r--;
      }
      break;
801009fd:	90                   	nop
801009fe:	eb 01                	jmp    80100a01 <consoleread+0xdc>
    }
    *dst++ = c;
    --n;
    if(c == '\n')
      break;
80100a00:	90                   	nop
  }
  release(&input.lock);
80100a01:	c7 04 24 a0 dd 10 80 	movl   $0x8010dda0,(%esp)
80100a08:	e8 a8 46 00 00       	call   801050b5 <release>
  ilock(ip);
80100a0d:	8b 45 08             	mov    0x8(%ebp),%eax
80100a10:	89 04 24             	mov    %eax,(%esp)
80100a13:	e8 48 0e 00 00       	call   80101860 <ilock>

  return target - n;
80100a18:	8b 45 10             	mov    0x10(%ebp),%eax
80100a1b:	8b 55 f4             	mov    -0xc(%ebp),%edx
80100a1e:	89 d1                	mov    %edx,%ecx
80100a20:	29 c1                	sub    %eax,%ecx
80100a22:	89 c8                	mov    %ecx,%eax
}
80100a24:	c9                   	leave  
80100a25:	c3                   	ret    

80100a26 <consolewrite>:

int
consolewrite(struct inode *ip, char *buf, int n)
{
80100a26:	55                   	push   %ebp
80100a27:	89 e5                	mov    %esp,%ebp
80100a29:	83 ec 28             	sub    $0x28,%esp
  int i;

  iunlock(ip);
80100a2c:	8b 45 08             	mov    0x8(%ebp),%eax
80100a2f:	89 04 24             	mov    %eax,(%esp)
80100a32:	e8 77 0f 00 00       	call   801019ae <iunlock>
  acquire(&cons.lock);
80100a37:	c7 04 24 c0 b5 10 80 	movl   $0x8010b5c0,(%esp)
80100a3e:	e8 10 46 00 00       	call   80105053 <acquire>
  for(i = 0; i < n; i++)
80100a43:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80100a4a:	eb 1d                	jmp    80100a69 <consolewrite+0x43>
    consputc(buf[i] & 0xff);
80100a4c:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100a4f:	03 45 0c             	add    0xc(%ebp),%eax
80100a52:	0f b6 00             	movzbl (%eax),%eax
80100a55:	0f be c0             	movsbl %al,%eax
80100a58:	25 ff 00 00 00       	and    $0xff,%eax
80100a5d:	89 04 24             	mov    %eax,(%esp)
80100a60:	e8 eb fc ff ff       	call   80100750 <consputc>
{
  int i;

  iunlock(ip);
  acquire(&cons.lock);
  for(i = 0; i < n; i++)
80100a65:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100a69:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100a6c:	3b 45 10             	cmp    0x10(%ebp),%eax
80100a6f:	7c db                	jl     80100a4c <consolewrite+0x26>
    consputc(buf[i] & 0xff);
  release(&cons.lock);
80100a71:	c7 04 24 c0 b5 10 80 	movl   $0x8010b5c0,(%esp)
80100a78:	e8 38 46 00 00       	call   801050b5 <release>
  ilock(ip);
80100a7d:	8b 45 08             	mov    0x8(%ebp),%eax
80100a80:	89 04 24             	mov    %eax,(%esp)
80100a83:	e8 d8 0d 00 00       	call   80101860 <ilock>

  return n;
80100a88:	8b 45 10             	mov    0x10(%ebp),%eax
}
80100a8b:	c9                   	leave  
80100a8c:	c3                   	ret    

80100a8d <consoleinit>:

void
consoleinit(void)
{
80100a8d:	55                   	push   %ebp
80100a8e:	89 e5                	mov    %esp,%ebp
80100a90:	83 ec 18             	sub    $0x18,%esp
  initlock(&cons.lock, "console");
80100a93:	c7 44 24 04 f7 86 10 	movl   $0x801086f7,0x4(%esp)
80100a9a:	80 
80100a9b:	c7 04 24 c0 b5 10 80 	movl   $0x8010b5c0,(%esp)
80100aa2:	e8 8b 45 00 00       	call   80105032 <initlock>
  initlock(&input.lock, "input");
80100aa7:	c7 44 24 04 ff 86 10 	movl   $0x801086ff,0x4(%esp)
80100aae:	80 
80100aaf:	c7 04 24 a0 dd 10 80 	movl   $0x8010dda0,(%esp)
80100ab6:	e8 77 45 00 00       	call   80105032 <initlock>

  devsw[CONSOLE].write = consolewrite;
80100abb:	c7 05 0c e8 10 80 26 	movl   $0x80100a26,0x8010e80c
80100ac2:	0a 10 80 
  devsw[CONSOLE].read = consoleread;
80100ac5:	c7 05 08 e8 10 80 25 	movl   $0x80100925,0x8010e808
80100acc:	09 10 80 
  cons.locking = 1;
80100acf:	c7 05 f4 b5 10 80 01 	movl   $0x1,0x8010b5f4
80100ad6:	00 00 00 

  picenable(IRQ_KBD);
80100ad9:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80100ae0:	e8 c8 2f 00 00       	call   80103aad <picenable>
  ioapicenable(IRQ_KBD, 0);
80100ae5:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80100aec:	00 
80100aed:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80100af4:	e8 75 1e 00 00       	call   8010296e <ioapicenable>
}
80100af9:	c9                   	leave  
80100afa:	c3                   	ret    
	...

80100afc <exec>:
#include "x86.h"
#include "elf.h"

int
exec(char *path, char **argv)
{
80100afc:	55                   	push   %ebp
80100afd:	89 e5                	mov    %esp,%ebp
80100aff:	81 ec 38 01 00 00    	sub    $0x138,%esp
  struct elfhdr elf;
  struct inode *ip;
  struct proghdr ph;
  pde_t *pgdir, *oldpgdir;

  if((ip = namei(path)) == 0)
80100b05:	8b 45 08             	mov    0x8(%ebp),%eax
80100b08:	89 04 24             	mov    %eax,(%esp)
80100b0b:	e8 f2 18 00 00       	call   80102402 <namei>
80100b10:	89 45 d8             	mov    %eax,-0x28(%ebp)
80100b13:	83 7d d8 00          	cmpl   $0x0,-0x28(%ebp)
80100b17:	75 0a                	jne    80100b23 <exec+0x27>
    return -1;
80100b19:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80100b1e:	e9 d3 03 00 00       	jmp    80100ef6 <exec+0x3fa>
  ilock(ip);
80100b23:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100b26:	89 04 24             	mov    %eax,(%esp)
80100b29:	e8 32 0d 00 00       	call   80101860 <ilock>
  pgdir = 0;
80100b2e:	c7 45 d4 00 00 00 00 	movl   $0x0,-0x2c(%ebp)

  // Check ELF header
  if(readi(ip, (char*)&elf, 0, sizeof(elf)) < sizeof(elf))
80100b35:	c7 44 24 0c 34 00 00 	movl   $0x34,0xc(%esp)
80100b3c:	00 
80100b3d:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80100b44:	00 
80100b45:	8d 85 0c ff ff ff    	lea    -0xf4(%ebp),%eax
80100b4b:	89 44 24 04          	mov    %eax,0x4(%esp)
80100b4f:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100b52:	89 04 24             	mov    %eax,(%esp)
80100b55:	e8 fc 11 00 00       	call   80101d56 <readi>
80100b5a:	83 f8 33             	cmp    $0x33,%eax
80100b5d:	0f 86 4d 03 00 00    	jbe    80100eb0 <exec+0x3b4>
    goto bad;
  if(elf.magic != ELF_MAGIC)
80100b63:	8b 85 0c ff ff ff    	mov    -0xf4(%ebp),%eax
80100b69:	3d 7f 45 4c 46       	cmp    $0x464c457f,%eax
80100b6e:	0f 85 3f 03 00 00    	jne    80100eb3 <exec+0x3b7>
    goto bad;

  if((pgdir = setupkvm()) == 0)
80100b74:	e8 c8 72 00 00       	call   80107e41 <setupkvm>
80100b79:	89 45 d4             	mov    %eax,-0x2c(%ebp)
80100b7c:	83 7d d4 00          	cmpl   $0x0,-0x2c(%ebp)
80100b80:	0f 84 30 03 00 00    	je     80100eb6 <exec+0x3ba>
    goto bad;

  // Load program into memory.
  sz = 0;
80100b86:	c7 45 e0 00 00 00 00 	movl   $0x0,-0x20(%ebp)
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
80100b8d:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
80100b94:	8b 85 28 ff ff ff    	mov    -0xd8(%ebp),%eax
80100b9a:	89 45 e8             	mov    %eax,-0x18(%ebp)
80100b9d:	e9 c5 00 00 00       	jmp    80100c67 <exec+0x16b>
    if(readi(ip, (char*)&ph, off, sizeof(ph)) != sizeof(ph))
80100ba2:	8b 45 e8             	mov    -0x18(%ebp),%eax
80100ba5:	c7 44 24 0c 20 00 00 	movl   $0x20,0xc(%esp)
80100bac:	00 
80100bad:	89 44 24 08          	mov    %eax,0x8(%esp)
80100bb1:	8d 85 ec fe ff ff    	lea    -0x114(%ebp),%eax
80100bb7:	89 44 24 04          	mov    %eax,0x4(%esp)
80100bbb:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100bbe:	89 04 24             	mov    %eax,(%esp)
80100bc1:	e8 90 11 00 00       	call   80101d56 <readi>
80100bc6:	83 f8 20             	cmp    $0x20,%eax
80100bc9:	0f 85 ea 02 00 00    	jne    80100eb9 <exec+0x3bd>
      goto bad;
    if(ph.type != ELF_PROG_LOAD)
80100bcf:	8b 85 ec fe ff ff    	mov    -0x114(%ebp),%eax
80100bd5:	83 f8 01             	cmp    $0x1,%eax
80100bd8:	75 7f                	jne    80100c59 <exec+0x15d>
      continue;
    if(ph.memsz < ph.filesz)
80100bda:	8b 95 00 ff ff ff    	mov    -0x100(%ebp),%edx
80100be0:	8b 85 fc fe ff ff    	mov    -0x104(%ebp),%eax
80100be6:	39 c2                	cmp    %eax,%edx
80100be8:	0f 82 ce 02 00 00    	jb     80100ebc <exec+0x3c0>
      goto bad;
    if((sz = allocuvm(pgdir, sz, ph.vaddr + ph.memsz)) == 0)
80100bee:	8b 95 f4 fe ff ff    	mov    -0x10c(%ebp),%edx
80100bf4:	8b 85 00 ff ff ff    	mov    -0x100(%ebp),%eax
80100bfa:	01 d0                	add    %edx,%eax
80100bfc:	89 44 24 08          	mov    %eax,0x8(%esp)
80100c00:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100c03:	89 44 24 04          	mov    %eax,0x4(%esp)
80100c07:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100c0a:	89 04 24             	mov    %eax,(%esp)
80100c0d:	e8 01 76 00 00       	call   80108213 <allocuvm>
80100c12:	89 45 e0             	mov    %eax,-0x20(%ebp)
80100c15:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
80100c19:	0f 84 a0 02 00 00    	je     80100ebf <exec+0x3c3>
      goto bad;
    if(loaduvm(pgdir, (char*)ph.vaddr, ip, ph.off, ph.filesz) < 0)
80100c1f:	8b 8d fc fe ff ff    	mov    -0x104(%ebp),%ecx
80100c25:	8b 95 f0 fe ff ff    	mov    -0x110(%ebp),%edx
80100c2b:	8b 85 f4 fe ff ff    	mov    -0x10c(%ebp),%eax
80100c31:	89 4c 24 10          	mov    %ecx,0x10(%esp)
80100c35:	89 54 24 0c          	mov    %edx,0xc(%esp)
80100c39:	8b 55 d8             	mov    -0x28(%ebp),%edx
80100c3c:	89 54 24 08          	mov    %edx,0x8(%esp)
80100c40:	89 44 24 04          	mov    %eax,0x4(%esp)
80100c44:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100c47:	89 04 24             	mov    %eax,(%esp)
80100c4a:	e8 d5 74 00 00       	call   80108124 <loaduvm>
80100c4f:	85 c0                	test   %eax,%eax
80100c51:	0f 88 6b 02 00 00    	js     80100ec2 <exec+0x3c6>
80100c57:	eb 01                	jmp    80100c5a <exec+0x15e>
  sz = 0;
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    if(readi(ip, (char*)&ph, off, sizeof(ph)) != sizeof(ph))
      goto bad;
    if(ph.type != ELF_PROG_LOAD)
      continue;
80100c59:	90                   	nop
  if((pgdir = setupkvm()) == 0)
    goto bad;

  // Load program into memory.
  sz = 0;
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
80100c5a:	83 45 ec 01          	addl   $0x1,-0x14(%ebp)
80100c5e:	8b 45 e8             	mov    -0x18(%ebp),%eax
80100c61:	83 c0 20             	add    $0x20,%eax
80100c64:	89 45 e8             	mov    %eax,-0x18(%ebp)
80100c67:	0f b7 85 38 ff ff ff 	movzwl -0xc8(%ebp),%eax
80100c6e:	0f b7 c0             	movzwl %ax,%eax
80100c71:	3b 45 ec             	cmp    -0x14(%ebp),%eax
80100c74:	0f 8f 28 ff ff ff    	jg     80100ba2 <exec+0xa6>
    if((sz = allocuvm(pgdir, sz, ph.vaddr + ph.memsz)) == 0)
      goto bad;
    if(loaduvm(pgdir, (char*)ph.vaddr, ip, ph.off, ph.filesz) < 0)
      goto bad;
  }
  iunlockput(ip);
80100c7a:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100c7d:	89 04 24             	mov    %eax,(%esp)
80100c80:	e8 5f 0e 00 00       	call   80101ae4 <iunlockput>
  ip = 0;
80100c85:	c7 45 d8 00 00 00 00 	movl   $0x0,-0x28(%ebp)

  // Allocate two pages at the next page boundary.
  // Make the first inaccessible.  Use the second as the user stack.
  sz = PGROUNDUP(sz);
80100c8c:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100c8f:	05 ff 0f 00 00       	add    $0xfff,%eax
80100c94:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80100c99:	89 45 e0             	mov    %eax,-0x20(%ebp)
  if((sz = allocuvm(pgdir, sz, sz + 2*PGSIZE)) == 0)
80100c9c:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100c9f:	05 00 20 00 00       	add    $0x2000,%eax
80100ca4:	89 44 24 08          	mov    %eax,0x8(%esp)
80100ca8:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100cab:	89 44 24 04          	mov    %eax,0x4(%esp)
80100caf:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100cb2:	89 04 24             	mov    %eax,(%esp)
80100cb5:	e8 59 75 00 00       	call   80108213 <allocuvm>
80100cba:	89 45 e0             	mov    %eax,-0x20(%ebp)
80100cbd:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
80100cc1:	0f 84 fe 01 00 00    	je     80100ec5 <exec+0x3c9>
    goto bad;
  clearpteu(pgdir, (char*)(sz - 2*PGSIZE));
80100cc7:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100cca:	2d 00 20 00 00       	sub    $0x2000,%eax
80100ccf:	89 44 24 04          	mov    %eax,0x4(%esp)
80100cd3:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100cd6:	89 04 24             	mov    %eax,(%esp)
80100cd9:	e8 59 77 00 00       	call   80108437 <clearpteu>
  sp = sz;
80100cde:	8b 45 e0             	mov    -0x20(%ebp),%eax
80100ce1:	89 45 dc             	mov    %eax,-0x24(%ebp)

  // Push argument strings, prepare rest of stack in ustack.
  for(argc = 0; argv[argc]; argc++) {
80100ce4:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
80100ceb:	e9 81 00 00 00       	jmp    80100d71 <exec+0x275>
    if(argc >= MAXARG)
80100cf0:	83 7d e4 1f          	cmpl   $0x1f,-0x1c(%ebp)
80100cf4:	0f 87 ce 01 00 00    	ja     80100ec8 <exec+0x3cc>
      goto bad;
    sp = (sp - (strlen(argv[argc]) + 1)) & ~3;
80100cfa:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100cfd:	c1 e0 02             	shl    $0x2,%eax
80100d00:	03 45 0c             	add    0xc(%ebp),%eax
80100d03:	8b 00                	mov    (%eax),%eax
80100d05:	89 04 24             	mov    %eax,(%esp)
80100d08:	e8 13 48 00 00       	call   80105520 <strlen>
80100d0d:	f7 d0                	not    %eax
80100d0f:	03 45 dc             	add    -0x24(%ebp),%eax
80100d12:	83 e0 fc             	and    $0xfffffffc,%eax
80100d15:	89 45 dc             	mov    %eax,-0x24(%ebp)
    if(copyout(pgdir, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
80100d18:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d1b:	c1 e0 02             	shl    $0x2,%eax
80100d1e:	03 45 0c             	add    0xc(%ebp),%eax
80100d21:	8b 00                	mov    (%eax),%eax
80100d23:	89 04 24             	mov    %eax,(%esp)
80100d26:	e8 f5 47 00 00       	call   80105520 <strlen>
80100d2b:	83 c0 01             	add    $0x1,%eax
80100d2e:	89 c2                	mov    %eax,%edx
80100d30:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d33:	c1 e0 02             	shl    $0x2,%eax
80100d36:	03 45 0c             	add    0xc(%ebp),%eax
80100d39:	8b 00                	mov    (%eax),%eax
80100d3b:	89 54 24 0c          	mov    %edx,0xc(%esp)
80100d3f:	89 44 24 08          	mov    %eax,0x8(%esp)
80100d43:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100d46:	89 44 24 04          	mov    %eax,0x4(%esp)
80100d4a:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100d4d:	89 04 24             	mov    %eax,(%esp)
80100d50:	e8 a7 78 00 00       	call   801085fc <copyout>
80100d55:	85 c0                	test   %eax,%eax
80100d57:	0f 88 6e 01 00 00    	js     80100ecb <exec+0x3cf>
      goto bad;
    ustack[3+argc] = sp;
80100d5d:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d60:	8d 50 03             	lea    0x3(%eax),%edx
80100d63:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100d66:	89 84 95 40 ff ff ff 	mov    %eax,-0xc0(%ebp,%edx,4)
    goto bad;
  clearpteu(pgdir, (char*)(sz - 2*PGSIZE));
  sp = sz;

  // Push argument strings, prepare rest of stack in ustack.
  for(argc = 0; argv[argc]; argc++) {
80100d6d:	83 45 e4 01          	addl   $0x1,-0x1c(%ebp)
80100d71:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d74:	c1 e0 02             	shl    $0x2,%eax
80100d77:	03 45 0c             	add    0xc(%ebp),%eax
80100d7a:	8b 00                	mov    (%eax),%eax
80100d7c:	85 c0                	test   %eax,%eax
80100d7e:	0f 85 6c ff ff ff    	jne    80100cf0 <exec+0x1f4>
    sp = (sp - (strlen(argv[argc]) + 1)) & ~3;
    if(copyout(pgdir, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
      goto bad;
    ustack[3+argc] = sp;
  }
  ustack[3+argc] = 0;
80100d84:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100d87:	83 c0 03             	add    $0x3,%eax
80100d8a:	c7 84 85 40 ff ff ff 	movl   $0x0,-0xc0(%ebp,%eax,4)
80100d91:	00 00 00 00 

  ustack[0] = 0xffffffff;  // fake return PC
80100d95:	c7 85 40 ff ff ff ff 	movl   $0xffffffff,-0xc0(%ebp)
80100d9c:	ff ff ff 
  ustack[1] = argc;
80100d9f:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100da2:	89 85 44 ff ff ff    	mov    %eax,-0xbc(%ebp)
  ustack[2] = sp - (argc+1)*4;  // argv pointer
80100da8:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100dab:	83 c0 01             	add    $0x1,%eax
80100dae:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
80100db5:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100db8:	29 d0                	sub    %edx,%eax
80100dba:	89 85 48 ff ff ff    	mov    %eax,-0xb8(%ebp)

  sp -= (3+argc+1) * 4;
80100dc0:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100dc3:	83 c0 04             	add    $0x4,%eax
80100dc6:	c1 e0 02             	shl    $0x2,%eax
80100dc9:	29 45 dc             	sub    %eax,-0x24(%ebp)
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
80100dcc:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80100dcf:	83 c0 04             	add    $0x4,%eax
80100dd2:	c1 e0 02             	shl    $0x2,%eax
80100dd5:	89 44 24 0c          	mov    %eax,0xc(%esp)
80100dd9:	8d 85 40 ff ff ff    	lea    -0xc0(%ebp),%eax
80100ddf:	89 44 24 08          	mov    %eax,0x8(%esp)
80100de3:	8b 45 dc             	mov    -0x24(%ebp),%eax
80100de6:	89 44 24 04          	mov    %eax,0x4(%esp)
80100dea:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100ded:	89 04 24             	mov    %eax,(%esp)
80100df0:	e8 07 78 00 00       	call   801085fc <copyout>
80100df5:	85 c0                	test   %eax,%eax
80100df7:	0f 88 d1 00 00 00    	js     80100ece <exec+0x3d2>
    goto bad;

  // Save program name for debugging.
  for(last=s=path; *s; s++)
80100dfd:	8b 45 08             	mov    0x8(%ebp),%eax
80100e00:	89 45 f4             	mov    %eax,-0xc(%ebp)
80100e03:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e06:	89 45 f0             	mov    %eax,-0x10(%ebp)
80100e09:	eb 17                	jmp    80100e22 <exec+0x326>
    if(*s == '/')
80100e0b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e0e:	0f b6 00             	movzbl (%eax),%eax
80100e11:	3c 2f                	cmp    $0x2f,%al
80100e13:	75 09                	jne    80100e1e <exec+0x322>
      last = s+1;
80100e15:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e18:	83 c0 01             	add    $0x1,%eax
80100e1b:	89 45 f0             	mov    %eax,-0x10(%ebp)
  sp -= (3+argc+1) * 4;
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
    goto bad;

  // Save program name for debugging.
  for(last=s=path; *s; s++)
80100e1e:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80100e22:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100e25:	0f b6 00             	movzbl (%eax),%eax
80100e28:	84 c0                	test   %al,%al
80100e2a:	75 df                	jne    80100e0b <exec+0x30f>
    if(*s == '/')
      last = s+1;
  safestrcpy(proc->name, last, sizeof(proc->name));
80100e2c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e32:	8d 50 6c             	lea    0x6c(%eax),%edx
80100e35:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80100e3c:	00 
80100e3d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80100e40:	89 44 24 04          	mov    %eax,0x4(%esp)
80100e44:	89 14 24             	mov    %edx,(%esp)
80100e47:	e8 86 46 00 00       	call   801054d2 <safestrcpy>

  // Commit to the user image.
  oldpgdir = proc->pgdir;
80100e4c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e52:	8b 40 04             	mov    0x4(%eax),%eax
80100e55:	89 45 d0             	mov    %eax,-0x30(%ebp)
  proc->pgdir = pgdir;
80100e58:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e5e:	8b 55 d4             	mov    -0x2c(%ebp),%edx
80100e61:	89 50 04             	mov    %edx,0x4(%eax)
  proc->sz = sz;
80100e64:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e6a:	8b 55 e0             	mov    -0x20(%ebp),%edx
80100e6d:	89 10                	mov    %edx,(%eax)
  proc->tf->eip = elf.entry;  // main
80100e6f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e75:	8b 40 18             	mov    0x18(%eax),%eax
80100e78:	8b 95 24 ff ff ff    	mov    -0xdc(%ebp),%edx
80100e7e:	89 50 38             	mov    %edx,0x38(%eax)
  proc->tf->esp = sp;
80100e81:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e87:	8b 40 18             	mov    0x18(%eax),%eax
80100e8a:	8b 55 dc             	mov    -0x24(%ebp),%edx
80100e8d:	89 50 44             	mov    %edx,0x44(%eax)
  switchuvm(proc);
80100e90:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80100e96:	89 04 24             	mov    %eax,(%esp)
80100e99:	e8 94 70 00 00       	call   80107f32 <switchuvm>
  freevm(oldpgdir);
80100e9e:	8b 45 d0             	mov    -0x30(%ebp),%eax
80100ea1:	89 04 24             	mov    %eax,(%esp)
80100ea4:	e8 00 75 00 00       	call   801083a9 <freevm>
  return 0;
80100ea9:	b8 00 00 00 00       	mov    $0x0,%eax
80100eae:	eb 46                	jmp    80100ef6 <exec+0x3fa>
  ilock(ip);
  pgdir = 0;

  // Check ELF header
  if(readi(ip, (char*)&elf, 0, sizeof(elf)) < sizeof(elf))
    goto bad;
80100eb0:	90                   	nop
80100eb1:	eb 1c                	jmp    80100ecf <exec+0x3d3>
  if(elf.magic != ELF_MAGIC)
    goto bad;
80100eb3:	90                   	nop
80100eb4:	eb 19                	jmp    80100ecf <exec+0x3d3>

  if((pgdir = setupkvm()) == 0)
    goto bad;
80100eb6:	90                   	nop
80100eb7:	eb 16                	jmp    80100ecf <exec+0x3d3>

  // Load program into memory.
  sz = 0;
  for(i=0, off=elf.phoff; i<elf.phnum; i++, off+=sizeof(ph)){
    if(readi(ip, (char*)&ph, off, sizeof(ph)) != sizeof(ph))
      goto bad;
80100eb9:	90                   	nop
80100eba:	eb 13                	jmp    80100ecf <exec+0x3d3>
    if(ph.type != ELF_PROG_LOAD)
      continue;
    if(ph.memsz < ph.filesz)
      goto bad;
80100ebc:	90                   	nop
80100ebd:	eb 10                	jmp    80100ecf <exec+0x3d3>
    if((sz = allocuvm(pgdir, sz, ph.vaddr + ph.memsz)) == 0)
      goto bad;
80100ebf:	90                   	nop
80100ec0:	eb 0d                	jmp    80100ecf <exec+0x3d3>
    if(loaduvm(pgdir, (char*)ph.vaddr, ip, ph.off, ph.filesz) < 0)
      goto bad;
80100ec2:	90                   	nop
80100ec3:	eb 0a                	jmp    80100ecf <exec+0x3d3>

  // Allocate two pages at the next page boundary.
  // Make the first inaccessible.  Use the second as the user stack.
  sz = PGROUNDUP(sz);
  if((sz = allocuvm(pgdir, sz, sz + 2*PGSIZE)) == 0)
    goto bad;
80100ec5:	90                   	nop
80100ec6:	eb 07                	jmp    80100ecf <exec+0x3d3>
  sp = sz;

  // Push argument strings, prepare rest of stack in ustack.
  for(argc = 0; argv[argc]; argc++) {
    if(argc >= MAXARG)
      goto bad;
80100ec8:	90                   	nop
80100ec9:	eb 04                	jmp    80100ecf <exec+0x3d3>
    sp = (sp - (strlen(argv[argc]) + 1)) & ~3;
    if(copyout(pgdir, sp, argv[argc], strlen(argv[argc]) + 1) < 0)
      goto bad;
80100ecb:	90                   	nop
80100ecc:	eb 01                	jmp    80100ecf <exec+0x3d3>
  ustack[1] = argc;
  ustack[2] = sp - (argc+1)*4;  // argv pointer

  sp -= (3+argc+1) * 4;
  if(copyout(pgdir, sp, ustack, (3+argc+1)*4) < 0)
    goto bad;
80100ece:	90                   	nop
  switchuvm(proc);
  freevm(oldpgdir);
  return 0;

 bad:
  if(pgdir)
80100ecf:	83 7d d4 00          	cmpl   $0x0,-0x2c(%ebp)
80100ed3:	74 0b                	je     80100ee0 <exec+0x3e4>
    freevm(pgdir);
80100ed5:	8b 45 d4             	mov    -0x2c(%ebp),%eax
80100ed8:	89 04 24             	mov    %eax,(%esp)
80100edb:	e8 c9 74 00 00       	call   801083a9 <freevm>
  if(ip)
80100ee0:	83 7d d8 00          	cmpl   $0x0,-0x28(%ebp)
80100ee4:	74 0b                	je     80100ef1 <exec+0x3f5>
    iunlockput(ip);
80100ee6:	8b 45 d8             	mov    -0x28(%ebp),%eax
80100ee9:	89 04 24             	mov    %eax,(%esp)
80100eec:	e8 f3 0b 00 00       	call   80101ae4 <iunlockput>
  return -1;
80100ef1:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80100ef6:	c9                   	leave  
80100ef7:	c3                   	ret    

80100ef8 <fileinit>:
  struct file file[NFILE];
} ftable;

void
fileinit(void)
{
80100ef8:	55                   	push   %ebp
80100ef9:	89 e5                	mov    %esp,%ebp
80100efb:	83 ec 18             	sub    $0x18,%esp
  initlock(&ftable.lock, "ftable");
80100efe:	c7 44 24 04 05 87 10 	movl   $0x80108705,0x4(%esp)
80100f05:	80 
80100f06:	c7 04 24 60 de 10 80 	movl   $0x8010de60,(%esp)
80100f0d:	e8 20 41 00 00       	call   80105032 <initlock>
}
80100f12:	c9                   	leave  
80100f13:	c3                   	ret    

80100f14 <filealloc>:

// Allocate a file structure.
struct file*
filealloc(void)
{
80100f14:	55                   	push   %ebp
80100f15:	89 e5                	mov    %esp,%ebp
80100f17:	83 ec 28             	sub    $0x28,%esp
  struct file *f;

  acquire(&ftable.lock);
80100f1a:	c7 04 24 60 de 10 80 	movl   $0x8010de60,(%esp)
80100f21:	e8 2d 41 00 00       	call   80105053 <acquire>
  for(f = ftable.file; f < ftable.file + NFILE; f++){
80100f26:	c7 45 f4 94 de 10 80 	movl   $0x8010de94,-0xc(%ebp)
80100f2d:	eb 29                	jmp    80100f58 <filealloc+0x44>
    if(f->ref == 0){
80100f2f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100f32:	8b 40 04             	mov    0x4(%eax),%eax
80100f35:	85 c0                	test   %eax,%eax
80100f37:	75 1b                	jne    80100f54 <filealloc+0x40>
      f->ref = 1;
80100f39:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100f3c:	c7 40 04 01 00 00 00 	movl   $0x1,0x4(%eax)
      release(&ftable.lock);
80100f43:	c7 04 24 60 de 10 80 	movl   $0x8010de60,(%esp)
80100f4a:	e8 66 41 00 00       	call   801050b5 <release>
      return f;
80100f4f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80100f52:	eb 1e                	jmp    80100f72 <filealloc+0x5e>
filealloc(void)
{
  struct file *f;

  acquire(&ftable.lock);
  for(f = ftable.file; f < ftable.file + NFILE; f++){
80100f54:	83 45 f4 18          	addl   $0x18,-0xc(%ebp)
80100f58:	81 7d f4 f4 e7 10 80 	cmpl   $0x8010e7f4,-0xc(%ebp)
80100f5f:	72 ce                	jb     80100f2f <filealloc+0x1b>
      f->ref = 1;
      release(&ftable.lock);
      return f;
    }
  }
  release(&ftable.lock);
80100f61:	c7 04 24 60 de 10 80 	movl   $0x8010de60,(%esp)
80100f68:	e8 48 41 00 00       	call   801050b5 <release>
  return 0;
80100f6d:	b8 00 00 00 00       	mov    $0x0,%eax
}
80100f72:	c9                   	leave  
80100f73:	c3                   	ret    

80100f74 <filedup>:

// Increment ref count for file f.
struct file*
filedup(struct file *f)
{
80100f74:	55                   	push   %ebp
80100f75:	89 e5                	mov    %esp,%ebp
80100f77:	83 ec 18             	sub    $0x18,%esp
  acquire(&ftable.lock);
80100f7a:	c7 04 24 60 de 10 80 	movl   $0x8010de60,(%esp)
80100f81:	e8 cd 40 00 00       	call   80105053 <acquire>
  if(f->ref < 1)
80100f86:	8b 45 08             	mov    0x8(%ebp),%eax
80100f89:	8b 40 04             	mov    0x4(%eax),%eax
80100f8c:	85 c0                	test   %eax,%eax
80100f8e:	7f 0c                	jg     80100f9c <filedup+0x28>
    panic("filedup");
80100f90:	c7 04 24 0c 87 10 80 	movl   $0x8010870c,(%esp)
80100f97:	e8 a1 f5 ff ff       	call   8010053d <panic>
  f->ref++;
80100f9c:	8b 45 08             	mov    0x8(%ebp),%eax
80100f9f:	8b 40 04             	mov    0x4(%eax),%eax
80100fa2:	8d 50 01             	lea    0x1(%eax),%edx
80100fa5:	8b 45 08             	mov    0x8(%ebp),%eax
80100fa8:	89 50 04             	mov    %edx,0x4(%eax)
  release(&ftable.lock);
80100fab:	c7 04 24 60 de 10 80 	movl   $0x8010de60,(%esp)
80100fb2:	e8 fe 40 00 00       	call   801050b5 <release>
  return f;
80100fb7:	8b 45 08             	mov    0x8(%ebp),%eax
}
80100fba:	c9                   	leave  
80100fbb:	c3                   	ret    

80100fbc <fileclose>:

// Close file f.  (Decrement ref count, close when reaches 0.)
void
fileclose(struct file *f)
{
80100fbc:	55                   	push   %ebp
80100fbd:	89 e5                	mov    %esp,%ebp
80100fbf:	83 ec 38             	sub    $0x38,%esp
  struct file ff;

  acquire(&ftable.lock);
80100fc2:	c7 04 24 60 de 10 80 	movl   $0x8010de60,(%esp)
80100fc9:	e8 85 40 00 00       	call   80105053 <acquire>
  if(f->ref < 1)
80100fce:	8b 45 08             	mov    0x8(%ebp),%eax
80100fd1:	8b 40 04             	mov    0x4(%eax),%eax
80100fd4:	85 c0                	test   %eax,%eax
80100fd6:	7f 0c                	jg     80100fe4 <fileclose+0x28>
    panic("fileclose");
80100fd8:	c7 04 24 14 87 10 80 	movl   $0x80108714,(%esp)
80100fdf:	e8 59 f5 ff ff       	call   8010053d <panic>
  if(--f->ref > 0){
80100fe4:	8b 45 08             	mov    0x8(%ebp),%eax
80100fe7:	8b 40 04             	mov    0x4(%eax),%eax
80100fea:	8d 50 ff             	lea    -0x1(%eax),%edx
80100fed:	8b 45 08             	mov    0x8(%ebp),%eax
80100ff0:	89 50 04             	mov    %edx,0x4(%eax)
80100ff3:	8b 45 08             	mov    0x8(%ebp),%eax
80100ff6:	8b 40 04             	mov    0x4(%eax),%eax
80100ff9:	85 c0                	test   %eax,%eax
80100ffb:	7e 11                	jle    8010100e <fileclose+0x52>
    release(&ftable.lock);
80100ffd:	c7 04 24 60 de 10 80 	movl   $0x8010de60,(%esp)
80101004:	e8 ac 40 00 00       	call   801050b5 <release>
    return;
80101009:	e9 82 00 00 00       	jmp    80101090 <fileclose+0xd4>
  }
  ff = *f;
8010100e:	8b 45 08             	mov    0x8(%ebp),%eax
80101011:	8b 10                	mov    (%eax),%edx
80101013:	89 55 e0             	mov    %edx,-0x20(%ebp)
80101016:	8b 50 04             	mov    0x4(%eax),%edx
80101019:	89 55 e4             	mov    %edx,-0x1c(%ebp)
8010101c:	8b 50 08             	mov    0x8(%eax),%edx
8010101f:	89 55 e8             	mov    %edx,-0x18(%ebp)
80101022:	8b 50 0c             	mov    0xc(%eax),%edx
80101025:	89 55 ec             	mov    %edx,-0x14(%ebp)
80101028:	8b 50 10             	mov    0x10(%eax),%edx
8010102b:	89 55 f0             	mov    %edx,-0x10(%ebp)
8010102e:	8b 40 14             	mov    0x14(%eax),%eax
80101031:	89 45 f4             	mov    %eax,-0xc(%ebp)
  f->ref = 0;
80101034:	8b 45 08             	mov    0x8(%ebp),%eax
80101037:	c7 40 04 00 00 00 00 	movl   $0x0,0x4(%eax)
  f->type = FD_NONE;
8010103e:	8b 45 08             	mov    0x8(%ebp),%eax
80101041:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  release(&ftable.lock);
80101047:	c7 04 24 60 de 10 80 	movl   $0x8010de60,(%esp)
8010104e:	e8 62 40 00 00       	call   801050b5 <release>
  
  if(ff.type == FD_PIPE)
80101053:	8b 45 e0             	mov    -0x20(%ebp),%eax
80101056:	83 f8 01             	cmp    $0x1,%eax
80101059:	75 18                	jne    80101073 <fileclose+0xb7>
    pipeclose(ff.pipe, ff.writable);
8010105b:	0f b6 45 e9          	movzbl -0x17(%ebp),%eax
8010105f:	0f be d0             	movsbl %al,%edx
80101062:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101065:	89 54 24 04          	mov    %edx,0x4(%esp)
80101069:	89 04 24             	mov    %eax,(%esp)
8010106c:	e8 f6 2c 00 00       	call   80103d67 <pipeclose>
80101071:	eb 1d                	jmp    80101090 <fileclose+0xd4>
  else if(ff.type == FD_INODE){
80101073:	8b 45 e0             	mov    -0x20(%ebp),%eax
80101076:	83 f8 02             	cmp    $0x2,%eax
80101079:	75 15                	jne    80101090 <fileclose+0xd4>
    begin_trans();
8010107b:	e8 95 21 00 00       	call   80103215 <begin_trans>
    iput(ff.ip);
80101080:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101083:	89 04 24             	mov    %eax,(%esp)
80101086:	e8 88 09 00 00       	call   80101a13 <iput>
    commit_trans();
8010108b:	e8 ce 21 00 00       	call   8010325e <commit_trans>
  }
}
80101090:	c9                   	leave  
80101091:	c3                   	ret    

80101092 <filestat>:

// Get metadata about file f.
int
filestat(struct file *f, struct stat *st)
{
80101092:	55                   	push   %ebp
80101093:	89 e5                	mov    %esp,%ebp
80101095:	83 ec 18             	sub    $0x18,%esp
  if(f->type == FD_INODE){
80101098:	8b 45 08             	mov    0x8(%ebp),%eax
8010109b:	8b 00                	mov    (%eax),%eax
8010109d:	83 f8 02             	cmp    $0x2,%eax
801010a0:	75 38                	jne    801010da <filestat+0x48>
    ilock(f->ip);
801010a2:	8b 45 08             	mov    0x8(%ebp),%eax
801010a5:	8b 40 10             	mov    0x10(%eax),%eax
801010a8:	89 04 24             	mov    %eax,(%esp)
801010ab:	e8 b0 07 00 00       	call   80101860 <ilock>
    stati(f->ip, st);
801010b0:	8b 45 08             	mov    0x8(%ebp),%eax
801010b3:	8b 40 10             	mov    0x10(%eax),%eax
801010b6:	8b 55 0c             	mov    0xc(%ebp),%edx
801010b9:	89 54 24 04          	mov    %edx,0x4(%esp)
801010bd:	89 04 24             	mov    %eax,(%esp)
801010c0:	e8 4c 0c 00 00       	call   80101d11 <stati>
    iunlock(f->ip);
801010c5:	8b 45 08             	mov    0x8(%ebp),%eax
801010c8:	8b 40 10             	mov    0x10(%eax),%eax
801010cb:	89 04 24             	mov    %eax,(%esp)
801010ce:	e8 db 08 00 00       	call   801019ae <iunlock>
    return 0;
801010d3:	b8 00 00 00 00       	mov    $0x0,%eax
801010d8:	eb 05                	jmp    801010df <filestat+0x4d>
  }
  return -1;
801010da:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801010df:	c9                   	leave  
801010e0:	c3                   	ret    

801010e1 <fileread>:

// Read from file f.
int
fileread(struct file *f, char *addr, int n)
{
801010e1:	55                   	push   %ebp
801010e2:	89 e5                	mov    %esp,%ebp
801010e4:	83 ec 28             	sub    $0x28,%esp
  int r;

  if(f->readable == 0)
801010e7:	8b 45 08             	mov    0x8(%ebp),%eax
801010ea:	0f b6 40 08          	movzbl 0x8(%eax),%eax
801010ee:	84 c0                	test   %al,%al
801010f0:	75 0a                	jne    801010fc <fileread+0x1b>
    return -1;
801010f2:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801010f7:	e9 9f 00 00 00       	jmp    8010119b <fileread+0xba>
  if(f->type == FD_PIPE)
801010fc:	8b 45 08             	mov    0x8(%ebp),%eax
801010ff:	8b 00                	mov    (%eax),%eax
80101101:	83 f8 01             	cmp    $0x1,%eax
80101104:	75 1e                	jne    80101124 <fileread+0x43>
    return piperead(f->pipe, addr, n);
80101106:	8b 45 08             	mov    0x8(%ebp),%eax
80101109:	8b 40 0c             	mov    0xc(%eax),%eax
8010110c:	8b 55 10             	mov    0x10(%ebp),%edx
8010110f:	89 54 24 08          	mov    %edx,0x8(%esp)
80101113:	8b 55 0c             	mov    0xc(%ebp),%edx
80101116:	89 54 24 04          	mov    %edx,0x4(%esp)
8010111a:	89 04 24             	mov    %eax,(%esp)
8010111d:	e8 c7 2d 00 00       	call   80103ee9 <piperead>
80101122:	eb 77                	jmp    8010119b <fileread+0xba>
  if(f->type == FD_INODE){
80101124:	8b 45 08             	mov    0x8(%ebp),%eax
80101127:	8b 00                	mov    (%eax),%eax
80101129:	83 f8 02             	cmp    $0x2,%eax
8010112c:	75 61                	jne    8010118f <fileread+0xae>
    ilock(f->ip);
8010112e:	8b 45 08             	mov    0x8(%ebp),%eax
80101131:	8b 40 10             	mov    0x10(%eax),%eax
80101134:	89 04 24             	mov    %eax,(%esp)
80101137:	e8 24 07 00 00       	call   80101860 <ilock>
    if((r = readi(f->ip, addr, f->off, n)) > 0)
8010113c:	8b 4d 10             	mov    0x10(%ebp),%ecx
8010113f:	8b 45 08             	mov    0x8(%ebp),%eax
80101142:	8b 50 14             	mov    0x14(%eax),%edx
80101145:	8b 45 08             	mov    0x8(%ebp),%eax
80101148:	8b 40 10             	mov    0x10(%eax),%eax
8010114b:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
8010114f:	89 54 24 08          	mov    %edx,0x8(%esp)
80101153:	8b 55 0c             	mov    0xc(%ebp),%edx
80101156:	89 54 24 04          	mov    %edx,0x4(%esp)
8010115a:	89 04 24             	mov    %eax,(%esp)
8010115d:	e8 f4 0b 00 00       	call   80101d56 <readi>
80101162:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101165:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101169:	7e 11                	jle    8010117c <fileread+0x9b>
      f->off += r;
8010116b:	8b 45 08             	mov    0x8(%ebp),%eax
8010116e:	8b 50 14             	mov    0x14(%eax),%edx
80101171:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101174:	01 c2                	add    %eax,%edx
80101176:	8b 45 08             	mov    0x8(%ebp),%eax
80101179:	89 50 14             	mov    %edx,0x14(%eax)
    iunlock(f->ip);
8010117c:	8b 45 08             	mov    0x8(%ebp),%eax
8010117f:	8b 40 10             	mov    0x10(%eax),%eax
80101182:	89 04 24             	mov    %eax,(%esp)
80101185:	e8 24 08 00 00       	call   801019ae <iunlock>
    return r;
8010118a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010118d:	eb 0c                	jmp    8010119b <fileread+0xba>
  }
  panic("fileread");
8010118f:	c7 04 24 1e 87 10 80 	movl   $0x8010871e,(%esp)
80101196:	e8 a2 f3 ff ff       	call   8010053d <panic>
}
8010119b:	c9                   	leave  
8010119c:	c3                   	ret    

8010119d <filewrite>:

//PAGEBREAK!
// Write to file f.
int
filewrite(struct file *f, char *addr, int n)
{
8010119d:	55                   	push   %ebp
8010119e:	89 e5                	mov    %esp,%ebp
801011a0:	53                   	push   %ebx
801011a1:	83 ec 24             	sub    $0x24,%esp
  int r;

  if(f->writable == 0)
801011a4:	8b 45 08             	mov    0x8(%ebp),%eax
801011a7:	0f b6 40 09          	movzbl 0x9(%eax),%eax
801011ab:	84 c0                	test   %al,%al
801011ad:	75 0a                	jne    801011b9 <filewrite+0x1c>
    return -1;
801011af:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801011b4:	e9 23 01 00 00       	jmp    801012dc <filewrite+0x13f>
  if(f->type == FD_PIPE)
801011b9:	8b 45 08             	mov    0x8(%ebp),%eax
801011bc:	8b 00                	mov    (%eax),%eax
801011be:	83 f8 01             	cmp    $0x1,%eax
801011c1:	75 21                	jne    801011e4 <filewrite+0x47>
    return pipewrite(f->pipe, addr, n);
801011c3:	8b 45 08             	mov    0x8(%ebp),%eax
801011c6:	8b 40 0c             	mov    0xc(%eax),%eax
801011c9:	8b 55 10             	mov    0x10(%ebp),%edx
801011cc:	89 54 24 08          	mov    %edx,0x8(%esp)
801011d0:	8b 55 0c             	mov    0xc(%ebp),%edx
801011d3:	89 54 24 04          	mov    %edx,0x4(%esp)
801011d7:	89 04 24             	mov    %eax,(%esp)
801011da:	e8 1a 2c 00 00       	call   80103df9 <pipewrite>
801011df:	e9 f8 00 00 00       	jmp    801012dc <filewrite+0x13f>
  if(f->type == FD_INODE){
801011e4:	8b 45 08             	mov    0x8(%ebp),%eax
801011e7:	8b 00                	mov    (%eax),%eax
801011e9:	83 f8 02             	cmp    $0x2,%eax
801011ec:	0f 85 de 00 00 00    	jne    801012d0 <filewrite+0x133>
    // the maximum log transaction size, including
    // i-node, indirect block, allocation blocks,
    // and 2 blocks of slop for non-aligned writes.
    // this really belongs lower down, since writei()
    // might be writing a device like the console.
    int max = ((LOGSIZE-1-1-2) / 2) * 512;
801011f2:	c7 45 ec 00 06 00 00 	movl   $0x600,-0x14(%ebp)
    int i = 0;
801011f9:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
    while(i < n){
80101200:	e9 a8 00 00 00       	jmp    801012ad <filewrite+0x110>
      int n1 = n - i;
80101205:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101208:	8b 55 10             	mov    0x10(%ebp),%edx
8010120b:	89 d1                	mov    %edx,%ecx
8010120d:	29 c1                	sub    %eax,%ecx
8010120f:	89 c8                	mov    %ecx,%eax
80101211:	89 45 f0             	mov    %eax,-0x10(%ebp)
      if(n1 > max)
80101214:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101217:	3b 45 ec             	cmp    -0x14(%ebp),%eax
8010121a:	7e 06                	jle    80101222 <filewrite+0x85>
        n1 = max;
8010121c:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010121f:	89 45 f0             	mov    %eax,-0x10(%ebp)

      begin_trans();
80101222:	e8 ee 1f 00 00       	call   80103215 <begin_trans>
      ilock(f->ip);
80101227:	8b 45 08             	mov    0x8(%ebp),%eax
8010122a:	8b 40 10             	mov    0x10(%eax),%eax
8010122d:	89 04 24             	mov    %eax,(%esp)
80101230:	e8 2b 06 00 00       	call   80101860 <ilock>
      if ((r = writei(f->ip, addr + i, f->off, n1)) > 0)
80101235:	8b 5d f0             	mov    -0x10(%ebp),%ebx
80101238:	8b 45 08             	mov    0x8(%ebp),%eax
8010123b:	8b 48 14             	mov    0x14(%eax),%ecx
8010123e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101241:	89 c2                	mov    %eax,%edx
80101243:	03 55 0c             	add    0xc(%ebp),%edx
80101246:	8b 45 08             	mov    0x8(%ebp),%eax
80101249:	8b 40 10             	mov    0x10(%eax),%eax
8010124c:	89 5c 24 0c          	mov    %ebx,0xc(%esp)
80101250:	89 4c 24 08          	mov    %ecx,0x8(%esp)
80101254:	89 54 24 04          	mov    %edx,0x4(%esp)
80101258:	89 04 24             	mov    %eax,(%esp)
8010125b:	e8 61 0c 00 00       	call   80101ec1 <writei>
80101260:	89 45 e8             	mov    %eax,-0x18(%ebp)
80101263:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
80101267:	7e 11                	jle    8010127a <filewrite+0xdd>
        f->off += r;
80101269:	8b 45 08             	mov    0x8(%ebp),%eax
8010126c:	8b 50 14             	mov    0x14(%eax),%edx
8010126f:	8b 45 e8             	mov    -0x18(%ebp),%eax
80101272:	01 c2                	add    %eax,%edx
80101274:	8b 45 08             	mov    0x8(%ebp),%eax
80101277:	89 50 14             	mov    %edx,0x14(%eax)
      iunlock(f->ip);
8010127a:	8b 45 08             	mov    0x8(%ebp),%eax
8010127d:	8b 40 10             	mov    0x10(%eax),%eax
80101280:	89 04 24             	mov    %eax,(%esp)
80101283:	e8 26 07 00 00       	call   801019ae <iunlock>
      commit_trans();
80101288:	e8 d1 1f 00 00       	call   8010325e <commit_trans>

      if(r < 0)
8010128d:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
80101291:	78 28                	js     801012bb <filewrite+0x11e>
        break;
      if(r != n1)
80101293:	8b 45 e8             	mov    -0x18(%ebp),%eax
80101296:	3b 45 f0             	cmp    -0x10(%ebp),%eax
80101299:	74 0c                	je     801012a7 <filewrite+0x10a>
        panic("short filewrite");
8010129b:	c7 04 24 27 87 10 80 	movl   $0x80108727,(%esp)
801012a2:	e8 96 f2 ff ff       	call   8010053d <panic>
      i += r;
801012a7:	8b 45 e8             	mov    -0x18(%ebp),%eax
801012aa:	01 45 f4             	add    %eax,-0xc(%ebp)
    // and 2 blocks of slop for non-aligned writes.
    // this really belongs lower down, since writei()
    // might be writing a device like the console.
    int max = ((LOGSIZE-1-1-2) / 2) * 512;
    int i = 0;
    while(i < n){
801012ad:	8b 45 f4             	mov    -0xc(%ebp),%eax
801012b0:	3b 45 10             	cmp    0x10(%ebp),%eax
801012b3:	0f 8c 4c ff ff ff    	jl     80101205 <filewrite+0x68>
801012b9:	eb 01                	jmp    801012bc <filewrite+0x11f>
        f->off += r;
      iunlock(f->ip);
      commit_trans();

      if(r < 0)
        break;
801012bb:	90                   	nop
      if(r != n1)
        panic("short filewrite");
      i += r;
    }
    return i == n ? n : -1;
801012bc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801012bf:	3b 45 10             	cmp    0x10(%ebp),%eax
801012c2:	75 05                	jne    801012c9 <filewrite+0x12c>
801012c4:	8b 45 10             	mov    0x10(%ebp),%eax
801012c7:	eb 05                	jmp    801012ce <filewrite+0x131>
801012c9:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801012ce:	eb 0c                	jmp    801012dc <filewrite+0x13f>
  }
  panic("filewrite");
801012d0:	c7 04 24 37 87 10 80 	movl   $0x80108737,(%esp)
801012d7:	e8 61 f2 ff ff       	call   8010053d <panic>
}
801012dc:	83 c4 24             	add    $0x24,%esp
801012df:	5b                   	pop    %ebx
801012e0:	5d                   	pop    %ebp
801012e1:	c3                   	ret    
	...

801012e4 <readsb>:
static void itrunc(struct inode*);

// Read the super block.
void
readsb(int dev, struct superblock *sb)
{
801012e4:	55                   	push   %ebp
801012e5:	89 e5                	mov    %esp,%ebp
801012e7:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  
  bp = bread(dev, 1);
801012ea:	8b 45 08             	mov    0x8(%ebp),%eax
801012ed:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
801012f4:	00 
801012f5:	89 04 24             	mov    %eax,(%esp)
801012f8:	e8 a9 ee ff ff       	call   801001a6 <bread>
801012fd:	89 45 f4             	mov    %eax,-0xc(%ebp)
  memmove(sb, bp->data, sizeof(*sb));
80101300:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101303:	83 c0 18             	add    $0x18,%eax
80101306:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
8010130d:	00 
8010130e:	89 44 24 04          	mov    %eax,0x4(%esp)
80101312:	8b 45 0c             	mov    0xc(%ebp),%eax
80101315:	89 04 24             	mov    %eax,(%esp)
80101318:	e8 58 40 00 00       	call   80105375 <memmove>
  brelse(bp);
8010131d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101320:	89 04 24             	mov    %eax,(%esp)
80101323:	e8 ef ee ff ff       	call   80100217 <brelse>
}
80101328:	c9                   	leave  
80101329:	c3                   	ret    

8010132a <bzero>:

// Zero a block.
static void
bzero(int dev, int bno)
{
8010132a:	55                   	push   %ebp
8010132b:	89 e5                	mov    %esp,%ebp
8010132d:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  
  bp = bread(dev, bno);
80101330:	8b 55 0c             	mov    0xc(%ebp),%edx
80101333:	8b 45 08             	mov    0x8(%ebp),%eax
80101336:	89 54 24 04          	mov    %edx,0x4(%esp)
8010133a:	89 04 24             	mov    %eax,(%esp)
8010133d:	e8 64 ee ff ff       	call   801001a6 <bread>
80101342:	89 45 f4             	mov    %eax,-0xc(%ebp)
  memset(bp->data, 0, BSIZE);
80101345:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101348:	83 c0 18             	add    $0x18,%eax
8010134b:	c7 44 24 08 00 02 00 	movl   $0x200,0x8(%esp)
80101352:	00 
80101353:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010135a:	00 
8010135b:	89 04 24             	mov    %eax,(%esp)
8010135e:	e8 3f 3f 00 00       	call   801052a2 <memset>
  log_write(bp);
80101363:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101366:	89 04 24             	mov    %eax,(%esp)
80101369:	e8 48 1f 00 00       	call   801032b6 <log_write>
  brelse(bp);
8010136e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101371:	89 04 24             	mov    %eax,(%esp)
80101374:	e8 9e ee ff ff       	call   80100217 <brelse>
}
80101379:	c9                   	leave  
8010137a:	c3                   	ret    

8010137b <balloc>:
// Blocks. 

// Allocate a zeroed disk block.
static uint
balloc(uint dev)
{
8010137b:	55                   	push   %ebp
8010137c:	89 e5                	mov    %esp,%ebp
8010137e:	53                   	push   %ebx
8010137f:	83 ec 34             	sub    $0x34,%esp
  int b, bi, m;
  struct buf *bp;
  struct superblock sb;

  bp = 0;
80101382:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
  readsb(dev, &sb);
80101389:	8b 45 08             	mov    0x8(%ebp),%eax
8010138c:	8d 55 d8             	lea    -0x28(%ebp),%edx
8010138f:	89 54 24 04          	mov    %edx,0x4(%esp)
80101393:	89 04 24             	mov    %eax,(%esp)
80101396:	e8 49 ff ff ff       	call   801012e4 <readsb>
  for(b = 0; b < sb.size; b += BPB){
8010139b:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801013a2:	e9 11 01 00 00       	jmp    801014b8 <balloc+0x13d>
    bp = bread(dev, BBLOCK(b, sb.ninodes));
801013a7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801013aa:	8d 90 ff 0f 00 00    	lea    0xfff(%eax),%edx
801013b0:	85 c0                	test   %eax,%eax
801013b2:	0f 48 c2             	cmovs  %edx,%eax
801013b5:	c1 f8 0c             	sar    $0xc,%eax
801013b8:	8b 55 e0             	mov    -0x20(%ebp),%edx
801013bb:	c1 ea 03             	shr    $0x3,%edx
801013be:	01 d0                	add    %edx,%eax
801013c0:	83 c0 03             	add    $0x3,%eax
801013c3:	89 44 24 04          	mov    %eax,0x4(%esp)
801013c7:	8b 45 08             	mov    0x8(%ebp),%eax
801013ca:	89 04 24             	mov    %eax,(%esp)
801013cd:	e8 d4 ed ff ff       	call   801001a6 <bread>
801013d2:	89 45 ec             	mov    %eax,-0x14(%ebp)
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
801013d5:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
801013dc:	e9 a7 00 00 00       	jmp    80101488 <balloc+0x10d>
      m = 1 << (bi % 8);
801013e1:	8b 45 f0             	mov    -0x10(%ebp),%eax
801013e4:	89 c2                	mov    %eax,%edx
801013e6:	c1 fa 1f             	sar    $0x1f,%edx
801013e9:	c1 ea 1d             	shr    $0x1d,%edx
801013ec:	01 d0                	add    %edx,%eax
801013ee:	83 e0 07             	and    $0x7,%eax
801013f1:	29 d0                	sub    %edx,%eax
801013f3:	ba 01 00 00 00       	mov    $0x1,%edx
801013f8:	89 d3                	mov    %edx,%ebx
801013fa:	89 c1                	mov    %eax,%ecx
801013fc:	d3 e3                	shl    %cl,%ebx
801013fe:	89 d8                	mov    %ebx,%eax
80101400:	89 45 e8             	mov    %eax,-0x18(%ebp)
      if((bp->data[bi/8] & m) == 0){  // Is block free?
80101403:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101406:	8d 50 07             	lea    0x7(%eax),%edx
80101409:	85 c0                	test   %eax,%eax
8010140b:	0f 48 c2             	cmovs  %edx,%eax
8010140e:	c1 f8 03             	sar    $0x3,%eax
80101411:	8b 55 ec             	mov    -0x14(%ebp),%edx
80101414:	0f b6 44 02 18       	movzbl 0x18(%edx,%eax,1),%eax
80101419:	0f b6 c0             	movzbl %al,%eax
8010141c:	23 45 e8             	and    -0x18(%ebp),%eax
8010141f:	85 c0                	test   %eax,%eax
80101421:	75 61                	jne    80101484 <balloc+0x109>
        bp->data[bi/8] |= m;  // Mark block in use.
80101423:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101426:	8d 50 07             	lea    0x7(%eax),%edx
80101429:	85 c0                	test   %eax,%eax
8010142b:	0f 48 c2             	cmovs  %edx,%eax
8010142e:	c1 f8 03             	sar    $0x3,%eax
80101431:	8b 55 ec             	mov    -0x14(%ebp),%edx
80101434:	0f b6 54 02 18       	movzbl 0x18(%edx,%eax,1),%edx
80101439:	89 d1                	mov    %edx,%ecx
8010143b:	8b 55 e8             	mov    -0x18(%ebp),%edx
8010143e:	09 ca                	or     %ecx,%edx
80101440:	89 d1                	mov    %edx,%ecx
80101442:	8b 55 ec             	mov    -0x14(%ebp),%edx
80101445:	88 4c 02 18          	mov    %cl,0x18(%edx,%eax,1)
        log_write(bp);
80101449:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010144c:	89 04 24             	mov    %eax,(%esp)
8010144f:	e8 62 1e 00 00       	call   801032b6 <log_write>
        brelse(bp);
80101454:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101457:	89 04 24             	mov    %eax,(%esp)
8010145a:	e8 b8 ed ff ff       	call   80100217 <brelse>
        bzero(dev, b + bi);
8010145f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101462:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101465:	01 c2                	add    %eax,%edx
80101467:	8b 45 08             	mov    0x8(%ebp),%eax
8010146a:	89 54 24 04          	mov    %edx,0x4(%esp)
8010146e:	89 04 24             	mov    %eax,(%esp)
80101471:	e8 b4 fe ff ff       	call   8010132a <bzero>
        return b + bi;
80101476:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101479:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010147c:	01 d0                	add    %edx,%eax
      }
    }
    brelse(bp);
  }
  panic("balloc: out of blocks");
}
8010147e:	83 c4 34             	add    $0x34,%esp
80101481:	5b                   	pop    %ebx
80101482:	5d                   	pop    %ebp
80101483:	c3                   	ret    

  bp = 0;
  readsb(dev, &sb);
  for(b = 0; b < sb.size; b += BPB){
    bp = bread(dev, BBLOCK(b, sb.ninodes));
    for(bi = 0; bi < BPB && b + bi < sb.size; bi++){
80101484:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
80101488:	81 7d f0 ff 0f 00 00 	cmpl   $0xfff,-0x10(%ebp)
8010148f:	7f 15                	jg     801014a6 <balloc+0x12b>
80101491:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101494:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101497:	01 d0                	add    %edx,%eax
80101499:	89 c2                	mov    %eax,%edx
8010149b:	8b 45 d8             	mov    -0x28(%ebp),%eax
8010149e:	39 c2                	cmp    %eax,%edx
801014a0:	0f 82 3b ff ff ff    	jb     801013e1 <balloc+0x66>
        brelse(bp);
        bzero(dev, b + bi);
        return b + bi;
      }
    }
    brelse(bp);
801014a6:	8b 45 ec             	mov    -0x14(%ebp),%eax
801014a9:	89 04 24             	mov    %eax,(%esp)
801014ac:	e8 66 ed ff ff       	call   80100217 <brelse>
  struct buf *bp;
  struct superblock sb;

  bp = 0;
  readsb(dev, &sb);
  for(b = 0; b < sb.size; b += BPB){
801014b1:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
801014b8:	8b 55 f4             	mov    -0xc(%ebp),%edx
801014bb:	8b 45 d8             	mov    -0x28(%ebp),%eax
801014be:	39 c2                	cmp    %eax,%edx
801014c0:	0f 82 e1 fe ff ff    	jb     801013a7 <balloc+0x2c>
        return b + bi;
      }
    }
    brelse(bp);
  }
  panic("balloc: out of blocks");
801014c6:	c7 04 24 41 87 10 80 	movl   $0x80108741,(%esp)
801014cd:	e8 6b f0 ff ff       	call   8010053d <panic>

801014d2 <bfree>:
}

// Free a disk block.
static void
bfree(int dev, uint b)
{
801014d2:	55                   	push   %ebp
801014d3:	89 e5                	mov    %esp,%ebp
801014d5:	53                   	push   %ebx
801014d6:	83 ec 34             	sub    $0x34,%esp
  struct buf *bp;
  struct superblock sb;
  int bi, m;

  readsb(dev, &sb);
801014d9:	8d 45 dc             	lea    -0x24(%ebp),%eax
801014dc:	89 44 24 04          	mov    %eax,0x4(%esp)
801014e0:	8b 45 08             	mov    0x8(%ebp),%eax
801014e3:	89 04 24             	mov    %eax,(%esp)
801014e6:	e8 f9 fd ff ff       	call   801012e4 <readsb>
  bp = bread(dev, BBLOCK(b, sb.ninodes));
801014eb:	8b 45 0c             	mov    0xc(%ebp),%eax
801014ee:	89 c2                	mov    %eax,%edx
801014f0:	c1 ea 0c             	shr    $0xc,%edx
801014f3:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801014f6:	c1 e8 03             	shr    $0x3,%eax
801014f9:	01 d0                	add    %edx,%eax
801014fb:	8d 50 03             	lea    0x3(%eax),%edx
801014fe:	8b 45 08             	mov    0x8(%ebp),%eax
80101501:	89 54 24 04          	mov    %edx,0x4(%esp)
80101505:	89 04 24             	mov    %eax,(%esp)
80101508:	e8 99 ec ff ff       	call   801001a6 <bread>
8010150d:	89 45 f4             	mov    %eax,-0xc(%ebp)
  bi = b % BPB;
80101510:	8b 45 0c             	mov    0xc(%ebp),%eax
80101513:	25 ff 0f 00 00       	and    $0xfff,%eax
80101518:	89 45 f0             	mov    %eax,-0x10(%ebp)
  m = 1 << (bi % 8);
8010151b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010151e:	89 c2                	mov    %eax,%edx
80101520:	c1 fa 1f             	sar    $0x1f,%edx
80101523:	c1 ea 1d             	shr    $0x1d,%edx
80101526:	01 d0                	add    %edx,%eax
80101528:	83 e0 07             	and    $0x7,%eax
8010152b:	29 d0                	sub    %edx,%eax
8010152d:	ba 01 00 00 00       	mov    $0x1,%edx
80101532:	89 d3                	mov    %edx,%ebx
80101534:	89 c1                	mov    %eax,%ecx
80101536:	d3 e3                	shl    %cl,%ebx
80101538:	89 d8                	mov    %ebx,%eax
8010153a:	89 45 ec             	mov    %eax,-0x14(%ebp)
  if((bp->data[bi/8] & m) == 0)
8010153d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101540:	8d 50 07             	lea    0x7(%eax),%edx
80101543:	85 c0                	test   %eax,%eax
80101545:	0f 48 c2             	cmovs  %edx,%eax
80101548:	c1 f8 03             	sar    $0x3,%eax
8010154b:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010154e:	0f b6 44 02 18       	movzbl 0x18(%edx,%eax,1),%eax
80101553:	0f b6 c0             	movzbl %al,%eax
80101556:	23 45 ec             	and    -0x14(%ebp),%eax
80101559:	85 c0                	test   %eax,%eax
8010155b:	75 0c                	jne    80101569 <bfree+0x97>
    panic("freeing free block");
8010155d:	c7 04 24 57 87 10 80 	movl   $0x80108757,(%esp)
80101564:	e8 d4 ef ff ff       	call   8010053d <panic>
  bp->data[bi/8] &= ~m;
80101569:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010156c:	8d 50 07             	lea    0x7(%eax),%edx
8010156f:	85 c0                	test   %eax,%eax
80101571:	0f 48 c2             	cmovs  %edx,%eax
80101574:	c1 f8 03             	sar    $0x3,%eax
80101577:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010157a:	0f b6 54 02 18       	movzbl 0x18(%edx,%eax,1),%edx
8010157f:	8b 4d ec             	mov    -0x14(%ebp),%ecx
80101582:	f7 d1                	not    %ecx
80101584:	21 ca                	and    %ecx,%edx
80101586:	89 d1                	mov    %edx,%ecx
80101588:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010158b:	88 4c 02 18          	mov    %cl,0x18(%edx,%eax,1)
  log_write(bp);
8010158f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101592:	89 04 24             	mov    %eax,(%esp)
80101595:	e8 1c 1d 00 00       	call   801032b6 <log_write>
  brelse(bp);
8010159a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010159d:	89 04 24             	mov    %eax,(%esp)
801015a0:	e8 72 ec ff ff       	call   80100217 <brelse>
}
801015a5:	83 c4 34             	add    $0x34,%esp
801015a8:	5b                   	pop    %ebx
801015a9:	5d                   	pop    %ebp
801015aa:	c3                   	ret    

801015ab <iinit>:
  struct inode inode[NINODE];
} icache;

void
iinit(void)
{
801015ab:	55                   	push   %ebp
801015ac:	89 e5                	mov    %esp,%ebp
801015ae:	83 ec 18             	sub    $0x18,%esp
  initlock(&icache.lock, "icache");
801015b1:	c7 44 24 04 6a 87 10 	movl   $0x8010876a,0x4(%esp)
801015b8:	80 
801015b9:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
801015c0:	e8 6d 3a 00 00       	call   80105032 <initlock>
}
801015c5:	c9                   	leave  
801015c6:	c3                   	ret    

801015c7 <ialloc>:
//PAGEBREAK!
// Allocate a new inode with the given type on device dev.
// A free inode has a type of zero.
struct inode*
ialloc(uint dev, short type)
{
801015c7:	55                   	push   %ebp
801015c8:	89 e5                	mov    %esp,%ebp
801015ca:	83 ec 48             	sub    $0x48,%esp
801015cd:	8b 45 0c             	mov    0xc(%ebp),%eax
801015d0:	66 89 45 d4          	mov    %ax,-0x2c(%ebp)
  int inum;
  struct buf *bp;
  struct dinode *dip;
  struct superblock sb;

  readsb(dev, &sb);
801015d4:	8b 45 08             	mov    0x8(%ebp),%eax
801015d7:	8d 55 dc             	lea    -0x24(%ebp),%edx
801015da:	89 54 24 04          	mov    %edx,0x4(%esp)
801015de:	89 04 24             	mov    %eax,(%esp)
801015e1:	e8 fe fc ff ff       	call   801012e4 <readsb>

  for(inum = 1; inum < sb.ninodes; inum++){
801015e6:	c7 45 f4 01 00 00 00 	movl   $0x1,-0xc(%ebp)
801015ed:	e9 98 00 00 00       	jmp    8010168a <ialloc+0xc3>
    bp = bread(dev, IBLOCK(inum));
801015f2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801015f5:	c1 e8 03             	shr    $0x3,%eax
801015f8:	83 c0 02             	add    $0x2,%eax
801015fb:	89 44 24 04          	mov    %eax,0x4(%esp)
801015ff:	8b 45 08             	mov    0x8(%ebp),%eax
80101602:	89 04 24             	mov    %eax,(%esp)
80101605:	e8 9c eb ff ff       	call   801001a6 <bread>
8010160a:	89 45 f0             	mov    %eax,-0x10(%ebp)
    dip = (struct dinode*)bp->data + inum%IPB;
8010160d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101610:	8d 50 18             	lea    0x18(%eax),%edx
80101613:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101616:	83 e0 07             	and    $0x7,%eax
80101619:	c1 e0 06             	shl    $0x6,%eax
8010161c:	01 d0                	add    %edx,%eax
8010161e:	89 45 ec             	mov    %eax,-0x14(%ebp)
    if(dip->type == 0){  // a free inode
80101621:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101624:	0f b7 00             	movzwl (%eax),%eax
80101627:	66 85 c0             	test   %ax,%ax
8010162a:	75 4f                	jne    8010167b <ialloc+0xb4>
      memset(dip, 0, sizeof(*dip));
8010162c:	c7 44 24 08 40 00 00 	movl   $0x40,0x8(%esp)
80101633:	00 
80101634:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
8010163b:	00 
8010163c:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010163f:	89 04 24             	mov    %eax,(%esp)
80101642:	e8 5b 3c 00 00       	call   801052a2 <memset>
      dip->type = type;
80101647:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010164a:	0f b7 55 d4          	movzwl -0x2c(%ebp),%edx
8010164e:	66 89 10             	mov    %dx,(%eax)
      log_write(bp);   // mark it allocated on the disk
80101651:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101654:	89 04 24             	mov    %eax,(%esp)
80101657:	e8 5a 1c 00 00       	call   801032b6 <log_write>
      brelse(bp);
8010165c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010165f:	89 04 24             	mov    %eax,(%esp)
80101662:	e8 b0 eb ff ff       	call   80100217 <brelse>
      return iget(dev, inum);
80101667:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010166a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010166e:	8b 45 08             	mov    0x8(%ebp),%eax
80101671:	89 04 24             	mov    %eax,(%esp)
80101674:	e8 e3 00 00 00       	call   8010175c <iget>
    }
    brelse(bp);
  }
  panic("ialloc: no inodes");
}
80101679:	c9                   	leave  
8010167a:	c3                   	ret    
      dip->type = type;
      log_write(bp);   // mark it allocated on the disk
      brelse(bp);
      return iget(dev, inum);
    }
    brelse(bp);
8010167b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010167e:	89 04 24             	mov    %eax,(%esp)
80101681:	e8 91 eb ff ff       	call   80100217 <brelse>
  struct dinode *dip;
  struct superblock sb;

  readsb(dev, &sb);

  for(inum = 1; inum < sb.ninodes; inum++){
80101686:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
8010168a:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010168d:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80101690:	39 c2                	cmp    %eax,%edx
80101692:	0f 82 5a ff ff ff    	jb     801015f2 <ialloc+0x2b>
      brelse(bp);
      return iget(dev, inum);
    }
    brelse(bp);
  }
  panic("ialloc: no inodes");
80101698:	c7 04 24 71 87 10 80 	movl   $0x80108771,(%esp)
8010169f:	e8 99 ee ff ff       	call   8010053d <panic>

801016a4 <iupdate>:
}

// Copy a modified in-memory inode to disk.
void
iupdate(struct inode *ip)
{
801016a4:	55                   	push   %ebp
801016a5:	89 e5                	mov    %esp,%ebp
801016a7:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  struct dinode *dip;

  bp = bread(ip->dev, IBLOCK(ip->inum));
801016aa:	8b 45 08             	mov    0x8(%ebp),%eax
801016ad:	8b 40 04             	mov    0x4(%eax),%eax
801016b0:	c1 e8 03             	shr    $0x3,%eax
801016b3:	8d 50 02             	lea    0x2(%eax),%edx
801016b6:	8b 45 08             	mov    0x8(%ebp),%eax
801016b9:	8b 00                	mov    (%eax),%eax
801016bb:	89 54 24 04          	mov    %edx,0x4(%esp)
801016bf:	89 04 24             	mov    %eax,(%esp)
801016c2:	e8 df ea ff ff       	call   801001a6 <bread>
801016c7:	89 45 f4             	mov    %eax,-0xc(%ebp)
  dip = (struct dinode*)bp->data + ip->inum%IPB;
801016ca:	8b 45 f4             	mov    -0xc(%ebp),%eax
801016cd:	8d 50 18             	lea    0x18(%eax),%edx
801016d0:	8b 45 08             	mov    0x8(%ebp),%eax
801016d3:	8b 40 04             	mov    0x4(%eax),%eax
801016d6:	83 e0 07             	and    $0x7,%eax
801016d9:	c1 e0 06             	shl    $0x6,%eax
801016dc:	01 d0                	add    %edx,%eax
801016de:	89 45 f0             	mov    %eax,-0x10(%ebp)
  dip->type = ip->type;
801016e1:	8b 45 08             	mov    0x8(%ebp),%eax
801016e4:	0f b7 50 10          	movzwl 0x10(%eax),%edx
801016e8:	8b 45 f0             	mov    -0x10(%ebp),%eax
801016eb:	66 89 10             	mov    %dx,(%eax)
  dip->major = ip->major;
801016ee:	8b 45 08             	mov    0x8(%ebp),%eax
801016f1:	0f b7 50 12          	movzwl 0x12(%eax),%edx
801016f5:	8b 45 f0             	mov    -0x10(%ebp),%eax
801016f8:	66 89 50 02          	mov    %dx,0x2(%eax)
  dip->minor = ip->minor;
801016fc:	8b 45 08             	mov    0x8(%ebp),%eax
801016ff:	0f b7 50 14          	movzwl 0x14(%eax),%edx
80101703:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101706:	66 89 50 04          	mov    %dx,0x4(%eax)
  dip->nlink = ip->nlink;
8010170a:	8b 45 08             	mov    0x8(%ebp),%eax
8010170d:	0f b7 50 16          	movzwl 0x16(%eax),%edx
80101711:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101714:	66 89 50 06          	mov    %dx,0x6(%eax)
  dip->size = ip->size;
80101718:	8b 45 08             	mov    0x8(%ebp),%eax
8010171b:	8b 50 18             	mov    0x18(%eax),%edx
8010171e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101721:	89 50 08             	mov    %edx,0x8(%eax)
  memmove(dip->addrs, ip->addrs, sizeof(ip->addrs));
80101724:	8b 45 08             	mov    0x8(%ebp),%eax
80101727:	8d 50 1c             	lea    0x1c(%eax),%edx
8010172a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010172d:	83 c0 0c             	add    $0xc,%eax
80101730:	c7 44 24 08 34 00 00 	movl   $0x34,0x8(%esp)
80101737:	00 
80101738:	89 54 24 04          	mov    %edx,0x4(%esp)
8010173c:	89 04 24             	mov    %eax,(%esp)
8010173f:	e8 31 3c 00 00       	call   80105375 <memmove>
  log_write(bp);
80101744:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101747:	89 04 24             	mov    %eax,(%esp)
8010174a:	e8 67 1b 00 00       	call   801032b6 <log_write>
  brelse(bp);
8010174f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101752:	89 04 24             	mov    %eax,(%esp)
80101755:	e8 bd ea ff ff       	call   80100217 <brelse>
}
8010175a:	c9                   	leave  
8010175b:	c3                   	ret    

8010175c <iget>:
// Find the inode with number inum on device dev
// and return the in-memory copy. Does not lock
// the inode and does not read it from disk.
static struct inode*
iget(uint dev, uint inum)
{
8010175c:	55                   	push   %ebp
8010175d:	89 e5                	mov    %esp,%ebp
8010175f:	83 ec 28             	sub    $0x28,%esp
  struct inode *ip, *empty;

  acquire(&icache.lock);
80101762:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
80101769:	e8 e5 38 00 00       	call   80105053 <acquire>

  // Is the inode already cached?
  empty = 0;
8010176e:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
80101775:	c7 45 f4 94 e8 10 80 	movl   $0x8010e894,-0xc(%ebp)
8010177c:	eb 59                	jmp    801017d7 <iget+0x7b>
    if(ip->ref > 0 && ip->dev == dev && ip->inum == inum){
8010177e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101781:	8b 40 08             	mov    0x8(%eax),%eax
80101784:	85 c0                	test   %eax,%eax
80101786:	7e 35                	jle    801017bd <iget+0x61>
80101788:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010178b:	8b 00                	mov    (%eax),%eax
8010178d:	3b 45 08             	cmp    0x8(%ebp),%eax
80101790:	75 2b                	jne    801017bd <iget+0x61>
80101792:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101795:	8b 40 04             	mov    0x4(%eax),%eax
80101798:	3b 45 0c             	cmp    0xc(%ebp),%eax
8010179b:	75 20                	jne    801017bd <iget+0x61>
      ip->ref++;
8010179d:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017a0:	8b 40 08             	mov    0x8(%eax),%eax
801017a3:	8d 50 01             	lea    0x1(%eax),%edx
801017a6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017a9:	89 50 08             	mov    %edx,0x8(%eax)
      release(&icache.lock);
801017ac:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
801017b3:	e8 fd 38 00 00       	call   801050b5 <release>
      return ip;
801017b8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017bb:	eb 6f                	jmp    8010182c <iget+0xd0>
    }
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
801017bd:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801017c1:	75 10                	jne    801017d3 <iget+0x77>
801017c3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017c6:	8b 40 08             	mov    0x8(%eax),%eax
801017c9:	85 c0                	test   %eax,%eax
801017cb:	75 06                	jne    801017d3 <iget+0x77>
      empty = ip;
801017cd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017d0:	89 45 f0             	mov    %eax,-0x10(%ebp)

  acquire(&icache.lock);

  // Is the inode already cached?
  empty = 0;
  for(ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++){
801017d3:	83 45 f4 50          	addl   $0x50,-0xc(%ebp)
801017d7:	81 7d f4 34 f8 10 80 	cmpl   $0x8010f834,-0xc(%ebp)
801017de:	72 9e                	jb     8010177e <iget+0x22>
    if(empty == 0 && ip->ref == 0)    // Remember empty slot.
      empty = ip;
  }

  // Recycle an inode cache entry.
  if(empty == 0)
801017e0:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801017e4:	75 0c                	jne    801017f2 <iget+0x96>
    panic("iget: no inodes");
801017e6:	c7 04 24 83 87 10 80 	movl   $0x80108783,(%esp)
801017ed:	e8 4b ed ff ff       	call   8010053d <panic>

  ip = empty;
801017f2:	8b 45 f0             	mov    -0x10(%ebp),%eax
801017f5:	89 45 f4             	mov    %eax,-0xc(%ebp)
  ip->dev = dev;
801017f8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801017fb:	8b 55 08             	mov    0x8(%ebp),%edx
801017fe:	89 10                	mov    %edx,(%eax)
  ip->inum = inum;
80101800:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101803:	8b 55 0c             	mov    0xc(%ebp),%edx
80101806:	89 50 04             	mov    %edx,0x4(%eax)
  ip->ref = 1;
80101809:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010180c:	c7 40 08 01 00 00 00 	movl   $0x1,0x8(%eax)
  ip->flags = 0;
80101813:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101816:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
  release(&icache.lock);
8010181d:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
80101824:	e8 8c 38 00 00       	call   801050b5 <release>

  return ip;
80101829:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
8010182c:	c9                   	leave  
8010182d:	c3                   	ret    

8010182e <idup>:

// Increment reference count for ip.
// Returns ip to enable ip = idup(ip1) idiom.
struct inode*
idup(struct inode *ip)
{
8010182e:	55                   	push   %ebp
8010182f:	89 e5                	mov    %esp,%ebp
80101831:	83 ec 18             	sub    $0x18,%esp
  acquire(&icache.lock);
80101834:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
8010183b:	e8 13 38 00 00       	call   80105053 <acquire>
  ip->ref++;
80101840:	8b 45 08             	mov    0x8(%ebp),%eax
80101843:	8b 40 08             	mov    0x8(%eax),%eax
80101846:	8d 50 01             	lea    0x1(%eax),%edx
80101849:	8b 45 08             	mov    0x8(%ebp),%eax
8010184c:	89 50 08             	mov    %edx,0x8(%eax)
  release(&icache.lock);
8010184f:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
80101856:	e8 5a 38 00 00       	call   801050b5 <release>
  return ip;
8010185b:	8b 45 08             	mov    0x8(%ebp),%eax
}
8010185e:	c9                   	leave  
8010185f:	c3                   	ret    

80101860 <ilock>:

// Lock the given inode.
// Reads the inode from disk if necessary.
void
ilock(struct inode *ip)
{
80101860:	55                   	push   %ebp
80101861:	89 e5                	mov    %esp,%ebp
80101863:	83 ec 28             	sub    $0x28,%esp
  struct buf *bp;
  struct dinode *dip;

  if(ip == 0 || ip->ref < 1)
80101866:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
8010186a:	74 0a                	je     80101876 <ilock+0x16>
8010186c:	8b 45 08             	mov    0x8(%ebp),%eax
8010186f:	8b 40 08             	mov    0x8(%eax),%eax
80101872:	85 c0                	test   %eax,%eax
80101874:	7f 0c                	jg     80101882 <ilock+0x22>
    panic("ilock");
80101876:	c7 04 24 93 87 10 80 	movl   $0x80108793,(%esp)
8010187d:	e8 bb ec ff ff       	call   8010053d <panic>

  acquire(&icache.lock);
80101882:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
80101889:	e8 c5 37 00 00       	call   80105053 <acquire>
  while(ip->flags & I_BUSY)
8010188e:	eb 13                	jmp    801018a3 <ilock+0x43>
    sleep(ip, &icache.lock);
80101890:	c7 44 24 04 60 e8 10 	movl   $0x8010e860,0x4(%esp)
80101897:	80 
80101898:	8b 45 08             	mov    0x8(%ebp),%eax
8010189b:	89 04 24             	mov    %eax,(%esp)
8010189e:	e8 6f 2f 00 00       	call   80104812 <sleep>

  if(ip == 0 || ip->ref < 1)
    panic("ilock");

  acquire(&icache.lock);
  while(ip->flags & I_BUSY)
801018a3:	8b 45 08             	mov    0x8(%ebp),%eax
801018a6:	8b 40 0c             	mov    0xc(%eax),%eax
801018a9:	83 e0 01             	and    $0x1,%eax
801018ac:	84 c0                	test   %al,%al
801018ae:	75 e0                	jne    80101890 <ilock+0x30>
    sleep(ip, &icache.lock);
  ip->flags |= I_BUSY;
801018b0:	8b 45 08             	mov    0x8(%ebp),%eax
801018b3:	8b 40 0c             	mov    0xc(%eax),%eax
801018b6:	89 c2                	mov    %eax,%edx
801018b8:	83 ca 01             	or     $0x1,%edx
801018bb:	8b 45 08             	mov    0x8(%ebp),%eax
801018be:	89 50 0c             	mov    %edx,0xc(%eax)
  release(&icache.lock);
801018c1:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
801018c8:	e8 e8 37 00 00       	call   801050b5 <release>

  if(!(ip->flags & I_VALID)){
801018cd:	8b 45 08             	mov    0x8(%ebp),%eax
801018d0:	8b 40 0c             	mov    0xc(%eax),%eax
801018d3:	83 e0 02             	and    $0x2,%eax
801018d6:	85 c0                	test   %eax,%eax
801018d8:	0f 85 ce 00 00 00    	jne    801019ac <ilock+0x14c>
    bp = bread(ip->dev, IBLOCK(ip->inum));
801018de:	8b 45 08             	mov    0x8(%ebp),%eax
801018e1:	8b 40 04             	mov    0x4(%eax),%eax
801018e4:	c1 e8 03             	shr    $0x3,%eax
801018e7:	8d 50 02             	lea    0x2(%eax),%edx
801018ea:	8b 45 08             	mov    0x8(%ebp),%eax
801018ed:	8b 00                	mov    (%eax),%eax
801018ef:	89 54 24 04          	mov    %edx,0x4(%esp)
801018f3:	89 04 24             	mov    %eax,(%esp)
801018f6:	e8 ab e8 ff ff       	call   801001a6 <bread>
801018fb:	89 45 f4             	mov    %eax,-0xc(%ebp)
    dip = (struct dinode*)bp->data + ip->inum%IPB;
801018fe:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101901:	8d 50 18             	lea    0x18(%eax),%edx
80101904:	8b 45 08             	mov    0x8(%ebp),%eax
80101907:	8b 40 04             	mov    0x4(%eax),%eax
8010190a:	83 e0 07             	and    $0x7,%eax
8010190d:	c1 e0 06             	shl    $0x6,%eax
80101910:	01 d0                	add    %edx,%eax
80101912:	89 45 f0             	mov    %eax,-0x10(%ebp)
    ip->type = dip->type;
80101915:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101918:	0f b7 10             	movzwl (%eax),%edx
8010191b:	8b 45 08             	mov    0x8(%ebp),%eax
8010191e:	66 89 50 10          	mov    %dx,0x10(%eax)
    ip->major = dip->major;
80101922:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101925:	0f b7 50 02          	movzwl 0x2(%eax),%edx
80101929:	8b 45 08             	mov    0x8(%ebp),%eax
8010192c:	66 89 50 12          	mov    %dx,0x12(%eax)
    ip->minor = dip->minor;
80101930:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101933:	0f b7 50 04          	movzwl 0x4(%eax),%edx
80101937:	8b 45 08             	mov    0x8(%ebp),%eax
8010193a:	66 89 50 14          	mov    %dx,0x14(%eax)
    ip->nlink = dip->nlink;
8010193e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101941:	0f b7 50 06          	movzwl 0x6(%eax),%edx
80101945:	8b 45 08             	mov    0x8(%ebp),%eax
80101948:	66 89 50 16          	mov    %dx,0x16(%eax)
    ip->size = dip->size;
8010194c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010194f:	8b 50 08             	mov    0x8(%eax),%edx
80101952:	8b 45 08             	mov    0x8(%ebp),%eax
80101955:	89 50 18             	mov    %edx,0x18(%eax)
    memmove(ip->addrs, dip->addrs, sizeof(ip->addrs));
80101958:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010195b:	8d 50 0c             	lea    0xc(%eax),%edx
8010195e:	8b 45 08             	mov    0x8(%ebp),%eax
80101961:	83 c0 1c             	add    $0x1c,%eax
80101964:	c7 44 24 08 34 00 00 	movl   $0x34,0x8(%esp)
8010196b:	00 
8010196c:	89 54 24 04          	mov    %edx,0x4(%esp)
80101970:	89 04 24             	mov    %eax,(%esp)
80101973:	e8 fd 39 00 00       	call   80105375 <memmove>
    brelse(bp);
80101978:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010197b:	89 04 24             	mov    %eax,(%esp)
8010197e:	e8 94 e8 ff ff       	call   80100217 <brelse>
    ip->flags |= I_VALID;
80101983:	8b 45 08             	mov    0x8(%ebp),%eax
80101986:	8b 40 0c             	mov    0xc(%eax),%eax
80101989:	89 c2                	mov    %eax,%edx
8010198b:	83 ca 02             	or     $0x2,%edx
8010198e:	8b 45 08             	mov    0x8(%ebp),%eax
80101991:	89 50 0c             	mov    %edx,0xc(%eax)
    if(ip->type == 0)
80101994:	8b 45 08             	mov    0x8(%ebp),%eax
80101997:	0f b7 40 10          	movzwl 0x10(%eax),%eax
8010199b:	66 85 c0             	test   %ax,%ax
8010199e:	75 0c                	jne    801019ac <ilock+0x14c>
      panic("ilock: no type");
801019a0:	c7 04 24 99 87 10 80 	movl   $0x80108799,(%esp)
801019a7:	e8 91 eb ff ff       	call   8010053d <panic>
  }
}
801019ac:	c9                   	leave  
801019ad:	c3                   	ret    

801019ae <iunlock>:

// Unlock the given inode.
void
iunlock(struct inode *ip)
{
801019ae:	55                   	push   %ebp
801019af:	89 e5                	mov    %esp,%ebp
801019b1:	83 ec 18             	sub    $0x18,%esp
  if(ip == 0 || !(ip->flags & I_BUSY) || ip->ref < 1)
801019b4:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801019b8:	74 17                	je     801019d1 <iunlock+0x23>
801019ba:	8b 45 08             	mov    0x8(%ebp),%eax
801019bd:	8b 40 0c             	mov    0xc(%eax),%eax
801019c0:	83 e0 01             	and    $0x1,%eax
801019c3:	85 c0                	test   %eax,%eax
801019c5:	74 0a                	je     801019d1 <iunlock+0x23>
801019c7:	8b 45 08             	mov    0x8(%ebp),%eax
801019ca:	8b 40 08             	mov    0x8(%eax),%eax
801019cd:	85 c0                	test   %eax,%eax
801019cf:	7f 0c                	jg     801019dd <iunlock+0x2f>
    panic("iunlock");
801019d1:	c7 04 24 a8 87 10 80 	movl   $0x801087a8,(%esp)
801019d8:	e8 60 eb ff ff       	call   8010053d <panic>

  acquire(&icache.lock);
801019dd:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
801019e4:	e8 6a 36 00 00       	call   80105053 <acquire>
  ip->flags &= ~I_BUSY;
801019e9:	8b 45 08             	mov    0x8(%ebp),%eax
801019ec:	8b 40 0c             	mov    0xc(%eax),%eax
801019ef:	89 c2                	mov    %eax,%edx
801019f1:	83 e2 fe             	and    $0xfffffffe,%edx
801019f4:	8b 45 08             	mov    0x8(%ebp),%eax
801019f7:	89 50 0c             	mov    %edx,0xc(%eax)
  wakeup(ip);
801019fa:	8b 45 08             	mov    0x8(%ebp),%eax
801019fd:	89 04 24             	mov    %eax,(%esp)
80101a00:	e8 e7 2e 00 00       	call   801048ec <wakeup>
  release(&icache.lock);
80101a05:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
80101a0c:	e8 a4 36 00 00       	call   801050b5 <release>
}
80101a11:	c9                   	leave  
80101a12:	c3                   	ret    

80101a13 <iput>:
// be recycled.
// If that was the last reference and the inode has no links
// to it, free the inode (and its content) on disk.
void
iput(struct inode *ip)
{
80101a13:	55                   	push   %ebp
80101a14:	89 e5                	mov    %esp,%ebp
80101a16:	83 ec 18             	sub    $0x18,%esp
  acquire(&icache.lock);
80101a19:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
80101a20:	e8 2e 36 00 00       	call   80105053 <acquire>
  if(ip->ref == 1 && (ip->flags & I_VALID) && ip->nlink == 0){
80101a25:	8b 45 08             	mov    0x8(%ebp),%eax
80101a28:	8b 40 08             	mov    0x8(%eax),%eax
80101a2b:	83 f8 01             	cmp    $0x1,%eax
80101a2e:	0f 85 93 00 00 00    	jne    80101ac7 <iput+0xb4>
80101a34:	8b 45 08             	mov    0x8(%ebp),%eax
80101a37:	8b 40 0c             	mov    0xc(%eax),%eax
80101a3a:	83 e0 02             	and    $0x2,%eax
80101a3d:	85 c0                	test   %eax,%eax
80101a3f:	0f 84 82 00 00 00    	je     80101ac7 <iput+0xb4>
80101a45:	8b 45 08             	mov    0x8(%ebp),%eax
80101a48:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80101a4c:	66 85 c0             	test   %ax,%ax
80101a4f:	75 76                	jne    80101ac7 <iput+0xb4>
    // inode has no links: truncate and free inode.
    if(ip->flags & I_BUSY)
80101a51:	8b 45 08             	mov    0x8(%ebp),%eax
80101a54:	8b 40 0c             	mov    0xc(%eax),%eax
80101a57:	83 e0 01             	and    $0x1,%eax
80101a5a:	84 c0                	test   %al,%al
80101a5c:	74 0c                	je     80101a6a <iput+0x57>
      panic("iput busy");
80101a5e:	c7 04 24 b0 87 10 80 	movl   $0x801087b0,(%esp)
80101a65:	e8 d3 ea ff ff       	call   8010053d <panic>
    ip->flags |= I_BUSY;
80101a6a:	8b 45 08             	mov    0x8(%ebp),%eax
80101a6d:	8b 40 0c             	mov    0xc(%eax),%eax
80101a70:	89 c2                	mov    %eax,%edx
80101a72:	83 ca 01             	or     $0x1,%edx
80101a75:	8b 45 08             	mov    0x8(%ebp),%eax
80101a78:	89 50 0c             	mov    %edx,0xc(%eax)
    release(&icache.lock);
80101a7b:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
80101a82:	e8 2e 36 00 00       	call   801050b5 <release>
    itrunc(ip);
80101a87:	8b 45 08             	mov    0x8(%ebp),%eax
80101a8a:	89 04 24             	mov    %eax,(%esp)
80101a8d:	e8 72 01 00 00       	call   80101c04 <itrunc>
    ip->type = 0;
80101a92:	8b 45 08             	mov    0x8(%ebp),%eax
80101a95:	66 c7 40 10 00 00    	movw   $0x0,0x10(%eax)
    iupdate(ip);
80101a9b:	8b 45 08             	mov    0x8(%ebp),%eax
80101a9e:	89 04 24             	mov    %eax,(%esp)
80101aa1:	e8 fe fb ff ff       	call   801016a4 <iupdate>
    acquire(&icache.lock);
80101aa6:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
80101aad:	e8 a1 35 00 00       	call   80105053 <acquire>
    ip->flags = 0;
80101ab2:	8b 45 08             	mov    0x8(%ebp),%eax
80101ab5:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
    wakeup(ip);
80101abc:	8b 45 08             	mov    0x8(%ebp),%eax
80101abf:	89 04 24             	mov    %eax,(%esp)
80101ac2:	e8 25 2e 00 00       	call   801048ec <wakeup>
  }
  ip->ref--;
80101ac7:	8b 45 08             	mov    0x8(%ebp),%eax
80101aca:	8b 40 08             	mov    0x8(%eax),%eax
80101acd:	8d 50 ff             	lea    -0x1(%eax),%edx
80101ad0:	8b 45 08             	mov    0x8(%ebp),%eax
80101ad3:	89 50 08             	mov    %edx,0x8(%eax)
  release(&icache.lock);
80101ad6:	c7 04 24 60 e8 10 80 	movl   $0x8010e860,(%esp)
80101add:	e8 d3 35 00 00       	call   801050b5 <release>
}
80101ae2:	c9                   	leave  
80101ae3:	c3                   	ret    

80101ae4 <iunlockput>:

// Common idiom: unlock, then put.
void
iunlockput(struct inode *ip)
{
80101ae4:	55                   	push   %ebp
80101ae5:	89 e5                	mov    %esp,%ebp
80101ae7:	83 ec 18             	sub    $0x18,%esp
  iunlock(ip);
80101aea:	8b 45 08             	mov    0x8(%ebp),%eax
80101aed:	89 04 24             	mov    %eax,(%esp)
80101af0:	e8 b9 fe ff ff       	call   801019ae <iunlock>
  iput(ip);
80101af5:	8b 45 08             	mov    0x8(%ebp),%eax
80101af8:	89 04 24             	mov    %eax,(%esp)
80101afb:	e8 13 ff ff ff       	call   80101a13 <iput>
}
80101b00:	c9                   	leave  
80101b01:	c3                   	ret    

80101b02 <bmap>:

// Return the disk block address of the nth block in inode ip.
// If there is no such block, bmap allocates one.
static uint
bmap(struct inode *ip, uint bn)
{
80101b02:	55                   	push   %ebp
80101b03:	89 e5                	mov    %esp,%ebp
80101b05:	53                   	push   %ebx
80101b06:	83 ec 24             	sub    $0x24,%esp
  uint addr, *a;
  struct buf *bp;

  if(bn < NDIRECT){
80101b09:	83 7d 0c 0b          	cmpl   $0xb,0xc(%ebp)
80101b0d:	77 3e                	ja     80101b4d <bmap+0x4b>
    if((addr = ip->addrs[bn]) == 0)
80101b0f:	8b 45 08             	mov    0x8(%ebp),%eax
80101b12:	8b 55 0c             	mov    0xc(%ebp),%edx
80101b15:	83 c2 04             	add    $0x4,%edx
80101b18:	8b 44 90 0c          	mov    0xc(%eax,%edx,4),%eax
80101b1c:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b1f:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101b23:	75 20                	jne    80101b45 <bmap+0x43>
      ip->addrs[bn] = addr = balloc(ip->dev);
80101b25:	8b 45 08             	mov    0x8(%ebp),%eax
80101b28:	8b 00                	mov    (%eax),%eax
80101b2a:	89 04 24             	mov    %eax,(%esp)
80101b2d:	e8 49 f8 ff ff       	call   8010137b <balloc>
80101b32:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b35:	8b 45 08             	mov    0x8(%ebp),%eax
80101b38:	8b 55 0c             	mov    0xc(%ebp),%edx
80101b3b:	8d 4a 04             	lea    0x4(%edx),%ecx
80101b3e:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101b41:	89 54 88 0c          	mov    %edx,0xc(%eax,%ecx,4)
    return addr;
80101b45:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101b48:	e9 b1 00 00 00       	jmp    80101bfe <bmap+0xfc>
  }
  bn -= NDIRECT;
80101b4d:	83 6d 0c 0c          	subl   $0xc,0xc(%ebp)

  if(bn < NINDIRECT){
80101b51:	83 7d 0c 7f          	cmpl   $0x7f,0xc(%ebp)
80101b55:	0f 87 97 00 00 00    	ja     80101bf2 <bmap+0xf0>
    // Load indirect block, allocating if necessary.
    if((addr = ip->addrs[NDIRECT]) == 0)
80101b5b:	8b 45 08             	mov    0x8(%ebp),%eax
80101b5e:	8b 40 4c             	mov    0x4c(%eax),%eax
80101b61:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b64:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101b68:	75 19                	jne    80101b83 <bmap+0x81>
      ip->addrs[NDIRECT] = addr = balloc(ip->dev);
80101b6a:	8b 45 08             	mov    0x8(%ebp),%eax
80101b6d:	8b 00                	mov    (%eax),%eax
80101b6f:	89 04 24             	mov    %eax,(%esp)
80101b72:	e8 04 f8 ff ff       	call   8010137b <balloc>
80101b77:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101b7a:	8b 45 08             	mov    0x8(%ebp),%eax
80101b7d:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101b80:	89 50 4c             	mov    %edx,0x4c(%eax)
    bp = bread(ip->dev, addr);
80101b83:	8b 45 08             	mov    0x8(%ebp),%eax
80101b86:	8b 00                	mov    (%eax),%eax
80101b88:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101b8b:	89 54 24 04          	mov    %edx,0x4(%esp)
80101b8f:	89 04 24             	mov    %eax,(%esp)
80101b92:	e8 0f e6 ff ff       	call   801001a6 <bread>
80101b97:	89 45 f0             	mov    %eax,-0x10(%ebp)
    a = (uint*)bp->data;
80101b9a:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101b9d:	83 c0 18             	add    $0x18,%eax
80101ba0:	89 45 ec             	mov    %eax,-0x14(%ebp)
    if((addr = a[bn]) == 0){
80101ba3:	8b 45 0c             	mov    0xc(%ebp),%eax
80101ba6:	c1 e0 02             	shl    $0x2,%eax
80101ba9:	03 45 ec             	add    -0x14(%ebp),%eax
80101bac:	8b 00                	mov    (%eax),%eax
80101bae:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101bb1:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80101bb5:	75 2b                	jne    80101be2 <bmap+0xe0>
      a[bn] = addr = balloc(ip->dev);
80101bb7:	8b 45 0c             	mov    0xc(%ebp),%eax
80101bba:	c1 e0 02             	shl    $0x2,%eax
80101bbd:	89 c3                	mov    %eax,%ebx
80101bbf:	03 5d ec             	add    -0x14(%ebp),%ebx
80101bc2:	8b 45 08             	mov    0x8(%ebp),%eax
80101bc5:	8b 00                	mov    (%eax),%eax
80101bc7:	89 04 24             	mov    %eax,(%esp)
80101bca:	e8 ac f7 ff ff       	call   8010137b <balloc>
80101bcf:	89 45 f4             	mov    %eax,-0xc(%ebp)
80101bd2:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101bd5:	89 03                	mov    %eax,(%ebx)
      log_write(bp);
80101bd7:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101bda:	89 04 24             	mov    %eax,(%esp)
80101bdd:	e8 d4 16 00 00       	call   801032b6 <log_write>
    }
    brelse(bp);
80101be2:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101be5:	89 04 24             	mov    %eax,(%esp)
80101be8:	e8 2a e6 ff ff       	call   80100217 <brelse>
    return addr;
80101bed:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101bf0:	eb 0c                	jmp    80101bfe <bmap+0xfc>
  }

  panic("bmap: out of range");
80101bf2:	c7 04 24 ba 87 10 80 	movl   $0x801087ba,(%esp)
80101bf9:	e8 3f e9 ff ff       	call   8010053d <panic>
}
80101bfe:	83 c4 24             	add    $0x24,%esp
80101c01:	5b                   	pop    %ebx
80101c02:	5d                   	pop    %ebp
80101c03:	c3                   	ret    

80101c04 <itrunc>:
// to it (no directory entries referring to it)
// and has no in-memory reference to it (is
// not an open file or current directory).
static void
itrunc(struct inode *ip)
{
80101c04:	55                   	push   %ebp
80101c05:	89 e5                	mov    %esp,%ebp
80101c07:	83 ec 28             	sub    $0x28,%esp
  int i, j;
  struct buf *bp;
  uint *a;

  for(i = 0; i < NDIRECT; i++){
80101c0a:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80101c11:	eb 44                	jmp    80101c57 <itrunc+0x53>
    if(ip->addrs[i]){
80101c13:	8b 45 08             	mov    0x8(%ebp),%eax
80101c16:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101c19:	83 c2 04             	add    $0x4,%edx
80101c1c:	8b 44 90 0c          	mov    0xc(%eax,%edx,4),%eax
80101c20:	85 c0                	test   %eax,%eax
80101c22:	74 2f                	je     80101c53 <itrunc+0x4f>
      bfree(ip->dev, ip->addrs[i]);
80101c24:	8b 45 08             	mov    0x8(%ebp),%eax
80101c27:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101c2a:	83 c2 04             	add    $0x4,%edx
80101c2d:	8b 54 90 0c          	mov    0xc(%eax,%edx,4),%edx
80101c31:	8b 45 08             	mov    0x8(%ebp),%eax
80101c34:	8b 00                	mov    (%eax),%eax
80101c36:	89 54 24 04          	mov    %edx,0x4(%esp)
80101c3a:	89 04 24             	mov    %eax,(%esp)
80101c3d:	e8 90 f8 ff ff       	call   801014d2 <bfree>
      ip->addrs[i] = 0;
80101c42:	8b 45 08             	mov    0x8(%ebp),%eax
80101c45:	8b 55 f4             	mov    -0xc(%ebp),%edx
80101c48:	83 c2 04             	add    $0x4,%edx
80101c4b:	c7 44 90 0c 00 00 00 	movl   $0x0,0xc(%eax,%edx,4)
80101c52:	00 
{
  int i, j;
  struct buf *bp;
  uint *a;

  for(i = 0; i < NDIRECT; i++){
80101c53:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80101c57:	83 7d f4 0b          	cmpl   $0xb,-0xc(%ebp)
80101c5b:	7e b6                	jle    80101c13 <itrunc+0xf>
      bfree(ip->dev, ip->addrs[i]);
      ip->addrs[i] = 0;
    }
  }
  
  if(ip->addrs[NDIRECT]){
80101c5d:	8b 45 08             	mov    0x8(%ebp),%eax
80101c60:	8b 40 4c             	mov    0x4c(%eax),%eax
80101c63:	85 c0                	test   %eax,%eax
80101c65:	0f 84 8f 00 00 00    	je     80101cfa <itrunc+0xf6>
    bp = bread(ip->dev, ip->addrs[NDIRECT]);
80101c6b:	8b 45 08             	mov    0x8(%ebp),%eax
80101c6e:	8b 50 4c             	mov    0x4c(%eax),%edx
80101c71:	8b 45 08             	mov    0x8(%ebp),%eax
80101c74:	8b 00                	mov    (%eax),%eax
80101c76:	89 54 24 04          	mov    %edx,0x4(%esp)
80101c7a:	89 04 24             	mov    %eax,(%esp)
80101c7d:	e8 24 e5 ff ff       	call   801001a6 <bread>
80101c82:	89 45 ec             	mov    %eax,-0x14(%ebp)
    a = (uint*)bp->data;
80101c85:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101c88:	83 c0 18             	add    $0x18,%eax
80101c8b:	89 45 e8             	mov    %eax,-0x18(%ebp)
    for(j = 0; j < NINDIRECT; j++){
80101c8e:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
80101c95:	eb 2f                	jmp    80101cc6 <itrunc+0xc2>
      if(a[j])
80101c97:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101c9a:	c1 e0 02             	shl    $0x2,%eax
80101c9d:	03 45 e8             	add    -0x18(%ebp),%eax
80101ca0:	8b 00                	mov    (%eax),%eax
80101ca2:	85 c0                	test   %eax,%eax
80101ca4:	74 1c                	je     80101cc2 <itrunc+0xbe>
        bfree(ip->dev, a[j]);
80101ca6:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101ca9:	c1 e0 02             	shl    $0x2,%eax
80101cac:	03 45 e8             	add    -0x18(%ebp),%eax
80101caf:	8b 10                	mov    (%eax),%edx
80101cb1:	8b 45 08             	mov    0x8(%ebp),%eax
80101cb4:	8b 00                	mov    (%eax),%eax
80101cb6:	89 54 24 04          	mov    %edx,0x4(%esp)
80101cba:	89 04 24             	mov    %eax,(%esp)
80101cbd:	e8 10 f8 ff ff       	call   801014d2 <bfree>
  }
  
  if(ip->addrs[NDIRECT]){
    bp = bread(ip->dev, ip->addrs[NDIRECT]);
    a = (uint*)bp->data;
    for(j = 0; j < NINDIRECT; j++){
80101cc2:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
80101cc6:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101cc9:	83 f8 7f             	cmp    $0x7f,%eax
80101ccc:	76 c9                	jbe    80101c97 <itrunc+0x93>
      if(a[j])
        bfree(ip->dev, a[j]);
    }
    brelse(bp);
80101cce:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101cd1:	89 04 24             	mov    %eax,(%esp)
80101cd4:	e8 3e e5 ff ff       	call   80100217 <brelse>
    bfree(ip->dev, ip->addrs[NDIRECT]);
80101cd9:	8b 45 08             	mov    0x8(%ebp),%eax
80101cdc:	8b 50 4c             	mov    0x4c(%eax),%edx
80101cdf:	8b 45 08             	mov    0x8(%ebp),%eax
80101ce2:	8b 00                	mov    (%eax),%eax
80101ce4:	89 54 24 04          	mov    %edx,0x4(%esp)
80101ce8:	89 04 24             	mov    %eax,(%esp)
80101ceb:	e8 e2 f7 ff ff       	call   801014d2 <bfree>
    ip->addrs[NDIRECT] = 0;
80101cf0:	8b 45 08             	mov    0x8(%ebp),%eax
80101cf3:	c7 40 4c 00 00 00 00 	movl   $0x0,0x4c(%eax)
  }

  ip->size = 0;
80101cfa:	8b 45 08             	mov    0x8(%ebp),%eax
80101cfd:	c7 40 18 00 00 00 00 	movl   $0x0,0x18(%eax)
  iupdate(ip);
80101d04:	8b 45 08             	mov    0x8(%ebp),%eax
80101d07:	89 04 24             	mov    %eax,(%esp)
80101d0a:	e8 95 f9 ff ff       	call   801016a4 <iupdate>
}
80101d0f:	c9                   	leave  
80101d10:	c3                   	ret    

80101d11 <stati>:

// Copy stat information from inode.
void
stati(struct inode *ip, struct stat *st)
{
80101d11:	55                   	push   %ebp
80101d12:	89 e5                	mov    %esp,%ebp
  st->dev = ip->dev;
80101d14:	8b 45 08             	mov    0x8(%ebp),%eax
80101d17:	8b 00                	mov    (%eax),%eax
80101d19:	89 c2                	mov    %eax,%edx
80101d1b:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d1e:	89 50 04             	mov    %edx,0x4(%eax)
  st->ino = ip->inum;
80101d21:	8b 45 08             	mov    0x8(%ebp),%eax
80101d24:	8b 50 04             	mov    0x4(%eax),%edx
80101d27:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d2a:	89 50 08             	mov    %edx,0x8(%eax)
  st->type = ip->type;
80101d2d:	8b 45 08             	mov    0x8(%ebp),%eax
80101d30:	0f b7 50 10          	movzwl 0x10(%eax),%edx
80101d34:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d37:	66 89 10             	mov    %dx,(%eax)
  st->nlink = ip->nlink;
80101d3a:	8b 45 08             	mov    0x8(%ebp),%eax
80101d3d:	0f b7 50 16          	movzwl 0x16(%eax),%edx
80101d41:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d44:	66 89 50 0c          	mov    %dx,0xc(%eax)
  st->size = ip->size;
80101d48:	8b 45 08             	mov    0x8(%ebp),%eax
80101d4b:	8b 50 18             	mov    0x18(%eax),%edx
80101d4e:	8b 45 0c             	mov    0xc(%ebp),%eax
80101d51:	89 50 10             	mov    %edx,0x10(%eax)
}
80101d54:	5d                   	pop    %ebp
80101d55:	c3                   	ret    

80101d56 <readi>:

//PAGEBREAK!
// Read data from inode.
int
readi(struct inode *ip, char *dst, uint off, uint n)
{
80101d56:	55                   	push   %ebp
80101d57:	89 e5                	mov    %esp,%ebp
80101d59:	53                   	push   %ebx
80101d5a:	83 ec 24             	sub    $0x24,%esp
  uint tot, m;
  struct buf *bp;

  if(ip->type == T_DEV){
80101d5d:	8b 45 08             	mov    0x8(%ebp),%eax
80101d60:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80101d64:	66 83 f8 03          	cmp    $0x3,%ax
80101d68:	75 60                	jne    80101dca <readi+0x74>
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].read)
80101d6a:	8b 45 08             	mov    0x8(%ebp),%eax
80101d6d:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101d71:	66 85 c0             	test   %ax,%ax
80101d74:	78 20                	js     80101d96 <readi+0x40>
80101d76:	8b 45 08             	mov    0x8(%ebp),%eax
80101d79:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101d7d:	66 83 f8 09          	cmp    $0x9,%ax
80101d81:	7f 13                	jg     80101d96 <readi+0x40>
80101d83:	8b 45 08             	mov    0x8(%ebp),%eax
80101d86:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101d8a:	98                   	cwtl   
80101d8b:	8b 04 c5 00 e8 10 80 	mov    -0x7fef1800(,%eax,8),%eax
80101d92:	85 c0                	test   %eax,%eax
80101d94:	75 0a                	jne    80101da0 <readi+0x4a>
      return -1;
80101d96:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101d9b:	e9 1b 01 00 00       	jmp    80101ebb <readi+0x165>
    return devsw[ip->major].read(ip, dst, n);
80101da0:	8b 45 08             	mov    0x8(%ebp),%eax
80101da3:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101da7:	98                   	cwtl   
80101da8:	8b 14 c5 00 e8 10 80 	mov    -0x7fef1800(,%eax,8),%edx
80101daf:	8b 45 14             	mov    0x14(%ebp),%eax
80101db2:	89 44 24 08          	mov    %eax,0x8(%esp)
80101db6:	8b 45 0c             	mov    0xc(%ebp),%eax
80101db9:	89 44 24 04          	mov    %eax,0x4(%esp)
80101dbd:	8b 45 08             	mov    0x8(%ebp),%eax
80101dc0:	89 04 24             	mov    %eax,(%esp)
80101dc3:	ff d2                	call   *%edx
80101dc5:	e9 f1 00 00 00       	jmp    80101ebb <readi+0x165>
  }

  if(off > ip->size || off + n < off)
80101dca:	8b 45 08             	mov    0x8(%ebp),%eax
80101dcd:	8b 40 18             	mov    0x18(%eax),%eax
80101dd0:	3b 45 10             	cmp    0x10(%ebp),%eax
80101dd3:	72 0d                	jb     80101de2 <readi+0x8c>
80101dd5:	8b 45 14             	mov    0x14(%ebp),%eax
80101dd8:	8b 55 10             	mov    0x10(%ebp),%edx
80101ddb:	01 d0                	add    %edx,%eax
80101ddd:	3b 45 10             	cmp    0x10(%ebp),%eax
80101de0:	73 0a                	jae    80101dec <readi+0x96>
    return -1;
80101de2:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101de7:	e9 cf 00 00 00       	jmp    80101ebb <readi+0x165>
  if(off + n > ip->size)
80101dec:	8b 45 14             	mov    0x14(%ebp),%eax
80101def:	8b 55 10             	mov    0x10(%ebp),%edx
80101df2:	01 c2                	add    %eax,%edx
80101df4:	8b 45 08             	mov    0x8(%ebp),%eax
80101df7:	8b 40 18             	mov    0x18(%eax),%eax
80101dfa:	39 c2                	cmp    %eax,%edx
80101dfc:	76 0c                	jbe    80101e0a <readi+0xb4>
    n = ip->size - off;
80101dfe:	8b 45 08             	mov    0x8(%ebp),%eax
80101e01:	8b 40 18             	mov    0x18(%eax),%eax
80101e04:	2b 45 10             	sub    0x10(%ebp),%eax
80101e07:	89 45 14             	mov    %eax,0x14(%ebp)

  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
80101e0a:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80101e11:	e9 96 00 00 00       	jmp    80101eac <readi+0x156>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
80101e16:	8b 45 10             	mov    0x10(%ebp),%eax
80101e19:	c1 e8 09             	shr    $0x9,%eax
80101e1c:	89 44 24 04          	mov    %eax,0x4(%esp)
80101e20:	8b 45 08             	mov    0x8(%ebp),%eax
80101e23:	89 04 24             	mov    %eax,(%esp)
80101e26:	e8 d7 fc ff ff       	call   80101b02 <bmap>
80101e2b:	8b 55 08             	mov    0x8(%ebp),%edx
80101e2e:	8b 12                	mov    (%edx),%edx
80101e30:	89 44 24 04          	mov    %eax,0x4(%esp)
80101e34:	89 14 24             	mov    %edx,(%esp)
80101e37:	e8 6a e3 ff ff       	call   801001a6 <bread>
80101e3c:	89 45 f0             	mov    %eax,-0x10(%ebp)
    m = min(n - tot, BSIZE - off%BSIZE);
80101e3f:	8b 45 10             	mov    0x10(%ebp),%eax
80101e42:	89 c2                	mov    %eax,%edx
80101e44:	81 e2 ff 01 00 00    	and    $0x1ff,%edx
80101e4a:	b8 00 02 00 00       	mov    $0x200,%eax
80101e4f:	89 c1                	mov    %eax,%ecx
80101e51:	29 d1                	sub    %edx,%ecx
80101e53:	89 ca                	mov    %ecx,%edx
80101e55:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101e58:	8b 4d 14             	mov    0x14(%ebp),%ecx
80101e5b:	89 cb                	mov    %ecx,%ebx
80101e5d:	29 c3                	sub    %eax,%ebx
80101e5f:	89 d8                	mov    %ebx,%eax
80101e61:	39 c2                	cmp    %eax,%edx
80101e63:	0f 46 c2             	cmovbe %edx,%eax
80101e66:	89 45 ec             	mov    %eax,-0x14(%ebp)
    memmove(dst, bp->data + off%BSIZE, m);
80101e69:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101e6c:	8d 50 18             	lea    0x18(%eax),%edx
80101e6f:	8b 45 10             	mov    0x10(%ebp),%eax
80101e72:	25 ff 01 00 00       	and    $0x1ff,%eax
80101e77:	01 c2                	add    %eax,%edx
80101e79:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101e7c:	89 44 24 08          	mov    %eax,0x8(%esp)
80101e80:	89 54 24 04          	mov    %edx,0x4(%esp)
80101e84:	8b 45 0c             	mov    0xc(%ebp),%eax
80101e87:	89 04 24             	mov    %eax,(%esp)
80101e8a:	e8 e6 34 00 00       	call   80105375 <memmove>
    brelse(bp);
80101e8f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101e92:	89 04 24             	mov    %eax,(%esp)
80101e95:	e8 7d e3 ff ff       	call   80100217 <brelse>
  if(off > ip->size || off + n < off)
    return -1;
  if(off + n > ip->size)
    n = ip->size - off;

  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
80101e9a:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101e9d:	01 45 f4             	add    %eax,-0xc(%ebp)
80101ea0:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101ea3:	01 45 10             	add    %eax,0x10(%ebp)
80101ea6:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101ea9:	01 45 0c             	add    %eax,0xc(%ebp)
80101eac:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101eaf:	3b 45 14             	cmp    0x14(%ebp),%eax
80101eb2:	0f 82 5e ff ff ff    	jb     80101e16 <readi+0xc0>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
    m = min(n - tot, BSIZE - off%BSIZE);
    memmove(dst, bp->data + off%BSIZE, m);
    brelse(bp);
  }
  return n;
80101eb8:	8b 45 14             	mov    0x14(%ebp),%eax
}
80101ebb:	83 c4 24             	add    $0x24,%esp
80101ebe:	5b                   	pop    %ebx
80101ebf:	5d                   	pop    %ebp
80101ec0:	c3                   	ret    

80101ec1 <writei>:

// PAGEBREAK!
// Write data to inode.
int
writei(struct inode *ip, char *src, uint off, uint n)
{
80101ec1:	55                   	push   %ebp
80101ec2:	89 e5                	mov    %esp,%ebp
80101ec4:	53                   	push   %ebx
80101ec5:	83 ec 24             	sub    $0x24,%esp
  uint tot, m;
  struct buf *bp;

  if(ip->type == T_DEV){
80101ec8:	8b 45 08             	mov    0x8(%ebp),%eax
80101ecb:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80101ecf:	66 83 f8 03          	cmp    $0x3,%ax
80101ed3:	75 60                	jne    80101f35 <writei+0x74>
    if(ip->major < 0 || ip->major >= NDEV || !devsw[ip->major].write)
80101ed5:	8b 45 08             	mov    0x8(%ebp),%eax
80101ed8:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101edc:	66 85 c0             	test   %ax,%ax
80101edf:	78 20                	js     80101f01 <writei+0x40>
80101ee1:	8b 45 08             	mov    0x8(%ebp),%eax
80101ee4:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101ee8:	66 83 f8 09          	cmp    $0x9,%ax
80101eec:	7f 13                	jg     80101f01 <writei+0x40>
80101eee:	8b 45 08             	mov    0x8(%ebp),%eax
80101ef1:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101ef5:	98                   	cwtl   
80101ef6:	8b 04 c5 04 e8 10 80 	mov    -0x7fef17fc(,%eax,8),%eax
80101efd:	85 c0                	test   %eax,%eax
80101eff:	75 0a                	jne    80101f0b <writei+0x4a>
      return -1;
80101f01:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101f06:	e9 46 01 00 00       	jmp    80102051 <writei+0x190>
    return devsw[ip->major].write(ip, src, n);
80101f0b:	8b 45 08             	mov    0x8(%ebp),%eax
80101f0e:	0f b7 40 12          	movzwl 0x12(%eax),%eax
80101f12:	98                   	cwtl   
80101f13:	8b 14 c5 04 e8 10 80 	mov    -0x7fef17fc(,%eax,8),%edx
80101f1a:	8b 45 14             	mov    0x14(%ebp),%eax
80101f1d:	89 44 24 08          	mov    %eax,0x8(%esp)
80101f21:	8b 45 0c             	mov    0xc(%ebp),%eax
80101f24:	89 44 24 04          	mov    %eax,0x4(%esp)
80101f28:	8b 45 08             	mov    0x8(%ebp),%eax
80101f2b:	89 04 24             	mov    %eax,(%esp)
80101f2e:	ff d2                	call   *%edx
80101f30:	e9 1c 01 00 00       	jmp    80102051 <writei+0x190>
  }

  if(off > ip->size || off + n < off)
80101f35:	8b 45 08             	mov    0x8(%ebp),%eax
80101f38:	8b 40 18             	mov    0x18(%eax),%eax
80101f3b:	3b 45 10             	cmp    0x10(%ebp),%eax
80101f3e:	72 0d                	jb     80101f4d <writei+0x8c>
80101f40:	8b 45 14             	mov    0x14(%ebp),%eax
80101f43:	8b 55 10             	mov    0x10(%ebp),%edx
80101f46:	01 d0                	add    %edx,%eax
80101f48:	3b 45 10             	cmp    0x10(%ebp),%eax
80101f4b:	73 0a                	jae    80101f57 <writei+0x96>
    return -1;
80101f4d:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101f52:	e9 fa 00 00 00       	jmp    80102051 <writei+0x190>
  if(off + n > MAXFILE*BSIZE)
80101f57:	8b 45 14             	mov    0x14(%ebp),%eax
80101f5a:	8b 55 10             	mov    0x10(%ebp),%edx
80101f5d:	01 d0                	add    %edx,%eax
80101f5f:	3d 00 18 01 00       	cmp    $0x11800,%eax
80101f64:	76 0a                	jbe    80101f70 <writei+0xaf>
    return -1;
80101f66:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80101f6b:	e9 e1 00 00 00       	jmp    80102051 <writei+0x190>

  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
80101f70:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80101f77:	e9 a1 00 00 00       	jmp    8010201d <writei+0x15c>
    bp = bread(ip->dev, bmap(ip, off/BSIZE));
80101f7c:	8b 45 10             	mov    0x10(%ebp),%eax
80101f7f:	c1 e8 09             	shr    $0x9,%eax
80101f82:	89 44 24 04          	mov    %eax,0x4(%esp)
80101f86:	8b 45 08             	mov    0x8(%ebp),%eax
80101f89:	89 04 24             	mov    %eax,(%esp)
80101f8c:	e8 71 fb ff ff       	call   80101b02 <bmap>
80101f91:	8b 55 08             	mov    0x8(%ebp),%edx
80101f94:	8b 12                	mov    (%edx),%edx
80101f96:	89 44 24 04          	mov    %eax,0x4(%esp)
80101f9a:	89 14 24             	mov    %edx,(%esp)
80101f9d:	e8 04 e2 ff ff       	call   801001a6 <bread>
80101fa2:	89 45 f0             	mov    %eax,-0x10(%ebp)
    m = min(n - tot, BSIZE - off%BSIZE);
80101fa5:	8b 45 10             	mov    0x10(%ebp),%eax
80101fa8:	89 c2                	mov    %eax,%edx
80101faa:	81 e2 ff 01 00 00    	and    $0x1ff,%edx
80101fb0:	b8 00 02 00 00       	mov    $0x200,%eax
80101fb5:	89 c1                	mov    %eax,%ecx
80101fb7:	29 d1                	sub    %edx,%ecx
80101fb9:	89 ca                	mov    %ecx,%edx
80101fbb:	8b 45 f4             	mov    -0xc(%ebp),%eax
80101fbe:	8b 4d 14             	mov    0x14(%ebp),%ecx
80101fc1:	89 cb                	mov    %ecx,%ebx
80101fc3:	29 c3                	sub    %eax,%ebx
80101fc5:	89 d8                	mov    %ebx,%eax
80101fc7:	39 c2                	cmp    %eax,%edx
80101fc9:	0f 46 c2             	cmovbe %edx,%eax
80101fcc:	89 45 ec             	mov    %eax,-0x14(%ebp)
    memmove(bp->data + off%BSIZE, src, m);
80101fcf:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101fd2:	8d 50 18             	lea    0x18(%eax),%edx
80101fd5:	8b 45 10             	mov    0x10(%ebp),%eax
80101fd8:	25 ff 01 00 00       	and    $0x1ff,%eax
80101fdd:	01 c2                	add    %eax,%edx
80101fdf:	8b 45 ec             	mov    -0x14(%ebp),%eax
80101fe2:	89 44 24 08          	mov    %eax,0x8(%esp)
80101fe6:	8b 45 0c             	mov    0xc(%ebp),%eax
80101fe9:	89 44 24 04          	mov    %eax,0x4(%esp)
80101fed:	89 14 24             	mov    %edx,(%esp)
80101ff0:	e8 80 33 00 00       	call   80105375 <memmove>
    log_write(bp);
80101ff5:	8b 45 f0             	mov    -0x10(%ebp),%eax
80101ff8:	89 04 24             	mov    %eax,(%esp)
80101ffb:	e8 b6 12 00 00       	call   801032b6 <log_write>
    brelse(bp);
80102000:	8b 45 f0             	mov    -0x10(%ebp),%eax
80102003:	89 04 24             	mov    %eax,(%esp)
80102006:	e8 0c e2 ff ff       	call   80100217 <brelse>
  if(off > ip->size || off + n < off)
    return -1;
  if(off + n > MAXFILE*BSIZE)
    return -1;

  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
8010200b:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010200e:	01 45 f4             	add    %eax,-0xc(%ebp)
80102011:	8b 45 ec             	mov    -0x14(%ebp),%eax
80102014:	01 45 10             	add    %eax,0x10(%ebp)
80102017:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010201a:	01 45 0c             	add    %eax,0xc(%ebp)
8010201d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102020:	3b 45 14             	cmp    0x14(%ebp),%eax
80102023:	0f 82 53 ff ff ff    	jb     80101f7c <writei+0xbb>
    memmove(bp->data + off%BSIZE, src, m);
    log_write(bp);
    brelse(bp);
  }

  if(n > 0 && off > ip->size){
80102029:	83 7d 14 00          	cmpl   $0x0,0x14(%ebp)
8010202d:	74 1f                	je     8010204e <writei+0x18d>
8010202f:	8b 45 08             	mov    0x8(%ebp),%eax
80102032:	8b 40 18             	mov    0x18(%eax),%eax
80102035:	3b 45 10             	cmp    0x10(%ebp),%eax
80102038:	73 14                	jae    8010204e <writei+0x18d>
    ip->size = off;
8010203a:	8b 45 08             	mov    0x8(%ebp),%eax
8010203d:	8b 55 10             	mov    0x10(%ebp),%edx
80102040:	89 50 18             	mov    %edx,0x18(%eax)
    iupdate(ip);
80102043:	8b 45 08             	mov    0x8(%ebp),%eax
80102046:	89 04 24             	mov    %eax,(%esp)
80102049:	e8 56 f6 ff ff       	call   801016a4 <iupdate>
  }
  return n;
8010204e:	8b 45 14             	mov    0x14(%ebp),%eax
}
80102051:	83 c4 24             	add    $0x24,%esp
80102054:	5b                   	pop    %ebx
80102055:	5d                   	pop    %ebp
80102056:	c3                   	ret    

80102057 <namecmp>:
//PAGEBREAK!
// Directories

int
namecmp(const char *s, const char *t)
{
80102057:	55                   	push   %ebp
80102058:	89 e5                	mov    %esp,%ebp
8010205a:	83 ec 18             	sub    $0x18,%esp
  return strncmp(s, t, DIRSIZ);
8010205d:	c7 44 24 08 0e 00 00 	movl   $0xe,0x8(%esp)
80102064:	00 
80102065:	8b 45 0c             	mov    0xc(%ebp),%eax
80102068:	89 44 24 04          	mov    %eax,0x4(%esp)
8010206c:	8b 45 08             	mov    0x8(%ebp),%eax
8010206f:	89 04 24             	mov    %eax,(%esp)
80102072:	e8 a2 33 00 00       	call   80105419 <strncmp>
}
80102077:	c9                   	leave  
80102078:	c3                   	ret    

80102079 <dirlookup>:

// Look for a directory entry in a directory.
// If found, set *poff to byte offset of entry.
struct inode*
dirlookup(struct inode *dp, char *name, uint *poff)
{
80102079:	55                   	push   %ebp
8010207a:	89 e5                	mov    %esp,%ebp
8010207c:	83 ec 38             	sub    $0x38,%esp
  uint off, inum;
  struct dirent de;

  if(dp->type != T_DIR)
8010207f:	8b 45 08             	mov    0x8(%ebp),%eax
80102082:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80102086:	66 83 f8 01          	cmp    $0x1,%ax
8010208a:	74 0c                	je     80102098 <dirlookup+0x1f>
    panic("dirlookup not DIR");
8010208c:	c7 04 24 cd 87 10 80 	movl   $0x801087cd,(%esp)
80102093:	e8 a5 e4 ff ff       	call   8010053d <panic>

  for(off = 0; off < dp->size; off += sizeof(de)){
80102098:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010209f:	e9 87 00 00 00       	jmp    8010212b <dirlookup+0xb2>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
801020a4:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
801020ab:	00 
801020ac:	8b 45 f4             	mov    -0xc(%ebp),%eax
801020af:	89 44 24 08          	mov    %eax,0x8(%esp)
801020b3:	8d 45 e0             	lea    -0x20(%ebp),%eax
801020b6:	89 44 24 04          	mov    %eax,0x4(%esp)
801020ba:	8b 45 08             	mov    0x8(%ebp),%eax
801020bd:	89 04 24             	mov    %eax,(%esp)
801020c0:	e8 91 fc ff ff       	call   80101d56 <readi>
801020c5:	83 f8 10             	cmp    $0x10,%eax
801020c8:	74 0c                	je     801020d6 <dirlookup+0x5d>
      panic("dirlink read");
801020ca:	c7 04 24 df 87 10 80 	movl   $0x801087df,(%esp)
801020d1:	e8 67 e4 ff ff       	call   8010053d <panic>
    if(de.inum == 0)
801020d6:	0f b7 45 e0          	movzwl -0x20(%ebp),%eax
801020da:	66 85 c0             	test   %ax,%ax
801020dd:	74 47                	je     80102126 <dirlookup+0xad>
      continue;
    if(namecmp(name, de.name) == 0){
801020df:	8d 45 e0             	lea    -0x20(%ebp),%eax
801020e2:	83 c0 02             	add    $0x2,%eax
801020e5:	89 44 24 04          	mov    %eax,0x4(%esp)
801020e9:	8b 45 0c             	mov    0xc(%ebp),%eax
801020ec:	89 04 24             	mov    %eax,(%esp)
801020ef:	e8 63 ff ff ff       	call   80102057 <namecmp>
801020f4:	85 c0                	test   %eax,%eax
801020f6:	75 2f                	jne    80102127 <dirlookup+0xae>
      // entry matches path element
      if(poff)
801020f8:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801020fc:	74 08                	je     80102106 <dirlookup+0x8d>
        *poff = off;
801020fe:	8b 45 10             	mov    0x10(%ebp),%eax
80102101:	8b 55 f4             	mov    -0xc(%ebp),%edx
80102104:	89 10                	mov    %edx,(%eax)
      inum = de.inum;
80102106:	0f b7 45 e0          	movzwl -0x20(%ebp),%eax
8010210a:	0f b7 c0             	movzwl %ax,%eax
8010210d:	89 45 f0             	mov    %eax,-0x10(%ebp)
      return iget(dp->dev, inum);
80102110:	8b 45 08             	mov    0x8(%ebp),%eax
80102113:	8b 00                	mov    (%eax),%eax
80102115:	8b 55 f0             	mov    -0x10(%ebp),%edx
80102118:	89 54 24 04          	mov    %edx,0x4(%esp)
8010211c:	89 04 24             	mov    %eax,(%esp)
8010211f:	e8 38 f6 ff ff       	call   8010175c <iget>
80102124:	eb 19                	jmp    8010213f <dirlookup+0xc6>

  for(off = 0; off < dp->size; off += sizeof(de)){
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
      panic("dirlink read");
    if(de.inum == 0)
      continue;
80102126:	90                   	nop
  struct dirent de;

  if(dp->type != T_DIR)
    panic("dirlookup not DIR");

  for(off = 0; off < dp->size; off += sizeof(de)){
80102127:	83 45 f4 10          	addl   $0x10,-0xc(%ebp)
8010212b:	8b 45 08             	mov    0x8(%ebp),%eax
8010212e:	8b 40 18             	mov    0x18(%eax),%eax
80102131:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80102134:	0f 87 6a ff ff ff    	ja     801020a4 <dirlookup+0x2b>
      inum = de.inum;
      return iget(dp->dev, inum);
    }
  }

  return 0;
8010213a:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010213f:	c9                   	leave  
80102140:	c3                   	ret    

80102141 <dirlink>:

// Write a new directory entry (name, inum) into the directory dp.
int
dirlink(struct inode *dp, char *name, uint inum)
{
80102141:	55                   	push   %ebp
80102142:	89 e5                	mov    %esp,%ebp
80102144:	83 ec 38             	sub    $0x38,%esp
  int off;
  struct dirent de;
  struct inode *ip;

  // Check that name is not present.
  if((ip = dirlookup(dp, name, 0)) != 0){
80102147:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
8010214e:	00 
8010214f:	8b 45 0c             	mov    0xc(%ebp),%eax
80102152:	89 44 24 04          	mov    %eax,0x4(%esp)
80102156:	8b 45 08             	mov    0x8(%ebp),%eax
80102159:	89 04 24             	mov    %eax,(%esp)
8010215c:	e8 18 ff ff ff       	call   80102079 <dirlookup>
80102161:	89 45 f0             	mov    %eax,-0x10(%ebp)
80102164:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80102168:	74 15                	je     8010217f <dirlink+0x3e>
    iput(ip);
8010216a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010216d:	89 04 24             	mov    %eax,(%esp)
80102170:	e8 9e f8 ff ff       	call   80101a13 <iput>
    return -1;
80102175:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010217a:	e9 b8 00 00 00       	jmp    80102237 <dirlink+0xf6>
  }

  // Look for an empty dirent.
  for(off = 0; off < dp->size; off += sizeof(de)){
8010217f:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80102186:	eb 44                	jmp    801021cc <dirlink+0x8b>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80102188:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010218b:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
80102192:	00 
80102193:	89 44 24 08          	mov    %eax,0x8(%esp)
80102197:	8d 45 e0             	lea    -0x20(%ebp),%eax
8010219a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010219e:	8b 45 08             	mov    0x8(%ebp),%eax
801021a1:	89 04 24             	mov    %eax,(%esp)
801021a4:	e8 ad fb ff ff       	call   80101d56 <readi>
801021a9:	83 f8 10             	cmp    $0x10,%eax
801021ac:	74 0c                	je     801021ba <dirlink+0x79>
      panic("dirlink read");
801021ae:	c7 04 24 df 87 10 80 	movl   $0x801087df,(%esp)
801021b5:	e8 83 e3 ff ff       	call   8010053d <panic>
    if(de.inum == 0)
801021ba:	0f b7 45 e0          	movzwl -0x20(%ebp),%eax
801021be:	66 85 c0             	test   %ax,%ax
801021c1:	74 18                	je     801021db <dirlink+0x9a>
    iput(ip);
    return -1;
  }

  // Look for an empty dirent.
  for(off = 0; off < dp->size; off += sizeof(de)){
801021c3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801021c6:	83 c0 10             	add    $0x10,%eax
801021c9:	89 45 f4             	mov    %eax,-0xc(%ebp)
801021cc:	8b 55 f4             	mov    -0xc(%ebp),%edx
801021cf:	8b 45 08             	mov    0x8(%ebp),%eax
801021d2:	8b 40 18             	mov    0x18(%eax),%eax
801021d5:	39 c2                	cmp    %eax,%edx
801021d7:	72 af                	jb     80102188 <dirlink+0x47>
801021d9:	eb 01                	jmp    801021dc <dirlink+0x9b>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
      panic("dirlink read");
    if(de.inum == 0)
      break;
801021db:	90                   	nop
  }

  strncpy(de.name, name, DIRSIZ);
801021dc:	c7 44 24 08 0e 00 00 	movl   $0xe,0x8(%esp)
801021e3:	00 
801021e4:	8b 45 0c             	mov    0xc(%ebp),%eax
801021e7:	89 44 24 04          	mov    %eax,0x4(%esp)
801021eb:	8d 45 e0             	lea    -0x20(%ebp),%eax
801021ee:	83 c0 02             	add    $0x2,%eax
801021f1:	89 04 24             	mov    %eax,(%esp)
801021f4:	e8 78 32 00 00       	call   80105471 <strncpy>
  de.inum = inum;
801021f9:	8b 45 10             	mov    0x10(%ebp),%eax
801021fc:	66 89 45 e0          	mov    %ax,-0x20(%ebp)
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80102200:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102203:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
8010220a:	00 
8010220b:	89 44 24 08          	mov    %eax,0x8(%esp)
8010220f:	8d 45 e0             	lea    -0x20(%ebp),%eax
80102212:	89 44 24 04          	mov    %eax,0x4(%esp)
80102216:	8b 45 08             	mov    0x8(%ebp),%eax
80102219:	89 04 24             	mov    %eax,(%esp)
8010221c:	e8 a0 fc ff ff       	call   80101ec1 <writei>
80102221:	83 f8 10             	cmp    $0x10,%eax
80102224:	74 0c                	je     80102232 <dirlink+0xf1>
    panic("dirlink");
80102226:	c7 04 24 ec 87 10 80 	movl   $0x801087ec,(%esp)
8010222d:	e8 0b e3 ff ff       	call   8010053d <panic>
  
  return 0;
80102232:	b8 00 00 00 00       	mov    $0x0,%eax
}
80102237:	c9                   	leave  
80102238:	c3                   	ret    

80102239 <skipelem>:
//   skipelem("a", name) = "", setting name = "a"
//   skipelem("", name) = skipelem("////", name) = 0
//
static char*
skipelem(char *path, char *name)
{
80102239:	55                   	push   %ebp
8010223a:	89 e5                	mov    %esp,%ebp
8010223c:	83 ec 28             	sub    $0x28,%esp
  char *s;
  int len;

  while(*path == '/')
8010223f:	eb 04                	jmp    80102245 <skipelem+0xc>
    path++;
80102241:	83 45 08 01          	addl   $0x1,0x8(%ebp)
skipelem(char *path, char *name)
{
  char *s;
  int len;

  while(*path == '/')
80102245:	8b 45 08             	mov    0x8(%ebp),%eax
80102248:	0f b6 00             	movzbl (%eax),%eax
8010224b:	3c 2f                	cmp    $0x2f,%al
8010224d:	74 f2                	je     80102241 <skipelem+0x8>
    path++;
  if(*path == 0)
8010224f:	8b 45 08             	mov    0x8(%ebp),%eax
80102252:	0f b6 00             	movzbl (%eax),%eax
80102255:	84 c0                	test   %al,%al
80102257:	75 0a                	jne    80102263 <skipelem+0x2a>
    return 0;
80102259:	b8 00 00 00 00       	mov    $0x0,%eax
8010225e:	e9 86 00 00 00       	jmp    801022e9 <skipelem+0xb0>
  s = path;
80102263:	8b 45 08             	mov    0x8(%ebp),%eax
80102266:	89 45 f4             	mov    %eax,-0xc(%ebp)
  while(*path != '/' && *path != 0)
80102269:	eb 04                	jmp    8010226f <skipelem+0x36>
    path++;
8010226b:	83 45 08 01          	addl   $0x1,0x8(%ebp)
  while(*path == '/')
    path++;
  if(*path == 0)
    return 0;
  s = path;
  while(*path != '/' && *path != 0)
8010226f:	8b 45 08             	mov    0x8(%ebp),%eax
80102272:	0f b6 00             	movzbl (%eax),%eax
80102275:	3c 2f                	cmp    $0x2f,%al
80102277:	74 0a                	je     80102283 <skipelem+0x4a>
80102279:	8b 45 08             	mov    0x8(%ebp),%eax
8010227c:	0f b6 00             	movzbl (%eax),%eax
8010227f:	84 c0                	test   %al,%al
80102281:	75 e8                	jne    8010226b <skipelem+0x32>
    path++;
  len = path - s;
80102283:	8b 55 08             	mov    0x8(%ebp),%edx
80102286:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102289:	89 d1                	mov    %edx,%ecx
8010228b:	29 c1                	sub    %eax,%ecx
8010228d:	89 c8                	mov    %ecx,%eax
8010228f:	89 45 f0             	mov    %eax,-0x10(%ebp)
  if(len >= DIRSIZ)
80102292:	83 7d f0 0d          	cmpl   $0xd,-0x10(%ebp)
80102296:	7e 1c                	jle    801022b4 <skipelem+0x7b>
    memmove(name, s, DIRSIZ);
80102298:	c7 44 24 08 0e 00 00 	movl   $0xe,0x8(%esp)
8010229f:	00 
801022a0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801022a3:	89 44 24 04          	mov    %eax,0x4(%esp)
801022a7:	8b 45 0c             	mov    0xc(%ebp),%eax
801022aa:	89 04 24             	mov    %eax,(%esp)
801022ad:	e8 c3 30 00 00       	call   80105375 <memmove>
  else {
    memmove(name, s, len);
    name[len] = 0;
  }
  while(*path == '/')
801022b2:	eb 28                	jmp    801022dc <skipelem+0xa3>
    path++;
  len = path - s;
  if(len >= DIRSIZ)
    memmove(name, s, DIRSIZ);
  else {
    memmove(name, s, len);
801022b4:	8b 45 f0             	mov    -0x10(%ebp),%eax
801022b7:	89 44 24 08          	mov    %eax,0x8(%esp)
801022bb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801022be:	89 44 24 04          	mov    %eax,0x4(%esp)
801022c2:	8b 45 0c             	mov    0xc(%ebp),%eax
801022c5:	89 04 24             	mov    %eax,(%esp)
801022c8:	e8 a8 30 00 00       	call   80105375 <memmove>
    name[len] = 0;
801022cd:	8b 45 f0             	mov    -0x10(%ebp),%eax
801022d0:	03 45 0c             	add    0xc(%ebp),%eax
801022d3:	c6 00 00             	movb   $0x0,(%eax)
  }
  while(*path == '/')
801022d6:	eb 04                	jmp    801022dc <skipelem+0xa3>
    path++;
801022d8:	83 45 08 01          	addl   $0x1,0x8(%ebp)
    memmove(name, s, DIRSIZ);
  else {
    memmove(name, s, len);
    name[len] = 0;
  }
  while(*path == '/')
801022dc:	8b 45 08             	mov    0x8(%ebp),%eax
801022df:	0f b6 00             	movzbl (%eax),%eax
801022e2:	3c 2f                	cmp    $0x2f,%al
801022e4:	74 f2                	je     801022d8 <skipelem+0x9f>
    path++;
  return path;
801022e6:	8b 45 08             	mov    0x8(%ebp),%eax
}
801022e9:	c9                   	leave  
801022ea:	c3                   	ret    

801022eb <namex>:
// Look up and return the inode for a path name.
// If parent != 0, return the inode for the parent and copy the final
// path element into name, which must have room for DIRSIZ bytes.
static struct inode*
namex(char *path, int nameiparent, char *name)
{
801022eb:	55                   	push   %ebp
801022ec:	89 e5                	mov    %esp,%ebp
801022ee:	83 ec 28             	sub    $0x28,%esp
  struct inode *ip, *next;

  if(*path == '/')
801022f1:	8b 45 08             	mov    0x8(%ebp),%eax
801022f4:	0f b6 00             	movzbl (%eax),%eax
801022f7:	3c 2f                	cmp    $0x2f,%al
801022f9:	75 1c                	jne    80102317 <namex+0x2c>
    ip = iget(ROOTDEV, ROOTINO);
801022fb:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80102302:	00 
80102303:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
8010230a:	e8 4d f4 ff ff       	call   8010175c <iget>
8010230f:	89 45 f4             	mov    %eax,-0xc(%ebp)
  else
    ip = idup(proc->cwd);

  while((path = skipelem(path, name)) != 0){
80102312:	e9 af 00 00 00       	jmp    801023c6 <namex+0xdb>
  struct inode *ip, *next;

  if(*path == '/')
    ip = iget(ROOTDEV, ROOTINO);
  else
    ip = idup(proc->cwd);
80102317:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010231d:	8b 40 68             	mov    0x68(%eax),%eax
80102320:	89 04 24             	mov    %eax,(%esp)
80102323:	e8 06 f5 ff ff       	call   8010182e <idup>
80102328:	89 45 f4             	mov    %eax,-0xc(%ebp)

  while((path = skipelem(path, name)) != 0){
8010232b:	e9 96 00 00 00       	jmp    801023c6 <namex+0xdb>
    ilock(ip);
80102330:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102333:	89 04 24             	mov    %eax,(%esp)
80102336:	e8 25 f5 ff ff       	call   80101860 <ilock>
    if(ip->type != T_DIR){
8010233b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010233e:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80102342:	66 83 f8 01          	cmp    $0x1,%ax
80102346:	74 15                	je     8010235d <namex+0x72>
      iunlockput(ip);
80102348:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010234b:	89 04 24             	mov    %eax,(%esp)
8010234e:	e8 91 f7 ff ff       	call   80101ae4 <iunlockput>
      return 0;
80102353:	b8 00 00 00 00       	mov    $0x0,%eax
80102358:	e9 a3 00 00 00       	jmp    80102400 <namex+0x115>
    }
    if(nameiparent && *path == '\0'){
8010235d:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80102361:	74 1d                	je     80102380 <namex+0x95>
80102363:	8b 45 08             	mov    0x8(%ebp),%eax
80102366:	0f b6 00             	movzbl (%eax),%eax
80102369:	84 c0                	test   %al,%al
8010236b:	75 13                	jne    80102380 <namex+0x95>
      // Stop one level early.
      iunlock(ip);
8010236d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102370:	89 04 24             	mov    %eax,(%esp)
80102373:	e8 36 f6 ff ff       	call   801019ae <iunlock>
      return ip;
80102378:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010237b:	e9 80 00 00 00       	jmp    80102400 <namex+0x115>
    }
    if((next = dirlookup(ip, name, 0)) == 0){
80102380:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80102387:	00 
80102388:	8b 45 10             	mov    0x10(%ebp),%eax
8010238b:	89 44 24 04          	mov    %eax,0x4(%esp)
8010238f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102392:	89 04 24             	mov    %eax,(%esp)
80102395:	e8 df fc ff ff       	call   80102079 <dirlookup>
8010239a:	89 45 f0             	mov    %eax,-0x10(%ebp)
8010239d:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
801023a1:	75 12                	jne    801023b5 <namex+0xca>
      iunlockput(ip);
801023a3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801023a6:	89 04 24             	mov    %eax,(%esp)
801023a9:	e8 36 f7 ff ff       	call   80101ae4 <iunlockput>
      return 0;
801023ae:	b8 00 00 00 00       	mov    $0x0,%eax
801023b3:	eb 4b                	jmp    80102400 <namex+0x115>
    }
    iunlockput(ip);
801023b5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801023b8:	89 04 24             	mov    %eax,(%esp)
801023bb:	e8 24 f7 ff ff       	call   80101ae4 <iunlockput>
    ip = next;
801023c0:	8b 45 f0             	mov    -0x10(%ebp),%eax
801023c3:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(*path == '/')
    ip = iget(ROOTDEV, ROOTINO);
  else
    ip = idup(proc->cwd);

  while((path = skipelem(path, name)) != 0){
801023c6:	8b 45 10             	mov    0x10(%ebp),%eax
801023c9:	89 44 24 04          	mov    %eax,0x4(%esp)
801023cd:	8b 45 08             	mov    0x8(%ebp),%eax
801023d0:	89 04 24             	mov    %eax,(%esp)
801023d3:	e8 61 fe ff ff       	call   80102239 <skipelem>
801023d8:	89 45 08             	mov    %eax,0x8(%ebp)
801023db:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801023df:	0f 85 4b ff ff ff    	jne    80102330 <namex+0x45>
      return 0;
    }
    iunlockput(ip);
    ip = next;
  }
  if(nameiparent){
801023e5:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
801023e9:	74 12                	je     801023fd <namex+0x112>
    iput(ip);
801023eb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801023ee:	89 04 24             	mov    %eax,(%esp)
801023f1:	e8 1d f6 ff ff       	call   80101a13 <iput>
    return 0;
801023f6:	b8 00 00 00 00       	mov    $0x0,%eax
801023fb:	eb 03                	jmp    80102400 <namex+0x115>
  }
  return ip;
801023fd:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80102400:	c9                   	leave  
80102401:	c3                   	ret    

80102402 <namei>:

struct inode*
namei(char *path)
{
80102402:	55                   	push   %ebp
80102403:	89 e5                	mov    %esp,%ebp
80102405:	83 ec 28             	sub    $0x28,%esp
  char name[DIRSIZ];
  return namex(path, 0, name);
80102408:	8d 45 ea             	lea    -0x16(%ebp),%eax
8010240b:	89 44 24 08          	mov    %eax,0x8(%esp)
8010240f:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102416:	00 
80102417:	8b 45 08             	mov    0x8(%ebp),%eax
8010241a:	89 04 24             	mov    %eax,(%esp)
8010241d:	e8 c9 fe ff ff       	call   801022eb <namex>
}
80102422:	c9                   	leave  
80102423:	c3                   	ret    

80102424 <nameiparent>:

struct inode*
nameiparent(char *path, char *name)
{
80102424:	55                   	push   %ebp
80102425:	89 e5                	mov    %esp,%ebp
80102427:	83 ec 18             	sub    $0x18,%esp
  return namex(path, 1, name);
8010242a:	8b 45 0c             	mov    0xc(%ebp),%eax
8010242d:	89 44 24 08          	mov    %eax,0x8(%esp)
80102431:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80102438:	00 
80102439:	8b 45 08             	mov    0x8(%ebp),%eax
8010243c:	89 04 24             	mov    %eax,(%esp)
8010243f:	e8 a7 fe ff ff       	call   801022eb <namex>
}
80102444:	c9                   	leave  
80102445:	c3                   	ret    
	...

80102448 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
80102448:	55                   	push   %ebp
80102449:	89 e5                	mov    %esp,%ebp
8010244b:	53                   	push   %ebx
8010244c:	83 ec 14             	sub    $0x14,%esp
8010244f:	8b 45 08             	mov    0x8(%ebp),%eax
80102452:	66 89 45 e8          	mov    %ax,-0x18(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102456:	0f b7 55 e8          	movzwl -0x18(%ebp),%edx
8010245a:	66 89 55 ea          	mov    %dx,-0x16(%ebp)
8010245e:	0f b7 55 ea          	movzwl -0x16(%ebp),%edx
80102462:	ec                   	in     (%dx),%al
80102463:	89 c3                	mov    %eax,%ebx
80102465:	88 5d fb             	mov    %bl,-0x5(%ebp)
  return data;
80102468:	0f b6 45 fb          	movzbl -0x5(%ebp),%eax
}
8010246c:	83 c4 14             	add    $0x14,%esp
8010246f:	5b                   	pop    %ebx
80102470:	5d                   	pop    %ebp
80102471:	c3                   	ret    

80102472 <insl>:

static inline void
insl(int port, void *addr, int cnt)
{
80102472:	55                   	push   %ebp
80102473:	89 e5                	mov    %esp,%ebp
80102475:	57                   	push   %edi
80102476:	53                   	push   %ebx
  asm volatile("cld; rep insl" :
80102477:	8b 55 08             	mov    0x8(%ebp),%edx
8010247a:	8b 4d 0c             	mov    0xc(%ebp),%ecx
8010247d:	8b 45 10             	mov    0x10(%ebp),%eax
80102480:	89 cb                	mov    %ecx,%ebx
80102482:	89 df                	mov    %ebx,%edi
80102484:	89 c1                	mov    %eax,%ecx
80102486:	fc                   	cld    
80102487:	f3 6d                	rep insl (%dx),%es:(%edi)
80102489:	89 c8                	mov    %ecx,%eax
8010248b:	89 fb                	mov    %edi,%ebx
8010248d:	89 5d 0c             	mov    %ebx,0xc(%ebp)
80102490:	89 45 10             	mov    %eax,0x10(%ebp)
               "=D" (addr), "=c" (cnt) :
               "d" (port), "0" (addr), "1" (cnt) :
               "memory", "cc");
}
80102493:	5b                   	pop    %ebx
80102494:	5f                   	pop    %edi
80102495:	5d                   	pop    %ebp
80102496:	c3                   	ret    

80102497 <outb>:

static inline void
outb(ushort port, uchar data)
{
80102497:	55                   	push   %ebp
80102498:	89 e5                	mov    %esp,%ebp
8010249a:	83 ec 08             	sub    $0x8,%esp
8010249d:	8b 55 08             	mov    0x8(%ebp),%edx
801024a0:	8b 45 0c             	mov    0xc(%ebp),%eax
801024a3:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
801024a7:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801024aa:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
801024ae:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
801024b2:	ee                   	out    %al,(%dx)
}
801024b3:	c9                   	leave  
801024b4:	c3                   	ret    

801024b5 <outsl>:
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
}

static inline void
outsl(int port, const void *addr, int cnt)
{
801024b5:	55                   	push   %ebp
801024b6:	89 e5                	mov    %esp,%ebp
801024b8:	56                   	push   %esi
801024b9:	53                   	push   %ebx
  asm volatile("cld; rep outsl" :
801024ba:	8b 55 08             	mov    0x8(%ebp),%edx
801024bd:	8b 4d 0c             	mov    0xc(%ebp),%ecx
801024c0:	8b 45 10             	mov    0x10(%ebp),%eax
801024c3:	89 cb                	mov    %ecx,%ebx
801024c5:	89 de                	mov    %ebx,%esi
801024c7:	89 c1                	mov    %eax,%ecx
801024c9:	fc                   	cld    
801024ca:	f3 6f                	rep outsl %ds:(%esi),(%dx)
801024cc:	89 c8                	mov    %ecx,%eax
801024ce:	89 f3                	mov    %esi,%ebx
801024d0:	89 5d 0c             	mov    %ebx,0xc(%ebp)
801024d3:	89 45 10             	mov    %eax,0x10(%ebp)
               "=S" (addr), "=c" (cnt) :
               "d" (port), "0" (addr), "1" (cnt) :
               "cc");
}
801024d6:	5b                   	pop    %ebx
801024d7:	5e                   	pop    %esi
801024d8:	5d                   	pop    %ebp
801024d9:	c3                   	ret    

801024da <idewait>:
static void idestart(struct buf*);

// Wait for IDE disk to become ready.
static int
idewait(int checkerr)
{
801024da:	55                   	push   %ebp
801024db:	89 e5                	mov    %esp,%ebp
801024dd:	83 ec 14             	sub    $0x14,%esp
  int r;

  while(((r = inb(0x1f7)) & (IDE_BSY|IDE_DRDY)) != IDE_DRDY) 
801024e0:	90                   	nop
801024e1:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
801024e8:	e8 5b ff ff ff       	call   80102448 <inb>
801024ed:	0f b6 c0             	movzbl %al,%eax
801024f0:	89 45 fc             	mov    %eax,-0x4(%ebp)
801024f3:	8b 45 fc             	mov    -0x4(%ebp),%eax
801024f6:	25 c0 00 00 00       	and    $0xc0,%eax
801024fb:	83 f8 40             	cmp    $0x40,%eax
801024fe:	75 e1                	jne    801024e1 <idewait+0x7>
    ;
  if(checkerr && (r & (IDE_DF|IDE_ERR)) != 0)
80102500:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80102504:	74 11                	je     80102517 <idewait+0x3d>
80102506:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102509:	83 e0 21             	and    $0x21,%eax
8010250c:	85 c0                	test   %eax,%eax
8010250e:	74 07                	je     80102517 <idewait+0x3d>
    return -1;
80102510:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80102515:	eb 05                	jmp    8010251c <idewait+0x42>
  return 0;
80102517:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010251c:	c9                   	leave  
8010251d:	c3                   	ret    

8010251e <ideinit>:

void
ideinit(void)
{
8010251e:	55                   	push   %ebp
8010251f:	89 e5                	mov    %esp,%ebp
80102521:	83 ec 28             	sub    $0x28,%esp
  int i;

  initlock(&idelock, "ide");
80102524:	c7 44 24 04 f4 87 10 	movl   $0x801087f4,0x4(%esp)
8010252b:	80 
8010252c:	c7 04 24 00 b6 10 80 	movl   $0x8010b600,(%esp)
80102533:	e8 fa 2a 00 00       	call   80105032 <initlock>
  picenable(IRQ_IDE);
80102538:	c7 04 24 0e 00 00 00 	movl   $0xe,(%esp)
8010253f:	e8 69 15 00 00       	call   80103aad <picenable>
  ioapicenable(IRQ_IDE, ncpu - 1);
80102544:	a1 00 ff 10 80       	mov    0x8010ff00,%eax
80102549:	83 e8 01             	sub    $0x1,%eax
8010254c:	89 44 24 04          	mov    %eax,0x4(%esp)
80102550:	c7 04 24 0e 00 00 00 	movl   $0xe,(%esp)
80102557:	e8 12 04 00 00       	call   8010296e <ioapicenable>
  idewait(0);
8010255c:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80102563:	e8 72 ff ff ff       	call   801024da <idewait>
  
  // Check if disk 1 is present
  outb(0x1f6, 0xe0 | (1<<4));
80102568:	c7 44 24 04 f0 00 00 	movl   $0xf0,0x4(%esp)
8010256f:	00 
80102570:	c7 04 24 f6 01 00 00 	movl   $0x1f6,(%esp)
80102577:	e8 1b ff ff ff       	call   80102497 <outb>
  for(i=0; i<1000; i++){
8010257c:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80102583:	eb 20                	jmp    801025a5 <ideinit+0x87>
    if(inb(0x1f7) != 0){
80102585:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
8010258c:	e8 b7 fe ff ff       	call   80102448 <inb>
80102591:	84 c0                	test   %al,%al
80102593:	74 0c                	je     801025a1 <ideinit+0x83>
      havedisk1 = 1;
80102595:	c7 05 38 b6 10 80 01 	movl   $0x1,0x8010b638
8010259c:	00 00 00 
      break;
8010259f:	eb 0d                	jmp    801025ae <ideinit+0x90>
  ioapicenable(IRQ_IDE, ncpu - 1);
  idewait(0);
  
  // Check if disk 1 is present
  outb(0x1f6, 0xe0 | (1<<4));
  for(i=0; i<1000; i++){
801025a1:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801025a5:	81 7d f4 e7 03 00 00 	cmpl   $0x3e7,-0xc(%ebp)
801025ac:	7e d7                	jle    80102585 <ideinit+0x67>
      break;
    }
  }
  
  // Switch back to disk 0.
  outb(0x1f6, 0xe0 | (0<<4));
801025ae:	c7 44 24 04 e0 00 00 	movl   $0xe0,0x4(%esp)
801025b5:	00 
801025b6:	c7 04 24 f6 01 00 00 	movl   $0x1f6,(%esp)
801025bd:	e8 d5 fe ff ff       	call   80102497 <outb>
}
801025c2:	c9                   	leave  
801025c3:	c3                   	ret    

801025c4 <idestart>:

// Start the request for b.  Caller must hold idelock.
static void
idestart(struct buf *b)
{
801025c4:	55                   	push   %ebp
801025c5:	89 e5                	mov    %esp,%ebp
801025c7:	83 ec 18             	sub    $0x18,%esp
  if(b == 0)
801025ca:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801025ce:	75 0c                	jne    801025dc <idestart+0x18>
    panic("idestart");
801025d0:	c7 04 24 f8 87 10 80 	movl   $0x801087f8,(%esp)
801025d7:	e8 61 df ff ff       	call   8010053d <panic>

  idewait(0);
801025dc:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801025e3:	e8 f2 fe ff ff       	call   801024da <idewait>
  outb(0x3f6, 0);  // generate interrupt
801025e8:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801025ef:	00 
801025f0:	c7 04 24 f6 03 00 00 	movl   $0x3f6,(%esp)
801025f7:	e8 9b fe ff ff       	call   80102497 <outb>
  outb(0x1f2, 1);  // number of sectors
801025fc:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80102603:	00 
80102604:	c7 04 24 f2 01 00 00 	movl   $0x1f2,(%esp)
8010260b:	e8 87 fe ff ff       	call   80102497 <outb>
  outb(0x1f3, b->sector & 0xff);
80102610:	8b 45 08             	mov    0x8(%ebp),%eax
80102613:	8b 40 08             	mov    0x8(%eax),%eax
80102616:	0f b6 c0             	movzbl %al,%eax
80102619:	89 44 24 04          	mov    %eax,0x4(%esp)
8010261d:	c7 04 24 f3 01 00 00 	movl   $0x1f3,(%esp)
80102624:	e8 6e fe ff ff       	call   80102497 <outb>
  outb(0x1f4, (b->sector >> 8) & 0xff);
80102629:	8b 45 08             	mov    0x8(%ebp),%eax
8010262c:	8b 40 08             	mov    0x8(%eax),%eax
8010262f:	c1 e8 08             	shr    $0x8,%eax
80102632:	0f b6 c0             	movzbl %al,%eax
80102635:	89 44 24 04          	mov    %eax,0x4(%esp)
80102639:	c7 04 24 f4 01 00 00 	movl   $0x1f4,(%esp)
80102640:	e8 52 fe ff ff       	call   80102497 <outb>
  outb(0x1f5, (b->sector >> 16) & 0xff);
80102645:	8b 45 08             	mov    0x8(%ebp),%eax
80102648:	8b 40 08             	mov    0x8(%eax),%eax
8010264b:	c1 e8 10             	shr    $0x10,%eax
8010264e:	0f b6 c0             	movzbl %al,%eax
80102651:	89 44 24 04          	mov    %eax,0x4(%esp)
80102655:	c7 04 24 f5 01 00 00 	movl   $0x1f5,(%esp)
8010265c:	e8 36 fe ff ff       	call   80102497 <outb>
  outb(0x1f6, 0xe0 | ((b->dev&1)<<4) | ((b->sector>>24)&0x0f));
80102661:	8b 45 08             	mov    0x8(%ebp),%eax
80102664:	8b 40 04             	mov    0x4(%eax),%eax
80102667:	83 e0 01             	and    $0x1,%eax
8010266a:	89 c2                	mov    %eax,%edx
8010266c:	c1 e2 04             	shl    $0x4,%edx
8010266f:	8b 45 08             	mov    0x8(%ebp),%eax
80102672:	8b 40 08             	mov    0x8(%eax),%eax
80102675:	c1 e8 18             	shr    $0x18,%eax
80102678:	83 e0 0f             	and    $0xf,%eax
8010267b:	09 d0                	or     %edx,%eax
8010267d:	83 c8 e0             	or     $0xffffffe0,%eax
80102680:	0f b6 c0             	movzbl %al,%eax
80102683:	89 44 24 04          	mov    %eax,0x4(%esp)
80102687:	c7 04 24 f6 01 00 00 	movl   $0x1f6,(%esp)
8010268e:	e8 04 fe ff ff       	call   80102497 <outb>
  if(b->flags & B_DIRTY){
80102693:	8b 45 08             	mov    0x8(%ebp),%eax
80102696:	8b 00                	mov    (%eax),%eax
80102698:	83 e0 04             	and    $0x4,%eax
8010269b:	85 c0                	test   %eax,%eax
8010269d:	74 34                	je     801026d3 <idestart+0x10f>
    outb(0x1f7, IDE_CMD_WRITE);
8010269f:	c7 44 24 04 30 00 00 	movl   $0x30,0x4(%esp)
801026a6:	00 
801026a7:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
801026ae:	e8 e4 fd ff ff       	call   80102497 <outb>
    outsl(0x1f0, b->data, 512/4);
801026b3:	8b 45 08             	mov    0x8(%ebp),%eax
801026b6:	83 c0 18             	add    $0x18,%eax
801026b9:	c7 44 24 08 80 00 00 	movl   $0x80,0x8(%esp)
801026c0:	00 
801026c1:	89 44 24 04          	mov    %eax,0x4(%esp)
801026c5:	c7 04 24 f0 01 00 00 	movl   $0x1f0,(%esp)
801026cc:	e8 e4 fd ff ff       	call   801024b5 <outsl>
801026d1:	eb 14                	jmp    801026e7 <idestart+0x123>
  } else {
    outb(0x1f7, IDE_CMD_READ);
801026d3:	c7 44 24 04 20 00 00 	movl   $0x20,0x4(%esp)
801026da:	00 
801026db:	c7 04 24 f7 01 00 00 	movl   $0x1f7,(%esp)
801026e2:	e8 b0 fd ff ff       	call   80102497 <outb>
  }
}
801026e7:	c9                   	leave  
801026e8:	c3                   	ret    

801026e9 <ideintr>:

// Interrupt handler.
void
ideintr(void)
{
801026e9:	55                   	push   %ebp
801026ea:	89 e5                	mov    %esp,%ebp
801026ec:	83 ec 28             	sub    $0x28,%esp
  struct buf *b;

  // First queued buffer is the active request.
  acquire(&idelock);
801026ef:	c7 04 24 00 b6 10 80 	movl   $0x8010b600,(%esp)
801026f6:	e8 58 29 00 00       	call   80105053 <acquire>
  if((b = idequeue) == 0){
801026fb:	a1 34 b6 10 80       	mov    0x8010b634,%eax
80102700:	89 45 f4             	mov    %eax,-0xc(%ebp)
80102703:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80102707:	75 11                	jne    8010271a <ideintr+0x31>
    release(&idelock);
80102709:	c7 04 24 00 b6 10 80 	movl   $0x8010b600,(%esp)
80102710:	e8 a0 29 00 00       	call   801050b5 <release>
    // cprintf("spurious IDE interrupt\n");
    return;
80102715:	e9 90 00 00 00       	jmp    801027aa <ideintr+0xc1>
  }
  idequeue = b->qnext;
8010271a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010271d:	8b 40 14             	mov    0x14(%eax),%eax
80102720:	a3 34 b6 10 80       	mov    %eax,0x8010b634

  // Read data if needed.
  if(!(b->flags & B_DIRTY) && idewait(1) >= 0)
80102725:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102728:	8b 00                	mov    (%eax),%eax
8010272a:	83 e0 04             	and    $0x4,%eax
8010272d:	85 c0                	test   %eax,%eax
8010272f:	75 2e                	jne    8010275f <ideintr+0x76>
80102731:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80102738:	e8 9d fd ff ff       	call   801024da <idewait>
8010273d:	85 c0                	test   %eax,%eax
8010273f:	78 1e                	js     8010275f <ideintr+0x76>
    insl(0x1f0, b->data, 512/4);
80102741:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102744:	83 c0 18             	add    $0x18,%eax
80102747:	c7 44 24 08 80 00 00 	movl   $0x80,0x8(%esp)
8010274e:	00 
8010274f:	89 44 24 04          	mov    %eax,0x4(%esp)
80102753:	c7 04 24 f0 01 00 00 	movl   $0x1f0,(%esp)
8010275a:	e8 13 fd ff ff       	call   80102472 <insl>
  
  // Wake process waiting for this buf.
  b->flags |= B_VALID;
8010275f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102762:	8b 00                	mov    (%eax),%eax
80102764:	89 c2                	mov    %eax,%edx
80102766:	83 ca 02             	or     $0x2,%edx
80102769:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010276c:	89 10                	mov    %edx,(%eax)
  b->flags &= ~B_DIRTY;
8010276e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102771:	8b 00                	mov    (%eax),%eax
80102773:	89 c2                	mov    %eax,%edx
80102775:	83 e2 fb             	and    $0xfffffffb,%edx
80102778:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010277b:	89 10                	mov    %edx,(%eax)
  wakeup(b);
8010277d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102780:	89 04 24             	mov    %eax,(%esp)
80102783:	e8 64 21 00 00       	call   801048ec <wakeup>
  
  // Start disk on next buf in queue.
  if(idequeue != 0)
80102788:	a1 34 b6 10 80       	mov    0x8010b634,%eax
8010278d:	85 c0                	test   %eax,%eax
8010278f:	74 0d                	je     8010279e <ideintr+0xb5>
    idestart(idequeue);
80102791:	a1 34 b6 10 80       	mov    0x8010b634,%eax
80102796:	89 04 24             	mov    %eax,(%esp)
80102799:	e8 26 fe ff ff       	call   801025c4 <idestart>

  release(&idelock);
8010279e:	c7 04 24 00 b6 10 80 	movl   $0x8010b600,(%esp)
801027a5:	e8 0b 29 00 00       	call   801050b5 <release>
}
801027aa:	c9                   	leave  
801027ab:	c3                   	ret    

801027ac <iderw>:
// Sync buf with disk. 
// If B_DIRTY is set, write buf to disk, clear B_DIRTY, set B_VALID.
// Else if B_VALID is not set, read buf from disk, set B_VALID.
void
iderw(struct buf *b)
{
801027ac:	55                   	push   %ebp
801027ad:	89 e5                	mov    %esp,%ebp
801027af:	83 ec 28             	sub    $0x28,%esp
  struct buf **pp;

  if(!(b->flags & B_BUSY))
801027b2:	8b 45 08             	mov    0x8(%ebp),%eax
801027b5:	8b 00                	mov    (%eax),%eax
801027b7:	83 e0 01             	and    $0x1,%eax
801027ba:	85 c0                	test   %eax,%eax
801027bc:	75 0c                	jne    801027ca <iderw+0x1e>
    panic("iderw: buf not busy");
801027be:	c7 04 24 01 88 10 80 	movl   $0x80108801,(%esp)
801027c5:	e8 73 dd ff ff       	call   8010053d <panic>
  if((b->flags & (B_VALID|B_DIRTY)) == B_VALID)
801027ca:	8b 45 08             	mov    0x8(%ebp),%eax
801027cd:	8b 00                	mov    (%eax),%eax
801027cf:	83 e0 06             	and    $0x6,%eax
801027d2:	83 f8 02             	cmp    $0x2,%eax
801027d5:	75 0c                	jne    801027e3 <iderw+0x37>
    panic("iderw: nothing to do");
801027d7:	c7 04 24 15 88 10 80 	movl   $0x80108815,(%esp)
801027de:	e8 5a dd ff ff       	call   8010053d <panic>
  if(b->dev != 0 && !havedisk1)
801027e3:	8b 45 08             	mov    0x8(%ebp),%eax
801027e6:	8b 40 04             	mov    0x4(%eax),%eax
801027e9:	85 c0                	test   %eax,%eax
801027eb:	74 15                	je     80102802 <iderw+0x56>
801027ed:	a1 38 b6 10 80       	mov    0x8010b638,%eax
801027f2:	85 c0                	test   %eax,%eax
801027f4:	75 0c                	jne    80102802 <iderw+0x56>
    panic("iderw: ide disk 1 not present");
801027f6:	c7 04 24 2a 88 10 80 	movl   $0x8010882a,(%esp)
801027fd:	e8 3b dd ff ff       	call   8010053d <panic>

  acquire(&idelock);  //DOC:acquire-lock
80102802:	c7 04 24 00 b6 10 80 	movl   $0x8010b600,(%esp)
80102809:	e8 45 28 00 00       	call   80105053 <acquire>

  // Append b to idequeue.
  b->qnext = 0;
8010280e:	8b 45 08             	mov    0x8(%ebp),%eax
80102811:	c7 40 14 00 00 00 00 	movl   $0x0,0x14(%eax)
  for(pp=&idequeue; *pp; pp=&(*pp)->qnext)  //DOC:insert-queue
80102818:	c7 45 f4 34 b6 10 80 	movl   $0x8010b634,-0xc(%ebp)
8010281f:	eb 0b                	jmp    8010282c <iderw+0x80>
80102821:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102824:	8b 00                	mov    (%eax),%eax
80102826:	83 c0 14             	add    $0x14,%eax
80102829:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010282c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010282f:	8b 00                	mov    (%eax),%eax
80102831:	85 c0                	test   %eax,%eax
80102833:	75 ec                	jne    80102821 <iderw+0x75>
    ;
  *pp = b;
80102835:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102838:	8b 55 08             	mov    0x8(%ebp),%edx
8010283b:	89 10                	mov    %edx,(%eax)
  
  // Start disk if necessary.
  if(idequeue == b)
8010283d:	a1 34 b6 10 80       	mov    0x8010b634,%eax
80102842:	3b 45 08             	cmp    0x8(%ebp),%eax
80102845:	75 22                	jne    80102869 <iderw+0xbd>
    idestart(b);
80102847:	8b 45 08             	mov    0x8(%ebp),%eax
8010284a:	89 04 24             	mov    %eax,(%esp)
8010284d:	e8 72 fd ff ff       	call   801025c4 <idestart>
  
  // Wait for request to finish.
  while((b->flags & (B_VALID|B_DIRTY)) != B_VALID){
80102852:	eb 15                	jmp    80102869 <iderw+0xbd>
    sleep(b, &idelock);
80102854:	c7 44 24 04 00 b6 10 	movl   $0x8010b600,0x4(%esp)
8010285b:	80 
8010285c:	8b 45 08             	mov    0x8(%ebp),%eax
8010285f:	89 04 24             	mov    %eax,(%esp)
80102862:	e8 ab 1f 00 00       	call   80104812 <sleep>
80102867:	eb 01                	jmp    8010286a <iderw+0xbe>
  // Start disk if necessary.
  if(idequeue == b)
    idestart(b);
  
  // Wait for request to finish.
  while((b->flags & (B_VALID|B_DIRTY)) != B_VALID){
80102869:	90                   	nop
8010286a:	8b 45 08             	mov    0x8(%ebp),%eax
8010286d:	8b 00                	mov    (%eax),%eax
8010286f:	83 e0 06             	and    $0x6,%eax
80102872:	83 f8 02             	cmp    $0x2,%eax
80102875:	75 dd                	jne    80102854 <iderw+0xa8>
    sleep(b, &idelock);
  }

  release(&idelock);
80102877:	c7 04 24 00 b6 10 80 	movl   $0x8010b600,(%esp)
8010287e:	e8 32 28 00 00       	call   801050b5 <release>
}
80102883:	c9                   	leave  
80102884:	c3                   	ret    
80102885:	00 00                	add    %al,(%eax)
	...

80102888 <ioapicread>:
  uint data;
};

static uint
ioapicread(int reg)
{
80102888:	55                   	push   %ebp
80102889:	89 e5                	mov    %esp,%ebp
  ioapic->reg = reg;
8010288b:	a1 34 f8 10 80       	mov    0x8010f834,%eax
80102890:	8b 55 08             	mov    0x8(%ebp),%edx
80102893:	89 10                	mov    %edx,(%eax)
  return ioapic->data;
80102895:	a1 34 f8 10 80       	mov    0x8010f834,%eax
8010289a:	8b 40 10             	mov    0x10(%eax),%eax
}
8010289d:	5d                   	pop    %ebp
8010289e:	c3                   	ret    

8010289f <ioapicwrite>:

static void
ioapicwrite(int reg, uint data)
{
8010289f:	55                   	push   %ebp
801028a0:	89 e5                	mov    %esp,%ebp
  ioapic->reg = reg;
801028a2:	a1 34 f8 10 80       	mov    0x8010f834,%eax
801028a7:	8b 55 08             	mov    0x8(%ebp),%edx
801028aa:	89 10                	mov    %edx,(%eax)
  ioapic->data = data;
801028ac:	a1 34 f8 10 80       	mov    0x8010f834,%eax
801028b1:	8b 55 0c             	mov    0xc(%ebp),%edx
801028b4:	89 50 10             	mov    %edx,0x10(%eax)
}
801028b7:	5d                   	pop    %ebp
801028b8:	c3                   	ret    

801028b9 <ioapicinit>:

void
ioapicinit(void)
{
801028b9:	55                   	push   %ebp
801028ba:	89 e5                	mov    %esp,%ebp
801028bc:	83 ec 28             	sub    $0x28,%esp
  int i, id, maxintr;

  if(!ismp)
801028bf:	a1 04 f9 10 80       	mov    0x8010f904,%eax
801028c4:	85 c0                	test   %eax,%eax
801028c6:	0f 84 9f 00 00 00    	je     8010296b <ioapicinit+0xb2>
    return;

  ioapic = (volatile struct ioapic*)IOAPIC;
801028cc:	c7 05 34 f8 10 80 00 	movl   $0xfec00000,0x8010f834
801028d3:	00 c0 fe 
  maxintr = (ioapicread(REG_VER) >> 16) & 0xFF;
801028d6:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801028dd:	e8 a6 ff ff ff       	call   80102888 <ioapicread>
801028e2:	c1 e8 10             	shr    $0x10,%eax
801028e5:	25 ff 00 00 00       	and    $0xff,%eax
801028ea:	89 45 f0             	mov    %eax,-0x10(%ebp)
  id = ioapicread(REG_ID) >> 24;
801028ed:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801028f4:	e8 8f ff ff ff       	call   80102888 <ioapicread>
801028f9:	c1 e8 18             	shr    $0x18,%eax
801028fc:	89 45 ec             	mov    %eax,-0x14(%ebp)
  if(id != ioapicid)
801028ff:	0f b6 05 00 f9 10 80 	movzbl 0x8010f900,%eax
80102906:	0f b6 c0             	movzbl %al,%eax
80102909:	3b 45 ec             	cmp    -0x14(%ebp),%eax
8010290c:	74 0c                	je     8010291a <ioapicinit+0x61>
    cprintf("ioapicinit: id isn't equal to ioapicid; not a MP\n");
8010290e:	c7 04 24 48 88 10 80 	movl   $0x80108848,(%esp)
80102915:	e8 87 da ff ff       	call   801003a1 <cprintf>

  // Mark all interrupts edge-triggered, active high, disabled,
  // and not routed to any CPUs.
  for(i = 0; i <= maxintr; i++){
8010291a:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80102921:	eb 3e                	jmp    80102961 <ioapicinit+0xa8>
    ioapicwrite(REG_TABLE+2*i, INT_DISABLED | (T_IRQ0 + i));
80102923:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102926:	83 c0 20             	add    $0x20,%eax
80102929:	0d 00 00 01 00       	or     $0x10000,%eax
8010292e:	8b 55 f4             	mov    -0xc(%ebp),%edx
80102931:	83 c2 08             	add    $0x8,%edx
80102934:	01 d2                	add    %edx,%edx
80102936:	89 44 24 04          	mov    %eax,0x4(%esp)
8010293a:	89 14 24             	mov    %edx,(%esp)
8010293d:	e8 5d ff ff ff       	call   8010289f <ioapicwrite>
    ioapicwrite(REG_TABLE+2*i+1, 0);
80102942:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102945:	83 c0 08             	add    $0x8,%eax
80102948:	01 c0                	add    %eax,%eax
8010294a:	83 c0 01             	add    $0x1,%eax
8010294d:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102954:	00 
80102955:	89 04 24             	mov    %eax,(%esp)
80102958:	e8 42 ff ff ff       	call   8010289f <ioapicwrite>
  if(id != ioapicid)
    cprintf("ioapicinit: id isn't equal to ioapicid; not a MP\n");

  // Mark all interrupts edge-triggered, active high, disabled,
  // and not routed to any CPUs.
  for(i = 0; i <= maxintr; i++){
8010295d:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80102961:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102964:	3b 45 f0             	cmp    -0x10(%ebp),%eax
80102967:	7e ba                	jle    80102923 <ioapicinit+0x6a>
80102969:	eb 01                	jmp    8010296c <ioapicinit+0xb3>
ioapicinit(void)
{
  int i, id, maxintr;

  if(!ismp)
    return;
8010296b:	90                   	nop
  // and not routed to any CPUs.
  for(i = 0; i <= maxintr; i++){
    ioapicwrite(REG_TABLE+2*i, INT_DISABLED | (T_IRQ0 + i));
    ioapicwrite(REG_TABLE+2*i+1, 0);
  }
}
8010296c:	c9                   	leave  
8010296d:	c3                   	ret    

8010296e <ioapicenable>:

void
ioapicenable(int irq, int cpunum)
{
8010296e:	55                   	push   %ebp
8010296f:	89 e5                	mov    %esp,%ebp
80102971:	83 ec 08             	sub    $0x8,%esp
  if(!ismp)
80102974:	a1 04 f9 10 80       	mov    0x8010f904,%eax
80102979:	85 c0                	test   %eax,%eax
8010297b:	74 39                	je     801029b6 <ioapicenable+0x48>
    return;

  // Mark interrupt edge-triggered, active high,
  // enabled, and routed to the given cpunum,
  // which happens to be that cpu's APIC ID.
  ioapicwrite(REG_TABLE+2*irq, T_IRQ0 + irq);
8010297d:	8b 45 08             	mov    0x8(%ebp),%eax
80102980:	83 c0 20             	add    $0x20,%eax
80102983:	8b 55 08             	mov    0x8(%ebp),%edx
80102986:	83 c2 08             	add    $0x8,%edx
80102989:	01 d2                	add    %edx,%edx
8010298b:	89 44 24 04          	mov    %eax,0x4(%esp)
8010298f:	89 14 24             	mov    %edx,(%esp)
80102992:	e8 08 ff ff ff       	call   8010289f <ioapicwrite>
  ioapicwrite(REG_TABLE+2*irq+1, cpunum << 24);
80102997:	8b 45 0c             	mov    0xc(%ebp),%eax
8010299a:	c1 e0 18             	shl    $0x18,%eax
8010299d:	8b 55 08             	mov    0x8(%ebp),%edx
801029a0:	83 c2 08             	add    $0x8,%edx
801029a3:	01 d2                	add    %edx,%edx
801029a5:	83 c2 01             	add    $0x1,%edx
801029a8:	89 44 24 04          	mov    %eax,0x4(%esp)
801029ac:	89 14 24             	mov    %edx,(%esp)
801029af:	e8 eb fe ff ff       	call   8010289f <ioapicwrite>
801029b4:	eb 01                	jmp    801029b7 <ioapicenable+0x49>

void
ioapicenable(int irq, int cpunum)
{
  if(!ismp)
    return;
801029b6:	90                   	nop
  // Mark interrupt edge-triggered, active high,
  // enabled, and routed to the given cpunum,
  // which happens to be that cpu's APIC ID.
  ioapicwrite(REG_TABLE+2*irq, T_IRQ0 + irq);
  ioapicwrite(REG_TABLE+2*irq+1, cpunum << 24);
}
801029b7:	c9                   	leave  
801029b8:	c3                   	ret    
801029b9:	00 00                	add    %al,(%eax)
	...

801029bc <v2p>:
#define KERNBASE 0x80000000         // First kernel virtual address
#define KERNLINK (KERNBASE+EXTMEM)  // Address where kernel is linked

#ifndef __ASSEMBLER__

static inline uint v2p(void *a) { return ((uint) (a))  - KERNBASE; }
801029bc:	55                   	push   %ebp
801029bd:	89 e5                	mov    %esp,%ebp
801029bf:	8b 45 08             	mov    0x8(%ebp),%eax
801029c2:	05 00 00 00 80       	add    $0x80000000,%eax
801029c7:	5d                   	pop    %ebp
801029c8:	c3                   	ret    

801029c9 <kinit1>:
// the pages mapped by entrypgdir on free list.
// 2. main() calls kinit2() with the rest of the physical pages
// after installing a full page table that maps them on all cores.
void
kinit1(void *vstart, void *vend)
{
801029c9:	55                   	push   %ebp
801029ca:	89 e5                	mov    %esp,%ebp
801029cc:	83 ec 18             	sub    $0x18,%esp
  initlock(&kmem.lock, "kmem");
801029cf:	c7 44 24 04 7a 88 10 	movl   $0x8010887a,0x4(%esp)
801029d6:	80 
801029d7:	c7 04 24 40 f8 10 80 	movl   $0x8010f840,(%esp)
801029de:	e8 4f 26 00 00       	call   80105032 <initlock>
  kmem.use_lock = 0;
801029e3:	c7 05 74 f8 10 80 00 	movl   $0x0,0x8010f874
801029ea:	00 00 00 
  freerange(vstart, vend);
801029ed:	8b 45 0c             	mov    0xc(%ebp),%eax
801029f0:	89 44 24 04          	mov    %eax,0x4(%esp)
801029f4:	8b 45 08             	mov    0x8(%ebp),%eax
801029f7:	89 04 24             	mov    %eax,(%esp)
801029fa:	e8 26 00 00 00       	call   80102a25 <freerange>
}
801029ff:	c9                   	leave  
80102a00:	c3                   	ret    

80102a01 <kinit2>:

void
kinit2(void *vstart, void *vend)
{
80102a01:	55                   	push   %ebp
80102a02:	89 e5                	mov    %esp,%ebp
80102a04:	83 ec 18             	sub    $0x18,%esp
  freerange(vstart, vend);
80102a07:	8b 45 0c             	mov    0xc(%ebp),%eax
80102a0a:	89 44 24 04          	mov    %eax,0x4(%esp)
80102a0e:	8b 45 08             	mov    0x8(%ebp),%eax
80102a11:	89 04 24             	mov    %eax,(%esp)
80102a14:	e8 0c 00 00 00       	call   80102a25 <freerange>
  kmem.use_lock = 1;
80102a19:	c7 05 74 f8 10 80 01 	movl   $0x1,0x8010f874
80102a20:	00 00 00 
}
80102a23:	c9                   	leave  
80102a24:	c3                   	ret    

80102a25 <freerange>:

void
freerange(void *vstart, void *vend)
{
80102a25:	55                   	push   %ebp
80102a26:	89 e5                	mov    %esp,%ebp
80102a28:	83 ec 28             	sub    $0x28,%esp
  char *p;
  p = (char*)PGROUNDUP((uint)vstart);
80102a2b:	8b 45 08             	mov    0x8(%ebp),%eax
80102a2e:	05 ff 0f 00 00       	add    $0xfff,%eax
80102a33:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80102a38:	89 45 f4             	mov    %eax,-0xc(%ebp)
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102a3b:	eb 12                	jmp    80102a4f <freerange+0x2a>
    kfree(p);
80102a3d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102a40:	89 04 24             	mov    %eax,(%esp)
80102a43:	e8 16 00 00 00       	call   80102a5e <kfree>
void
freerange(void *vstart, void *vend)
{
  char *p;
  p = (char*)PGROUNDUP((uint)vstart);
  for(; p + PGSIZE <= (char*)vend; p += PGSIZE)
80102a48:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
80102a4f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102a52:	05 00 10 00 00       	add    $0x1000,%eax
80102a57:	3b 45 0c             	cmp    0xc(%ebp),%eax
80102a5a:	76 e1                	jbe    80102a3d <freerange+0x18>
    kfree(p);
}
80102a5c:	c9                   	leave  
80102a5d:	c3                   	ret    

80102a5e <kfree>:
// which normally should have been returned by a
// call to kalloc().  (The exception is when
// initializing the allocator; see kinit above.)
void
kfree(char *v)
{
80102a5e:	55                   	push   %ebp
80102a5f:	89 e5                	mov    %esp,%ebp
80102a61:	83 ec 28             	sub    $0x28,%esp
  struct run *r;

  if((uint)v % PGSIZE || v < end || v2p(v) >= PHYSTOP)
80102a64:	8b 45 08             	mov    0x8(%ebp),%eax
80102a67:	25 ff 0f 00 00       	and    $0xfff,%eax
80102a6c:	85 c0                	test   %eax,%eax
80102a6e:	75 1b                	jne    80102a8b <kfree+0x2d>
80102a70:	81 7d 08 5c 2d 11 80 	cmpl   $0x80112d5c,0x8(%ebp)
80102a77:	72 12                	jb     80102a8b <kfree+0x2d>
80102a79:	8b 45 08             	mov    0x8(%ebp),%eax
80102a7c:	89 04 24             	mov    %eax,(%esp)
80102a7f:	e8 38 ff ff ff       	call   801029bc <v2p>
80102a84:	3d ff ff ff 0d       	cmp    $0xdffffff,%eax
80102a89:	76 0c                	jbe    80102a97 <kfree+0x39>
    panic("kfree");
80102a8b:	c7 04 24 7f 88 10 80 	movl   $0x8010887f,(%esp)
80102a92:	e8 a6 da ff ff       	call   8010053d <panic>

  // Fill with junk to catch dangling refs.
  memset(v, 1, PGSIZE);
80102a97:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80102a9e:	00 
80102a9f:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80102aa6:	00 
80102aa7:	8b 45 08             	mov    0x8(%ebp),%eax
80102aaa:	89 04 24             	mov    %eax,(%esp)
80102aad:	e8 f0 27 00 00       	call   801052a2 <memset>

  if(kmem.use_lock)
80102ab2:	a1 74 f8 10 80       	mov    0x8010f874,%eax
80102ab7:	85 c0                	test   %eax,%eax
80102ab9:	74 0c                	je     80102ac7 <kfree+0x69>
    acquire(&kmem.lock);
80102abb:	c7 04 24 40 f8 10 80 	movl   $0x8010f840,(%esp)
80102ac2:	e8 8c 25 00 00       	call   80105053 <acquire>
  r = (struct run*)v;
80102ac7:	8b 45 08             	mov    0x8(%ebp),%eax
80102aca:	89 45 f4             	mov    %eax,-0xc(%ebp)
  r->next = kmem.freelist;
80102acd:	8b 15 78 f8 10 80    	mov    0x8010f878,%edx
80102ad3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102ad6:	89 10                	mov    %edx,(%eax)
  kmem.freelist = r;
80102ad8:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102adb:	a3 78 f8 10 80       	mov    %eax,0x8010f878
  if(kmem.use_lock)
80102ae0:	a1 74 f8 10 80       	mov    0x8010f874,%eax
80102ae5:	85 c0                	test   %eax,%eax
80102ae7:	74 0c                	je     80102af5 <kfree+0x97>
    release(&kmem.lock);
80102ae9:	c7 04 24 40 f8 10 80 	movl   $0x8010f840,(%esp)
80102af0:	e8 c0 25 00 00       	call   801050b5 <release>
}
80102af5:	c9                   	leave  
80102af6:	c3                   	ret    

80102af7 <kalloc>:
// Allocate one 4096-byte page of physical memory.
// Returns a pointer that the kernel can use.
// Returns 0 if the memory cannot be allocated.
char*
kalloc(void)
{
80102af7:	55                   	push   %ebp
80102af8:	89 e5                	mov    %esp,%ebp
80102afa:	83 ec 28             	sub    $0x28,%esp
  struct run *r;

  if(kmem.use_lock)
80102afd:	a1 74 f8 10 80       	mov    0x8010f874,%eax
80102b02:	85 c0                	test   %eax,%eax
80102b04:	74 0c                	je     80102b12 <kalloc+0x1b>
    acquire(&kmem.lock);
80102b06:	c7 04 24 40 f8 10 80 	movl   $0x8010f840,(%esp)
80102b0d:	e8 41 25 00 00       	call   80105053 <acquire>
  r = kmem.freelist;
80102b12:	a1 78 f8 10 80       	mov    0x8010f878,%eax
80102b17:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(r)
80102b1a:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80102b1e:	74 0a                	je     80102b2a <kalloc+0x33>
    kmem.freelist = r->next;
80102b20:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102b23:	8b 00                	mov    (%eax),%eax
80102b25:	a3 78 f8 10 80       	mov    %eax,0x8010f878
  if(kmem.use_lock)
80102b2a:	a1 74 f8 10 80       	mov    0x8010f874,%eax
80102b2f:	85 c0                	test   %eax,%eax
80102b31:	74 0c                	je     80102b3f <kalloc+0x48>
    release(&kmem.lock);
80102b33:	c7 04 24 40 f8 10 80 	movl   $0x8010f840,(%esp)
80102b3a:	e8 76 25 00 00       	call   801050b5 <release>
  return (char*)r;
80102b3f:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80102b42:	c9                   	leave  
80102b43:	c3                   	ret    

80102b44 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
80102b44:	55                   	push   %ebp
80102b45:	89 e5                	mov    %esp,%ebp
80102b47:	53                   	push   %ebx
80102b48:	83 ec 14             	sub    $0x14,%esp
80102b4b:	8b 45 08             	mov    0x8(%ebp),%eax
80102b4e:	66 89 45 e8          	mov    %ax,-0x18(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80102b52:	0f b7 55 e8          	movzwl -0x18(%ebp),%edx
80102b56:	66 89 55 ea          	mov    %dx,-0x16(%ebp)
80102b5a:	0f b7 55 ea          	movzwl -0x16(%ebp),%edx
80102b5e:	ec                   	in     (%dx),%al
80102b5f:	89 c3                	mov    %eax,%ebx
80102b61:	88 5d fb             	mov    %bl,-0x5(%ebp)
  return data;
80102b64:	0f b6 45 fb          	movzbl -0x5(%ebp),%eax
}
80102b68:	83 c4 14             	add    $0x14,%esp
80102b6b:	5b                   	pop    %ebx
80102b6c:	5d                   	pop    %ebp
80102b6d:	c3                   	ret    

80102b6e <kbdgetc>:
#include "defs.h"
#include "kbd.h"

int
kbdgetc(void)
{
80102b6e:	55                   	push   %ebp
80102b6f:	89 e5                	mov    %esp,%ebp
80102b71:	83 ec 14             	sub    $0x14,%esp
  static uchar *charcode[4] = {
    normalmap, shiftmap, ctlmap, ctlmap
  };
  uint st, data, c;

  st = inb(KBSTATP);
80102b74:	c7 04 24 64 00 00 00 	movl   $0x64,(%esp)
80102b7b:	e8 c4 ff ff ff       	call   80102b44 <inb>
80102b80:	0f b6 c0             	movzbl %al,%eax
80102b83:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if((st & KBS_DIB) == 0)
80102b86:	8b 45 f4             	mov    -0xc(%ebp),%eax
80102b89:	83 e0 01             	and    $0x1,%eax
80102b8c:	85 c0                	test   %eax,%eax
80102b8e:	75 0a                	jne    80102b9a <kbdgetc+0x2c>
    return -1;
80102b90:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80102b95:	e9 23 01 00 00       	jmp    80102cbd <kbdgetc+0x14f>
  data = inb(KBDATAP);
80102b9a:	c7 04 24 60 00 00 00 	movl   $0x60,(%esp)
80102ba1:	e8 9e ff ff ff       	call   80102b44 <inb>
80102ba6:	0f b6 c0             	movzbl %al,%eax
80102ba9:	89 45 fc             	mov    %eax,-0x4(%ebp)

  if(data == 0xE0){
80102bac:	81 7d fc e0 00 00 00 	cmpl   $0xe0,-0x4(%ebp)
80102bb3:	75 17                	jne    80102bcc <kbdgetc+0x5e>
    shift |= E0ESC;
80102bb5:	a1 3c b6 10 80       	mov    0x8010b63c,%eax
80102bba:	83 c8 40             	or     $0x40,%eax
80102bbd:	a3 3c b6 10 80       	mov    %eax,0x8010b63c
    return 0;
80102bc2:	b8 00 00 00 00       	mov    $0x0,%eax
80102bc7:	e9 f1 00 00 00       	jmp    80102cbd <kbdgetc+0x14f>
  } else if(data & 0x80){
80102bcc:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102bcf:	25 80 00 00 00       	and    $0x80,%eax
80102bd4:	85 c0                	test   %eax,%eax
80102bd6:	74 45                	je     80102c1d <kbdgetc+0xaf>
    // Key released
    data = (shift & E0ESC ? data : data & 0x7F);
80102bd8:	a1 3c b6 10 80       	mov    0x8010b63c,%eax
80102bdd:	83 e0 40             	and    $0x40,%eax
80102be0:	85 c0                	test   %eax,%eax
80102be2:	75 08                	jne    80102bec <kbdgetc+0x7e>
80102be4:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102be7:	83 e0 7f             	and    $0x7f,%eax
80102bea:	eb 03                	jmp    80102bef <kbdgetc+0x81>
80102bec:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102bef:	89 45 fc             	mov    %eax,-0x4(%ebp)
    shift &= ~(shiftcode[data] | E0ESC);
80102bf2:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102bf5:	05 20 90 10 80       	add    $0x80109020,%eax
80102bfa:	0f b6 00             	movzbl (%eax),%eax
80102bfd:	83 c8 40             	or     $0x40,%eax
80102c00:	0f b6 c0             	movzbl %al,%eax
80102c03:	f7 d0                	not    %eax
80102c05:	89 c2                	mov    %eax,%edx
80102c07:	a1 3c b6 10 80       	mov    0x8010b63c,%eax
80102c0c:	21 d0                	and    %edx,%eax
80102c0e:	a3 3c b6 10 80       	mov    %eax,0x8010b63c
    return 0;
80102c13:	b8 00 00 00 00       	mov    $0x0,%eax
80102c18:	e9 a0 00 00 00       	jmp    80102cbd <kbdgetc+0x14f>
  } else if(shift & E0ESC){
80102c1d:	a1 3c b6 10 80       	mov    0x8010b63c,%eax
80102c22:	83 e0 40             	and    $0x40,%eax
80102c25:	85 c0                	test   %eax,%eax
80102c27:	74 14                	je     80102c3d <kbdgetc+0xcf>
    // Last character was an E0 escape; or with 0x80
    data |= 0x80;
80102c29:	81 4d fc 80 00 00 00 	orl    $0x80,-0x4(%ebp)
    shift &= ~E0ESC;
80102c30:	a1 3c b6 10 80       	mov    0x8010b63c,%eax
80102c35:	83 e0 bf             	and    $0xffffffbf,%eax
80102c38:	a3 3c b6 10 80       	mov    %eax,0x8010b63c
  }

  shift |= shiftcode[data];
80102c3d:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102c40:	05 20 90 10 80       	add    $0x80109020,%eax
80102c45:	0f b6 00             	movzbl (%eax),%eax
80102c48:	0f b6 d0             	movzbl %al,%edx
80102c4b:	a1 3c b6 10 80       	mov    0x8010b63c,%eax
80102c50:	09 d0                	or     %edx,%eax
80102c52:	a3 3c b6 10 80       	mov    %eax,0x8010b63c
  shift ^= togglecode[data];
80102c57:	8b 45 fc             	mov    -0x4(%ebp),%eax
80102c5a:	05 20 91 10 80       	add    $0x80109120,%eax
80102c5f:	0f b6 00             	movzbl (%eax),%eax
80102c62:	0f b6 d0             	movzbl %al,%edx
80102c65:	a1 3c b6 10 80       	mov    0x8010b63c,%eax
80102c6a:	31 d0                	xor    %edx,%eax
80102c6c:	a3 3c b6 10 80       	mov    %eax,0x8010b63c
  c = charcode[shift & (CTL | SHIFT)][data];
80102c71:	a1 3c b6 10 80       	mov    0x8010b63c,%eax
80102c76:	83 e0 03             	and    $0x3,%eax
80102c79:	8b 04 85 20 95 10 80 	mov    -0x7fef6ae0(,%eax,4),%eax
80102c80:	03 45 fc             	add    -0x4(%ebp),%eax
80102c83:	0f b6 00             	movzbl (%eax),%eax
80102c86:	0f b6 c0             	movzbl %al,%eax
80102c89:	89 45 f8             	mov    %eax,-0x8(%ebp)
  if(shift & CAPSLOCK){
80102c8c:	a1 3c b6 10 80       	mov    0x8010b63c,%eax
80102c91:	83 e0 08             	and    $0x8,%eax
80102c94:	85 c0                	test   %eax,%eax
80102c96:	74 22                	je     80102cba <kbdgetc+0x14c>
    if('a' <= c && c <= 'z')
80102c98:	83 7d f8 60          	cmpl   $0x60,-0x8(%ebp)
80102c9c:	76 0c                	jbe    80102caa <kbdgetc+0x13c>
80102c9e:	83 7d f8 7a          	cmpl   $0x7a,-0x8(%ebp)
80102ca2:	77 06                	ja     80102caa <kbdgetc+0x13c>
      c += 'A' - 'a';
80102ca4:	83 6d f8 20          	subl   $0x20,-0x8(%ebp)
80102ca8:	eb 10                	jmp    80102cba <kbdgetc+0x14c>
    else if('A' <= c && c <= 'Z')
80102caa:	83 7d f8 40          	cmpl   $0x40,-0x8(%ebp)
80102cae:	76 0a                	jbe    80102cba <kbdgetc+0x14c>
80102cb0:	83 7d f8 5a          	cmpl   $0x5a,-0x8(%ebp)
80102cb4:	77 04                	ja     80102cba <kbdgetc+0x14c>
      c += 'a' - 'A';
80102cb6:	83 45 f8 20          	addl   $0x20,-0x8(%ebp)
  }
  return c;
80102cba:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
80102cbd:	c9                   	leave  
80102cbe:	c3                   	ret    

80102cbf <kbdintr>:

void
kbdintr(void)
{
80102cbf:	55                   	push   %ebp
80102cc0:	89 e5                	mov    %esp,%ebp
80102cc2:	83 ec 18             	sub    $0x18,%esp
  consoleintr(kbdgetc);
80102cc5:	c7 04 24 6e 2b 10 80 	movl   $0x80102b6e,(%esp)
80102ccc:	e8 dc da ff ff       	call   801007ad <consoleintr>
}
80102cd1:	c9                   	leave  
80102cd2:	c3                   	ret    
	...

80102cd4 <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
80102cd4:	55                   	push   %ebp
80102cd5:	89 e5                	mov    %esp,%ebp
80102cd7:	83 ec 08             	sub    $0x8,%esp
80102cda:	8b 55 08             	mov    0x8(%ebp),%edx
80102cdd:	8b 45 0c             	mov    0xc(%ebp),%eax
80102ce0:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
80102ce4:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80102ce7:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
80102ceb:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
80102cef:	ee                   	out    %al,(%dx)
}
80102cf0:	c9                   	leave  
80102cf1:	c3                   	ret    

80102cf2 <readeflags>:
  asm volatile("ltr %0" : : "r" (sel));
}

static inline uint
readeflags(void)
{
80102cf2:	55                   	push   %ebp
80102cf3:	89 e5                	mov    %esp,%ebp
80102cf5:	53                   	push   %ebx
80102cf6:	83 ec 10             	sub    $0x10,%esp
  uint eflags;
  asm volatile("pushfl; popl %0" : "=r" (eflags));
80102cf9:	9c                   	pushf  
80102cfa:	5b                   	pop    %ebx
80102cfb:	89 5d f8             	mov    %ebx,-0x8(%ebp)
  return eflags;
80102cfe:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
80102d01:	83 c4 10             	add    $0x10,%esp
80102d04:	5b                   	pop    %ebx
80102d05:	5d                   	pop    %ebp
80102d06:	c3                   	ret    

80102d07 <lapicw>:

volatile uint *lapic;  // Initialized in mp.c

static void
lapicw(int index, int value)
{
80102d07:	55                   	push   %ebp
80102d08:	89 e5                	mov    %esp,%ebp
  lapic[index] = value;
80102d0a:	a1 7c f8 10 80       	mov    0x8010f87c,%eax
80102d0f:	8b 55 08             	mov    0x8(%ebp),%edx
80102d12:	c1 e2 02             	shl    $0x2,%edx
80102d15:	01 c2                	add    %eax,%edx
80102d17:	8b 45 0c             	mov    0xc(%ebp),%eax
80102d1a:	89 02                	mov    %eax,(%edx)
  lapic[ID];  // wait for write to finish, by reading
80102d1c:	a1 7c f8 10 80       	mov    0x8010f87c,%eax
80102d21:	83 c0 20             	add    $0x20,%eax
80102d24:	8b 00                	mov    (%eax),%eax
}
80102d26:	5d                   	pop    %ebp
80102d27:	c3                   	ret    

80102d28 <lapicinit>:
//PAGEBREAK!

void
lapicinit(void)
{
80102d28:	55                   	push   %ebp
80102d29:	89 e5                	mov    %esp,%ebp
80102d2b:	83 ec 08             	sub    $0x8,%esp
  if(!lapic) 
80102d2e:	a1 7c f8 10 80       	mov    0x8010f87c,%eax
80102d33:	85 c0                	test   %eax,%eax
80102d35:	0f 84 47 01 00 00    	je     80102e82 <lapicinit+0x15a>
    return;

  // Enable local APIC; set spurious interrupt vector.
  lapicw(SVR, ENABLE | (T_IRQ0 + IRQ_SPURIOUS));
80102d3b:	c7 44 24 04 3f 01 00 	movl   $0x13f,0x4(%esp)
80102d42:	00 
80102d43:	c7 04 24 3c 00 00 00 	movl   $0x3c,(%esp)
80102d4a:	e8 b8 ff ff ff       	call   80102d07 <lapicw>

  // The timer repeatedly counts down at bus frequency
  // from lapic[TICR] and then issues an interrupt.  
  // If xv6 cared more about precise timekeeping,
  // TICR would be calibrated using an external time source.
  lapicw(TDCR, X1);
80102d4f:	c7 44 24 04 0b 00 00 	movl   $0xb,0x4(%esp)
80102d56:	00 
80102d57:	c7 04 24 f8 00 00 00 	movl   $0xf8,(%esp)
80102d5e:	e8 a4 ff ff ff       	call   80102d07 <lapicw>
  lapicw(TIMER, PERIODIC | (T_IRQ0 + IRQ_TIMER));
80102d63:	c7 44 24 04 20 00 02 	movl   $0x20020,0x4(%esp)
80102d6a:	00 
80102d6b:	c7 04 24 c8 00 00 00 	movl   $0xc8,(%esp)
80102d72:	e8 90 ff ff ff       	call   80102d07 <lapicw>
  lapicw(TICR, 10000000); 
80102d77:	c7 44 24 04 80 96 98 	movl   $0x989680,0x4(%esp)
80102d7e:	00 
80102d7f:	c7 04 24 e0 00 00 00 	movl   $0xe0,(%esp)
80102d86:	e8 7c ff ff ff       	call   80102d07 <lapicw>

  // Disable logical interrupt lines.
  lapicw(LINT0, MASKED);
80102d8b:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
80102d92:	00 
80102d93:	c7 04 24 d4 00 00 00 	movl   $0xd4,(%esp)
80102d9a:	e8 68 ff ff ff       	call   80102d07 <lapicw>
  lapicw(LINT1, MASKED);
80102d9f:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
80102da6:	00 
80102da7:	c7 04 24 d8 00 00 00 	movl   $0xd8,(%esp)
80102dae:	e8 54 ff ff ff       	call   80102d07 <lapicw>

  // Disable performance counter overflow interrupts
  // on machines that provide that interrupt entry.
  if(((lapic[VER]>>16) & 0xFF) >= 4)
80102db3:	a1 7c f8 10 80       	mov    0x8010f87c,%eax
80102db8:	83 c0 30             	add    $0x30,%eax
80102dbb:	8b 00                	mov    (%eax),%eax
80102dbd:	c1 e8 10             	shr    $0x10,%eax
80102dc0:	25 ff 00 00 00       	and    $0xff,%eax
80102dc5:	83 f8 03             	cmp    $0x3,%eax
80102dc8:	76 14                	jbe    80102dde <lapicinit+0xb6>
    lapicw(PCINT, MASKED);
80102dca:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
80102dd1:	00 
80102dd2:	c7 04 24 d0 00 00 00 	movl   $0xd0,(%esp)
80102dd9:	e8 29 ff ff ff       	call   80102d07 <lapicw>

  // Map error interrupt to IRQ_ERROR.
  lapicw(ERROR, T_IRQ0 + IRQ_ERROR);
80102dde:	c7 44 24 04 33 00 00 	movl   $0x33,0x4(%esp)
80102de5:	00 
80102de6:	c7 04 24 dc 00 00 00 	movl   $0xdc,(%esp)
80102ded:	e8 15 ff ff ff       	call   80102d07 <lapicw>

  // Clear error status register (requires back-to-back writes).
  lapicw(ESR, 0);
80102df2:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102df9:	00 
80102dfa:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80102e01:	e8 01 ff ff ff       	call   80102d07 <lapicw>
  lapicw(ESR, 0);
80102e06:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102e0d:	00 
80102e0e:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80102e15:	e8 ed fe ff ff       	call   80102d07 <lapicw>

  // Ack any outstanding interrupts.
  lapicw(EOI, 0);
80102e1a:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102e21:	00 
80102e22:	c7 04 24 2c 00 00 00 	movl   $0x2c,(%esp)
80102e29:	e8 d9 fe ff ff       	call   80102d07 <lapicw>

  // Send an Init Level De-Assert to synchronise arbitration ID's.
  lapicw(ICRHI, 0);
80102e2e:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102e35:	00 
80102e36:	c7 04 24 c4 00 00 00 	movl   $0xc4,(%esp)
80102e3d:	e8 c5 fe ff ff       	call   80102d07 <lapicw>
  lapicw(ICRLO, BCAST | INIT | LEVEL);
80102e42:	c7 44 24 04 00 85 08 	movl   $0x88500,0x4(%esp)
80102e49:	00 
80102e4a:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
80102e51:	e8 b1 fe ff ff       	call   80102d07 <lapicw>
  while(lapic[ICRLO] & DELIVS)
80102e56:	90                   	nop
80102e57:	a1 7c f8 10 80       	mov    0x8010f87c,%eax
80102e5c:	05 00 03 00 00       	add    $0x300,%eax
80102e61:	8b 00                	mov    (%eax),%eax
80102e63:	25 00 10 00 00       	and    $0x1000,%eax
80102e68:	85 c0                	test   %eax,%eax
80102e6a:	75 eb                	jne    80102e57 <lapicinit+0x12f>
    ;

  // Enable interrupts on the APIC (but not on the processor).
  lapicw(TPR, 0);
80102e6c:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102e73:	00 
80102e74:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80102e7b:	e8 87 fe ff ff       	call   80102d07 <lapicw>
80102e80:	eb 01                	jmp    80102e83 <lapicinit+0x15b>

void
lapicinit(void)
{
  if(!lapic) 
    return;
80102e82:	90                   	nop
  while(lapic[ICRLO] & DELIVS)
    ;

  // Enable interrupts on the APIC (but not on the processor).
  lapicw(TPR, 0);
}
80102e83:	c9                   	leave  
80102e84:	c3                   	ret    

80102e85 <cpunum>:

int
cpunum(void)
{
80102e85:	55                   	push   %ebp
80102e86:	89 e5                	mov    %esp,%ebp
80102e88:	83 ec 18             	sub    $0x18,%esp
  // Cannot call cpu when interrupts are enabled:
  // result not guaranteed to last long enough to be used!
  // Would prefer to panic but even printing is chancy here:
  // almost everything, including cprintf and panic, calls cpu,
  // often indirectly through acquire and release.
  if(readeflags()&FL_IF){
80102e8b:	e8 62 fe ff ff       	call   80102cf2 <readeflags>
80102e90:	25 00 02 00 00       	and    $0x200,%eax
80102e95:	85 c0                	test   %eax,%eax
80102e97:	74 29                	je     80102ec2 <cpunum+0x3d>
    static int n;
    if(n++ == 0)
80102e99:	a1 40 b6 10 80       	mov    0x8010b640,%eax
80102e9e:	85 c0                	test   %eax,%eax
80102ea0:	0f 94 c2             	sete   %dl
80102ea3:	83 c0 01             	add    $0x1,%eax
80102ea6:	a3 40 b6 10 80       	mov    %eax,0x8010b640
80102eab:	84 d2                	test   %dl,%dl
80102ead:	74 13                	je     80102ec2 <cpunum+0x3d>
      cprintf("cpu called from %x with interrupts enabled\n",
80102eaf:	8b 45 04             	mov    0x4(%ebp),%eax
80102eb2:	89 44 24 04          	mov    %eax,0x4(%esp)
80102eb6:	c7 04 24 88 88 10 80 	movl   $0x80108888,(%esp)
80102ebd:	e8 df d4 ff ff       	call   801003a1 <cprintf>
        __builtin_return_address(0));
  }

  if(lapic)
80102ec2:	a1 7c f8 10 80       	mov    0x8010f87c,%eax
80102ec7:	85 c0                	test   %eax,%eax
80102ec9:	74 0f                	je     80102eda <cpunum+0x55>
    return lapic[ID]>>24;
80102ecb:	a1 7c f8 10 80       	mov    0x8010f87c,%eax
80102ed0:	83 c0 20             	add    $0x20,%eax
80102ed3:	8b 00                	mov    (%eax),%eax
80102ed5:	c1 e8 18             	shr    $0x18,%eax
80102ed8:	eb 05                	jmp    80102edf <cpunum+0x5a>
  return 0;
80102eda:	b8 00 00 00 00       	mov    $0x0,%eax
}
80102edf:	c9                   	leave  
80102ee0:	c3                   	ret    

80102ee1 <lapiceoi>:

// Acknowledge interrupt.
void
lapiceoi(void)
{
80102ee1:	55                   	push   %ebp
80102ee2:	89 e5                	mov    %esp,%ebp
80102ee4:	83 ec 08             	sub    $0x8,%esp
  if(lapic)
80102ee7:	a1 7c f8 10 80       	mov    0x8010f87c,%eax
80102eec:	85 c0                	test   %eax,%eax
80102eee:	74 14                	je     80102f04 <lapiceoi+0x23>
    lapicw(EOI, 0);
80102ef0:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80102ef7:	00 
80102ef8:	c7 04 24 2c 00 00 00 	movl   $0x2c,(%esp)
80102eff:	e8 03 fe ff ff       	call   80102d07 <lapicw>
}
80102f04:	c9                   	leave  
80102f05:	c3                   	ret    

80102f06 <microdelay>:

// Spin for a given number of microseconds.
// On real hardware would want to tune this dynamically.
void
microdelay(int us)
{
80102f06:	55                   	push   %ebp
80102f07:	89 e5                	mov    %esp,%ebp
}
80102f09:	5d                   	pop    %ebp
80102f0a:	c3                   	ret    

80102f0b <lapicstartap>:

// Start additional processor running entry code at addr.
// See Appendix B of MultiProcessor Specification.
void
lapicstartap(uchar apicid, uint addr)
{
80102f0b:	55                   	push   %ebp
80102f0c:	89 e5                	mov    %esp,%ebp
80102f0e:	83 ec 1c             	sub    $0x1c,%esp
80102f11:	8b 45 08             	mov    0x8(%ebp),%eax
80102f14:	88 45 ec             	mov    %al,-0x14(%ebp)
  ushort *wrv;
  
  // "The BSP must initialize CMOS shutdown code to 0AH
  // and the warm reset vector (DWORD based at 40:67) to point at
  // the AP startup code prior to the [universal startup algorithm]."
  outb(IO_RTC, 0xF);  // offset 0xF is shutdown code
80102f17:	c7 44 24 04 0f 00 00 	movl   $0xf,0x4(%esp)
80102f1e:	00 
80102f1f:	c7 04 24 70 00 00 00 	movl   $0x70,(%esp)
80102f26:	e8 a9 fd ff ff       	call   80102cd4 <outb>
  outb(IO_RTC+1, 0x0A);
80102f2b:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80102f32:	00 
80102f33:	c7 04 24 71 00 00 00 	movl   $0x71,(%esp)
80102f3a:	e8 95 fd ff ff       	call   80102cd4 <outb>
  wrv = (ushort*)P2V((0x40<<4 | 0x67));  // Warm reset vector
80102f3f:	c7 45 f8 67 04 00 80 	movl   $0x80000467,-0x8(%ebp)
  wrv[0] = 0;
80102f46:	8b 45 f8             	mov    -0x8(%ebp),%eax
80102f49:	66 c7 00 00 00       	movw   $0x0,(%eax)
  wrv[1] = addr >> 4;
80102f4e:	8b 45 f8             	mov    -0x8(%ebp),%eax
80102f51:	8d 50 02             	lea    0x2(%eax),%edx
80102f54:	8b 45 0c             	mov    0xc(%ebp),%eax
80102f57:	c1 e8 04             	shr    $0x4,%eax
80102f5a:	66 89 02             	mov    %ax,(%edx)

  // "Universal startup algorithm."
  // Send INIT (level-triggered) interrupt to reset other CPU.
  lapicw(ICRHI, apicid<<24);
80102f5d:	0f b6 45 ec          	movzbl -0x14(%ebp),%eax
80102f61:	c1 e0 18             	shl    $0x18,%eax
80102f64:	89 44 24 04          	mov    %eax,0x4(%esp)
80102f68:	c7 04 24 c4 00 00 00 	movl   $0xc4,(%esp)
80102f6f:	e8 93 fd ff ff       	call   80102d07 <lapicw>
  lapicw(ICRLO, INIT | LEVEL | ASSERT);
80102f74:	c7 44 24 04 00 c5 00 	movl   $0xc500,0x4(%esp)
80102f7b:	00 
80102f7c:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
80102f83:	e8 7f fd ff ff       	call   80102d07 <lapicw>
  microdelay(200);
80102f88:	c7 04 24 c8 00 00 00 	movl   $0xc8,(%esp)
80102f8f:	e8 72 ff ff ff       	call   80102f06 <microdelay>
  lapicw(ICRLO, INIT | LEVEL);
80102f94:	c7 44 24 04 00 85 00 	movl   $0x8500,0x4(%esp)
80102f9b:	00 
80102f9c:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
80102fa3:	e8 5f fd ff ff       	call   80102d07 <lapicw>
  microdelay(100);    // should be 10ms, but too slow in Bochs!
80102fa8:	c7 04 24 64 00 00 00 	movl   $0x64,(%esp)
80102faf:	e8 52 ff ff ff       	call   80102f06 <microdelay>
  // Send startup IPI (twice!) to enter code.
  // Regular hardware is supposed to only accept a STARTUP
  // when it is in the halted state due to an INIT.  So the second
  // should be ignored, but it is part of the official Intel algorithm.
  // Bochs complains about the second one.  Too bad for Bochs.
  for(i = 0; i < 2; i++){
80102fb4:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
80102fbb:	eb 40                	jmp    80102ffd <lapicstartap+0xf2>
    lapicw(ICRHI, apicid<<24);
80102fbd:	0f b6 45 ec          	movzbl -0x14(%ebp),%eax
80102fc1:	c1 e0 18             	shl    $0x18,%eax
80102fc4:	89 44 24 04          	mov    %eax,0x4(%esp)
80102fc8:	c7 04 24 c4 00 00 00 	movl   $0xc4,(%esp)
80102fcf:	e8 33 fd ff ff       	call   80102d07 <lapicw>
    lapicw(ICRLO, STARTUP | (addr>>12));
80102fd4:	8b 45 0c             	mov    0xc(%ebp),%eax
80102fd7:	c1 e8 0c             	shr    $0xc,%eax
80102fda:	80 cc 06             	or     $0x6,%ah
80102fdd:	89 44 24 04          	mov    %eax,0x4(%esp)
80102fe1:	c7 04 24 c0 00 00 00 	movl   $0xc0,(%esp)
80102fe8:	e8 1a fd ff ff       	call   80102d07 <lapicw>
    microdelay(200);
80102fed:	c7 04 24 c8 00 00 00 	movl   $0xc8,(%esp)
80102ff4:	e8 0d ff ff ff       	call   80102f06 <microdelay>
  // Send startup IPI (twice!) to enter code.
  // Regular hardware is supposed to only accept a STARTUP
  // when it is in the halted state due to an INIT.  So the second
  // should be ignored, but it is part of the official Intel algorithm.
  // Bochs complains about the second one.  Too bad for Bochs.
  for(i = 0; i < 2; i++){
80102ff9:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80102ffd:	83 7d fc 01          	cmpl   $0x1,-0x4(%ebp)
80103001:	7e ba                	jle    80102fbd <lapicstartap+0xb2>
    lapicw(ICRHI, apicid<<24);
    lapicw(ICRLO, STARTUP | (addr>>12));
    microdelay(200);
  }
}
80103003:	c9                   	leave  
80103004:	c3                   	ret    
80103005:	00 00                	add    %al,(%eax)
	...

80103008 <initlog>:

static void recover_from_log(void);

void
initlog(void)
{
80103008:	55                   	push   %ebp
80103009:	89 e5                	mov    %esp,%ebp
8010300b:	83 ec 28             	sub    $0x28,%esp
  if (sizeof(struct logheader) >= BSIZE)
    panic("initlog: too big logheader");

  struct superblock sb;
  initlock(&log.lock, "log");
8010300e:	c7 44 24 04 b4 88 10 	movl   $0x801088b4,0x4(%esp)
80103015:	80 
80103016:	c7 04 24 80 f8 10 80 	movl   $0x8010f880,(%esp)
8010301d:	e8 10 20 00 00       	call   80105032 <initlock>
  readsb(ROOTDEV, &sb);
80103022:	8d 45 e8             	lea    -0x18(%ebp),%eax
80103025:	89 44 24 04          	mov    %eax,0x4(%esp)
80103029:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80103030:	e8 af e2 ff ff       	call   801012e4 <readsb>
  log.start = sb.size - sb.nlog;
80103035:	8b 55 e8             	mov    -0x18(%ebp),%edx
80103038:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010303b:	89 d1                	mov    %edx,%ecx
8010303d:	29 c1                	sub    %eax,%ecx
8010303f:	89 c8                	mov    %ecx,%eax
80103041:	a3 b4 f8 10 80       	mov    %eax,0x8010f8b4
  log.size = sb.nlog;
80103046:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103049:	a3 b8 f8 10 80       	mov    %eax,0x8010f8b8
  log.dev = ROOTDEV;
8010304e:	c7 05 c0 f8 10 80 01 	movl   $0x1,0x8010f8c0
80103055:	00 00 00 
  recover_from_log();
80103058:	e8 97 01 00 00       	call   801031f4 <recover_from_log>
}
8010305d:	c9                   	leave  
8010305e:	c3                   	ret    

8010305f <install_trans>:

// Copy committed blocks from log to their home location
static void 
install_trans(void)
{
8010305f:	55                   	push   %ebp
80103060:	89 e5                	mov    %esp,%ebp
80103062:	83 ec 28             	sub    $0x28,%esp
  int tail;

  for (tail = 0; tail < log.lh.n; tail++) {
80103065:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010306c:	e9 89 00 00 00       	jmp    801030fa <install_trans+0x9b>
    struct buf *lbuf = bread(log.dev, log.start+tail+1); // read log block
80103071:	a1 b4 f8 10 80       	mov    0x8010f8b4,%eax
80103076:	03 45 f4             	add    -0xc(%ebp),%eax
80103079:	83 c0 01             	add    $0x1,%eax
8010307c:	89 c2                	mov    %eax,%edx
8010307e:	a1 c0 f8 10 80       	mov    0x8010f8c0,%eax
80103083:	89 54 24 04          	mov    %edx,0x4(%esp)
80103087:	89 04 24             	mov    %eax,(%esp)
8010308a:	e8 17 d1 ff ff       	call   801001a6 <bread>
8010308f:	89 45 f0             	mov    %eax,-0x10(%ebp)
    struct buf *dbuf = bread(log.dev, log.lh.sector[tail]); // read dst
80103092:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103095:	83 c0 10             	add    $0x10,%eax
80103098:	8b 04 85 88 f8 10 80 	mov    -0x7fef0778(,%eax,4),%eax
8010309f:	89 c2                	mov    %eax,%edx
801030a1:	a1 c0 f8 10 80       	mov    0x8010f8c0,%eax
801030a6:	89 54 24 04          	mov    %edx,0x4(%esp)
801030aa:	89 04 24             	mov    %eax,(%esp)
801030ad:	e8 f4 d0 ff ff       	call   801001a6 <bread>
801030b2:	89 45 ec             	mov    %eax,-0x14(%ebp)
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
801030b5:	8b 45 f0             	mov    -0x10(%ebp),%eax
801030b8:	8d 50 18             	lea    0x18(%eax),%edx
801030bb:	8b 45 ec             	mov    -0x14(%ebp),%eax
801030be:	83 c0 18             	add    $0x18,%eax
801030c1:	c7 44 24 08 00 02 00 	movl   $0x200,0x8(%esp)
801030c8:	00 
801030c9:	89 54 24 04          	mov    %edx,0x4(%esp)
801030cd:	89 04 24             	mov    %eax,(%esp)
801030d0:	e8 a0 22 00 00       	call   80105375 <memmove>
    bwrite(dbuf);  // write dst to disk
801030d5:	8b 45 ec             	mov    -0x14(%ebp),%eax
801030d8:	89 04 24             	mov    %eax,(%esp)
801030db:	e8 fd d0 ff ff       	call   801001dd <bwrite>
    brelse(lbuf); 
801030e0:	8b 45 f0             	mov    -0x10(%ebp),%eax
801030e3:	89 04 24             	mov    %eax,(%esp)
801030e6:	e8 2c d1 ff ff       	call   80100217 <brelse>
    brelse(dbuf);
801030eb:	8b 45 ec             	mov    -0x14(%ebp),%eax
801030ee:	89 04 24             	mov    %eax,(%esp)
801030f1:	e8 21 d1 ff ff       	call   80100217 <brelse>
static void 
install_trans(void)
{
  int tail;

  for (tail = 0; tail < log.lh.n; tail++) {
801030f6:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801030fa:	a1 c4 f8 10 80       	mov    0x8010f8c4,%eax
801030ff:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80103102:	0f 8f 69 ff ff ff    	jg     80103071 <install_trans+0x12>
    memmove(dbuf->data, lbuf->data, BSIZE);  // copy block to dst
    bwrite(dbuf);  // write dst to disk
    brelse(lbuf); 
    brelse(dbuf);
  }
}
80103108:	c9                   	leave  
80103109:	c3                   	ret    

8010310a <read_head>:

// Read the log header from disk into the in-memory log header
static void
read_head(void)
{
8010310a:	55                   	push   %ebp
8010310b:	89 e5                	mov    %esp,%ebp
8010310d:	83 ec 28             	sub    $0x28,%esp
  struct buf *buf = bread(log.dev, log.start);
80103110:	a1 b4 f8 10 80       	mov    0x8010f8b4,%eax
80103115:	89 c2                	mov    %eax,%edx
80103117:	a1 c0 f8 10 80       	mov    0x8010f8c0,%eax
8010311c:	89 54 24 04          	mov    %edx,0x4(%esp)
80103120:	89 04 24             	mov    %eax,(%esp)
80103123:	e8 7e d0 ff ff       	call   801001a6 <bread>
80103128:	89 45 f0             	mov    %eax,-0x10(%ebp)
  struct logheader *lh = (struct logheader *) (buf->data);
8010312b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010312e:	83 c0 18             	add    $0x18,%eax
80103131:	89 45 ec             	mov    %eax,-0x14(%ebp)
  int i;
  log.lh.n = lh->n;
80103134:	8b 45 ec             	mov    -0x14(%ebp),%eax
80103137:	8b 00                	mov    (%eax),%eax
80103139:	a3 c4 f8 10 80       	mov    %eax,0x8010f8c4
  for (i = 0; i < log.lh.n; i++) {
8010313e:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103145:	eb 1b                	jmp    80103162 <read_head+0x58>
    log.lh.sector[i] = lh->sector[i];
80103147:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010314a:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010314d:	8b 44 90 04          	mov    0x4(%eax,%edx,4),%eax
80103151:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103154:	83 c2 10             	add    $0x10,%edx
80103157:	89 04 95 88 f8 10 80 	mov    %eax,-0x7fef0778(,%edx,4)
{
  struct buf *buf = bread(log.dev, log.start);
  struct logheader *lh = (struct logheader *) (buf->data);
  int i;
  log.lh.n = lh->n;
  for (i = 0; i < log.lh.n; i++) {
8010315e:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80103162:	a1 c4 f8 10 80       	mov    0x8010f8c4,%eax
80103167:	3b 45 f4             	cmp    -0xc(%ebp),%eax
8010316a:	7f db                	jg     80103147 <read_head+0x3d>
    log.lh.sector[i] = lh->sector[i];
  }
  brelse(buf);
8010316c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010316f:	89 04 24             	mov    %eax,(%esp)
80103172:	e8 a0 d0 ff ff       	call   80100217 <brelse>
}
80103177:	c9                   	leave  
80103178:	c3                   	ret    

80103179 <write_head>:
// Write in-memory log header to disk.
// This is the true point at which the
// current transaction commits.
static void
write_head(void)
{
80103179:	55                   	push   %ebp
8010317a:	89 e5                	mov    %esp,%ebp
8010317c:	83 ec 28             	sub    $0x28,%esp
  struct buf *buf = bread(log.dev, log.start);
8010317f:	a1 b4 f8 10 80       	mov    0x8010f8b4,%eax
80103184:	89 c2                	mov    %eax,%edx
80103186:	a1 c0 f8 10 80       	mov    0x8010f8c0,%eax
8010318b:	89 54 24 04          	mov    %edx,0x4(%esp)
8010318f:	89 04 24             	mov    %eax,(%esp)
80103192:	e8 0f d0 ff ff       	call   801001a6 <bread>
80103197:	89 45 f0             	mov    %eax,-0x10(%ebp)
  struct logheader *hb = (struct logheader *) (buf->data);
8010319a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010319d:	83 c0 18             	add    $0x18,%eax
801031a0:	89 45 ec             	mov    %eax,-0x14(%ebp)
  int i;
  hb->n = log.lh.n;
801031a3:	8b 15 c4 f8 10 80    	mov    0x8010f8c4,%edx
801031a9:	8b 45 ec             	mov    -0x14(%ebp),%eax
801031ac:	89 10                	mov    %edx,(%eax)
  for (i = 0; i < log.lh.n; i++) {
801031ae:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801031b5:	eb 1b                	jmp    801031d2 <write_head+0x59>
    hb->sector[i] = log.lh.sector[i];
801031b7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801031ba:	83 c0 10             	add    $0x10,%eax
801031bd:	8b 0c 85 88 f8 10 80 	mov    -0x7fef0778(,%eax,4),%ecx
801031c4:	8b 45 ec             	mov    -0x14(%ebp),%eax
801031c7:	8b 55 f4             	mov    -0xc(%ebp),%edx
801031ca:	89 4c 90 04          	mov    %ecx,0x4(%eax,%edx,4)
{
  struct buf *buf = bread(log.dev, log.start);
  struct logheader *hb = (struct logheader *) (buf->data);
  int i;
  hb->n = log.lh.n;
  for (i = 0; i < log.lh.n; i++) {
801031ce:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
801031d2:	a1 c4 f8 10 80       	mov    0x8010f8c4,%eax
801031d7:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801031da:	7f db                	jg     801031b7 <write_head+0x3e>
    hb->sector[i] = log.lh.sector[i];
  }
  bwrite(buf);
801031dc:	8b 45 f0             	mov    -0x10(%ebp),%eax
801031df:	89 04 24             	mov    %eax,(%esp)
801031e2:	e8 f6 cf ff ff       	call   801001dd <bwrite>
  brelse(buf);
801031e7:	8b 45 f0             	mov    -0x10(%ebp),%eax
801031ea:	89 04 24             	mov    %eax,(%esp)
801031ed:	e8 25 d0 ff ff       	call   80100217 <brelse>
}
801031f2:	c9                   	leave  
801031f3:	c3                   	ret    

801031f4 <recover_from_log>:

static void
recover_from_log(void)
{
801031f4:	55                   	push   %ebp
801031f5:	89 e5                	mov    %esp,%ebp
801031f7:	83 ec 08             	sub    $0x8,%esp
  read_head();      
801031fa:	e8 0b ff ff ff       	call   8010310a <read_head>
  install_trans(); // if committed, copy from log to disk
801031ff:	e8 5b fe ff ff       	call   8010305f <install_trans>
  log.lh.n = 0;
80103204:	c7 05 c4 f8 10 80 00 	movl   $0x0,0x8010f8c4
8010320b:	00 00 00 
  write_head(); // clear the log
8010320e:	e8 66 ff ff ff       	call   80103179 <write_head>
}
80103213:	c9                   	leave  
80103214:	c3                   	ret    

80103215 <begin_trans>:

void
begin_trans(void)
{
80103215:	55                   	push   %ebp
80103216:	89 e5                	mov    %esp,%ebp
80103218:	83 ec 18             	sub    $0x18,%esp
  acquire(&log.lock);
8010321b:	c7 04 24 80 f8 10 80 	movl   $0x8010f880,(%esp)
80103222:	e8 2c 1e 00 00       	call   80105053 <acquire>
  while (log.busy) {
80103227:	eb 14                	jmp    8010323d <begin_trans+0x28>
    sleep(&log, &log.lock);
80103229:	c7 44 24 04 80 f8 10 	movl   $0x8010f880,0x4(%esp)
80103230:	80 
80103231:	c7 04 24 80 f8 10 80 	movl   $0x8010f880,(%esp)
80103238:	e8 d5 15 00 00       	call   80104812 <sleep>

void
begin_trans(void)
{
  acquire(&log.lock);
  while (log.busy) {
8010323d:	a1 bc f8 10 80       	mov    0x8010f8bc,%eax
80103242:	85 c0                	test   %eax,%eax
80103244:	75 e3                	jne    80103229 <begin_trans+0x14>
    sleep(&log, &log.lock);
  }
  log.busy = 1;
80103246:	c7 05 bc f8 10 80 01 	movl   $0x1,0x8010f8bc
8010324d:	00 00 00 
  release(&log.lock);
80103250:	c7 04 24 80 f8 10 80 	movl   $0x8010f880,(%esp)
80103257:	e8 59 1e 00 00       	call   801050b5 <release>
}
8010325c:	c9                   	leave  
8010325d:	c3                   	ret    

8010325e <commit_trans>:

void
commit_trans(void)
{
8010325e:	55                   	push   %ebp
8010325f:	89 e5                	mov    %esp,%ebp
80103261:	83 ec 18             	sub    $0x18,%esp
  if (log.lh.n > 0) {
80103264:	a1 c4 f8 10 80       	mov    0x8010f8c4,%eax
80103269:	85 c0                	test   %eax,%eax
8010326b:	7e 19                	jle    80103286 <commit_trans+0x28>
    write_head();    // Write header to disk -- the real commit
8010326d:	e8 07 ff ff ff       	call   80103179 <write_head>
    install_trans(); // Now install writes to home locations
80103272:	e8 e8 fd ff ff       	call   8010305f <install_trans>
    log.lh.n = 0; 
80103277:	c7 05 c4 f8 10 80 00 	movl   $0x0,0x8010f8c4
8010327e:	00 00 00 
    write_head();    // Erase the transaction from the log
80103281:	e8 f3 fe ff ff       	call   80103179 <write_head>
  }
  
  acquire(&log.lock);
80103286:	c7 04 24 80 f8 10 80 	movl   $0x8010f880,(%esp)
8010328d:	e8 c1 1d 00 00       	call   80105053 <acquire>
  log.busy = 0;
80103292:	c7 05 bc f8 10 80 00 	movl   $0x0,0x8010f8bc
80103299:	00 00 00 
  wakeup(&log);
8010329c:	c7 04 24 80 f8 10 80 	movl   $0x8010f880,(%esp)
801032a3:	e8 44 16 00 00       	call   801048ec <wakeup>
  release(&log.lock);
801032a8:	c7 04 24 80 f8 10 80 	movl   $0x8010f880,(%esp)
801032af:	e8 01 1e 00 00       	call   801050b5 <release>
}
801032b4:	c9                   	leave  
801032b5:	c3                   	ret    

801032b6 <log_write>:
//   modify bp->data[]
//   log_write(bp)
//   brelse(bp)
void
log_write(struct buf *b)
{
801032b6:	55                   	push   %ebp
801032b7:	89 e5                	mov    %esp,%ebp
801032b9:	83 ec 28             	sub    $0x28,%esp
  int i;

  if (log.lh.n >= LOGSIZE || log.lh.n >= log.size - 1)
801032bc:	a1 c4 f8 10 80       	mov    0x8010f8c4,%eax
801032c1:	83 f8 09             	cmp    $0x9,%eax
801032c4:	7f 12                	jg     801032d8 <log_write+0x22>
801032c6:	a1 c4 f8 10 80       	mov    0x8010f8c4,%eax
801032cb:	8b 15 b8 f8 10 80    	mov    0x8010f8b8,%edx
801032d1:	83 ea 01             	sub    $0x1,%edx
801032d4:	39 d0                	cmp    %edx,%eax
801032d6:	7c 0c                	jl     801032e4 <log_write+0x2e>
    panic("too big a transaction");
801032d8:	c7 04 24 b8 88 10 80 	movl   $0x801088b8,(%esp)
801032df:	e8 59 d2 ff ff       	call   8010053d <panic>
  if (!log.busy)
801032e4:	a1 bc f8 10 80       	mov    0x8010f8bc,%eax
801032e9:	85 c0                	test   %eax,%eax
801032eb:	75 0c                	jne    801032f9 <log_write+0x43>
    panic("write outside of trans");
801032ed:	c7 04 24 ce 88 10 80 	movl   $0x801088ce,(%esp)
801032f4:	e8 44 d2 ff ff       	call   8010053d <panic>

  for (i = 0; i < log.lh.n; i++) {
801032f9:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103300:	eb 1d                	jmp    8010331f <log_write+0x69>
    if (log.lh.sector[i] == b->sector)   // log absorbtion?
80103302:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103305:	83 c0 10             	add    $0x10,%eax
80103308:	8b 04 85 88 f8 10 80 	mov    -0x7fef0778(,%eax,4),%eax
8010330f:	89 c2                	mov    %eax,%edx
80103311:	8b 45 08             	mov    0x8(%ebp),%eax
80103314:	8b 40 08             	mov    0x8(%eax),%eax
80103317:	39 c2                	cmp    %eax,%edx
80103319:	74 10                	je     8010332b <log_write+0x75>
  if (log.lh.n >= LOGSIZE || log.lh.n >= log.size - 1)
    panic("too big a transaction");
  if (!log.busy)
    panic("write outside of trans");

  for (i = 0; i < log.lh.n; i++) {
8010331b:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
8010331f:	a1 c4 f8 10 80       	mov    0x8010f8c4,%eax
80103324:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80103327:	7f d9                	jg     80103302 <log_write+0x4c>
80103329:	eb 01                	jmp    8010332c <log_write+0x76>
    if (log.lh.sector[i] == b->sector)   // log absorbtion?
      break;
8010332b:	90                   	nop
  }
  log.lh.sector[i] = b->sector;
8010332c:	8b 45 08             	mov    0x8(%ebp),%eax
8010332f:	8b 40 08             	mov    0x8(%eax),%eax
80103332:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103335:	83 c2 10             	add    $0x10,%edx
80103338:	89 04 95 88 f8 10 80 	mov    %eax,-0x7fef0778(,%edx,4)
  struct buf *lbuf = bread(b->dev, log.start+i+1);
8010333f:	a1 b4 f8 10 80       	mov    0x8010f8b4,%eax
80103344:	03 45 f4             	add    -0xc(%ebp),%eax
80103347:	83 c0 01             	add    $0x1,%eax
8010334a:	89 c2                	mov    %eax,%edx
8010334c:	8b 45 08             	mov    0x8(%ebp),%eax
8010334f:	8b 40 04             	mov    0x4(%eax),%eax
80103352:	89 54 24 04          	mov    %edx,0x4(%esp)
80103356:	89 04 24             	mov    %eax,(%esp)
80103359:	e8 48 ce ff ff       	call   801001a6 <bread>
8010335e:	89 45 f0             	mov    %eax,-0x10(%ebp)
  memmove(lbuf->data, b->data, BSIZE);
80103361:	8b 45 08             	mov    0x8(%ebp),%eax
80103364:	8d 50 18             	lea    0x18(%eax),%edx
80103367:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010336a:	83 c0 18             	add    $0x18,%eax
8010336d:	c7 44 24 08 00 02 00 	movl   $0x200,0x8(%esp)
80103374:	00 
80103375:	89 54 24 04          	mov    %edx,0x4(%esp)
80103379:	89 04 24             	mov    %eax,(%esp)
8010337c:	e8 f4 1f 00 00       	call   80105375 <memmove>
  bwrite(lbuf);
80103381:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103384:	89 04 24             	mov    %eax,(%esp)
80103387:	e8 51 ce ff ff       	call   801001dd <bwrite>
  brelse(lbuf);
8010338c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010338f:	89 04 24             	mov    %eax,(%esp)
80103392:	e8 80 ce ff ff       	call   80100217 <brelse>
  if (i == log.lh.n)
80103397:	a1 c4 f8 10 80       	mov    0x8010f8c4,%eax
8010339c:	3b 45 f4             	cmp    -0xc(%ebp),%eax
8010339f:	75 0d                	jne    801033ae <log_write+0xf8>
    log.lh.n++;
801033a1:	a1 c4 f8 10 80       	mov    0x8010f8c4,%eax
801033a6:	83 c0 01             	add    $0x1,%eax
801033a9:	a3 c4 f8 10 80       	mov    %eax,0x8010f8c4
  b->flags |= B_DIRTY; // XXX prevent eviction
801033ae:	8b 45 08             	mov    0x8(%ebp),%eax
801033b1:	8b 00                	mov    (%eax),%eax
801033b3:	89 c2                	mov    %eax,%edx
801033b5:	83 ca 04             	or     $0x4,%edx
801033b8:	8b 45 08             	mov    0x8(%ebp),%eax
801033bb:	89 10                	mov    %edx,(%eax)
}
801033bd:	c9                   	leave  
801033be:	c3                   	ret    
	...

801033c0 <v2p>:
801033c0:	55                   	push   %ebp
801033c1:	89 e5                	mov    %esp,%ebp
801033c3:	8b 45 08             	mov    0x8(%ebp),%eax
801033c6:	05 00 00 00 80       	add    $0x80000000,%eax
801033cb:	5d                   	pop    %ebp
801033cc:	c3                   	ret    

801033cd <p2v>:
static inline void *p2v(uint a) { return (void *) ((a) + KERNBASE); }
801033cd:	55                   	push   %ebp
801033ce:	89 e5                	mov    %esp,%ebp
801033d0:	8b 45 08             	mov    0x8(%ebp),%eax
801033d3:	05 00 00 00 80       	add    $0x80000000,%eax
801033d8:	5d                   	pop    %ebp
801033d9:	c3                   	ret    

801033da <xchg>:
  asm volatile("sti");
}

static inline uint
xchg(volatile uint *addr, uint newval)
{
801033da:	55                   	push   %ebp
801033db:	89 e5                	mov    %esp,%ebp
801033dd:	53                   	push   %ebx
801033de:	83 ec 10             	sub    $0x10,%esp
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
               "+m" (*addr), "=a" (result) :
801033e1:	8b 55 08             	mov    0x8(%ebp),%edx
xchg(volatile uint *addr, uint newval)
{
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
801033e4:	8b 45 0c             	mov    0xc(%ebp),%eax
               "+m" (*addr), "=a" (result) :
801033e7:	8b 4d 08             	mov    0x8(%ebp),%ecx
xchg(volatile uint *addr, uint newval)
{
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
801033ea:	89 c3                	mov    %eax,%ebx
801033ec:	89 d8                	mov    %ebx,%eax
801033ee:	f0 87 02             	lock xchg %eax,(%edx)
801033f1:	89 c3                	mov    %eax,%ebx
801033f3:	89 5d f8             	mov    %ebx,-0x8(%ebp)
               "+m" (*addr), "=a" (result) :
               "1" (newval) :
               "cc");
  return result;
801033f6:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
801033f9:	83 c4 10             	add    $0x10,%esp
801033fc:	5b                   	pop    %ebx
801033fd:	5d                   	pop    %ebp
801033fe:	c3                   	ret    

801033ff <main>:
// Bootstrap processor starts running C code here.
// Allocate a real stack and switch to it, first
// doing some setup required for memory allocator to work.
int
main(void)
{
801033ff:	55                   	push   %ebp
80103400:	89 e5                	mov    %esp,%ebp
80103402:	83 e4 f0             	and    $0xfffffff0,%esp
80103405:	83 ec 10             	sub    $0x10,%esp
  kinit1(end, P2V(4*1024*1024)); // phys page allocator
80103408:	c7 44 24 04 00 00 40 	movl   $0x80400000,0x4(%esp)
8010340f:	80 
80103410:	c7 04 24 5c 2d 11 80 	movl   $0x80112d5c,(%esp)
80103417:	e8 ad f5 ff ff       	call   801029c9 <kinit1>
  kvmalloc();      // kernel page table
8010341c:	e8 dd 4a 00 00       	call   80107efe <kvmalloc>
  mpinit();        // collect info about this machine
80103421:	e8 57 04 00 00       	call   8010387d <mpinit>
  lapicinit();
80103426:	e8 fd f8 ff ff       	call   80102d28 <lapicinit>
  seginit();       // set up segments
8010342b:	e8 71 44 00 00       	call   801078a1 <seginit>
  cprintf("\ncpu%d: starting xv6\n\n", cpu->id);
80103430:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80103436:	0f b6 00             	movzbl (%eax),%eax
80103439:	0f b6 c0             	movzbl %al,%eax
8010343c:	89 44 24 04          	mov    %eax,0x4(%esp)
80103440:	c7 04 24 e5 88 10 80 	movl   $0x801088e5,(%esp)
80103447:	e8 55 cf ff ff       	call   801003a1 <cprintf>
  picinit();       // interrupt controller
8010344c:	e8 91 06 00 00       	call   80103ae2 <picinit>
  ioapicinit();    // another interrupt controller
80103451:	e8 63 f4 ff ff       	call   801028b9 <ioapicinit>
  consoleinit();   // I/O devices & their interrupts
80103456:	e8 32 d6 ff ff       	call   80100a8d <consoleinit>
  uartinit();      // serial port
8010345b:	e8 8c 37 00 00       	call   80106bec <uartinit>
  pinit();         // process table
80103460:	e8 92 0b 00 00       	call   80103ff7 <pinit>
  tvinit();        // trap vectors
80103465:	e8 25 33 00 00       	call   8010678f <tvinit>
  binit();         // buffer cache
8010346a:	e8 c5 cb ff ff       	call   80100034 <binit>
  fileinit();      // file table
8010346f:	e8 84 da ff ff       	call   80100ef8 <fileinit>
  iinit();         // inode cache
80103474:	e8 32 e1 ff ff       	call   801015ab <iinit>
  ideinit();       // disk
80103479:	e8 a0 f0 ff ff       	call   8010251e <ideinit>
  if(!ismp)
8010347e:	a1 04 f9 10 80       	mov    0x8010f904,%eax
80103483:	85 c0                	test   %eax,%eax
80103485:	75 05                	jne    8010348c <main+0x8d>
    timerinit();   // uniprocessor timer
80103487:	e8 46 32 00 00       	call   801066d2 <timerinit>

  InitialPriorityTables();
8010348c:	e8 77 17 00 00       	call   80104c08 <InitialPriorityTables>

  startothers();   // start other processors
80103491:	e8 7f 00 00 00       	call   80103515 <startothers>
  kinit2(P2V(4*1024*1024), P2V(PHYSTOP)); // must come after startothers()
80103496:	c7 44 24 04 00 00 00 	movl   $0x8e000000,0x4(%esp)
8010349d:	8e 
8010349e:	c7 04 24 00 00 40 80 	movl   $0x80400000,(%esp)
801034a5:	e8 57 f5 ff ff       	call   80102a01 <kinit2>
  userinit();      // first user process
801034aa:	e8 63 0c 00 00       	call   80104112 <userinit>
  // Finish setting up this processor in mpmain.


  mpmain();
801034af:	e8 1a 00 00 00       	call   801034ce <mpmain>

801034b4 <mpenter>:
}

// Other CPUs jump here from entryother.S.
static void
mpenter(void)
{
801034b4:	55                   	push   %ebp
801034b5:	89 e5                	mov    %esp,%ebp
801034b7:	83 ec 08             	sub    $0x8,%esp
  switchkvm(); 
801034ba:	e8 56 4a 00 00       	call   80107f15 <switchkvm>
  seginit();
801034bf:	e8 dd 43 00 00       	call   801078a1 <seginit>
  lapicinit();
801034c4:	e8 5f f8 ff ff       	call   80102d28 <lapicinit>
  mpmain();
801034c9:	e8 00 00 00 00       	call   801034ce <mpmain>

801034ce <mpmain>:
}

// Common CPU setup code.
static void
mpmain(void)
{
801034ce:	55                   	push   %ebp
801034cf:	89 e5                	mov    %esp,%ebp
801034d1:	83 ec 18             	sub    $0x18,%esp
  cprintf("cpu%d: starting\n", cpu->id);
801034d4:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801034da:	0f b6 00             	movzbl (%eax),%eax
801034dd:	0f b6 c0             	movzbl %al,%eax
801034e0:	89 44 24 04          	mov    %eax,0x4(%esp)
801034e4:	c7 04 24 fc 88 10 80 	movl   $0x801088fc,(%esp)
801034eb:	e8 b1 ce ff ff       	call   801003a1 <cprintf>
  idtinit();       // load idt register
801034f0:	e8 0e 34 00 00       	call   80106903 <idtinit>
  xchg(&cpu->started, 1); // tell startothers() we're up
801034f5:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801034fb:	05 a8 00 00 00       	add    $0xa8,%eax
80103500:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80103507:	00 
80103508:	89 04 24             	mov    %eax,(%esp)
8010350b:	e8 ca fe ff ff       	call   801033da <xchg>

  scheduler();     // start running processes
80103510:	e8 4e 11 00 00       	call   80104663 <scheduler>

80103515 <startothers>:
pde_t entrypgdir[];  // For entry.S

// Start the non-boot (AP) processors.
static void
startothers(void)
{
80103515:	55                   	push   %ebp
80103516:	89 e5                	mov    %esp,%ebp
80103518:	53                   	push   %ebx
80103519:	83 ec 24             	sub    $0x24,%esp
  char *stack;

  // Write entry code to unused memory at 0x7000.
  // The linker has placed the image of entryother.S in
  // _binary_entryother_start.
  code = p2v(0x7000);
8010351c:	c7 04 24 00 70 00 00 	movl   $0x7000,(%esp)
80103523:	e8 a5 fe ff ff       	call   801033cd <p2v>
80103528:	89 45 f0             	mov    %eax,-0x10(%ebp)
  memmove(code, _binary_entryother_start, (uint)_binary_entryother_size);
8010352b:	b8 8a 00 00 00       	mov    $0x8a,%eax
80103530:	89 44 24 08          	mov    %eax,0x8(%esp)
80103534:	c7 44 24 04 0c b5 10 	movl   $0x8010b50c,0x4(%esp)
8010353b:	80 
8010353c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010353f:	89 04 24             	mov    %eax,(%esp)
80103542:	e8 2e 1e 00 00       	call   80105375 <memmove>

  for(c = cpus; c < cpus+ncpu; c++){
80103547:	c7 45 f4 20 f9 10 80 	movl   $0x8010f920,-0xc(%ebp)
8010354e:	e9 86 00 00 00       	jmp    801035d9 <startothers+0xc4>
    if(c == cpus+cpunum())  // We've started already.
80103553:	e8 2d f9 ff ff       	call   80102e85 <cpunum>
80103558:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
8010355e:	05 20 f9 10 80       	add    $0x8010f920,%eax
80103563:	3b 45 f4             	cmp    -0xc(%ebp),%eax
80103566:	74 69                	je     801035d1 <startothers+0xbc>
      continue;

    // Tell entryother.S what stack to use, where to enter, and what 
    // pgdir to use. We cannot use kpgdir yet, because the AP processor
    // is running in low  memory, so we use entrypgdir for the APs too.
    stack = kalloc();
80103568:	e8 8a f5 ff ff       	call   80102af7 <kalloc>
8010356d:	89 45 ec             	mov    %eax,-0x14(%ebp)
    *(void**)(code-4) = stack + KSTACKSIZE;
80103570:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103573:	83 e8 04             	sub    $0x4,%eax
80103576:	8b 55 ec             	mov    -0x14(%ebp),%edx
80103579:	81 c2 00 10 00 00    	add    $0x1000,%edx
8010357f:	89 10                	mov    %edx,(%eax)
    *(void**)(code-8) = mpenter;
80103581:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103584:	83 e8 08             	sub    $0x8,%eax
80103587:	c7 00 b4 34 10 80    	movl   $0x801034b4,(%eax)
    *(int**)(code-12) = (void *) v2p(entrypgdir);
8010358d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103590:	8d 58 f4             	lea    -0xc(%eax),%ebx
80103593:	c7 04 24 00 a0 10 80 	movl   $0x8010a000,(%esp)
8010359a:	e8 21 fe ff ff       	call   801033c0 <v2p>
8010359f:	89 03                	mov    %eax,(%ebx)

    lapicstartap(c->id, v2p(code));
801035a1:	8b 45 f0             	mov    -0x10(%ebp),%eax
801035a4:	89 04 24             	mov    %eax,(%esp)
801035a7:	e8 14 fe ff ff       	call   801033c0 <v2p>
801035ac:	8b 55 f4             	mov    -0xc(%ebp),%edx
801035af:	0f b6 12             	movzbl (%edx),%edx
801035b2:	0f b6 d2             	movzbl %dl,%edx
801035b5:	89 44 24 04          	mov    %eax,0x4(%esp)
801035b9:	89 14 24             	mov    %edx,(%esp)
801035bc:	e8 4a f9 ff ff       	call   80102f0b <lapicstartap>

    // wait for cpu to finish mpmain()
    while(c->started == 0)
801035c1:	90                   	nop
801035c2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801035c5:	8b 80 a8 00 00 00    	mov    0xa8(%eax),%eax
801035cb:	85 c0                	test   %eax,%eax
801035cd:	74 f3                	je     801035c2 <startothers+0xad>
801035cf:	eb 01                	jmp    801035d2 <startothers+0xbd>
  code = p2v(0x7000);
  memmove(code, _binary_entryother_start, (uint)_binary_entryother_size);

  for(c = cpus; c < cpus+ncpu; c++){
    if(c == cpus+cpunum())  // We've started already.
      continue;
801035d1:	90                   	nop
  // The linker has placed the image of entryother.S in
  // _binary_entryother_start.
  code = p2v(0x7000);
  memmove(code, _binary_entryother_start, (uint)_binary_entryother_size);

  for(c = cpus; c < cpus+ncpu; c++){
801035d2:	81 45 f4 bc 00 00 00 	addl   $0xbc,-0xc(%ebp)
801035d9:	a1 00 ff 10 80       	mov    0x8010ff00,%eax
801035de:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
801035e4:	05 20 f9 10 80       	add    $0x8010f920,%eax
801035e9:	3b 45 f4             	cmp    -0xc(%ebp),%eax
801035ec:	0f 87 61 ff ff ff    	ja     80103553 <startothers+0x3e>

    // wait for cpu to finish mpmain()
    while(c->started == 0)
      ;
  }
}
801035f2:	83 c4 24             	add    $0x24,%esp
801035f5:	5b                   	pop    %ebx
801035f6:	5d                   	pop    %ebp
801035f7:	c3                   	ret    

801035f8 <p2v>:
801035f8:	55                   	push   %ebp
801035f9:	89 e5                	mov    %esp,%ebp
801035fb:	8b 45 08             	mov    0x8(%ebp),%eax
801035fe:	05 00 00 00 80       	add    $0x80000000,%eax
80103603:	5d                   	pop    %ebp
80103604:	c3                   	ret    

80103605 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
80103605:	55                   	push   %ebp
80103606:	89 e5                	mov    %esp,%ebp
80103608:	53                   	push   %ebx
80103609:	83 ec 14             	sub    $0x14,%esp
8010360c:	8b 45 08             	mov    0x8(%ebp),%eax
8010360f:	66 89 45 e8          	mov    %ax,-0x18(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80103613:	0f b7 55 e8          	movzwl -0x18(%ebp),%edx
80103617:	66 89 55 ea          	mov    %dx,-0x16(%ebp)
8010361b:	0f b7 55 ea          	movzwl -0x16(%ebp),%edx
8010361f:	ec                   	in     (%dx),%al
80103620:	89 c3                	mov    %eax,%ebx
80103622:	88 5d fb             	mov    %bl,-0x5(%ebp)
  return data;
80103625:	0f b6 45 fb          	movzbl -0x5(%ebp),%eax
}
80103629:	83 c4 14             	add    $0x14,%esp
8010362c:	5b                   	pop    %ebx
8010362d:	5d                   	pop    %ebp
8010362e:	c3                   	ret    

8010362f <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
8010362f:	55                   	push   %ebp
80103630:	89 e5                	mov    %esp,%ebp
80103632:	83 ec 08             	sub    $0x8,%esp
80103635:	8b 55 08             	mov    0x8(%ebp),%edx
80103638:	8b 45 0c             	mov    0xc(%ebp),%eax
8010363b:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
8010363f:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80103642:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
80103646:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
8010364a:	ee                   	out    %al,(%dx)
}
8010364b:	c9                   	leave  
8010364c:	c3                   	ret    

8010364d <mpbcpu>:
int ncpu;
uchar ioapicid;

int
mpbcpu(void)
{
8010364d:	55                   	push   %ebp
8010364e:	89 e5                	mov    %esp,%ebp
  return bcpu-cpus;
80103650:	a1 44 b6 10 80       	mov    0x8010b644,%eax
80103655:	89 c2                	mov    %eax,%edx
80103657:	b8 20 f9 10 80       	mov    $0x8010f920,%eax
8010365c:	89 d1                	mov    %edx,%ecx
8010365e:	29 c1                	sub    %eax,%ecx
80103660:	89 c8                	mov    %ecx,%eax
80103662:	c1 f8 02             	sar    $0x2,%eax
80103665:	69 c0 cf 46 7d 67    	imul   $0x677d46cf,%eax,%eax
}
8010366b:	5d                   	pop    %ebp
8010366c:	c3                   	ret    

8010366d <sum>:

static uchar
sum(uchar *addr, int len)
{
8010366d:	55                   	push   %ebp
8010366e:	89 e5                	mov    %esp,%ebp
80103670:	83 ec 10             	sub    $0x10,%esp
  int i, sum;
  
  sum = 0;
80103673:	c7 45 f8 00 00 00 00 	movl   $0x0,-0x8(%ebp)
  for(i=0; i<len; i++)
8010367a:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
80103681:	eb 13                	jmp    80103696 <sum+0x29>
    sum += addr[i];
80103683:	8b 45 fc             	mov    -0x4(%ebp),%eax
80103686:	03 45 08             	add    0x8(%ebp),%eax
80103689:	0f b6 00             	movzbl (%eax),%eax
8010368c:	0f b6 c0             	movzbl %al,%eax
8010368f:	01 45 f8             	add    %eax,-0x8(%ebp)
sum(uchar *addr, int len)
{
  int i, sum;
  
  sum = 0;
  for(i=0; i<len; i++)
80103692:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80103696:	8b 45 fc             	mov    -0x4(%ebp),%eax
80103699:	3b 45 0c             	cmp    0xc(%ebp),%eax
8010369c:	7c e5                	jl     80103683 <sum+0x16>
    sum += addr[i];
  return sum;
8010369e:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
801036a1:	c9                   	leave  
801036a2:	c3                   	ret    

801036a3 <mpsearch1>:

// Look for an MP structure in the len bytes at addr.
static struct mp*
mpsearch1(uint a, int len)
{
801036a3:	55                   	push   %ebp
801036a4:	89 e5                	mov    %esp,%ebp
801036a6:	83 ec 28             	sub    $0x28,%esp
  uchar *e, *p, *addr;

  addr = p2v(a);
801036a9:	8b 45 08             	mov    0x8(%ebp),%eax
801036ac:	89 04 24             	mov    %eax,(%esp)
801036af:	e8 44 ff ff ff       	call   801035f8 <p2v>
801036b4:	89 45 f0             	mov    %eax,-0x10(%ebp)
  e = addr+len;
801036b7:	8b 45 0c             	mov    0xc(%ebp),%eax
801036ba:	03 45 f0             	add    -0x10(%ebp),%eax
801036bd:	89 45 ec             	mov    %eax,-0x14(%ebp)
  for(p = addr; p < e; p += sizeof(struct mp))
801036c0:	8b 45 f0             	mov    -0x10(%ebp),%eax
801036c3:	89 45 f4             	mov    %eax,-0xc(%ebp)
801036c6:	eb 3f                	jmp    80103707 <mpsearch1+0x64>
    if(memcmp(p, "_MP_", 4) == 0 && sum(p, sizeof(struct mp)) == 0)
801036c8:	c7 44 24 08 04 00 00 	movl   $0x4,0x8(%esp)
801036cf:	00 
801036d0:	c7 44 24 04 10 89 10 	movl   $0x80108910,0x4(%esp)
801036d7:	80 
801036d8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801036db:	89 04 24             	mov    %eax,(%esp)
801036de:	e8 36 1c 00 00       	call   80105319 <memcmp>
801036e3:	85 c0                	test   %eax,%eax
801036e5:	75 1c                	jne    80103703 <mpsearch1+0x60>
801036e7:	c7 44 24 04 10 00 00 	movl   $0x10,0x4(%esp)
801036ee:	00 
801036ef:	8b 45 f4             	mov    -0xc(%ebp),%eax
801036f2:	89 04 24             	mov    %eax,(%esp)
801036f5:	e8 73 ff ff ff       	call   8010366d <sum>
801036fa:	84 c0                	test   %al,%al
801036fc:	75 05                	jne    80103703 <mpsearch1+0x60>
      return (struct mp*)p;
801036fe:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103701:	eb 11                	jmp    80103714 <mpsearch1+0x71>
{
  uchar *e, *p, *addr;

  addr = p2v(a);
  e = addr+len;
  for(p = addr; p < e; p += sizeof(struct mp))
80103703:	83 45 f4 10          	addl   $0x10,-0xc(%ebp)
80103707:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010370a:	3b 45 ec             	cmp    -0x14(%ebp),%eax
8010370d:	72 b9                	jb     801036c8 <mpsearch1+0x25>
    if(memcmp(p, "_MP_", 4) == 0 && sum(p, sizeof(struct mp)) == 0)
      return (struct mp*)p;
  return 0;
8010370f:	b8 00 00 00 00       	mov    $0x0,%eax
}
80103714:	c9                   	leave  
80103715:	c3                   	ret    

80103716 <mpsearch>:
// 1) in the first KB of the EBDA;
// 2) in the last KB of system base memory;
// 3) in the BIOS ROM between 0xE0000 and 0xFFFFF.
static struct mp*
mpsearch(void)
{
80103716:	55                   	push   %ebp
80103717:	89 e5                	mov    %esp,%ebp
80103719:	83 ec 28             	sub    $0x28,%esp
  uchar *bda;
  uint p;
  struct mp *mp;

  bda = (uchar *) P2V(0x400);
8010371c:	c7 45 f4 00 04 00 80 	movl   $0x80000400,-0xc(%ebp)
  if((p = ((bda[0x0F]<<8)| bda[0x0E]) << 4)){
80103723:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103726:	83 c0 0f             	add    $0xf,%eax
80103729:	0f b6 00             	movzbl (%eax),%eax
8010372c:	0f b6 c0             	movzbl %al,%eax
8010372f:	89 c2                	mov    %eax,%edx
80103731:	c1 e2 08             	shl    $0x8,%edx
80103734:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103737:	83 c0 0e             	add    $0xe,%eax
8010373a:	0f b6 00             	movzbl (%eax),%eax
8010373d:	0f b6 c0             	movzbl %al,%eax
80103740:	09 d0                	or     %edx,%eax
80103742:	c1 e0 04             	shl    $0x4,%eax
80103745:	89 45 f0             	mov    %eax,-0x10(%ebp)
80103748:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
8010374c:	74 21                	je     8010376f <mpsearch+0x59>
    if((mp = mpsearch1(p, 1024)))
8010374e:	c7 44 24 04 00 04 00 	movl   $0x400,0x4(%esp)
80103755:	00 
80103756:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103759:	89 04 24             	mov    %eax,(%esp)
8010375c:	e8 42 ff ff ff       	call   801036a3 <mpsearch1>
80103761:	89 45 ec             	mov    %eax,-0x14(%ebp)
80103764:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80103768:	74 50                	je     801037ba <mpsearch+0xa4>
      return mp;
8010376a:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010376d:	eb 5f                	jmp    801037ce <mpsearch+0xb8>
  } else {
    p = ((bda[0x14]<<8)|bda[0x13])*1024;
8010376f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103772:	83 c0 14             	add    $0x14,%eax
80103775:	0f b6 00             	movzbl (%eax),%eax
80103778:	0f b6 c0             	movzbl %al,%eax
8010377b:	89 c2                	mov    %eax,%edx
8010377d:	c1 e2 08             	shl    $0x8,%edx
80103780:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103783:	83 c0 13             	add    $0x13,%eax
80103786:	0f b6 00             	movzbl (%eax),%eax
80103789:	0f b6 c0             	movzbl %al,%eax
8010378c:	09 d0                	or     %edx,%eax
8010378e:	c1 e0 0a             	shl    $0xa,%eax
80103791:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if((mp = mpsearch1(p-1024, 1024)))
80103794:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103797:	2d 00 04 00 00       	sub    $0x400,%eax
8010379c:	c7 44 24 04 00 04 00 	movl   $0x400,0x4(%esp)
801037a3:	00 
801037a4:	89 04 24             	mov    %eax,(%esp)
801037a7:	e8 f7 fe ff ff       	call   801036a3 <mpsearch1>
801037ac:	89 45 ec             	mov    %eax,-0x14(%ebp)
801037af:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
801037b3:	74 05                	je     801037ba <mpsearch+0xa4>
      return mp;
801037b5:	8b 45 ec             	mov    -0x14(%ebp),%eax
801037b8:	eb 14                	jmp    801037ce <mpsearch+0xb8>
  }
  return mpsearch1(0xF0000, 0x10000);
801037ba:	c7 44 24 04 00 00 01 	movl   $0x10000,0x4(%esp)
801037c1:	00 
801037c2:	c7 04 24 00 00 0f 00 	movl   $0xf0000,(%esp)
801037c9:	e8 d5 fe ff ff       	call   801036a3 <mpsearch1>
}
801037ce:	c9                   	leave  
801037cf:	c3                   	ret    

801037d0 <mpconfig>:
// Check for correct signature, calculate the checksum and,
// if correct, check the version.
// To do: check extended table checksum.
static struct mpconf*
mpconfig(struct mp **pmp)
{
801037d0:	55                   	push   %ebp
801037d1:	89 e5                	mov    %esp,%ebp
801037d3:	83 ec 28             	sub    $0x28,%esp
  struct mpconf *conf;
  struct mp *mp;

  if((mp = mpsearch()) == 0 || mp->physaddr == 0)
801037d6:	e8 3b ff ff ff       	call   80103716 <mpsearch>
801037db:	89 45 f4             	mov    %eax,-0xc(%ebp)
801037de:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801037e2:	74 0a                	je     801037ee <mpconfig+0x1e>
801037e4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801037e7:	8b 40 04             	mov    0x4(%eax),%eax
801037ea:	85 c0                	test   %eax,%eax
801037ec:	75 0a                	jne    801037f8 <mpconfig+0x28>
    return 0;
801037ee:	b8 00 00 00 00       	mov    $0x0,%eax
801037f3:	e9 83 00 00 00       	jmp    8010387b <mpconfig+0xab>
  conf = (struct mpconf*) p2v((uint) mp->physaddr);
801037f8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801037fb:	8b 40 04             	mov    0x4(%eax),%eax
801037fe:	89 04 24             	mov    %eax,(%esp)
80103801:	e8 f2 fd ff ff       	call   801035f8 <p2v>
80103806:	89 45 f0             	mov    %eax,-0x10(%ebp)
  if(memcmp(conf, "PCMP", 4) != 0)
80103809:	c7 44 24 08 04 00 00 	movl   $0x4,0x8(%esp)
80103810:	00 
80103811:	c7 44 24 04 15 89 10 	movl   $0x80108915,0x4(%esp)
80103818:	80 
80103819:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010381c:	89 04 24             	mov    %eax,(%esp)
8010381f:	e8 f5 1a 00 00       	call   80105319 <memcmp>
80103824:	85 c0                	test   %eax,%eax
80103826:	74 07                	je     8010382f <mpconfig+0x5f>
    return 0;
80103828:	b8 00 00 00 00       	mov    $0x0,%eax
8010382d:	eb 4c                	jmp    8010387b <mpconfig+0xab>
  if(conf->version != 1 && conf->version != 4)
8010382f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80103832:	0f b6 40 06          	movzbl 0x6(%eax),%eax
80103836:	3c 01                	cmp    $0x1,%al
80103838:	74 12                	je     8010384c <mpconfig+0x7c>
8010383a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010383d:	0f b6 40 06          	movzbl 0x6(%eax),%eax
80103841:	3c 04                	cmp    $0x4,%al
80103843:	74 07                	je     8010384c <mpconfig+0x7c>
    return 0;
80103845:	b8 00 00 00 00       	mov    $0x0,%eax
8010384a:	eb 2f                	jmp    8010387b <mpconfig+0xab>
  if(sum((uchar*)conf, conf->length) != 0)
8010384c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010384f:	0f b7 40 04          	movzwl 0x4(%eax),%eax
80103853:	0f b7 c0             	movzwl %ax,%eax
80103856:	89 44 24 04          	mov    %eax,0x4(%esp)
8010385a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010385d:	89 04 24             	mov    %eax,(%esp)
80103860:	e8 08 fe ff ff       	call   8010366d <sum>
80103865:	84 c0                	test   %al,%al
80103867:	74 07                	je     80103870 <mpconfig+0xa0>
    return 0;
80103869:	b8 00 00 00 00       	mov    $0x0,%eax
8010386e:	eb 0b                	jmp    8010387b <mpconfig+0xab>
  *pmp = mp;
80103870:	8b 45 08             	mov    0x8(%ebp),%eax
80103873:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103876:	89 10                	mov    %edx,(%eax)
  return conf;
80103878:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
8010387b:	c9                   	leave  
8010387c:	c3                   	ret    

8010387d <mpinit>:

void
mpinit(void)
{
8010387d:	55                   	push   %ebp
8010387e:	89 e5                	mov    %esp,%ebp
80103880:	83 ec 38             	sub    $0x38,%esp
  struct mp *mp;
  struct mpconf *conf;
  struct mpproc *proc;
  struct mpioapic *ioapic;

  bcpu = &cpus[0];
80103883:	c7 05 44 b6 10 80 20 	movl   $0x8010f920,0x8010b644
8010388a:	f9 10 80 
  if((conf = mpconfig(&mp)) == 0)
8010388d:	8d 45 e0             	lea    -0x20(%ebp),%eax
80103890:	89 04 24             	mov    %eax,(%esp)
80103893:	e8 38 ff ff ff       	call   801037d0 <mpconfig>
80103898:	89 45 f0             	mov    %eax,-0x10(%ebp)
8010389b:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
8010389f:	0f 84 9c 01 00 00    	je     80103a41 <mpinit+0x1c4>
    return;
  ismp = 1;
801038a5:	c7 05 04 f9 10 80 01 	movl   $0x1,0x8010f904
801038ac:	00 00 00 
  lapic = (uint*)conf->lapicaddr;
801038af:	8b 45 f0             	mov    -0x10(%ebp),%eax
801038b2:	8b 40 24             	mov    0x24(%eax),%eax
801038b5:	a3 7c f8 10 80       	mov    %eax,0x8010f87c
  for(p=(uchar*)(conf+1), e=(uchar*)conf+conf->length; p<e; ){
801038ba:	8b 45 f0             	mov    -0x10(%ebp),%eax
801038bd:	83 c0 2c             	add    $0x2c,%eax
801038c0:	89 45 f4             	mov    %eax,-0xc(%ebp)
801038c3:	8b 45 f0             	mov    -0x10(%ebp),%eax
801038c6:	0f b7 40 04          	movzwl 0x4(%eax),%eax
801038ca:	0f b7 c0             	movzwl %ax,%eax
801038cd:	03 45 f0             	add    -0x10(%ebp),%eax
801038d0:	89 45 ec             	mov    %eax,-0x14(%ebp)
801038d3:	e9 f4 00 00 00       	jmp    801039cc <mpinit+0x14f>
    switch(*p){
801038d8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801038db:	0f b6 00             	movzbl (%eax),%eax
801038de:	0f b6 c0             	movzbl %al,%eax
801038e1:	83 f8 04             	cmp    $0x4,%eax
801038e4:	0f 87 bf 00 00 00    	ja     801039a9 <mpinit+0x12c>
801038ea:	8b 04 85 58 89 10 80 	mov    -0x7fef76a8(,%eax,4),%eax
801038f1:	ff e0                	jmp    *%eax
    case MPPROC:
      proc = (struct mpproc*)p;
801038f3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801038f6:	89 45 e8             	mov    %eax,-0x18(%ebp)
      if(ncpu != proc->apicid){
801038f9:	8b 45 e8             	mov    -0x18(%ebp),%eax
801038fc:	0f b6 40 01          	movzbl 0x1(%eax),%eax
80103900:	0f b6 d0             	movzbl %al,%edx
80103903:	a1 00 ff 10 80       	mov    0x8010ff00,%eax
80103908:	39 c2                	cmp    %eax,%edx
8010390a:	74 2d                	je     80103939 <mpinit+0xbc>
        cprintf("mpinit: ncpu=%d apicid=%d\n", ncpu, proc->apicid);
8010390c:	8b 45 e8             	mov    -0x18(%ebp),%eax
8010390f:	0f b6 40 01          	movzbl 0x1(%eax),%eax
80103913:	0f b6 d0             	movzbl %al,%edx
80103916:	a1 00 ff 10 80       	mov    0x8010ff00,%eax
8010391b:	89 54 24 08          	mov    %edx,0x8(%esp)
8010391f:	89 44 24 04          	mov    %eax,0x4(%esp)
80103923:	c7 04 24 1a 89 10 80 	movl   $0x8010891a,(%esp)
8010392a:	e8 72 ca ff ff       	call   801003a1 <cprintf>
        ismp = 0;
8010392f:	c7 05 04 f9 10 80 00 	movl   $0x0,0x8010f904
80103936:	00 00 00 
      }
      if(proc->flags & MPBOOT)
80103939:	8b 45 e8             	mov    -0x18(%ebp),%eax
8010393c:	0f b6 40 03          	movzbl 0x3(%eax),%eax
80103940:	0f b6 c0             	movzbl %al,%eax
80103943:	83 e0 02             	and    $0x2,%eax
80103946:	85 c0                	test   %eax,%eax
80103948:	74 15                	je     8010395f <mpinit+0xe2>
        bcpu = &cpus[ncpu];
8010394a:	a1 00 ff 10 80       	mov    0x8010ff00,%eax
8010394f:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
80103955:	05 20 f9 10 80       	add    $0x8010f920,%eax
8010395a:	a3 44 b6 10 80       	mov    %eax,0x8010b644
      cpus[ncpu].id = ncpu;
8010395f:	8b 15 00 ff 10 80    	mov    0x8010ff00,%edx
80103965:	a1 00 ff 10 80       	mov    0x8010ff00,%eax
8010396a:	69 d2 bc 00 00 00    	imul   $0xbc,%edx,%edx
80103970:	81 c2 20 f9 10 80    	add    $0x8010f920,%edx
80103976:	88 02                	mov    %al,(%edx)
      ncpu++;
80103978:	a1 00 ff 10 80       	mov    0x8010ff00,%eax
8010397d:	83 c0 01             	add    $0x1,%eax
80103980:	a3 00 ff 10 80       	mov    %eax,0x8010ff00
      p += sizeof(struct mpproc);
80103985:	83 45 f4 14          	addl   $0x14,-0xc(%ebp)
      continue;
80103989:	eb 41                	jmp    801039cc <mpinit+0x14f>
    case MPIOAPIC:
      ioapic = (struct mpioapic*)p;
8010398b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010398e:	89 45 e4             	mov    %eax,-0x1c(%ebp)
      ioapicid = ioapic->apicno;
80103991:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80103994:	0f b6 40 01          	movzbl 0x1(%eax),%eax
80103998:	a2 00 f9 10 80       	mov    %al,0x8010f900
      p += sizeof(struct mpioapic);
8010399d:	83 45 f4 08          	addl   $0x8,-0xc(%ebp)
      continue;
801039a1:	eb 29                	jmp    801039cc <mpinit+0x14f>
    case MPBUS:
    case MPIOINTR:
    case MPLINTR:
      p += 8;
801039a3:	83 45 f4 08          	addl   $0x8,-0xc(%ebp)
      continue;
801039a7:	eb 23                	jmp    801039cc <mpinit+0x14f>
    default:
      cprintf("mpinit: unknown config type %x\n", *p);
801039a9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801039ac:	0f b6 00             	movzbl (%eax),%eax
801039af:	0f b6 c0             	movzbl %al,%eax
801039b2:	89 44 24 04          	mov    %eax,0x4(%esp)
801039b6:	c7 04 24 38 89 10 80 	movl   $0x80108938,(%esp)
801039bd:	e8 df c9 ff ff       	call   801003a1 <cprintf>
      ismp = 0;
801039c2:	c7 05 04 f9 10 80 00 	movl   $0x0,0x8010f904
801039c9:	00 00 00 
  bcpu = &cpus[0];
  if((conf = mpconfig(&mp)) == 0)
    return;
  ismp = 1;
  lapic = (uint*)conf->lapicaddr;
  for(p=(uchar*)(conf+1), e=(uchar*)conf+conf->length; p<e; ){
801039cc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801039cf:	3b 45 ec             	cmp    -0x14(%ebp),%eax
801039d2:	0f 82 00 ff ff ff    	jb     801038d8 <mpinit+0x5b>
    default:
      cprintf("mpinit: unknown config type %x\n", *p);
      ismp = 0;
    }
  }
  if(!ismp){
801039d8:	a1 04 f9 10 80       	mov    0x8010f904,%eax
801039dd:	85 c0                	test   %eax,%eax
801039df:	75 1d                	jne    801039fe <mpinit+0x181>
    // Didn't like what we found; fall back to no MP.
    ncpu = 1;
801039e1:	c7 05 00 ff 10 80 01 	movl   $0x1,0x8010ff00
801039e8:	00 00 00 
    lapic = 0;
801039eb:	c7 05 7c f8 10 80 00 	movl   $0x0,0x8010f87c
801039f2:	00 00 00 
    ioapicid = 0;
801039f5:	c6 05 00 f9 10 80 00 	movb   $0x0,0x8010f900
    return;
801039fc:	eb 44                	jmp    80103a42 <mpinit+0x1c5>
  }

  if(mp->imcrp){
801039fe:	8b 45 e0             	mov    -0x20(%ebp),%eax
80103a01:	0f b6 40 0c          	movzbl 0xc(%eax),%eax
80103a05:	84 c0                	test   %al,%al
80103a07:	74 39                	je     80103a42 <mpinit+0x1c5>
    // Bochs doesn't support IMCR, so this doesn't run on Bochs.
    // But it would on real hardware.
    outb(0x22, 0x70);   // Select IMCR
80103a09:	c7 44 24 04 70 00 00 	movl   $0x70,0x4(%esp)
80103a10:	00 
80103a11:	c7 04 24 22 00 00 00 	movl   $0x22,(%esp)
80103a18:	e8 12 fc ff ff       	call   8010362f <outb>
    outb(0x23, inb(0x23) | 1);  // Mask external interrupts.
80103a1d:	c7 04 24 23 00 00 00 	movl   $0x23,(%esp)
80103a24:	e8 dc fb ff ff       	call   80103605 <inb>
80103a29:	83 c8 01             	or     $0x1,%eax
80103a2c:	0f b6 c0             	movzbl %al,%eax
80103a2f:	89 44 24 04          	mov    %eax,0x4(%esp)
80103a33:	c7 04 24 23 00 00 00 	movl   $0x23,(%esp)
80103a3a:	e8 f0 fb ff ff       	call   8010362f <outb>
80103a3f:	eb 01                	jmp    80103a42 <mpinit+0x1c5>
  struct mpproc *proc;
  struct mpioapic *ioapic;

  bcpu = &cpus[0];
  if((conf = mpconfig(&mp)) == 0)
    return;
80103a41:	90                   	nop
    // Bochs doesn't support IMCR, so this doesn't run on Bochs.
    // But it would on real hardware.
    outb(0x22, 0x70);   // Select IMCR
    outb(0x23, inb(0x23) | 1);  // Mask external interrupts.
  }
}
80103a42:	c9                   	leave  
80103a43:	c3                   	ret    

80103a44 <outb>:
80103a44:	55                   	push   %ebp
80103a45:	89 e5                	mov    %esp,%ebp
80103a47:	83 ec 08             	sub    $0x8,%esp
80103a4a:	8b 55 08             	mov    0x8(%ebp),%edx
80103a4d:	8b 45 0c             	mov    0xc(%ebp),%eax
80103a50:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
80103a54:	88 45 f8             	mov    %al,-0x8(%ebp)
80103a57:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
80103a5b:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
80103a5f:	ee                   	out    %al,(%dx)
80103a60:	c9                   	leave  
80103a61:	c3                   	ret    

80103a62 <picsetmask>:
80103a62:	55                   	push   %ebp
80103a63:	89 e5                	mov    %esp,%ebp
80103a65:	83 ec 0c             	sub    $0xc,%esp
80103a68:	8b 45 08             	mov    0x8(%ebp),%eax
80103a6b:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
80103a6f:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80103a73:	66 a3 00 b0 10 80    	mov    %ax,0x8010b000
80103a79:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80103a7d:	0f b6 c0             	movzbl %al,%eax
80103a80:	89 44 24 04          	mov    %eax,0x4(%esp)
80103a84:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103a8b:	e8 b4 ff ff ff       	call   80103a44 <outb>
80103a90:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80103a94:	66 c1 e8 08          	shr    $0x8,%ax
80103a98:	0f b6 c0             	movzbl %al,%eax
80103a9b:	89 44 24 04          	mov    %eax,0x4(%esp)
80103a9f:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103aa6:	e8 99 ff ff ff       	call   80103a44 <outb>
80103aab:	c9                   	leave  
80103aac:	c3                   	ret    

80103aad <picenable>:
80103aad:	55                   	push   %ebp
80103aae:	89 e5                	mov    %esp,%ebp
80103ab0:	53                   	push   %ebx
80103ab1:	83 ec 04             	sub    $0x4,%esp
80103ab4:	8b 45 08             	mov    0x8(%ebp),%eax
80103ab7:	ba 01 00 00 00       	mov    $0x1,%edx
80103abc:	89 d3                	mov    %edx,%ebx
80103abe:	89 c1                	mov    %eax,%ecx
80103ac0:	d3 e3                	shl    %cl,%ebx
80103ac2:	89 d8                	mov    %ebx,%eax
80103ac4:	89 c2                	mov    %eax,%edx
80103ac6:	f7 d2                	not    %edx
80103ac8:	0f b7 05 00 b0 10 80 	movzwl 0x8010b000,%eax
80103acf:	21 d0                	and    %edx,%eax
80103ad1:	0f b7 c0             	movzwl %ax,%eax
80103ad4:	89 04 24             	mov    %eax,(%esp)
80103ad7:	e8 86 ff ff ff       	call   80103a62 <picsetmask>
80103adc:	83 c4 04             	add    $0x4,%esp
80103adf:	5b                   	pop    %ebx
80103ae0:	5d                   	pop    %ebp
80103ae1:	c3                   	ret    

80103ae2 <picinit>:
80103ae2:	55                   	push   %ebp
80103ae3:	89 e5                	mov    %esp,%ebp
80103ae5:	83 ec 08             	sub    $0x8,%esp
80103ae8:	c7 44 24 04 ff 00 00 	movl   $0xff,0x4(%esp)
80103aef:	00 
80103af0:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103af7:	e8 48 ff ff ff       	call   80103a44 <outb>
80103afc:	c7 44 24 04 ff 00 00 	movl   $0xff,0x4(%esp)
80103b03:	00 
80103b04:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103b0b:	e8 34 ff ff ff       	call   80103a44 <outb>
80103b10:	c7 44 24 04 11 00 00 	movl   $0x11,0x4(%esp)
80103b17:	00 
80103b18:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80103b1f:	e8 20 ff ff ff       	call   80103a44 <outb>
80103b24:	c7 44 24 04 20 00 00 	movl   $0x20,0x4(%esp)
80103b2b:	00 
80103b2c:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103b33:	e8 0c ff ff ff       	call   80103a44 <outb>
80103b38:	c7 44 24 04 04 00 00 	movl   $0x4,0x4(%esp)
80103b3f:	00 
80103b40:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103b47:	e8 f8 fe ff ff       	call   80103a44 <outb>
80103b4c:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
80103b53:	00 
80103b54:	c7 04 24 21 00 00 00 	movl   $0x21,(%esp)
80103b5b:	e8 e4 fe ff ff       	call   80103a44 <outb>
80103b60:	c7 44 24 04 11 00 00 	movl   $0x11,0x4(%esp)
80103b67:	00 
80103b68:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80103b6f:	e8 d0 fe ff ff       	call   80103a44 <outb>
80103b74:	c7 44 24 04 28 00 00 	movl   $0x28,0x4(%esp)
80103b7b:	00 
80103b7c:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103b83:	e8 bc fe ff ff       	call   80103a44 <outb>
80103b88:	c7 44 24 04 02 00 00 	movl   $0x2,0x4(%esp)
80103b8f:	00 
80103b90:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103b97:	e8 a8 fe ff ff       	call   80103a44 <outb>
80103b9c:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
80103ba3:	00 
80103ba4:	c7 04 24 a1 00 00 00 	movl   $0xa1,(%esp)
80103bab:	e8 94 fe ff ff       	call   80103a44 <outb>
80103bb0:	c7 44 24 04 68 00 00 	movl   $0x68,0x4(%esp)
80103bb7:	00 
80103bb8:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80103bbf:	e8 80 fe ff ff       	call   80103a44 <outb>
80103bc4:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80103bcb:	00 
80103bcc:	c7 04 24 20 00 00 00 	movl   $0x20,(%esp)
80103bd3:	e8 6c fe ff ff       	call   80103a44 <outb>
80103bd8:	c7 44 24 04 68 00 00 	movl   $0x68,0x4(%esp)
80103bdf:	00 
80103be0:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80103be7:	e8 58 fe ff ff       	call   80103a44 <outb>
80103bec:	c7 44 24 04 0a 00 00 	movl   $0xa,0x4(%esp)
80103bf3:	00 
80103bf4:	c7 04 24 a0 00 00 00 	movl   $0xa0,(%esp)
80103bfb:	e8 44 fe ff ff       	call   80103a44 <outb>
80103c00:	0f b7 05 00 b0 10 80 	movzwl 0x8010b000,%eax
80103c07:	66 83 f8 ff          	cmp    $0xffff,%ax
80103c0b:	74 12                	je     80103c1f <picinit+0x13d>
80103c0d:	0f b7 05 00 b0 10 80 	movzwl 0x8010b000,%eax
80103c14:	0f b7 c0             	movzwl %ax,%eax
80103c17:	89 04 24             	mov    %eax,(%esp)
80103c1a:	e8 43 fe ff ff       	call   80103a62 <picsetmask>
80103c1f:	c9                   	leave  
80103c20:	c3                   	ret    
80103c21:	00 00                	add    %al,(%eax)
	...

80103c24 <pipealloc>:
  int writeopen;  // write fd is still open
};

int
pipealloc(struct file **f0, struct file **f1)
{
80103c24:	55                   	push   %ebp
80103c25:	89 e5                	mov    %esp,%ebp
80103c27:	83 ec 28             	sub    $0x28,%esp
  struct pipe *p;

  p = 0;
80103c2a:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
  *f0 = *f1 = 0;
80103c31:	8b 45 0c             	mov    0xc(%ebp),%eax
80103c34:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
80103c3a:	8b 45 0c             	mov    0xc(%ebp),%eax
80103c3d:	8b 10                	mov    (%eax),%edx
80103c3f:	8b 45 08             	mov    0x8(%ebp),%eax
80103c42:	89 10                	mov    %edx,(%eax)
  if((*f0 = filealloc()) == 0 || (*f1 = filealloc()) == 0)
80103c44:	e8 cb d2 ff ff       	call   80100f14 <filealloc>
80103c49:	8b 55 08             	mov    0x8(%ebp),%edx
80103c4c:	89 02                	mov    %eax,(%edx)
80103c4e:	8b 45 08             	mov    0x8(%ebp),%eax
80103c51:	8b 00                	mov    (%eax),%eax
80103c53:	85 c0                	test   %eax,%eax
80103c55:	0f 84 c8 00 00 00    	je     80103d23 <pipealloc+0xff>
80103c5b:	e8 b4 d2 ff ff       	call   80100f14 <filealloc>
80103c60:	8b 55 0c             	mov    0xc(%ebp),%edx
80103c63:	89 02                	mov    %eax,(%edx)
80103c65:	8b 45 0c             	mov    0xc(%ebp),%eax
80103c68:	8b 00                	mov    (%eax),%eax
80103c6a:	85 c0                	test   %eax,%eax
80103c6c:	0f 84 b1 00 00 00    	je     80103d23 <pipealloc+0xff>
    goto bad;
  if((p = (struct pipe*)kalloc()) == 0)
80103c72:	e8 80 ee ff ff       	call   80102af7 <kalloc>
80103c77:	89 45 f4             	mov    %eax,-0xc(%ebp)
80103c7a:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80103c7e:	0f 84 9e 00 00 00    	je     80103d22 <pipealloc+0xfe>
    goto bad;
  p->readopen = 1;
80103c84:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103c87:	c7 80 3c 02 00 00 01 	movl   $0x1,0x23c(%eax)
80103c8e:	00 00 00 
  p->writeopen = 1;
80103c91:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103c94:	c7 80 40 02 00 00 01 	movl   $0x1,0x240(%eax)
80103c9b:	00 00 00 
  p->nwrite = 0;
80103c9e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103ca1:	c7 80 38 02 00 00 00 	movl   $0x0,0x238(%eax)
80103ca8:	00 00 00 
  p->nread = 0;
80103cab:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103cae:	c7 80 34 02 00 00 00 	movl   $0x0,0x234(%eax)
80103cb5:	00 00 00 
  initlock(&p->lock, "pipe");
80103cb8:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103cbb:	c7 44 24 04 6c 89 10 	movl   $0x8010896c,0x4(%esp)
80103cc2:	80 
80103cc3:	89 04 24             	mov    %eax,(%esp)
80103cc6:	e8 67 13 00 00       	call   80105032 <initlock>
  (*f0)->type = FD_PIPE;
80103ccb:	8b 45 08             	mov    0x8(%ebp),%eax
80103cce:	8b 00                	mov    (%eax),%eax
80103cd0:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
  (*f0)->readable = 1;
80103cd6:	8b 45 08             	mov    0x8(%ebp),%eax
80103cd9:	8b 00                	mov    (%eax),%eax
80103cdb:	c6 40 08 01          	movb   $0x1,0x8(%eax)
  (*f0)->writable = 0;
80103cdf:	8b 45 08             	mov    0x8(%ebp),%eax
80103ce2:	8b 00                	mov    (%eax),%eax
80103ce4:	c6 40 09 00          	movb   $0x0,0x9(%eax)
  (*f0)->pipe = p;
80103ce8:	8b 45 08             	mov    0x8(%ebp),%eax
80103ceb:	8b 00                	mov    (%eax),%eax
80103ced:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103cf0:	89 50 0c             	mov    %edx,0xc(%eax)
  (*f1)->type = FD_PIPE;
80103cf3:	8b 45 0c             	mov    0xc(%ebp),%eax
80103cf6:	8b 00                	mov    (%eax),%eax
80103cf8:	c7 00 01 00 00 00    	movl   $0x1,(%eax)
  (*f1)->readable = 0;
80103cfe:	8b 45 0c             	mov    0xc(%ebp),%eax
80103d01:	8b 00                	mov    (%eax),%eax
80103d03:	c6 40 08 00          	movb   $0x0,0x8(%eax)
  (*f1)->writable = 1;
80103d07:	8b 45 0c             	mov    0xc(%ebp),%eax
80103d0a:	8b 00                	mov    (%eax),%eax
80103d0c:	c6 40 09 01          	movb   $0x1,0x9(%eax)
  (*f1)->pipe = p;
80103d10:	8b 45 0c             	mov    0xc(%ebp),%eax
80103d13:	8b 00                	mov    (%eax),%eax
80103d15:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103d18:	89 50 0c             	mov    %edx,0xc(%eax)
  return 0;
80103d1b:	b8 00 00 00 00       	mov    $0x0,%eax
80103d20:	eb 43                	jmp    80103d65 <pipealloc+0x141>
  p = 0;
  *f0 = *f1 = 0;
  if((*f0 = filealloc()) == 0 || (*f1 = filealloc()) == 0)
    goto bad;
  if((p = (struct pipe*)kalloc()) == 0)
    goto bad;
80103d22:	90                   	nop
  (*f1)->pipe = p;
  return 0;

//PAGEBREAK: 20
 bad:
  if(p)
80103d23:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80103d27:	74 0b                	je     80103d34 <pipealloc+0x110>
    kfree((char*)p);
80103d29:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103d2c:	89 04 24             	mov    %eax,(%esp)
80103d2f:	e8 2a ed ff ff       	call   80102a5e <kfree>
  if(*f0)
80103d34:	8b 45 08             	mov    0x8(%ebp),%eax
80103d37:	8b 00                	mov    (%eax),%eax
80103d39:	85 c0                	test   %eax,%eax
80103d3b:	74 0d                	je     80103d4a <pipealloc+0x126>
    fileclose(*f0);
80103d3d:	8b 45 08             	mov    0x8(%ebp),%eax
80103d40:	8b 00                	mov    (%eax),%eax
80103d42:	89 04 24             	mov    %eax,(%esp)
80103d45:	e8 72 d2 ff ff       	call   80100fbc <fileclose>
  if(*f1)
80103d4a:	8b 45 0c             	mov    0xc(%ebp),%eax
80103d4d:	8b 00                	mov    (%eax),%eax
80103d4f:	85 c0                	test   %eax,%eax
80103d51:	74 0d                	je     80103d60 <pipealloc+0x13c>
    fileclose(*f1);
80103d53:	8b 45 0c             	mov    0xc(%ebp),%eax
80103d56:	8b 00                	mov    (%eax),%eax
80103d58:	89 04 24             	mov    %eax,(%esp)
80103d5b:	e8 5c d2 ff ff       	call   80100fbc <fileclose>
  return -1;
80103d60:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80103d65:	c9                   	leave  
80103d66:	c3                   	ret    

80103d67 <pipeclose>:

void
pipeclose(struct pipe *p, int writable)
{
80103d67:	55                   	push   %ebp
80103d68:	89 e5                	mov    %esp,%ebp
80103d6a:	83 ec 18             	sub    $0x18,%esp
  acquire(&p->lock);
80103d6d:	8b 45 08             	mov    0x8(%ebp),%eax
80103d70:	89 04 24             	mov    %eax,(%esp)
80103d73:	e8 db 12 00 00       	call   80105053 <acquire>
  if(writable){
80103d78:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80103d7c:	74 1f                	je     80103d9d <pipeclose+0x36>
    p->writeopen = 0;
80103d7e:	8b 45 08             	mov    0x8(%ebp),%eax
80103d81:	c7 80 40 02 00 00 00 	movl   $0x0,0x240(%eax)
80103d88:	00 00 00 
    wakeup(&p->nread);
80103d8b:	8b 45 08             	mov    0x8(%ebp),%eax
80103d8e:	05 34 02 00 00       	add    $0x234,%eax
80103d93:	89 04 24             	mov    %eax,(%esp)
80103d96:	e8 51 0b 00 00       	call   801048ec <wakeup>
80103d9b:	eb 1d                	jmp    80103dba <pipeclose+0x53>
  } else {
    p->readopen = 0;
80103d9d:	8b 45 08             	mov    0x8(%ebp),%eax
80103da0:	c7 80 3c 02 00 00 00 	movl   $0x0,0x23c(%eax)
80103da7:	00 00 00 
    wakeup(&p->nwrite);
80103daa:	8b 45 08             	mov    0x8(%ebp),%eax
80103dad:	05 38 02 00 00       	add    $0x238,%eax
80103db2:	89 04 24             	mov    %eax,(%esp)
80103db5:	e8 32 0b 00 00       	call   801048ec <wakeup>
  }
  if(p->readopen == 0 && p->writeopen == 0){
80103dba:	8b 45 08             	mov    0x8(%ebp),%eax
80103dbd:	8b 80 3c 02 00 00    	mov    0x23c(%eax),%eax
80103dc3:	85 c0                	test   %eax,%eax
80103dc5:	75 25                	jne    80103dec <pipeclose+0x85>
80103dc7:	8b 45 08             	mov    0x8(%ebp),%eax
80103dca:	8b 80 40 02 00 00    	mov    0x240(%eax),%eax
80103dd0:	85 c0                	test   %eax,%eax
80103dd2:	75 18                	jne    80103dec <pipeclose+0x85>
    release(&p->lock);
80103dd4:	8b 45 08             	mov    0x8(%ebp),%eax
80103dd7:	89 04 24             	mov    %eax,(%esp)
80103dda:	e8 d6 12 00 00       	call   801050b5 <release>
    kfree((char*)p);
80103ddf:	8b 45 08             	mov    0x8(%ebp),%eax
80103de2:	89 04 24             	mov    %eax,(%esp)
80103de5:	e8 74 ec ff ff       	call   80102a5e <kfree>
80103dea:	eb 0b                	jmp    80103df7 <pipeclose+0x90>
  } else
    release(&p->lock);
80103dec:	8b 45 08             	mov    0x8(%ebp),%eax
80103def:	89 04 24             	mov    %eax,(%esp)
80103df2:	e8 be 12 00 00       	call   801050b5 <release>
}
80103df7:	c9                   	leave  
80103df8:	c3                   	ret    

80103df9 <pipewrite>:

//PAGEBREAK: 40
int
pipewrite(struct pipe *p, char *addr, int n)
{
80103df9:	55                   	push   %ebp
80103dfa:	89 e5                	mov    %esp,%ebp
80103dfc:	53                   	push   %ebx
80103dfd:	83 ec 24             	sub    $0x24,%esp
  int i;

  acquire(&p->lock);
80103e00:	8b 45 08             	mov    0x8(%ebp),%eax
80103e03:	89 04 24             	mov    %eax,(%esp)
80103e06:	e8 48 12 00 00       	call   80105053 <acquire>
  for(i = 0; i < n; i++){
80103e0b:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103e12:	e9 a6 00 00 00       	jmp    80103ebd <pipewrite+0xc4>
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
      if(p->readopen == 0 || proc->killed){
80103e17:	8b 45 08             	mov    0x8(%ebp),%eax
80103e1a:	8b 80 3c 02 00 00    	mov    0x23c(%eax),%eax
80103e20:	85 c0                	test   %eax,%eax
80103e22:	74 0d                	je     80103e31 <pipewrite+0x38>
80103e24:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80103e2a:	8b 40 24             	mov    0x24(%eax),%eax
80103e2d:	85 c0                	test   %eax,%eax
80103e2f:	74 15                	je     80103e46 <pipewrite+0x4d>
        release(&p->lock);
80103e31:	8b 45 08             	mov    0x8(%ebp),%eax
80103e34:	89 04 24             	mov    %eax,(%esp)
80103e37:	e8 79 12 00 00       	call   801050b5 <release>
        return -1;
80103e3c:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80103e41:	e9 9d 00 00 00       	jmp    80103ee3 <pipewrite+0xea>
      }
      wakeup(&p->nread);
80103e46:	8b 45 08             	mov    0x8(%ebp),%eax
80103e49:	05 34 02 00 00       	add    $0x234,%eax
80103e4e:	89 04 24             	mov    %eax,(%esp)
80103e51:	e8 96 0a 00 00       	call   801048ec <wakeup>
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
80103e56:	8b 45 08             	mov    0x8(%ebp),%eax
80103e59:	8b 55 08             	mov    0x8(%ebp),%edx
80103e5c:	81 c2 38 02 00 00    	add    $0x238,%edx
80103e62:	89 44 24 04          	mov    %eax,0x4(%esp)
80103e66:	89 14 24             	mov    %edx,(%esp)
80103e69:	e8 a4 09 00 00       	call   80104812 <sleep>
80103e6e:	eb 01                	jmp    80103e71 <pipewrite+0x78>
{
  int i;

  acquire(&p->lock);
  for(i = 0; i < n; i++){
    while(p->nwrite == p->nread + PIPESIZE){  //DOC: pipewrite-full
80103e70:	90                   	nop
80103e71:	8b 45 08             	mov    0x8(%ebp),%eax
80103e74:	8b 90 38 02 00 00    	mov    0x238(%eax),%edx
80103e7a:	8b 45 08             	mov    0x8(%ebp),%eax
80103e7d:	8b 80 34 02 00 00    	mov    0x234(%eax),%eax
80103e83:	05 00 02 00 00       	add    $0x200,%eax
80103e88:	39 c2                	cmp    %eax,%edx
80103e8a:	74 8b                	je     80103e17 <pipewrite+0x1e>
        return -1;
      }
      wakeup(&p->nread);
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
    }
    p->data[p->nwrite++ % PIPESIZE] = addr[i];
80103e8c:	8b 45 08             	mov    0x8(%ebp),%eax
80103e8f:	8b 80 38 02 00 00    	mov    0x238(%eax),%eax
80103e95:	89 c3                	mov    %eax,%ebx
80103e97:	81 e3 ff 01 00 00    	and    $0x1ff,%ebx
80103e9d:	8b 55 f4             	mov    -0xc(%ebp),%edx
80103ea0:	03 55 0c             	add    0xc(%ebp),%edx
80103ea3:	0f b6 0a             	movzbl (%edx),%ecx
80103ea6:	8b 55 08             	mov    0x8(%ebp),%edx
80103ea9:	88 4c 1a 34          	mov    %cl,0x34(%edx,%ebx,1)
80103ead:	8d 50 01             	lea    0x1(%eax),%edx
80103eb0:	8b 45 08             	mov    0x8(%ebp),%eax
80103eb3:	89 90 38 02 00 00    	mov    %edx,0x238(%eax)
pipewrite(struct pipe *p, char *addr, int n)
{
  int i;

  acquire(&p->lock);
  for(i = 0; i < n; i++){
80103eb9:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80103ebd:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103ec0:	3b 45 10             	cmp    0x10(%ebp),%eax
80103ec3:	7c ab                	jl     80103e70 <pipewrite+0x77>
      wakeup(&p->nread);
      sleep(&p->nwrite, &p->lock);  //DOC: pipewrite-sleep
    }
    p->data[p->nwrite++ % PIPESIZE] = addr[i];
  }
  wakeup(&p->nread);  //DOC: pipewrite-wakeup1
80103ec5:	8b 45 08             	mov    0x8(%ebp),%eax
80103ec8:	05 34 02 00 00       	add    $0x234,%eax
80103ecd:	89 04 24             	mov    %eax,(%esp)
80103ed0:	e8 17 0a 00 00       	call   801048ec <wakeup>
  release(&p->lock);
80103ed5:	8b 45 08             	mov    0x8(%ebp),%eax
80103ed8:	89 04 24             	mov    %eax,(%esp)
80103edb:	e8 d5 11 00 00       	call   801050b5 <release>
  return n;
80103ee0:	8b 45 10             	mov    0x10(%ebp),%eax
}
80103ee3:	83 c4 24             	add    $0x24,%esp
80103ee6:	5b                   	pop    %ebx
80103ee7:	5d                   	pop    %ebp
80103ee8:	c3                   	ret    

80103ee9 <piperead>:

int
piperead(struct pipe *p, char *addr, int n)
{
80103ee9:	55                   	push   %ebp
80103eea:	89 e5                	mov    %esp,%ebp
80103eec:	53                   	push   %ebx
80103eed:	83 ec 24             	sub    $0x24,%esp
  int i;

  acquire(&p->lock);
80103ef0:	8b 45 08             	mov    0x8(%ebp),%eax
80103ef3:	89 04 24             	mov    %eax,(%esp)
80103ef6:	e8 58 11 00 00       	call   80105053 <acquire>
  while(p->nread == p->nwrite && p->writeopen){  //DOC: pipe-empty
80103efb:	eb 3a                	jmp    80103f37 <piperead+0x4e>
    if(proc->killed){
80103efd:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80103f03:	8b 40 24             	mov    0x24(%eax),%eax
80103f06:	85 c0                	test   %eax,%eax
80103f08:	74 15                	je     80103f1f <piperead+0x36>
      release(&p->lock);
80103f0a:	8b 45 08             	mov    0x8(%ebp),%eax
80103f0d:	89 04 24             	mov    %eax,(%esp)
80103f10:	e8 a0 11 00 00       	call   801050b5 <release>
      return -1;
80103f15:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80103f1a:	e9 b6 00 00 00       	jmp    80103fd5 <piperead+0xec>
    }
    sleep(&p->nread, &p->lock); //DOC: piperead-sleep
80103f1f:	8b 45 08             	mov    0x8(%ebp),%eax
80103f22:	8b 55 08             	mov    0x8(%ebp),%edx
80103f25:	81 c2 34 02 00 00    	add    $0x234,%edx
80103f2b:	89 44 24 04          	mov    %eax,0x4(%esp)
80103f2f:	89 14 24             	mov    %edx,(%esp)
80103f32:	e8 db 08 00 00       	call   80104812 <sleep>
piperead(struct pipe *p, char *addr, int n)
{
  int i;

  acquire(&p->lock);
  while(p->nread == p->nwrite && p->writeopen){  //DOC: pipe-empty
80103f37:	8b 45 08             	mov    0x8(%ebp),%eax
80103f3a:	8b 90 34 02 00 00    	mov    0x234(%eax),%edx
80103f40:	8b 45 08             	mov    0x8(%ebp),%eax
80103f43:	8b 80 38 02 00 00    	mov    0x238(%eax),%eax
80103f49:	39 c2                	cmp    %eax,%edx
80103f4b:	75 0d                	jne    80103f5a <piperead+0x71>
80103f4d:	8b 45 08             	mov    0x8(%ebp),%eax
80103f50:	8b 80 40 02 00 00    	mov    0x240(%eax),%eax
80103f56:	85 c0                	test   %eax,%eax
80103f58:	75 a3                	jne    80103efd <piperead+0x14>
      release(&p->lock);
      return -1;
    }
    sleep(&p->nread, &p->lock); //DOC: piperead-sleep
  }
  for(i = 0; i < n; i++){  //DOC: piperead-copy
80103f5a:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80103f61:	eb 49                	jmp    80103fac <piperead+0xc3>
    if(p->nread == p->nwrite)
80103f63:	8b 45 08             	mov    0x8(%ebp),%eax
80103f66:	8b 90 34 02 00 00    	mov    0x234(%eax),%edx
80103f6c:	8b 45 08             	mov    0x8(%ebp),%eax
80103f6f:	8b 80 38 02 00 00    	mov    0x238(%eax),%eax
80103f75:	39 c2                	cmp    %eax,%edx
80103f77:	74 3d                	je     80103fb6 <piperead+0xcd>
      break;
    addr[i] = p->data[p->nread++ % PIPESIZE];
80103f79:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103f7c:	89 c2                	mov    %eax,%edx
80103f7e:	03 55 0c             	add    0xc(%ebp),%edx
80103f81:	8b 45 08             	mov    0x8(%ebp),%eax
80103f84:	8b 80 34 02 00 00    	mov    0x234(%eax),%eax
80103f8a:	89 c3                	mov    %eax,%ebx
80103f8c:	81 e3 ff 01 00 00    	and    $0x1ff,%ebx
80103f92:	8b 4d 08             	mov    0x8(%ebp),%ecx
80103f95:	0f b6 4c 19 34       	movzbl 0x34(%ecx,%ebx,1),%ecx
80103f9a:	88 0a                	mov    %cl,(%edx)
80103f9c:	8d 50 01             	lea    0x1(%eax),%edx
80103f9f:	8b 45 08             	mov    0x8(%ebp),%eax
80103fa2:	89 90 34 02 00 00    	mov    %edx,0x234(%eax)
      release(&p->lock);
      return -1;
    }
    sleep(&p->nread, &p->lock); //DOC: piperead-sleep
  }
  for(i = 0; i < n; i++){  //DOC: piperead-copy
80103fa8:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80103fac:	8b 45 f4             	mov    -0xc(%ebp),%eax
80103faf:	3b 45 10             	cmp    0x10(%ebp),%eax
80103fb2:	7c af                	jl     80103f63 <piperead+0x7a>
80103fb4:	eb 01                	jmp    80103fb7 <piperead+0xce>
    if(p->nread == p->nwrite)
      break;
80103fb6:	90                   	nop
    addr[i] = p->data[p->nread++ % PIPESIZE];
  }
  wakeup(&p->nwrite);  //DOC: piperead-wakeup
80103fb7:	8b 45 08             	mov    0x8(%ebp),%eax
80103fba:	05 38 02 00 00       	add    $0x238,%eax
80103fbf:	89 04 24             	mov    %eax,(%esp)
80103fc2:	e8 25 09 00 00       	call   801048ec <wakeup>
  release(&p->lock);
80103fc7:	8b 45 08             	mov    0x8(%ebp),%eax
80103fca:	89 04 24             	mov    %eax,(%esp)
80103fcd:	e8 e3 10 00 00       	call   801050b5 <release>
  return i;
80103fd2:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80103fd5:	83 c4 24             	add    $0x24,%esp
80103fd8:	5b                   	pop    %ebx
80103fd9:	5d                   	pop    %ebp
80103fda:	c3                   	ret    
	...

80103fdc <readeflags>:
  asm volatile("ltr %0" : : "r" (sel));
}

static inline uint
readeflags(void)
{
80103fdc:	55                   	push   %ebp
80103fdd:	89 e5                	mov    %esp,%ebp
80103fdf:	53                   	push   %ebx
80103fe0:	83 ec 10             	sub    $0x10,%esp
  uint eflags;
  asm volatile("pushfl; popl %0" : "=r" (eflags));
80103fe3:	9c                   	pushf  
80103fe4:	5b                   	pop    %ebx
80103fe5:	89 5d f8             	mov    %ebx,-0x8(%ebp)
  return eflags;
80103fe8:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
80103feb:	83 c4 10             	add    $0x10,%esp
80103fee:	5b                   	pop    %ebx
80103fef:	5d                   	pop    %ebp
80103ff0:	c3                   	ret    

80103ff1 <sti>:
  asm volatile("cli");
}

static inline void
sti(void)
{
80103ff1:	55                   	push   %ebp
80103ff2:	89 e5                	mov    %esp,%ebp
  asm volatile("sti");
80103ff4:	fb                   	sti    
}
80103ff5:	5d                   	pop    %ebp
80103ff6:	c3                   	ret    

80103ff7 <pinit>:
extern void forkret(void);
extern void trapret(void);

static void wakeup1(void *chan);

void pinit(void) {
80103ff7:	55                   	push   %ebp
80103ff8:	89 e5                	mov    %esp,%ebp
80103ffa:	83 ec 18             	sub    $0x18,%esp
	initlock(&ptable.lock, "ptable");
80103ffd:	c7 44 24 04 74 89 10 	movl   $0x80108974,0x4(%esp)
80104004:	80 
80104005:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
8010400c:	e8 21 10 00 00       	call   80105032 <initlock>
}
80104011:	c9                   	leave  
80104012:	c3                   	ret    

80104013 <allocproc>:
// Look in the process table for an UNUSED proc.
// If found, change state to EMBRYO and initialize
// state required to run in the kernel.
// Otherwise return 0.
static struct proc*
allocproc(void) {
80104013:	55                   	push   %ebp
80104014:	89 e5                	mov    %esp,%ebp
80104016:	83 ec 28             	sub    $0x28,%esp
	struct proc *p;
	char *sp;

	acquire(&ptable.lock);
80104019:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104020:	e8 2e 10 00 00       	call   80105053 <acquire>
	for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104025:	c7 45 f4 54 ff 10 80 	movl   $0x8010ff54,-0xc(%ebp)
8010402c:	eb 0e                	jmp    8010403c <allocproc+0x29>
		if (p->state == UNUSED)
8010402e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104031:	8b 40 0c             	mov    0xc(%eax),%eax
80104034:	85 c0                	test   %eax,%eax
80104036:	74 23                	je     8010405b <allocproc+0x48>
allocproc(void) {
	struct proc *p;
	char *sp;

	acquire(&ptable.lock);
	for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
80104038:	83 6d f4 80          	subl   $0xffffff80,-0xc(%ebp)
8010403c:	81 7d f4 54 1f 11 80 	cmpl   $0x80111f54,-0xc(%ebp)
80104043:	72 e9                	jb     8010402e <allocproc+0x1b>
		if (p->state == UNUSED)
			goto found;
	release(&ptable.lock);
80104045:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
8010404c:	e8 64 10 00 00       	call   801050b5 <release>
	return 0;
80104051:	b8 00 00 00 00       	mov    $0x0,%eax
80104056:	e9 b5 00 00 00       	jmp    80104110 <allocproc+0xfd>
	char *sp;

	acquire(&ptable.lock);
	for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
		if (p->state == UNUSED)
			goto found;
8010405b:	90                   	nop
	release(&ptable.lock);
	return 0;

	found: p->state = EMBRYO;
8010405c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010405f:	c7 40 0c 01 00 00 00 	movl   $0x1,0xc(%eax)
	p->pid = nextpid++;
80104066:	a1 04 b0 10 80       	mov    0x8010b004,%eax
8010406b:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010406e:	89 42 10             	mov    %eax,0x10(%edx)
80104071:	83 c0 01             	add    $0x1,%eax
80104074:	a3 04 b0 10 80       	mov    %eax,0x8010b004
	release(&ptable.lock);
80104079:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104080:	e8 30 10 00 00       	call   801050b5 <release>

	// Allocate kernel stack.
	if ((p->kstack = kalloc()) == 0) {
80104085:	e8 6d ea ff ff       	call   80102af7 <kalloc>
8010408a:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010408d:	89 42 08             	mov    %eax,0x8(%edx)
80104090:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104093:	8b 40 08             	mov    0x8(%eax),%eax
80104096:	85 c0                	test   %eax,%eax
80104098:	75 11                	jne    801040ab <allocproc+0x98>
		p->state = UNUSED;
8010409a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010409d:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
		return 0;
801040a4:	b8 00 00 00 00       	mov    $0x0,%eax
801040a9:	eb 65                	jmp    80104110 <allocproc+0xfd>
	}
	sp = p->kstack + KSTACKSIZE;
801040ab:	8b 45 f4             	mov    -0xc(%ebp),%eax
801040ae:	8b 40 08             	mov    0x8(%eax),%eax
801040b1:	05 00 10 00 00       	add    $0x1000,%eax
801040b6:	89 45 f0             	mov    %eax,-0x10(%ebp)

	// Leave room for trap frame.
	sp -= sizeof *p->tf;
801040b9:	83 6d f0 4c          	subl   $0x4c,-0x10(%ebp)
	p->tf = (struct trapframe*) sp;
801040bd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801040c0:	8b 55 f0             	mov    -0x10(%ebp),%edx
801040c3:	89 50 18             	mov    %edx,0x18(%eax)

	// Set up new context to start executing at forkret,
	// which returns to trapret.
	sp -= 4;
801040c6:	83 6d f0 04          	subl   $0x4,-0x10(%ebp)
	*(uint*) sp = (uint) trapret;
801040ca:	ba 44 67 10 80       	mov    $0x80106744,%edx
801040cf:	8b 45 f0             	mov    -0x10(%ebp),%eax
801040d2:	89 10                	mov    %edx,(%eax)

	sp -= sizeof *p->context;
801040d4:	83 6d f0 14          	subl   $0x14,-0x10(%ebp)
	p->context = (struct context*) sp;
801040d8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801040db:	8b 55 f0             	mov    -0x10(%ebp),%edx
801040de:	89 50 1c             	mov    %edx,0x1c(%eax)
	memset(p->context, 0, sizeof *p->context);
801040e1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801040e4:	8b 40 1c             	mov    0x1c(%eax),%eax
801040e7:	c7 44 24 08 14 00 00 	movl   $0x14,0x8(%esp)
801040ee:	00 
801040ef:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801040f6:	00 
801040f7:	89 04 24             	mov    %eax,(%esp)
801040fa:	e8 a3 11 00 00       	call   801052a2 <memset>
	p->context->eip = (uint) forkret;
801040ff:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104102:	8b 40 1c             	mov    0x1c(%eax),%eax
80104105:	ba e6 47 10 80       	mov    $0x801047e6,%edx
8010410a:	89 50 10             	mov    %edx,0x10(%eax)

	return p;
8010410d:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80104110:	c9                   	leave  
80104111:	c3                   	ret    

80104112 <userinit>:

//PAGEBREAK: 32
// Set up first user process.
void userinit(void) {
80104112:	55                   	push   %ebp
80104113:	89 e5                	mov    %esp,%ebp
80104115:	83 ec 28             	sub    $0x28,%esp
	struct proc *p;
	extern char _binary_initcode_start[], _binary_initcode_size[];

	p = allocproc();
80104118:	e8 f6 fe ff ff       	call   80104013 <allocproc>
8010411d:	89 45 f4             	mov    %eax,-0xc(%ebp)
	initproc = p;
80104120:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104123:	a3 48 b6 10 80       	mov    %eax,0x8010b648
	if ((p->pgdir = setupkvm()) == 0)
80104128:	e8 14 3d 00 00       	call   80107e41 <setupkvm>
8010412d:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104130:	89 42 04             	mov    %eax,0x4(%edx)
80104133:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104136:	8b 40 04             	mov    0x4(%eax),%eax
80104139:	85 c0                	test   %eax,%eax
8010413b:	75 0c                	jne    80104149 <userinit+0x37>
		panic("userinit: out of memory?");
8010413d:	c7 04 24 7b 89 10 80 	movl   $0x8010897b,(%esp)
80104144:	e8 f4 c3 ff ff       	call   8010053d <panic>
	inituvm(p->pgdir, _binary_initcode_start, (int) _binary_initcode_size);
80104149:	ba 2c 00 00 00       	mov    $0x2c,%edx
8010414e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104151:	8b 40 04             	mov    0x4(%eax),%eax
80104154:	89 54 24 08          	mov    %edx,0x8(%esp)
80104158:	c7 44 24 04 e0 b4 10 	movl   $0x8010b4e0,0x4(%esp)
8010415f:	80 
80104160:	89 04 24             	mov    %eax,(%esp)
80104163:	e8 31 3f 00 00       	call   80108099 <inituvm>
	p->sz = PGSIZE;
80104168:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010416b:	c7 00 00 10 00 00    	movl   $0x1000,(%eax)
	memset(p->tf, 0, sizeof(*p->tf));
80104171:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104174:	8b 40 18             	mov    0x18(%eax),%eax
80104177:	c7 44 24 08 4c 00 00 	movl   $0x4c,0x8(%esp)
8010417e:	00 
8010417f:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80104186:	00 
80104187:	89 04 24             	mov    %eax,(%esp)
8010418a:	e8 13 11 00 00       	call   801052a2 <memset>
	p->tf->cs = (SEG_UCODE << 3) | DPL_USER;
8010418f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104192:	8b 40 18             	mov    0x18(%eax),%eax
80104195:	66 c7 40 3c 23 00    	movw   $0x23,0x3c(%eax)
	p->tf->ds = (SEG_UDATA << 3) | DPL_USER;
8010419b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010419e:	8b 40 18             	mov    0x18(%eax),%eax
801041a1:	66 c7 40 2c 2b 00    	movw   $0x2b,0x2c(%eax)
	p->tf->es = p->tf->ds;
801041a7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801041aa:	8b 40 18             	mov    0x18(%eax),%eax
801041ad:	8b 55 f4             	mov    -0xc(%ebp),%edx
801041b0:	8b 52 18             	mov    0x18(%edx),%edx
801041b3:	0f b7 52 2c          	movzwl 0x2c(%edx),%edx
801041b7:	66 89 50 28          	mov    %dx,0x28(%eax)
	p->tf->ss = p->tf->ds;
801041bb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801041be:	8b 40 18             	mov    0x18(%eax),%eax
801041c1:	8b 55 f4             	mov    -0xc(%ebp),%edx
801041c4:	8b 52 18             	mov    0x18(%edx),%edx
801041c7:	0f b7 52 2c          	movzwl 0x2c(%edx),%edx
801041cb:	66 89 50 48          	mov    %dx,0x48(%eax)
	p->tf->eflags = FL_IF;
801041cf:	8b 45 f4             	mov    -0xc(%ebp),%eax
801041d2:	8b 40 18             	mov    0x18(%eax),%eax
801041d5:	c7 40 40 00 02 00 00 	movl   $0x200,0x40(%eax)
	p->tf->esp = PGSIZE;
801041dc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801041df:	8b 40 18             	mov    0x18(%eax),%eax
801041e2:	c7 40 44 00 10 00 00 	movl   $0x1000,0x44(%eax)
	p->tf->eip = 0; // beginning of initcode.S
801041e9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801041ec:	8b 40 18             	mov    0x18(%eax),%eax
801041ef:	c7 40 38 00 00 00 00 	movl   $0x0,0x38(%eax)

	safestrcpy(p->name, "initcode", sizeof(p->name));
801041f6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801041f9:	83 c0 6c             	add    $0x6c,%eax
801041fc:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80104203:	00 
80104204:	c7 44 24 04 94 89 10 	movl   $0x80108994,0x4(%esp)
8010420b:	80 
8010420c:	89 04 24             	mov    %eax,(%esp)
8010420f:	e8 be 12 00 00       	call   801054d2 <safestrcpy>
	p->cwd = namei("/");
80104214:	c7 04 24 9d 89 10 80 	movl   $0x8010899d,(%esp)
8010421b:	e8 e2 e1 ff ff       	call   80102402 <namei>
80104220:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104223:	89 42 68             	mov    %eax,0x68(%edx)

	SetProcessRunnable(p);
80104226:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104229:	89 04 24             	mov    %eax,(%esp)
8010422c:	e8 55 0a 00 00       	call   80104c86 <SetProcessRunnable>
}
80104231:	c9                   	leave  
80104232:	c3                   	ret    

80104233 <growproc>:

// Grow current process's memory by n bytes.
// Return 0 on success, -1 on failure.
int growproc(int n) {
80104233:	55                   	push   %ebp
80104234:	89 e5                	mov    %esp,%ebp
80104236:	83 ec 28             	sub    $0x28,%esp
	uint sz;

	sz = proc->sz;
80104239:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010423f:	8b 00                	mov    (%eax),%eax
80104241:	89 45 f4             	mov    %eax,-0xc(%ebp)
	if (n > 0) {
80104244:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80104248:	7e 34                	jle    8010427e <growproc+0x4b>
		if ((sz = allocuvm(proc->pgdir, sz, sz + n)) == 0)
8010424a:	8b 45 08             	mov    0x8(%ebp),%eax
8010424d:	89 c2                	mov    %eax,%edx
8010424f:	03 55 f4             	add    -0xc(%ebp),%edx
80104252:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104258:	8b 40 04             	mov    0x4(%eax),%eax
8010425b:	89 54 24 08          	mov    %edx,0x8(%esp)
8010425f:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104262:	89 54 24 04          	mov    %edx,0x4(%esp)
80104266:	89 04 24             	mov    %eax,(%esp)
80104269:	e8 a5 3f 00 00       	call   80108213 <allocuvm>
8010426e:	89 45 f4             	mov    %eax,-0xc(%ebp)
80104271:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80104275:	75 41                	jne    801042b8 <growproc+0x85>
			return -1;
80104277:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010427c:	eb 58                	jmp    801042d6 <growproc+0xa3>
	} else if (n < 0) {
8010427e:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80104282:	79 34                	jns    801042b8 <growproc+0x85>
		if ((sz = deallocuvm(proc->pgdir, sz, sz + n)) == 0)
80104284:	8b 45 08             	mov    0x8(%ebp),%eax
80104287:	89 c2                	mov    %eax,%edx
80104289:	03 55 f4             	add    -0xc(%ebp),%edx
8010428c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104292:	8b 40 04             	mov    0x4(%eax),%eax
80104295:	89 54 24 08          	mov    %edx,0x8(%esp)
80104299:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010429c:	89 54 24 04          	mov    %edx,0x4(%esp)
801042a0:	89 04 24             	mov    %eax,(%esp)
801042a3:	e8 45 40 00 00       	call   801082ed <deallocuvm>
801042a8:	89 45 f4             	mov    %eax,-0xc(%ebp)
801042ab:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801042af:	75 07                	jne    801042b8 <growproc+0x85>
			return -1;
801042b1:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801042b6:	eb 1e                	jmp    801042d6 <growproc+0xa3>
	}
	proc->sz = sz;
801042b8:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801042be:	8b 55 f4             	mov    -0xc(%ebp),%edx
801042c1:	89 10                	mov    %edx,(%eax)
	switchuvm(proc);
801042c3:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801042c9:	89 04 24             	mov    %eax,(%esp)
801042cc:	e8 61 3c 00 00       	call   80107f32 <switchuvm>
	return 0;
801042d1:	b8 00 00 00 00       	mov    $0x0,%eax
}
801042d6:	c9                   	leave  
801042d7:	c3                   	ret    

801042d8 <fork>:

// Create a new process copying p as the parent.
// Sets up stack to return as if from system call.
// Caller must set state of returned proc to RUNNABLE.
int fork(void) {
801042d8:	55                   	push   %ebp
801042d9:	89 e5                	mov    %esp,%ebp
801042db:	57                   	push   %edi
801042dc:	56                   	push   %esi
801042dd:	53                   	push   %ebx
801042de:	83 ec 2c             	sub    $0x2c,%esp

	int i, pid;
	struct proc *np;

	// Allocate process.
	if ((np = allocproc()) == 0)
801042e1:	e8 2d fd ff ff       	call   80104013 <allocproc>
801042e6:	89 45 e0             	mov    %eax,-0x20(%ebp)
801042e9:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
801042ed:	75 0a                	jne    801042f9 <fork+0x21>
		return -1;
801042ef:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801042f4:	e9 3b 01 00 00       	jmp    80104434 <fork+0x15c>

	// Copy process state from p.
	if ((np->pgdir = copyuvm(proc->pgdir, proc->sz)) == 0) {
801042f9:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801042ff:	8b 10                	mov    (%eax),%edx
80104301:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104307:	8b 40 04             	mov    0x4(%eax),%eax
8010430a:	89 54 24 04          	mov    %edx,0x4(%esp)
8010430e:	89 04 24             	mov    %eax,(%esp)
80104311:	e8 67 41 00 00       	call   8010847d <copyuvm>
80104316:	8b 55 e0             	mov    -0x20(%ebp),%edx
80104319:	89 42 04             	mov    %eax,0x4(%edx)
8010431c:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010431f:	8b 40 04             	mov    0x4(%eax),%eax
80104322:	85 c0                	test   %eax,%eax
80104324:	75 2c                	jne    80104352 <fork+0x7a>
		kfree(np->kstack);
80104326:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104329:	8b 40 08             	mov    0x8(%eax),%eax
8010432c:	89 04 24             	mov    %eax,(%esp)
8010432f:	e8 2a e7 ff ff       	call   80102a5e <kfree>
		np->kstack = 0;
80104334:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104337:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
		np->state = UNUSED;
8010433e:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104341:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
		return -1;
80104348:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010434d:	e9 e2 00 00 00       	jmp    80104434 <fork+0x15c>
	}
	np->sz = proc->sz;
80104352:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104358:	8b 10                	mov    (%eax),%edx
8010435a:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010435d:	89 10                	mov    %edx,(%eax)
	np->parent = proc;
8010435f:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80104366:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104369:	89 50 14             	mov    %edx,0x14(%eax)
	*np->tf = *proc->tf;
8010436c:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010436f:	8b 50 18             	mov    0x18(%eax),%edx
80104372:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104378:	8b 40 18             	mov    0x18(%eax),%eax
8010437b:	89 c3                	mov    %eax,%ebx
8010437d:	b8 13 00 00 00       	mov    $0x13,%eax
80104382:	89 d7                	mov    %edx,%edi
80104384:	89 de                	mov    %ebx,%esi
80104386:	89 c1                	mov    %eax,%ecx
80104388:	f3 a5                	rep movsl %ds:(%esi),%es:(%edi)

	// Clear %eax so that fork returns 0 in the child.
	np->tf->eax = 0;
8010438a:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010438d:	8b 40 18             	mov    0x18(%eax),%eax
80104390:	c7 40 1c 00 00 00 00 	movl   $0x0,0x1c(%eax)

	for (i = 0; i < NOFILE; i++)
80104397:	c7 45 e4 00 00 00 00 	movl   $0x0,-0x1c(%ebp)
8010439e:	eb 3d                	jmp    801043dd <fork+0x105>
		if (proc->ofile[i])
801043a0:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801043a6:	8b 55 e4             	mov    -0x1c(%ebp),%edx
801043a9:	83 c2 08             	add    $0x8,%edx
801043ac:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
801043b0:	85 c0                	test   %eax,%eax
801043b2:	74 25                	je     801043d9 <fork+0x101>
			np->ofile[i] = filedup(proc->ofile[i]);
801043b4:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801043ba:	8b 55 e4             	mov    -0x1c(%ebp),%edx
801043bd:	83 c2 08             	add    $0x8,%edx
801043c0:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
801043c4:	89 04 24             	mov    %eax,(%esp)
801043c7:	e8 a8 cb ff ff       	call   80100f74 <filedup>
801043cc:	8b 55 e0             	mov    -0x20(%ebp),%edx
801043cf:	8b 4d e4             	mov    -0x1c(%ebp),%ecx
801043d2:	83 c1 08             	add    $0x8,%ecx
801043d5:	89 44 8a 08          	mov    %eax,0x8(%edx,%ecx,4)
	*np->tf = *proc->tf;

	// Clear %eax so that fork returns 0 in the child.
	np->tf->eax = 0;

	for (i = 0; i < NOFILE; i++)
801043d9:	83 45 e4 01          	addl   $0x1,-0x1c(%ebp)
801043dd:	83 7d e4 0f          	cmpl   $0xf,-0x1c(%ebp)
801043e1:	7e bd                	jle    801043a0 <fork+0xc8>
		if (proc->ofile[i])
			np->ofile[i] = filedup(proc->ofile[i]);
	np->cwd = idup(proc->cwd);
801043e3:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801043e9:	8b 40 68             	mov    0x68(%eax),%eax
801043ec:	89 04 24             	mov    %eax,(%esp)
801043ef:	e8 3a d4 ff ff       	call   8010182e <idup>
801043f4:	8b 55 e0             	mov    -0x20(%ebp),%edx
801043f7:	89 42 68             	mov    %eax,0x68(%edx)

	pid = np->pid;
801043fa:	8b 45 e0             	mov    -0x20(%ebp),%eax
801043fd:	8b 40 10             	mov    0x10(%eax),%eax
80104400:	89 45 dc             	mov    %eax,-0x24(%ebp)
	SetProcessRunnable(np);
80104403:	8b 45 e0             	mov    -0x20(%ebp),%eax
80104406:	89 04 24             	mov    %eax,(%esp)
80104409:	e8 78 08 00 00       	call   80104c86 <SetProcessRunnable>
	safestrcpy(np->name, proc->name, sizeof(proc->name));
8010440e:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104414:	8d 50 6c             	lea    0x6c(%eax),%edx
80104417:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010441a:	83 c0 6c             	add    $0x6c,%eax
8010441d:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80104424:	00 
80104425:	89 54 24 04          	mov    %edx,0x4(%esp)
80104429:	89 04 24             	mov    %eax,(%esp)
8010442c:	e8 a1 10 00 00       	call   801054d2 <safestrcpy>
	return pid;
80104431:	8b 45 dc             	mov    -0x24(%ebp),%eax
}
80104434:	83 c4 2c             	add    $0x2c,%esp
80104437:	5b                   	pop    %ebx
80104438:	5e                   	pop    %esi
80104439:	5f                   	pop    %edi
8010443a:	5d                   	pop    %ebp
8010443b:	c3                   	ret    

8010443c <exit>:

// Exit the current process.  Does not return.
// An exited process remains in the zombie state
// until its parent calls wait() to find out it exited.
void exit(void) {
8010443c:	55                   	push   %ebp
8010443d:	89 e5                	mov    %esp,%ebp
8010443f:	83 ec 28             	sub    $0x28,%esp
	struct proc *p;
	int fd;

	if (proc == initproc)
80104442:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80104449:	a1 48 b6 10 80       	mov    0x8010b648,%eax
8010444e:	39 c2                	cmp    %eax,%edx
80104450:	75 0c                	jne    8010445e <exit+0x22>
		panic("init exiting");
80104452:	c7 04 24 9f 89 10 80 	movl   $0x8010899f,(%esp)
80104459:	e8 df c0 ff ff       	call   8010053d <panic>

	// Close all open files.
	for (fd = 0; fd < NOFILE; fd++) {
8010445e:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
80104465:	eb 44                	jmp    801044ab <exit+0x6f>
		if (proc->ofile[fd]) {
80104467:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010446d:	8b 55 f0             	mov    -0x10(%ebp),%edx
80104470:	83 c2 08             	add    $0x8,%edx
80104473:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
80104477:	85 c0                	test   %eax,%eax
80104479:	74 2c                	je     801044a7 <exit+0x6b>
			fileclose(proc->ofile[fd]);
8010447b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104481:	8b 55 f0             	mov    -0x10(%ebp),%edx
80104484:	83 c2 08             	add    $0x8,%edx
80104487:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
8010448b:	89 04 24             	mov    %eax,(%esp)
8010448e:	e8 29 cb ff ff       	call   80100fbc <fileclose>
			proc->ofile[fd] = 0;
80104493:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104499:	8b 55 f0             	mov    -0x10(%ebp),%edx
8010449c:	83 c2 08             	add    $0x8,%edx
8010449f:	c7 44 90 08 00 00 00 	movl   $0x0,0x8(%eax,%edx,4)
801044a6:	00 

	if (proc == initproc)
		panic("init exiting");

	// Close all open files.
	for (fd = 0; fd < NOFILE; fd++) {
801044a7:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
801044ab:	83 7d f0 0f          	cmpl   $0xf,-0x10(%ebp)
801044af:	7e b6                	jle    80104467 <exit+0x2b>
			fileclose(proc->ofile[fd]);
			proc->ofile[fd] = 0;
		}
	}

	iput(proc->cwd);
801044b1:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801044b7:	8b 40 68             	mov    0x68(%eax),%eax
801044ba:	89 04 24             	mov    %eax,(%esp)
801044bd:	e8 51 d5 ff ff       	call   80101a13 <iput>
	proc->cwd = 0;
801044c2:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801044c8:	c7 40 68 00 00 00 00 	movl   $0x0,0x68(%eax)

	acquire(&ptable.lock);
801044cf:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
801044d6:	e8 78 0b 00 00       	call   80105053 <acquire>

	// Parent might be sleeping in wait().
	wakeup1(proc->parent);
801044db:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801044e1:	8b 40 14             	mov    0x14(%eax),%eax
801044e4:	89 04 24             	mov    %eax,(%esp)
801044e7:	e8 c1 03 00 00       	call   801048ad <wakeup1>

	// Pass abandoned children to init.
	for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
801044ec:	c7 45 f4 54 ff 10 80 	movl   $0x8010ff54,-0xc(%ebp)
801044f3:	eb 38                	jmp    8010452d <exit+0xf1>
		if (p->parent == proc) {
801044f5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801044f8:	8b 50 14             	mov    0x14(%eax),%edx
801044fb:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104501:	39 c2                	cmp    %eax,%edx
80104503:	75 24                	jne    80104529 <exit+0xed>
			p->parent = initproc;
80104505:	8b 15 48 b6 10 80    	mov    0x8010b648,%edx
8010450b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010450e:	89 50 14             	mov    %edx,0x14(%eax)
			if (p->state == ZOMBIE)
80104511:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104514:	8b 40 0c             	mov    0xc(%eax),%eax
80104517:	83 f8 05             	cmp    $0x5,%eax
8010451a:	75 0d                	jne    80104529 <exit+0xed>
				wakeup1(initproc);
8010451c:	a1 48 b6 10 80       	mov    0x8010b648,%eax
80104521:	89 04 24             	mov    %eax,(%esp)
80104524:	e8 84 03 00 00       	call   801048ad <wakeup1>

	// Parent might be sleeping in wait().
	wakeup1(proc->parent);

	// Pass abandoned children to init.
	for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
80104529:	83 6d f4 80          	subl   $0xffffff80,-0xc(%ebp)
8010452d:	81 7d f4 54 1f 11 80 	cmpl   $0x80111f54,-0xc(%ebp)
80104534:	72 bf                	jb     801044f5 <exit+0xb9>
				wakeup1(initproc);
		}
	}

	// Jump into the scheduler, never to return.
	proc->state = ZOMBIE;
80104536:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010453c:	c7 40 0c 05 00 00 00 	movl   $0x5,0xc(%eax)
	sched();
80104543:	e8 b9 01 00 00       	call   80104701 <sched>
	panic("zombie exit");
80104548:	c7 04 24 ac 89 10 80 	movl   $0x801089ac,(%esp)
8010454f:	e8 e9 bf ff ff       	call   8010053d <panic>

80104554 <wait>:
}

// Wait for a child process to exit and return its pid.
// Return -1 if this process has no children.
int wait(void) {
80104554:	55                   	push   %ebp
80104555:	89 e5                	mov    %esp,%ebp
80104557:	83 ec 28             	sub    $0x28,%esp
	struct proc *p;
	int havekids, pid;

	acquire(&ptable.lock);
8010455a:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104561:	e8 ed 0a 00 00       	call   80105053 <acquire>
	for (;;) {
		// Scan through table looking for zombie children.
		havekids = 0;
80104566:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
		for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
8010456d:	c7 45 f4 54 ff 10 80 	movl   $0x8010ff54,-0xc(%ebp)
80104574:	e9 9a 00 00 00       	jmp    80104613 <wait+0xbf>
			if (p->parent != proc)
80104579:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010457c:	8b 50 14             	mov    0x14(%eax),%edx
8010457f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104585:	39 c2                	cmp    %eax,%edx
80104587:	0f 85 81 00 00 00    	jne    8010460e <wait+0xba>
				continue;
			havekids = 1;
8010458d:	c7 45 f0 01 00 00 00 	movl   $0x1,-0x10(%ebp)
			if (p->state == ZOMBIE) {
80104594:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104597:	8b 40 0c             	mov    0xc(%eax),%eax
8010459a:	83 f8 05             	cmp    $0x5,%eax
8010459d:	75 70                	jne    8010460f <wait+0xbb>
				// Found one.
				pid = p->pid;
8010459f:	8b 45 f4             	mov    -0xc(%ebp),%eax
801045a2:	8b 40 10             	mov    0x10(%eax),%eax
801045a5:	89 45 ec             	mov    %eax,-0x14(%ebp)
				kfree(p->kstack);
801045a8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801045ab:	8b 40 08             	mov    0x8(%eax),%eax
801045ae:	89 04 24             	mov    %eax,(%esp)
801045b1:	e8 a8 e4 ff ff       	call   80102a5e <kfree>
				p->kstack = 0;
801045b6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801045b9:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
				freevm(p->pgdir);
801045c0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801045c3:	8b 40 04             	mov    0x4(%eax),%eax
801045c6:	89 04 24             	mov    %eax,(%esp)
801045c9:	e8 db 3d 00 00       	call   801083a9 <freevm>
				p->state = UNUSED;
801045ce:	8b 45 f4             	mov    -0xc(%ebp),%eax
801045d1:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
				p->pid = 0;
801045d8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801045db:	c7 40 10 00 00 00 00 	movl   $0x0,0x10(%eax)
				p->parent = 0;
801045e2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801045e5:	c7 40 14 00 00 00 00 	movl   $0x0,0x14(%eax)
				p->name[0] = 0;
801045ec:	8b 45 f4             	mov    -0xc(%ebp),%eax
801045ef:	c6 40 6c 00          	movb   $0x0,0x6c(%eax)
				p->killed = 0;
801045f3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801045f6:	c7 40 24 00 00 00 00 	movl   $0x0,0x24(%eax)
				release(&ptable.lock);
801045fd:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104604:	e8 ac 0a 00 00       	call   801050b5 <release>
				return pid;
80104609:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010460c:	eb 53                	jmp    80104661 <wait+0x10d>
	for (;;) {
		// Scan through table looking for zombie children.
		havekids = 0;
		for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
			if (p->parent != proc)
				continue;
8010460e:	90                   	nop

	acquire(&ptable.lock);
	for (;;) {
		// Scan through table looking for zombie children.
		havekids = 0;
		for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
8010460f:	83 6d f4 80          	subl   $0xffffff80,-0xc(%ebp)
80104613:	81 7d f4 54 1f 11 80 	cmpl   $0x80111f54,-0xc(%ebp)
8010461a:	0f 82 59 ff ff ff    	jb     80104579 <wait+0x25>
				return pid;
			}
		}

		// No point waiting if we don't have any children.
		if (!havekids || proc->killed) {
80104620:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80104624:	74 0d                	je     80104633 <wait+0xdf>
80104626:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010462c:	8b 40 24             	mov    0x24(%eax),%eax
8010462f:	85 c0                	test   %eax,%eax
80104631:	74 13                	je     80104646 <wait+0xf2>
			release(&ptable.lock);
80104633:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
8010463a:	e8 76 0a 00 00       	call   801050b5 <release>
			return -1;
8010463f:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80104644:	eb 1b                	jmp    80104661 <wait+0x10d>
		}

		// Wait for children to exit.  (See wakeup1 call in proc_exit.)
		sleep(proc, &ptable.lock); //DOC: wait-sleep
80104646:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010464c:	c7 44 24 04 20 ff 10 	movl   $0x8010ff20,0x4(%esp)
80104653:	80 
80104654:	89 04 24             	mov    %eax,(%esp)
80104657:	e8 b6 01 00 00       	call   80104812 <sleep>
	}
8010465c:	e9 05 ff ff ff       	jmp    80104566 <wait+0x12>
}
80104661:	c9                   	leave  
80104662:	c3                   	ret    

80104663 <scheduler>:
// Scheduler never returns.  It loops, doing:
//  - choose a process to run
//  - swtch to start running that process
//  - eventually that process transfers control
//      via swtch back to the scheduler.
void scheduler(void) {
80104663:	55                   	push   %ebp
80104664:	89 e5                	mov    %esp,%ebp
80104666:	53                   	push   %ebx
80104667:	83 ec 24             	sub    $0x24,%esp
	struct proc *p;

	int priority = 0;
8010466a:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
	//TODO: find the correct place for this


	for (;;) {
		// Enable interrupts on this processor.
		sti();
80104671:	e8 7b f9 ff ff       	call   80103ff1 <sti>
		acquire(&ptable.lock);
80104676:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
8010467d:	e8 d1 09 00 00       	call   80105053 <acquire>
		
		p = GetNextRunnableProcess(priority);
80104682:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104685:	89 04 24             	mov    %eax,(%esp)
80104688:	e8 4a 06 00 00       	call   80104cd7 <GetNextRunnableProcess>
8010468d:	89 45 f0             	mov    %eax,-0x10(%ebp)
		if( p != 0)
80104690:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80104694:	74 28                	je     801046be <scheduler+0x5b>
		{

			SwitchToProccess(p);
80104696:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104699:	89 04 24             	mov    %eax,(%esp)
8010469c:	e8 12 05 00 00       	call   80104bb3 <SwitchToProccess>
			ps();
801046a1:	e8 47 04 00 00       	call   80104aed <ps>
			if(IsThereANoneEmptyHigherLevelPriorityTable(priority))
801046a6:	8b 45 f4             	mov    -0xc(%ebp),%eax
801046a9:	89 04 24             	mov    %eax,(%esp)
801046ac:	e8 48 06 00 00       	call   80104cf9 <IsThereANoneEmptyHigherLevelPriorityTable>
801046b1:	85 c0                	test   %eax,%eax
801046b3:	74 3b                	je     801046f0 <scheduler+0x8d>
				priority= 0; // Start from the highest priority
801046b5:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801046bc:	eb 32                	jmp    801046f0 <scheduler+0x8d>
		}
		else// the current priority Table is empty
			priority = (priority+1)%PRIORITYLEVELS;
801046be:	8b 45 f4             	mov    -0xc(%ebp),%eax
801046c1:	8d 48 01             	lea    0x1(%eax),%ecx
801046c4:	ba 67 66 66 66       	mov    $0x66666667,%edx
801046c9:	89 c8                	mov    %ecx,%eax
801046cb:	f7 ea                	imul   %edx
801046cd:	d1 fa                	sar    %edx
801046cf:	89 c8                	mov    %ecx,%eax
801046d1:	c1 f8 1f             	sar    $0x1f,%eax
801046d4:	89 d3                	mov    %edx,%ebx
801046d6:	29 c3                	sub    %eax,%ebx
801046d8:	89 d8                	mov    %ebx,%eax
801046da:	89 45 f4             	mov    %eax,-0xc(%ebp)
801046dd:	8b 55 f4             	mov    -0xc(%ebp),%edx
801046e0:	89 d0                	mov    %edx,%eax
801046e2:	c1 e0 02             	shl    $0x2,%eax
801046e5:	01 d0                	add    %edx,%eax
801046e7:	89 ca                	mov    %ecx,%edx
801046e9:	29 c2                	sub    %eax,%edx
801046eb:	89 d0                	mov    %edx,%eax
801046ed:	89 45 f4             	mov    %eax,-0xc(%ebp)
		release(&ptable.lock);
801046f0:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
801046f7:	e8 b9 09 00 00       	call   801050b5 <release>
	}
801046fc:	e9 70 ff ff ff       	jmp    80104671 <scheduler+0xe>

80104701 <sched>:
}


// Enter scheduler.  Must hold only ptable.lock
// and have changed proc->state.
void sched(void) {
80104701:	55                   	push   %ebp
80104702:	89 e5                	mov    %esp,%ebp
80104704:	83 ec 28             	sub    $0x28,%esp
	int intena;

	if (!holding(&ptable.lock))
80104707:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
8010470e:	e8 5e 0a 00 00       	call   80105171 <holding>
80104713:	85 c0                	test   %eax,%eax
80104715:	75 0c                	jne    80104723 <sched+0x22>
		panic("sched ptable.lock");
80104717:	c7 04 24 b8 89 10 80 	movl   $0x801089b8,(%esp)
8010471e:	e8 1a be ff ff       	call   8010053d <panic>
	if (cpu->ncli != 1)
80104723:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104729:	8b 80 ac 00 00 00    	mov    0xac(%eax),%eax
8010472f:	83 f8 01             	cmp    $0x1,%eax
80104732:	74 0c                	je     80104740 <sched+0x3f>
		panic("sched locks");
80104734:	c7 04 24 ca 89 10 80 	movl   $0x801089ca,(%esp)
8010473b:	e8 fd bd ff ff       	call   8010053d <panic>
	if (proc->state == RUNNING)
80104740:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104746:	8b 40 0c             	mov    0xc(%eax),%eax
80104749:	83 f8 04             	cmp    $0x4,%eax
8010474c:	75 0c                	jne    8010475a <sched+0x59>
		panic("sched running");
8010474e:	c7 04 24 d6 89 10 80 	movl   $0x801089d6,(%esp)
80104755:	e8 e3 bd ff ff       	call   8010053d <panic>
	if (readeflags() & FL_IF)
8010475a:	e8 7d f8 ff ff       	call   80103fdc <readeflags>
8010475f:	25 00 02 00 00       	and    $0x200,%eax
80104764:	85 c0                	test   %eax,%eax
80104766:	74 0c                	je     80104774 <sched+0x73>
		panic("sched interruptible");
80104768:	c7 04 24 e4 89 10 80 	movl   $0x801089e4,(%esp)
8010476f:	e8 c9 bd ff ff       	call   8010053d <panic>
	intena = cpu->intena;
80104774:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
8010477a:	8b 80 b0 00 00 00    	mov    0xb0(%eax),%eax
80104780:	89 45 f4             	mov    %eax,-0xc(%ebp)
	swtch(&proc->context, cpu->scheduler);
80104783:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80104789:	8b 40 04             	mov    0x4(%eax),%eax
8010478c:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80104793:	83 c2 1c             	add    $0x1c,%edx
80104796:	89 44 24 04          	mov    %eax,0x4(%esp)
8010479a:	89 14 24             	mov    %edx,(%esp)
8010479d:	e8 a6 0d 00 00       	call   80105548 <swtch>
	cpu->intena = intena;
801047a2:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801047a8:	8b 55 f4             	mov    -0xc(%ebp),%edx
801047ab:	89 90 b0 00 00 00    	mov    %edx,0xb0(%eax)
}
801047b1:	c9                   	leave  
801047b2:	c3                   	ret    

801047b3 <yield>:

// Give up the CPU for one scheduling round.
void yield(void) {
801047b3:	55                   	push   %ebp
801047b4:	89 e5                	mov    %esp,%ebp
801047b6:	83 ec 18             	sub    $0x18,%esp
	acquire(&ptable.lock); //DOC: yieldlock
801047b9:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
801047c0:	e8 8e 08 00 00       	call   80105053 <acquire>
	SetProcessRunnable(proc);
801047c5:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801047cb:	89 04 24             	mov    %eax,(%esp)
801047ce:	e8 b3 04 00 00       	call   80104c86 <SetProcessRunnable>
	sched();
801047d3:	e8 29 ff ff ff       	call   80104701 <sched>
	release(&ptable.lock);
801047d8:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
801047df:	e8 d1 08 00 00       	call   801050b5 <release>
}
801047e4:	c9                   	leave  
801047e5:	c3                   	ret    

801047e6 <forkret>:

// A fork child's very first scheduling by scheduler()
// will swtch here.  "Return" to user space.
void forkret(void) {
801047e6:	55                   	push   %ebp
801047e7:	89 e5                	mov    %esp,%ebp
801047e9:	83 ec 18             	sub    $0x18,%esp
	static int first = 1;
	// Still holding ptable.lock from scheduler.
	release(&ptable.lock);
801047ec:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
801047f3:	e8 bd 08 00 00       	call   801050b5 <release>

	if (first) {
801047f8:	a1 20 b0 10 80       	mov    0x8010b020,%eax
801047fd:	85 c0                	test   %eax,%eax
801047ff:	74 0f                	je     80104810 <forkret+0x2a>
		// Some initialization functions must be run in the context
		// of a regular process (e.g., they call sleep), and thus cannot
		// be run from main().
		first = 0;
80104801:	c7 05 20 b0 10 80 00 	movl   $0x0,0x8010b020
80104808:	00 00 00 
		initlog();
8010480b:	e8 f8 e7 ff ff       	call   80103008 <initlog>
	}

	// Return to "caller", actually trapret (see allocproc).
}
80104810:	c9                   	leave  
80104811:	c3                   	ret    

80104812 <sleep>:

// Atomically release lock and sleep on chan.
// Reacquires lock when awakened.
void sleep(void *chan, struct spinlock *lk) {
80104812:	55                   	push   %ebp
80104813:	89 e5                	mov    %esp,%ebp
80104815:	83 ec 18             	sub    $0x18,%esp
	if (proc == 0)
80104818:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010481e:	85 c0                	test   %eax,%eax
80104820:	75 0c                	jne    8010482e <sleep+0x1c>
		panic("sleep");
80104822:	c7 04 24 f8 89 10 80 	movl   $0x801089f8,(%esp)
80104829:	e8 0f bd ff ff       	call   8010053d <panic>

	if (lk == 0)
8010482e:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
80104832:	75 0c                	jne    80104840 <sleep+0x2e>
		panic("sleep without lk");
80104834:	c7 04 24 fe 89 10 80 	movl   $0x801089fe,(%esp)
8010483b:	e8 fd bc ff ff       	call   8010053d <panic>
	// change p->state and then call sched.
	// Once we hold ptable.lock, we can be
	// guaranteed that we won't miss any wakeup
	// (wakeup runs with ptable.lock locked),
	// so it's okay to release lk.
	if (lk != &ptable.lock) { //DOC: sleeplock0
80104840:	81 7d 0c 20 ff 10 80 	cmpl   $0x8010ff20,0xc(%ebp)
80104847:	74 17                	je     80104860 <sleep+0x4e>
		acquire(&ptable.lock); //DOC: sleeplock1
80104849:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104850:	e8 fe 07 00 00       	call   80105053 <acquire>
		release(lk);
80104855:	8b 45 0c             	mov    0xc(%ebp),%eax
80104858:	89 04 24             	mov    %eax,(%esp)
8010485b:	e8 55 08 00 00       	call   801050b5 <release>
	}

	// Go to sleep.
	proc->chan = chan;
80104860:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104866:	8b 55 08             	mov    0x8(%ebp),%edx
80104869:	89 50 20             	mov    %edx,0x20(%eax)
	proc->state = SLEEPING;
8010486c:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104872:	c7 40 0c 02 00 00 00 	movl   $0x2,0xc(%eax)
	sched();
80104879:	e8 83 fe ff ff       	call   80104701 <sched>

	// Tidy up.
	proc->chan = 0;
8010487e:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104884:	c7 40 20 00 00 00 00 	movl   $0x0,0x20(%eax)

	// Reacquire original lock.
	if (lk != &ptable.lock) { //DOC: sleeplock2
8010488b:	81 7d 0c 20 ff 10 80 	cmpl   $0x8010ff20,0xc(%ebp)
80104892:	74 17                	je     801048ab <sleep+0x99>
		release(&ptable.lock);
80104894:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
8010489b:	e8 15 08 00 00       	call   801050b5 <release>
		acquire(lk);
801048a0:	8b 45 0c             	mov    0xc(%ebp),%eax
801048a3:	89 04 24             	mov    %eax,(%esp)
801048a6:	e8 a8 07 00 00       	call   80105053 <acquire>
	}
}
801048ab:	c9                   	leave  
801048ac:	c3                   	ret    

801048ad <wakeup1>:

//PAGEBREAK!
// Wake up all processes sleeping on chan.
// The ptable lock must be held.
static void wakeup1(void *chan) {
801048ad:	55                   	push   %ebp
801048ae:	89 e5                	mov    %esp,%ebp
801048b0:	83 ec 28             	sub    $0x28,%esp
	struct proc *p;

	for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
801048b3:	c7 45 f4 54 ff 10 80 	movl   $0x8010ff54,-0xc(%ebp)
801048ba:	eb 25                	jmp    801048e1 <wakeup1+0x34>
		if (p->state == SLEEPING && p->chan == chan)
801048bc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801048bf:	8b 40 0c             	mov    0xc(%eax),%eax
801048c2:	83 f8 02             	cmp    $0x2,%eax
801048c5:	75 16                	jne    801048dd <wakeup1+0x30>
801048c7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801048ca:	8b 40 20             	mov    0x20(%eax),%eax
801048cd:	3b 45 08             	cmp    0x8(%ebp),%eax
801048d0:	75 0b                	jne    801048dd <wakeup1+0x30>
			SetProcessRunnable(p);
801048d2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801048d5:	89 04 24             	mov    %eax,(%esp)
801048d8:	e8 a9 03 00 00       	call   80104c86 <SetProcessRunnable>
// Wake up all processes sleeping on chan.
// The ptable lock must be held.
static void wakeup1(void *chan) {
	struct proc *p;

	for (p = ptable.proc; p < &ptable.proc[NPROC]; p++)
801048dd:	83 6d f4 80          	subl   $0xffffff80,-0xc(%ebp)
801048e1:	81 7d f4 54 1f 11 80 	cmpl   $0x80111f54,-0xc(%ebp)
801048e8:	72 d2                	jb     801048bc <wakeup1+0xf>
		if (p->state == SLEEPING && p->chan == chan)
			SetProcessRunnable(p);
}
801048ea:	c9                   	leave  
801048eb:	c3                   	ret    

801048ec <wakeup>:

// Wake up all processes sleeping on chan.
void wakeup(void *chan) {
801048ec:	55                   	push   %ebp
801048ed:	89 e5                	mov    %esp,%ebp
801048ef:	83 ec 18             	sub    $0x18,%esp
	acquire(&ptable.lock);
801048f2:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
801048f9:	e8 55 07 00 00       	call   80105053 <acquire>
	wakeup1(chan);
801048fe:	8b 45 08             	mov    0x8(%ebp),%eax
80104901:	89 04 24             	mov    %eax,(%esp)
80104904:	e8 a4 ff ff ff       	call   801048ad <wakeup1>
	release(&ptable.lock);
80104909:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104910:	e8 a0 07 00 00       	call   801050b5 <release>
}
80104915:	c9                   	leave  
80104916:	c3                   	ret    

80104917 <kill>:

// Kill the process with the given pid.
// Process won't exit until it returns
// to user space (see trap in trap.c).
int kill(int pid) {
80104917:	55                   	push   %ebp
80104918:	89 e5                	mov    %esp,%ebp
8010491a:	83 ec 28             	sub    $0x28,%esp
	struct proc *p;

	acquire(&ptable.lock);
8010491d:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104924:	e8 2a 07 00 00       	call   80105053 <acquire>
	for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
80104929:	c7 45 f4 54 ff 10 80 	movl   $0x8010ff54,-0xc(%ebp)
80104930:	eb 42                	jmp    80104974 <kill+0x5d>
		if (p->pid == pid) {
80104932:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104935:	8b 40 10             	mov    0x10(%eax),%eax
80104938:	3b 45 08             	cmp    0x8(%ebp),%eax
8010493b:	75 33                	jne    80104970 <kill+0x59>
			p->killed = 1;
8010493d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104940:	c7 40 24 01 00 00 00 	movl   $0x1,0x24(%eax)
			// Wake process from sleep if necessary.
			if (p->state == SLEEPING)
80104947:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010494a:	8b 40 0c             	mov    0xc(%eax),%eax
8010494d:	83 f8 02             	cmp    $0x2,%eax
80104950:	75 0b                	jne    8010495d <kill+0x46>
				SetProcessRunnable(p);
80104952:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104955:	89 04 24             	mov    %eax,(%esp)
80104958:	e8 29 03 00 00       	call   80104c86 <SetProcessRunnable>
			release(&ptable.lock);
8010495d:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104964:	e8 4c 07 00 00       	call   801050b5 <release>
			return 0;
80104969:	b8 00 00 00 00       	mov    $0x0,%eax
8010496e:	eb 1e                	jmp    8010498e <kill+0x77>
// to user space (see trap in trap.c).
int kill(int pid) {
	struct proc *p;

	acquire(&ptable.lock);
	for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
80104970:	83 6d f4 80          	subl   $0xffffff80,-0xc(%ebp)
80104974:	81 7d f4 54 1f 11 80 	cmpl   $0x80111f54,-0xc(%ebp)
8010497b:	72 b5                	jb     80104932 <kill+0x1b>
				SetProcessRunnable(p);
			release(&ptable.lock);
			return 0;
		}
	}
	release(&ptable.lock);
8010497d:	c7 04 24 20 ff 10 80 	movl   $0x8010ff20,(%esp)
80104984:	e8 2c 07 00 00       	call   801050b5 <release>
	return -1;
80104989:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
8010498e:	c9                   	leave  
8010498f:	c3                   	ret    

80104990 <procdump>:

//PAGEBREAK: 36
// Print a process listing to console.  For debugging.
// Runs when user types ^P on console.
// No lock to avoid wedging a stuck machine further.
void procdump(void) {
80104990:	55                   	push   %ebp
80104991:	89 e5                	mov    %esp,%ebp
80104993:	83 ec 58             	sub    $0x58,%esp
	int i;
	struct proc *p;
	char *state;
	uint pc[10];

	for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
80104996:	c7 45 f0 54 ff 10 80 	movl   $0x8010ff54,-0x10(%ebp)
8010499d:	e9 d8 00 00 00       	jmp    80104a7a <procdump+0xea>
		if (p->state == UNUSED)
801049a2:	8b 45 f0             	mov    -0x10(%ebp),%eax
801049a5:	8b 40 0c             	mov    0xc(%eax),%eax
801049a8:	85 c0                	test   %eax,%eax
801049aa:	0f 84 c5 00 00 00    	je     80104a75 <procdump+0xe5>
			continue;
		if (p->state >= 0 && p->state < NELEM(states) && states[p->state])
801049b0:	8b 45 f0             	mov    -0x10(%ebp),%eax
801049b3:	8b 40 0c             	mov    0xc(%eax),%eax
801049b6:	83 f8 05             	cmp    $0x5,%eax
801049b9:	77 23                	ja     801049de <procdump+0x4e>
801049bb:	8b 45 f0             	mov    -0x10(%ebp),%eax
801049be:	8b 40 0c             	mov    0xc(%eax),%eax
801049c1:	8b 04 85 08 b0 10 80 	mov    -0x7fef4ff8(,%eax,4),%eax
801049c8:	85 c0                	test   %eax,%eax
801049ca:	74 12                	je     801049de <procdump+0x4e>
			state = states[p->state];
801049cc:	8b 45 f0             	mov    -0x10(%ebp),%eax
801049cf:	8b 40 0c             	mov    0xc(%eax),%eax
801049d2:	8b 04 85 08 b0 10 80 	mov    -0x7fef4ff8(,%eax,4),%eax
801049d9:	89 45 ec             	mov    %eax,-0x14(%ebp)
801049dc:	eb 07                	jmp    801049e5 <procdump+0x55>
		else
			state = "???";
801049de:	c7 45 ec 0f 8a 10 80 	movl   $0x80108a0f,-0x14(%ebp)
		cprintf("%d %s %s ###", p->pid, state, p->name);
801049e5:	8b 45 f0             	mov    -0x10(%ebp),%eax
801049e8:	8d 50 6c             	lea    0x6c(%eax),%edx
801049eb:	8b 45 f0             	mov    -0x10(%ebp),%eax
801049ee:	8b 40 10             	mov    0x10(%eax),%eax
801049f1:	89 54 24 0c          	mov    %edx,0xc(%esp)
801049f5:	8b 55 ec             	mov    -0x14(%ebp),%edx
801049f8:	89 54 24 08          	mov    %edx,0x8(%esp)
801049fc:	89 44 24 04          	mov    %eax,0x4(%esp)
80104a00:	c7 04 24 13 8a 10 80 	movl   $0x80108a13,(%esp)
80104a07:	e8 95 b9 ff ff       	call   801003a1 <cprintf>
		if (p->state == SLEEPING) {
80104a0c:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104a0f:	8b 40 0c             	mov    0xc(%eax),%eax
80104a12:	83 f8 02             	cmp    $0x2,%eax
80104a15:	75 50                	jne    80104a67 <procdump+0xd7>
			getcallerpcs((uint*) p->context->ebp + 2, pc);
80104a17:	8b 45 f0             	mov    -0x10(%ebp),%eax
80104a1a:	8b 40 1c             	mov    0x1c(%eax),%eax
80104a1d:	8b 40 0c             	mov    0xc(%eax),%eax
80104a20:	83 c0 08             	add    $0x8,%eax
80104a23:	8d 55 c4             	lea    -0x3c(%ebp),%edx
80104a26:	89 54 24 04          	mov    %edx,0x4(%esp)
80104a2a:	89 04 24             	mov    %eax,(%esp)
80104a2d:	e8 d2 06 00 00       	call   80105104 <getcallerpcs>
			for (i = 0; i < 10 && pc[i] != 0; i++)
80104a32:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80104a39:	eb 1b                	jmp    80104a56 <procdump+0xc6>
				cprintf(" %p ###", pc[i]);
80104a3b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104a3e:	8b 44 85 c4          	mov    -0x3c(%ebp,%eax,4),%eax
80104a42:	89 44 24 04          	mov    %eax,0x4(%esp)
80104a46:	c7 04 24 20 8a 10 80 	movl   $0x80108a20,(%esp)
80104a4d:	e8 4f b9 ff ff       	call   801003a1 <cprintf>
		else
			state = "???";
		cprintf("%d %s %s ###", p->pid, state, p->name);
		if (p->state == SLEEPING) {
			getcallerpcs((uint*) p->context->ebp + 2, pc);
			for (i = 0; i < 10 && pc[i] != 0; i++)
80104a52:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80104a56:	83 7d f4 09          	cmpl   $0x9,-0xc(%ebp)
80104a5a:	7f 0b                	jg     80104a67 <procdump+0xd7>
80104a5c:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104a5f:	8b 44 85 c4          	mov    -0x3c(%ebp,%eax,4),%eax
80104a63:	85 c0                	test   %eax,%eax
80104a65:	75 d4                	jne    80104a3b <procdump+0xab>
				cprintf(" %p ###", pc[i]);
		}
		cprintf("\n");
80104a67:	c7 04 24 28 8a 10 80 	movl   $0x80108a28,(%esp)
80104a6e:	e8 2e b9 ff ff       	call   801003a1 <cprintf>
80104a73:	eb 01                	jmp    80104a76 <procdump+0xe6>
	char *state;
	uint pc[10];

	for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
		if (p->state == UNUSED)
			continue;
80104a75:	90                   	nop
	int i;
	struct proc *p;
	char *state;
	uint pc[10];

	for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
80104a76:	83 6d f0 80          	subl   $0xffffff80,-0x10(%ebp)
80104a7a:	81 7d f0 54 1f 11 80 	cmpl   $0x80111f54,-0x10(%ebp)
80104a81:	0f 82 1b ff ff ff    	jb     801049a2 <procdump+0x12>
			for (i = 0; i < 10 && pc[i] != 0; i++)
				cprintf(" %p ###", pc[i]);
		}
		cprintf("\n");
	}
}
80104a87:	c9                   	leave  
80104a88:	c3                   	ret    

80104a89 <printprocesslist>:
void printprocesslist(void) {
80104a89:	55                   	push   %ebp
80104a8a:	89 e5                	mov    %esp,%ebp
80104a8c:	83 ec 28             	sub    $0x28,%esp
	int i = 0;
80104a8f:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
	for (; i < NPROC; i++) {
80104a96:	eb 4d                	jmp    80104ae5 <printprocesslist+0x5c>
		if (ptable.proc[i].state != UNUSED) {
80104a98:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104a9b:	c1 e0 07             	shl    $0x7,%eax
80104a9e:	05 50 ff 10 80       	add    $0x8010ff50,%eax
80104aa3:	8b 40 10             	mov    0x10(%eax),%eax
80104aa6:	85 c0                	test   %eax,%eax
80104aa8:	74 37                	je     80104ae1 <printprocesslist+0x58>
			cprintf("process name : %s and pid : %d \n", ptable.proc[i].name,
80104aaa:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104aad:	c1 e0 07             	shl    $0x7,%eax
80104ab0:	05 60 ff 10 80       	add    $0x8010ff60,%eax
80104ab5:	8b 40 04             	mov    0x4(%eax),%eax
80104ab8:	8b 55 f4             	mov    -0xc(%ebp),%edx
80104abb:	c1 e2 07             	shl    $0x7,%edx
80104abe:	81 c2 90 00 00 00    	add    $0x90,%edx
80104ac4:	81 c2 20 ff 10 80    	add    $0x8010ff20,%edx
80104aca:	83 c2 10             	add    $0x10,%edx
80104acd:	89 44 24 08          	mov    %eax,0x8(%esp)
80104ad1:	89 54 24 04          	mov    %edx,0x4(%esp)
80104ad5:	c7 04 24 2c 8a 10 80 	movl   $0x80108a2c,(%esp)
80104adc:	e8 c0 b8 ff ff       	call   801003a1 <cprintf>
		cprintf("\n");
	}
}
void printprocesslist(void) {
	int i = 0;
	for (; i < NPROC; i++) {
80104ae1:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80104ae5:	83 7d f4 3f          	cmpl   $0x3f,-0xc(%ebp)
80104ae9:	7e ad                	jle    80104a98 <printprocesslist+0xf>
		if (ptable.proc[i].state != UNUSED) {
			cprintf("process name : %s and pid : %d \n", ptable.proc[i].name,
					ptable.proc[i].pid);
		}
	}
}
80104aeb:	c9                   	leave  
80104aec:	c3                   	ret    

80104aed <ps>:

int ps(void) {
80104aed:	55                   	push   %ebp
80104aee:	89 e5                	mov    %esp,%ebp
80104af0:	83 ec 28             	sub    $0x28,%esp
	//static char *states[] = { [UNUSED] "unused", [EMBRYO] "embryo", [SLEEPING
	//		] "sleep ", [RUNNABLE] "runble", [RUNNING] "run   ", [ZOMBIE
	//		] "zombie" };

	cprintf("#############%i################ \n");
80104af3:	c7 04 24 50 8a 10 80 	movl   $0x80108a50,(%esp)
80104afa:	e8 a2 b8 ff ff       	call   801003a1 <cprintf>

	int i;
	for(i=0;i<5;i++)
80104aff:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80104b06:	e9 97 00 00 00       	jmp    80104ba2 <ps+0xb5>
	{
		if(!IsQueueEmpty(&priorityTable[i]))
80104b0b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104b0e:	c1 e0 04             	shl    $0x4,%eax
80104b11:	89 c2                	mov    %eax,%edx
80104b13:	c1 e2 04             	shl    $0x4,%edx
80104b16:	01 d0                	add    %edx,%eax
80104b18:	05 60 1f 11 80       	add    $0x80111f60,%eax
80104b1d:	89 04 24             	mov    %eax,(%esp)
80104b20:	e8 31 03 00 00       	call   80104e56 <IsQueueEmpty>
80104b25:	85 c0                	test   %eax,%eax
80104b27:	75 71                	jne    80104b9a <ps+0xad>
		{
			int p;
			for(p=0;p<priorityTable[i].size;p++)
80104b29:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
80104b30:	eb 4c                	jmp    80104b7e <ps+0x91>
			{
				cprintf("process name->%s | process priority->%i \n"
						,priorityTable[i].proc[p]->name
						,priorityTable[i].proc[p]->priority);
80104b32:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104b35:	c1 e0 02             	shl    $0x2,%eax
80104b38:	89 c2                	mov    %eax,%edx
80104b3a:	c1 e2 04             	shl    $0x4,%edx
80104b3d:	01 d0                	add    %edx,%eax
80104b3f:	03 45 f0             	add    -0x10(%ebp),%eax
80104b42:	8b 04 85 60 1f 11 80 	mov    -0x7feee0a0(,%eax,4),%eax
		if(!IsQueueEmpty(&priorityTable[i]))
		{
			int p;
			for(p=0;p<priorityTable[i].size;p++)
			{
				cprintf("process name->%s | process priority->%i \n"
80104b49:	8b 50 7c             	mov    0x7c(%eax),%edx
						,priorityTable[i].proc[p]->name
80104b4c:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104b4f:	c1 e0 02             	shl    $0x2,%eax
80104b52:	89 c1                	mov    %eax,%ecx
80104b54:	c1 e1 04             	shl    $0x4,%ecx
80104b57:	01 c8                	add    %ecx,%eax
80104b59:	03 45 f0             	add    -0x10(%ebp),%eax
80104b5c:	8b 04 85 60 1f 11 80 	mov    -0x7feee0a0(,%eax,4),%eax
80104b63:	83 c0 6c             	add    $0x6c,%eax
		if(!IsQueueEmpty(&priorityTable[i]))
		{
			int p;
			for(p=0;p<priorityTable[i].size;p++)
			{
				cprintf("process name->%s | process priority->%i \n"
80104b66:	89 54 24 08          	mov    %edx,0x8(%esp)
80104b6a:	89 44 24 04          	mov    %eax,0x4(%esp)
80104b6e:	c7 04 24 74 8a 10 80 	movl   $0x80108a74,(%esp)
80104b75:	e8 27 b8 ff ff       	call   801003a1 <cprintf>
	for(i=0;i<5;i++)
	{
		if(!IsQueueEmpty(&priorityTable[i]))
		{
			int p;
			for(p=0;p<priorityTable[i].size;p++)
80104b7a:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
80104b7e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104b81:	c1 e0 04             	shl    $0x4,%eax
80104b84:	89 c2                	mov    %eax,%edx
80104b86:	c1 e2 04             	shl    $0x4,%edx
80104b89:	01 d0                	add    %edx,%eax
80104b8b:	05 60 20 11 80       	add    $0x80112060,%eax
80104b90:	8b 40 08             	mov    0x8(%eax),%eax
80104b93:	3b 45 f0             	cmp    -0x10(%ebp),%eax
80104b96:	7f 9a                	jg     80104b32 <ps+0x45>
80104b98:	eb 04                	jmp    80104b9e <ps+0xb1>

			}
		}
		else
		{
			i=i+1;
80104b9a:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
	//		] "zombie" };

	cprintf("#############%i################ \n");

	int i;
	for(i=0;i<5;i++)
80104b9e:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80104ba2:	83 7d f4 04          	cmpl   $0x4,-0xc(%ebp)
80104ba6:	0f 8e 5f ff ff ff    	jle    80104b0b <ps+0x1e>
	 }



	//cprintf("##########################################");
	return 0;
80104bac:	b8 00 00 00 00       	mov    $0x0,%eax
}
80104bb1:	c9                   	leave  
80104bb2:	c3                   	ret    

80104bb3 <SwitchToProccess>:

void SwitchToProccess(struct proc* process)
{
80104bb3:	55                   	push   %ebp
80104bb4:	89 e5                	mov    %esp,%ebp
80104bb6:	83 ec 18             	sub    $0x18,%esp
	proc = process;
80104bb9:	8b 45 08             	mov    0x8(%ebp),%eax
80104bbc:	65 a3 04 00 00 00    	mov    %eax,%gs:0x4
	switchuvm(process);
80104bc2:	8b 45 08             	mov    0x8(%ebp),%eax
80104bc5:	89 04 24             	mov    %eax,(%esp)
80104bc8:	e8 65 33 00 00       	call   80107f32 <switchuvm>
	process->state = RUNNING;
80104bcd:	8b 45 08             	mov    0x8(%ebp),%eax
80104bd0:	c7 40 0c 04 00 00 00 	movl   $0x4,0xc(%eax)
	swtch(&cpu->scheduler, proc->context);
80104bd7:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80104bdd:	8b 40 1c             	mov    0x1c(%eax),%eax
80104be0:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80104be7:	83 c2 04             	add    $0x4,%edx
80104bea:	89 44 24 04          	mov    %eax,0x4(%esp)
80104bee:	89 14 24             	mov    %edx,(%esp)
80104bf1:	e8 52 09 00 00       	call   80105548 <swtch>
	switchkvm();
80104bf6:	e8 1a 33 00 00       	call   80107f15 <switchkvm>

	// Process is done running for now.
	// It should have changed its p->state before coming back.
	proc = 0;
80104bfb:	65 c7 05 04 00 00 00 	movl   $0x0,%gs:0x4
80104c02:	00 00 00 00 
}
80104c06:	c9                   	leave  
80104c07:	c3                   	ret    

80104c08 <InitialPriorityTables>:

void InitialPriorityTables()
{
80104c08:	55                   	push   %ebp
80104c09:	89 e5                	mov    %esp,%ebp
80104c0b:	83 ec 10             	sub    $0x10,%esp
	int i;
	for(i = 0 ; i< PRIORITYLEVELS;i++)
80104c0e:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
80104c15:	eb 67                	jmp    80104c7e <InitialPriorityTables+0x76>
	{
		priorityTable[i].head = 0;
80104c17:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104c1a:	c1 e0 04             	shl    $0x4,%eax
80104c1d:	89 c2                	mov    %eax,%edx
80104c1f:	c1 e2 04             	shl    $0x4,%edx
80104c22:	01 d0                	add    %edx,%eax
80104c24:	05 60 20 11 80       	add    $0x80112060,%eax
80104c29:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
		priorityTable[i].tail = QUEUE_CAPACITY-1;   
80104c2f:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104c32:	c1 e0 04             	shl    $0x4,%eax
80104c35:	89 c2                	mov    %eax,%edx
80104c37:	c1 e2 04             	shl    $0x4,%edx
80104c3a:	01 d0                	add    %edx,%eax
80104c3c:	05 60 20 11 80       	add    $0x80112060,%eax
80104c41:	c7 40 04 3f 00 00 00 	movl   $0x3f,0x4(%eax)
		priorityTable[i].size=0;
80104c48:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104c4b:	c1 e0 04             	shl    $0x4,%eax
80104c4e:	89 c2                	mov    %eax,%edx
80104c50:	c1 e2 04             	shl    $0x4,%edx
80104c53:	01 d0                	add    %edx,%eax
80104c55:	05 60 20 11 80       	add    $0x80112060,%eax
80104c5a:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
		priorityTable[i].priority = i;
80104c61:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104c64:	c1 e0 04             	shl    $0x4,%eax
80104c67:	89 c2                	mov    %eax,%edx
80104c69:	c1 e2 04             	shl    $0x4,%edx
80104c6c:	01 d0                	add    %edx,%eax
80104c6e:	8d 90 60 20 11 80    	lea    -0x7feedfa0(%eax),%edx
80104c74:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104c77:	89 42 0c             	mov    %eax,0xc(%edx)
}

void InitialPriorityTables()
{
	int i;
	for(i = 0 ; i< PRIORITYLEVELS;i++)
80104c7a:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80104c7e:	83 7d fc 04          	cmpl   $0x4,-0x4(%ebp)
80104c82:	7e 93                	jle    80104c17 <InitialPriorityTables+0xf>
		priorityTable[i].head = 0;
		priorityTable[i].tail = QUEUE_CAPACITY-1;   
		priorityTable[i].size=0;
		priorityTable[i].priority = i;
	}
}
80104c84:	c9                   	leave  
80104c85:	c3                   	ret    

80104c86 <SetProcessRunnable>:

void SetProcessRunnable(struct proc* process)
{
80104c86:	55                   	push   %ebp
80104c87:	89 e5                	mov    %esp,%ebp
80104c89:	83 ec 18             	sub    $0x18,%esp
	process->state=RUNNABLE;
80104c8c:	8b 45 08             	mov    0x8(%ebp),%eax
80104c8f:	c7 40 0c 03 00 00 00 	movl   $0x3,0xc(%eax)
	InsertToPriorityTable(process->priority,process);
80104c96:	8b 45 08             	mov    0x8(%ebp),%eax
80104c99:	8b 40 7c             	mov    0x7c(%eax),%eax
80104c9c:	8b 55 08             	mov    0x8(%ebp),%edx
80104c9f:	89 54 24 04          	mov    %edx,0x4(%esp)
80104ca3:	89 04 24             	mov    %eax,(%esp)
80104ca6:	e8 02 00 00 00       	call   80104cad <InsertToPriorityTable>
}
80104cab:	c9                   	leave  
80104cac:	c3                   	ret    

80104cad <InsertToPriorityTable>:

int InsertToPriorityTable(int priority,struct proc* process)
{
80104cad:	55                   	push   %ebp
80104cae:	89 e5                	mov    %esp,%ebp
80104cb0:	83 ec 18             	sub    $0x18,%esp
	return EnqueueProcess(&priorityTable[priority],process);
80104cb3:	8b 45 08             	mov    0x8(%ebp),%eax
80104cb6:	c1 e0 04             	shl    $0x4,%eax
80104cb9:	89 c2                	mov    %eax,%edx
80104cbb:	c1 e2 04             	shl    $0x4,%edx
80104cbe:	01 d0                	add    %edx,%eax
80104cc0:	8d 90 60 1f 11 80    	lea    -0x7feee0a0(%eax),%edx
80104cc6:	8b 45 0c             	mov    0xc(%ebp),%eax
80104cc9:	89 44 24 04          	mov    %eax,0x4(%esp)
80104ccd:	89 14 24             	mov    %edx,(%esp)
80104cd0:	e8 7e 00 00 00       	call   80104d53 <EnqueueProcess>
}
80104cd5:	c9                   	leave  
80104cd6:	c3                   	ret    

80104cd7 <GetNextRunnableProcess>:



struct proc* GetNextRunnableProcess(int priority)
{
80104cd7:	55                   	push   %ebp
80104cd8:	89 e5                	mov    %esp,%ebp
80104cda:	83 ec 18             	sub    $0x18,%esp
	return DequeueProcess(&priorityTable[priority]);
80104cdd:	8b 45 08             	mov    0x8(%ebp),%eax
80104ce0:	c1 e0 04             	shl    $0x4,%eax
80104ce3:	89 c2                	mov    %eax,%edx
80104ce5:	c1 e2 04             	shl    $0x4,%edx
80104ce8:	01 d0                	add    %edx,%eax
80104cea:	05 60 1f 11 80       	add    $0x80111f60,%eax
80104cef:	89 04 24             	mov    %eax,(%esp)
80104cf2:	e8 d6 00 00 00       	call   80104dcd <DequeueProcess>
}
80104cf7:	c9                   	leave  
80104cf8:	c3                   	ret    

80104cf9 <IsThereANoneEmptyHigherLevelPriorityTable>:

bool IsThereANoneEmptyHigherLevelPriorityTable(int currentPriority)
{
80104cf9:	55                   	push   %ebp
80104cfa:	89 e5                	mov    %esp,%ebp
80104cfc:	83 ec 28             	sub    $0x28,%esp
	int i;
	for(i = 0 ; i< currentPriority ; i++)
80104cff:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80104d06:	eb 1a                	jmp    80104d22 <IsThereANoneEmptyHigherLevelPriorityTable+0x29>
	{
		if(!IsPriorityTableEmpty(i))
80104d08:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104d0b:	89 04 24             	mov    %eax,(%esp)
80104d0e:	e8 1e 00 00 00       	call   80104d31 <IsPriorityTableEmpty>
80104d13:	85 c0                	test   %eax,%eax
80104d15:	75 07                	jne    80104d1e <IsThereANoneEmptyHigherLevelPriorityTable+0x25>
			return true;
80104d17:	b8 01 00 00 00       	mov    $0x1,%eax
80104d1c:	eb 11                	jmp    80104d2f <IsThereANoneEmptyHigherLevelPriorityTable+0x36>
}

bool IsThereANoneEmptyHigherLevelPriorityTable(int currentPriority)
{
	int i;
	for(i = 0 ; i< currentPriority ; i++)
80104d1e:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80104d22:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104d25:	3b 45 08             	cmp    0x8(%ebp),%eax
80104d28:	7c de                	jl     80104d08 <IsThereANoneEmptyHigherLevelPriorityTable+0xf>
	{
		if(!IsPriorityTableEmpty(i))
			return true;
	}
	return false;
80104d2a:	b8 00 00 00 00       	mov    $0x0,%eax
}
80104d2f:	c9                   	leave  
80104d30:	c3                   	ret    

80104d31 <IsPriorityTableEmpty>:

bool IsPriorityTableEmpty(int priority)
{
80104d31:	55                   	push   %ebp
80104d32:	89 e5                	mov    %esp,%ebp
80104d34:	83 ec 18             	sub    $0x18,%esp
	return (IsQueueEmpty(&priorityTable[priority]));
80104d37:	8b 45 08             	mov    0x8(%ebp),%eax
80104d3a:	c1 e0 04             	shl    $0x4,%eax
80104d3d:	89 c2                	mov    %eax,%edx
80104d3f:	c1 e2 04             	shl    $0x4,%edx
80104d42:	01 d0                	add    %edx,%eax
80104d44:	05 60 1f 11 80       	add    $0x80111f60,%eax
80104d49:	89 04 24             	mov    %eax,(%esp)
80104d4c:	e8 05 01 00 00       	call   80104e56 <IsQueueEmpty>
}
80104d51:	c9                   	leave  
80104d52:	c3                   	ret    

80104d53 <EnqueueProcess>:



/****************************** QUEUE ******************************/
int EnqueueProcess(struct processQueue* queue, struct proc* process )
{
80104d53:	55                   	push   %ebp
80104d54:	89 e5                	mov    %esp,%ebp
80104d56:	83 ec 18             	sub    $0x18,%esp
	if(!IsQueueFull(queue))
80104d59:	8b 45 08             	mov    0x8(%ebp),%eax
80104d5c:	89 04 24             	mov    %eax,(%esp)
80104d5f:	e8 db 00 00 00       	call   80104e3f <IsQueueFull>
80104d64:	85 c0                	test   %eax,%eax
80104d66:	75 5e                	jne    80104dc6 <EnqueueProcess+0x73>
	{
		//queue.proc[queue.tail] = process;
		//queue.tail = (queue.tail++)%QUEUE_CAPACITY;
		//return 0;
		if(queue!=0)
80104d68:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
80104d6c:	74 58                	je     80104dc6 <EnqueueProcess+0x73>
		{
			queue->tail=(queue->tail+1)%QUEUE_CAPACITY;
80104d6e:	8b 45 08             	mov    0x8(%ebp),%eax
80104d71:	8b 80 04 01 00 00    	mov    0x104(%eax),%eax
80104d77:	8d 50 01             	lea    0x1(%eax),%edx
80104d7a:	89 d0                	mov    %edx,%eax
80104d7c:	c1 f8 1f             	sar    $0x1f,%eax
80104d7f:	c1 e8 1a             	shr    $0x1a,%eax
80104d82:	01 c2                	add    %eax,%edx
80104d84:	83 e2 3f             	and    $0x3f,%edx
80104d87:	89 d1                	mov    %edx,%ecx
80104d89:	29 c1                	sub    %eax,%ecx
80104d8b:	89 c8                	mov    %ecx,%eax
80104d8d:	89 c2                	mov    %eax,%edx
80104d8f:	8b 45 08             	mov    0x8(%ebp),%eax
80104d92:	89 90 04 01 00 00    	mov    %edx,0x104(%eax)
			queue->proc[queue->tail]=process;
80104d98:	8b 45 08             	mov    0x8(%ebp),%eax
80104d9b:	8b 90 04 01 00 00    	mov    0x104(%eax),%edx
80104da1:	8b 45 08             	mov    0x8(%ebp),%eax
80104da4:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80104da7:	89 0c 90             	mov    %ecx,(%eax,%edx,4)
			queue->size=(queue->size)+1;
80104daa:	8b 45 08             	mov    0x8(%ebp),%eax
80104dad:	8b 80 08 01 00 00    	mov    0x108(%eax),%eax
80104db3:	8d 50 01             	lea    0x1(%eax),%edx
80104db6:	8b 45 08             	mov    0x8(%ebp),%eax
80104db9:	89 90 08 01 00 00    	mov    %edx,0x108(%eax)
			return 0;
80104dbf:	b8 00 00 00 00       	mov    $0x0,%eax
80104dc4:	eb 05                	jmp    80104dcb <EnqueueProcess+0x78>
		}
	}
	return -1;
80104dc6:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80104dcb:	c9                   	leave  
80104dcc:	c3                   	ret    

80104dcd <DequeueProcess>:

struct proc* DequeueProcess(struct processQueue* queue)
{
80104dcd:	55                   	push   %ebp
80104dce:	89 e5                	mov    %esp,%ebp
80104dd0:	83 ec 28             	sub    $0x28,%esp
	if(!IsQueueEmpty(queue))
80104dd3:	8b 45 08             	mov    0x8(%ebp),%eax
80104dd6:	89 04 24             	mov    %eax,(%esp)
80104dd9:	e8 78 00 00 00       	call   80104e56 <IsQueueEmpty>
80104dde:	85 c0                	test   %eax,%eax
80104de0:	75 56                	jne    80104e38 <DequeueProcess+0x6b>
	{
		struct proc* p = queue->proc[queue->head];
80104de2:	8b 45 08             	mov    0x8(%ebp),%eax
80104de5:	8b 90 00 01 00 00    	mov    0x100(%eax),%edx
80104deb:	8b 45 08             	mov    0x8(%ebp),%eax
80104dee:	8b 04 90             	mov    (%eax,%edx,4),%eax
80104df1:	89 45 f4             	mov    %eax,-0xc(%ebp)
		queue->head= (queue->head+1)%QUEUE_CAPACITY;
80104df4:	8b 45 08             	mov    0x8(%ebp),%eax
80104df7:	8b 80 00 01 00 00    	mov    0x100(%eax),%eax
80104dfd:	8d 50 01             	lea    0x1(%eax),%edx
80104e00:	89 d0                	mov    %edx,%eax
80104e02:	c1 f8 1f             	sar    $0x1f,%eax
80104e05:	c1 e8 1a             	shr    $0x1a,%eax
80104e08:	01 c2                	add    %eax,%edx
80104e0a:	83 e2 3f             	and    $0x3f,%edx
80104e0d:	89 d1                	mov    %edx,%ecx
80104e0f:	29 c1                	sub    %eax,%ecx
80104e11:	89 c8                	mov    %ecx,%eax
80104e13:	89 c2                	mov    %eax,%edx
80104e15:	8b 45 08             	mov    0x8(%ebp),%eax
80104e18:	89 90 00 01 00 00    	mov    %edx,0x100(%eax)
		queue->size=queue->size-1;
80104e1e:	8b 45 08             	mov    0x8(%ebp),%eax
80104e21:	8b 80 08 01 00 00    	mov    0x108(%eax),%eax
80104e27:	8d 50 ff             	lea    -0x1(%eax),%edx
80104e2a:	8b 45 08             	mov    0x8(%ebp),%eax
80104e2d:	89 90 08 01 00 00    	mov    %edx,0x108(%eax)
		return p;
80104e33:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104e36:	eb 05                	jmp    80104e3d <DequeueProcess+0x70>
		
	}
	return 0;
80104e38:	b8 00 00 00 00       	mov    $0x0,%eax
}
80104e3d:	c9                   	leave  
80104e3e:	c3                   	ret    

80104e3f <IsQueueFull>:

bool IsQueueFull(struct processQueue* queue)
{
80104e3f:	55                   	push   %ebp
80104e40:	89 e5                	mov    %esp,%ebp
	//return (queue.head == queue.tail);
	return (queue->size>=QUEUE_CAPACITY);
80104e42:	8b 45 08             	mov    0x8(%ebp),%eax
80104e45:	8b 80 08 01 00 00    	mov    0x108(%eax),%eax
80104e4b:	83 f8 3f             	cmp    $0x3f,%eax
80104e4e:	0f 9f c0             	setg   %al
80104e51:	0f b6 c0             	movzbl %al,%eax
		
}
80104e54:	5d                   	pop    %ebp
80104e55:	c3                   	ret    

80104e56 <IsQueueEmpty>:

bool IsQueueEmpty(struct processQueue* queue)
{
80104e56:	55                   	push   %ebp
80104e57:	89 e5                	mov    %esp,%ebp
	//if tail is pointing to the next of head
	//return ((queue.head++)%QUEUE_CAPACITY == queue.tail);
	return (queue->size<=0);
80104e59:	8b 45 08             	mov    0x8(%ebp),%eax
80104e5c:	8b 80 08 01 00 00    	mov    0x108(%eax),%eax
80104e62:	85 c0                	test   %eax,%eax
80104e64:	0f 9e c0             	setle  %al
80104e67:	0f b6 c0             	movzbl %al,%eax
	
}
80104e6a:	5d                   	pop    %ebp
80104e6b:	c3                   	ret    

80104e6c <setpriority>:

/****************************** /QUEUE ******************************/

int setpriority(int pid,int priority)
{
80104e6c:	55                   	push   %ebp
80104e6d:	89 e5                	mov    %esp,%ebp
80104e6f:	83 ec 18             	sub    $0x18,%esp
	if(findprocesswithpid(pid)!=0)
80104e72:	8b 45 08             	mov    0x8(%ebp),%eax
80104e75:	89 04 24             	mov    %eax,(%esp)
80104e78:	e8 35 00 00 00       	call   80104eb2 <findprocesswithpid>
80104e7d:	85 c0                	test   %eax,%eax
80104e7f:	74 2a                	je     80104eab <setpriority+0x3f>
	{
		findprocesswithpid(pid)->priority=priority;
80104e81:	8b 45 08             	mov    0x8(%ebp),%eax
80104e84:	89 04 24             	mov    %eax,(%esp)
80104e87:	e8 26 00 00 00       	call   80104eb2 <findprocesswithpid>
80104e8c:	8b 55 0c             	mov    0xc(%ebp),%edx
80104e8f:	89 50 7c             	mov    %edx,0x7c(%eax)
		setpriorityforchildren(pid,priority);
80104e92:	8b 45 0c             	mov    0xc(%ebp),%eax
80104e95:	89 44 24 04          	mov    %eax,0x4(%esp)
80104e99:	8b 45 08             	mov    0x8(%ebp),%eax
80104e9c:	89 04 24             	mov    %eax,(%esp)
80104e9f:	e8 95 00 00 00       	call   80104f39 <setpriorityforchildren>
		return 0;
80104ea4:	b8 00 00 00 00       	mov    $0x0,%eax
80104ea9:	eb 05                	jmp    80104eb0 <setpriority+0x44>
	}
	else
		return -1;
80104eab:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80104eb0:	c9                   	leave  
80104eb1:	c3                   	ret    

80104eb2 <findprocesswithpid>:

struct proc* findprocesswithpid(int pid)
{
80104eb2:	55                   	push   %ebp
80104eb3:	89 e5                	mov    %esp,%ebp
80104eb5:	83 ec 10             	sub    $0x10,%esp
	int i;
	int j;
	struct proc* foundprocess;
	for (i=0;i<PRIORITYLEVELS;i++)
80104eb8:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
80104ebf:	eb 6d                	jmp    80104f2e <findprocesswithpid+0x7c>
	{
		for(j=0;j<priorityTable[i].size;j++)
80104ec1:	c7 45 f8 00 00 00 00 	movl   $0x0,-0x8(%ebp)
80104ec8:	eb 46                	jmp    80104f10 <findprocesswithpid+0x5e>
		{
			if(priorityTable[i].proc[j]->pid==pid)
80104eca:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104ecd:	c1 e0 02             	shl    $0x2,%eax
80104ed0:	89 c2                	mov    %eax,%edx
80104ed2:	c1 e2 04             	shl    $0x4,%edx
80104ed5:	01 d0                	add    %edx,%eax
80104ed7:	03 45 f8             	add    -0x8(%ebp),%eax
80104eda:	8b 04 85 60 1f 11 80 	mov    -0x7feee0a0(,%eax,4),%eax
80104ee1:	8b 40 10             	mov    0x10(%eax),%eax
80104ee4:	3b 45 08             	cmp    0x8(%ebp),%eax
80104ee7:	75 1c                	jne    80104f05 <findprocesswithpid+0x53>
			{
				foundprocess=priorityTable[i].proc[j];
80104ee9:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104eec:	c1 e0 02             	shl    $0x2,%eax
80104eef:	89 c2                	mov    %eax,%edx
80104ef1:	c1 e2 04             	shl    $0x4,%edx
80104ef4:	01 d0                	add    %edx,%eax
80104ef6:	03 45 f8             	add    -0x8(%ebp),%eax
80104ef9:	8b 04 85 60 1f 11 80 	mov    -0x7feee0a0(,%eax,4),%eax
80104f00:	89 45 f4             	mov    %eax,-0xc(%ebp)
80104f03:	eb 07                	jmp    80104f0c <findprocesswithpid+0x5a>
			}
			else
			{
				foundprocess=nullp;
80104f05:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
	int i;
	int j;
	struct proc* foundprocess;
	for (i=0;i<PRIORITYLEVELS;i++)
	{
		for(j=0;j<priorityTable[i].size;j++)
80104f0c:	83 45 f8 01          	addl   $0x1,-0x8(%ebp)
80104f10:	8b 45 fc             	mov    -0x4(%ebp),%eax
80104f13:	c1 e0 04             	shl    $0x4,%eax
80104f16:	89 c2                	mov    %eax,%edx
80104f18:	c1 e2 04             	shl    $0x4,%edx
80104f1b:	01 d0                	add    %edx,%eax
80104f1d:	05 60 20 11 80       	add    $0x80112060,%eax
80104f22:	8b 40 08             	mov    0x8(%eax),%eax
80104f25:	3b 45 f8             	cmp    -0x8(%ebp),%eax
80104f28:	7f a0                	jg     80104eca <findprocesswithpid+0x18>
struct proc* findprocesswithpid(int pid)
{
	int i;
	int j;
	struct proc* foundprocess;
	for (i=0;i<PRIORITYLEVELS;i++)
80104f2a:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80104f2e:	83 7d fc 04          	cmpl   $0x4,-0x4(%ebp)
80104f32:	7e 8d                	jle    80104ec1 <findprocesswithpid+0xf>
			{
				foundprocess=nullp;
			}
		}
	}
	return foundprocess;
80104f34:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80104f37:	c9                   	leave  
80104f38:	c3                   	ret    

80104f39 <setpriorityforchildren>:

void setpriorityforchildren(int parentpid,int priority)
{
80104f39:	55                   	push   %ebp
80104f3a:	89 e5                	mov    %esp,%ebp
80104f3c:	83 ec 28             	sub    $0x28,%esp
	int i;
	int j;
	struct proc* childprocess;
	for (i=0;i<PRIORITYLEVELS;i++)
80104f3f:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80104f46:	e9 92 00 00 00       	jmp    80104fdd <setpriorityforchildren+0xa4>
	{
		for(j=0;j<priorityTable[i].size;j++)
80104f4b:	c7 45 f0 00 00 00 00 	movl   $0x0,-0x10(%ebp)
80104f52:	eb 67                	jmp    80104fbb <setpriorityforchildren+0x82>
		{
			if(priorityTable[i].proc[j]->parent->pid==parentpid)
80104f54:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104f57:	c1 e0 02             	shl    $0x2,%eax
80104f5a:	89 c2                	mov    %eax,%edx
80104f5c:	c1 e2 04             	shl    $0x4,%edx
80104f5f:	01 d0                	add    %edx,%eax
80104f61:	03 45 f0             	add    -0x10(%ebp),%eax
80104f64:	8b 04 85 60 1f 11 80 	mov    -0x7feee0a0(,%eax,4),%eax
80104f6b:	8b 40 14             	mov    0x14(%eax),%eax
80104f6e:	8b 40 10             	mov    0x10(%eax),%eax
80104f71:	3b 45 08             	cmp    0x8(%ebp),%eax
80104f74:	75 3a                	jne    80104fb0 <setpriorityforchildren+0x77>
			{
				childprocess=priorityTable[i].proc[j];
80104f76:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104f79:	c1 e0 02             	shl    $0x2,%eax
80104f7c:	89 c2                	mov    %eax,%edx
80104f7e:	c1 e2 04             	shl    $0x4,%edx
80104f81:	01 d0                	add    %edx,%eax
80104f83:	03 45 f0             	add    -0x10(%ebp),%eax
80104f86:	8b 04 85 60 1f 11 80 	mov    -0x7feee0a0(,%eax,4),%eax
80104f8d:	89 45 ec             	mov    %eax,-0x14(%ebp)
				childprocess->priority=priority;
80104f90:	8b 55 0c             	mov    0xc(%ebp),%edx
80104f93:	8b 45 ec             	mov    -0x14(%ebp),%eax
80104f96:	89 50 7c             	mov    %edx,0x7c(%eax)
				setpriorityforchildren(childprocess->pid,priority);
80104f99:	8b 45 ec             	mov    -0x14(%ebp),%eax
80104f9c:	8b 40 10             	mov    0x10(%eax),%eax
80104f9f:	8b 55 0c             	mov    0xc(%ebp),%edx
80104fa2:	89 54 24 04          	mov    %edx,0x4(%esp)
80104fa6:	89 04 24             	mov    %eax,(%esp)
80104fa9:	e8 8b ff ff ff       	call   80104f39 <setpriorityforchildren>
80104fae:	eb 07                	jmp    80104fb7 <setpriorityforchildren+0x7e>

			}
			else
			{
				childprocess=nullp;
80104fb0:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%ebp)
	int i;
	int j;
	struct proc* childprocess;
	for (i=0;i<PRIORITYLEVELS;i++)
	{
		for(j=0;j<priorityTable[i].size;j++)
80104fb7:	83 45 f0 01          	addl   $0x1,-0x10(%ebp)
80104fbb:	8b 45 f4             	mov    -0xc(%ebp),%eax
80104fbe:	c1 e0 04             	shl    $0x4,%eax
80104fc1:	89 c2                	mov    %eax,%edx
80104fc3:	c1 e2 04             	shl    $0x4,%edx
80104fc6:	01 d0                	add    %edx,%eax
80104fc8:	05 60 20 11 80       	add    $0x80112060,%eax
80104fcd:	8b 40 08             	mov    0x8(%eax),%eax
80104fd0:	3b 45 f0             	cmp    -0x10(%ebp),%eax
80104fd3:	0f 8f 7b ff ff ff    	jg     80104f54 <setpriorityforchildren+0x1b>
void setpriorityforchildren(int parentpid,int priority)
{
	int i;
	int j;
	struct proc* childprocess;
	for (i=0;i<PRIORITYLEVELS;i++)
80104fd9:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80104fdd:	83 7d f4 04          	cmpl   $0x4,-0xc(%ebp)
80104fe1:	0f 8e 64 ff ff ff    	jle    80104f4b <setpriorityforchildren+0x12>
				childprocess=nullp;
			}
		}
	}

}
80104fe7:	c9                   	leave  
80104fe8:	c3                   	ret    
80104fe9:	00 00                	add    %al,(%eax)
	...

80104fec <readeflags>:
  asm volatile("ltr %0" : : "r" (sel));
}

static inline uint
readeflags(void)
{
80104fec:	55                   	push   %ebp
80104fed:	89 e5                	mov    %esp,%ebp
80104fef:	53                   	push   %ebx
80104ff0:	83 ec 10             	sub    $0x10,%esp
  uint eflags;
  asm volatile("pushfl; popl %0" : "=r" (eflags));
80104ff3:	9c                   	pushf  
80104ff4:	5b                   	pop    %ebx
80104ff5:	89 5d f8             	mov    %ebx,-0x8(%ebp)
  return eflags;
80104ff8:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
80104ffb:	83 c4 10             	add    $0x10,%esp
80104ffe:	5b                   	pop    %ebx
80104fff:	5d                   	pop    %ebp
80105000:	c3                   	ret    

80105001 <cli>:
  asm volatile("movw %0, %%gs" : : "r" (v));
}

static inline void
cli(void)
{
80105001:	55                   	push   %ebp
80105002:	89 e5                	mov    %esp,%ebp
  asm volatile("cli");
80105004:	fa                   	cli    
}
80105005:	5d                   	pop    %ebp
80105006:	c3                   	ret    

80105007 <sti>:

static inline void
sti(void)
{
80105007:	55                   	push   %ebp
80105008:	89 e5                	mov    %esp,%ebp
  asm volatile("sti");
8010500a:	fb                   	sti    
}
8010500b:	5d                   	pop    %ebp
8010500c:	c3                   	ret    

8010500d <xchg>:

static inline uint
xchg(volatile uint *addr, uint newval)
{
8010500d:	55                   	push   %ebp
8010500e:	89 e5                	mov    %esp,%ebp
80105010:	53                   	push   %ebx
80105011:	83 ec 10             	sub    $0x10,%esp
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
               "+m" (*addr), "=a" (result) :
80105014:	8b 55 08             	mov    0x8(%ebp),%edx
xchg(volatile uint *addr, uint newval)
{
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
80105017:	8b 45 0c             	mov    0xc(%ebp),%eax
               "+m" (*addr), "=a" (result) :
8010501a:	8b 4d 08             	mov    0x8(%ebp),%ecx
xchg(volatile uint *addr, uint newval)
{
  uint result;
  
  // The + in "+m" denotes a read-modify-write operand.
  asm volatile("lock; xchgl %0, %1" :
8010501d:	89 c3                	mov    %eax,%ebx
8010501f:	89 d8                	mov    %ebx,%eax
80105021:	f0 87 02             	lock xchg %eax,(%edx)
80105024:	89 c3                	mov    %eax,%ebx
80105026:	89 5d f8             	mov    %ebx,-0x8(%ebp)
               "+m" (*addr), "=a" (result) :
               "1" (newval) :
               "cc");
  return result;
80105029:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
8010502c:	83 c4 10             	add    $0x10,%esp
8010502f:	5b                   	pop    %ebx
80105030:	5d                   	pop    %ebp
80105031:	c3                   	ret    

80105032 <initlock>:
#include "proc.h"
#include "spinlock.h"

void
initlock(struct spinlock *lk, char *name)
{
80105032:	55                   	push   %ebp
80105033:	89 e5                	mov    %esp,%ebp
  lk->name = name;
80105035:	8b 45 08             	mov    0x8(%ebp),%eax
80105038:	8b 55 0c             	mov    0xc(%ebp),%edx
8010503b:	89 50 04             	mov    %edx,0x4(%eax)
  lk->locked = 0;
8010503e:	8b 45 08             	mov    0x8(%ebp),%eax
80105041:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
  lk->cpu = 0;
80105047:	8b 45 08             	mov    0x8(%ebp),%eax
8010504a:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
}
80105051:	5d                   	pop    %ebp
80105052:	c3                   	ret    

80105053 <acquire>:
// Loops (spins) until the lock is acquired.
// Holding a lock for a long time may cause
// other CPUs to waste time spinning to acquire it.
void
acquire(struct spinlock *lk)
{
80105053:	55                   	push   %ebp
80105054:	89 e5                	mov    %esp,%ebp
80105056:	83 ec 18             	sub    $0x18,%esp
  pushcli(); // disable interrupts to avoid deadlock.
80105059:	e8 3d 01 00 00       	call   8010519b <pushcli>
  if(holding(lk))
8010505e:	8b 45 08             	mov    0x8(%ebp),%eax
80105061:	89 04 24             	mov    %eax,(%esp)
80105064:	e8 08 01 00 00       	call   80105171 <holding>
80105069:	85 c0                	test   %eax,%eax
8010506b:	74 0c                	je     80105079 <acquire+0x26>
    panic("acquire");
8010506d:	c7 04 24 c8 8a 10 80 	movl   $0x80108ac8,(%esp)
80105074:	e8 c4 b4 ff ff       	call   8010053d <panic>

  // The xchg is atomic.
  // It also serializes, so that reads after acquire are not
  // reordered before it. 
  while(xchg(&lk->locked, 1) != 0)
80105079:	90                   	nop
8010507a:	8b 45 08             	mov    0x8(%ebp),%eax
8010507d:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80105084:	00 
80105085:	89 04 24             	mov    %eax,(%esp)
80105088:	e8 80 ff ff ff       	call   8010500d <xchg>
8010508d:	85 c0                	test   %eax,%eax
8010508f:	75 e9                	jne    8010507a <acquire+0x27>
    ;

  // Record info about lock acquisition for debugging.
  lk->cpu = cpu;
80105091:	8b 45 08             	mov    0x8(%ebp),%eax
80105094:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
8010509b:	89 50 08             	mov    %edx,0x8(%eax)
  getcallerpcs(&lk, lk->pcs);
8010509e:	8b 45 08             	mov    0x8(%ebp),%eax
801050a1:	83 c0 0c             	add    $0xc,%eax
801050a4:	89 44 24 04          	mov    %eax,0x4(%esp)
801050a8:	8d 45 08             	lea    0x8(%ebp),%eax
801050ab:	89 04 24             	mov    %eax,(%esp)
801050ae:	e8 51 00 00 00       	call   80105104 <getcallerpcs>
}
801050b3:	c9                   	leave  
801050b4:	c3                   	ret    

801050b5 <release>:

// Release the lock.
void
release(struct spinlock *lk)
{
801050b5:	55                   	push   %ebp
801050b6:	89 e5                	mov    %esp,%ebp
801050b8:	83 ec 18             	sub    $0x18,%esp
  if(!holding(lk))
801050bb:	8b 45 08             	mov    0x8(%ebp),%eax
801050be:	89 04 24             	mov    %eax,(%esp)
801050c1:	e8 ab 00 00 00       	call   80105171 <holding>
801050c6:	85 c0                	test   %eax,%eax
801050c8:	75 0c                	jne    801050d6 <release+0x21>
    panic("release");
801050ca:	c7 04 24 d0 8a 10 80 	movl   $0x80108ad0,(%esp)
801050d1:	e8 67 b4 ff ff       	call   8010053d <panic>

  lk->pcs[0] = 0;
801050d6:	8b 45 08             	mov    0x8(%ebp),%eax
801050d9:	c7 40 0c 00 00 00 00 	movl   $0x0,0xc(%eax)
  lk->cpu = 0;
801050e0:	8b 45 08             	mov    0x8(%ebp),%eax
801050e3:	c7 40 08 00 00 00 00 	movl   $0x0,0x8(%eax)
  // But the 2007 Intel 64 Architecture Memory Ordering White
  // Paper says that Intel 64 and IA-32 will not move a load
  // after a store. So lock->locked = 0 would work here.
  // The xchg being asm volatile ensures gcc emits it after
  // the above assignments (and after the critical section).
  xchg(&lk->locked, 0);
801050ea:	8b 45 08             	mov    0x8(%ebp),%eax
801050ed:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801050f4:	00 
801050f5:	89 04 24             	mov    %eax,(%esp)
801050f8:	e8 10 ff ff ff       	call   8010500d <xchg>

  popcli();
801050fd:	e8 e1 00 00 00       	call   801051e3 <popcli>
}
80105102:	c9                   	leave  
80105103:	c3                   	ret    

80105104 <getcallerpcs>:

// Record the current call stack in pcs[] by following the %ebp chain.
void
getcallerpcs(void *v, uint pcs[])
{
80105104:	55                   	push   %ebp
80105105:	89 e5                	mov    %esp,%ebp
80105107:	83 ec 10             	sub    $0x10,%esp
  uint *ebp;
  int i;
  
  ebp = (uint*)v - 2;
8010510a:	8b 45 08             	mov    0x8(%ebp),%eax
8010510d:	83 e8 08             	sub    $0x8,%eax
80105110:	89 45 fc             	mov    %eax,-0x4(%ebp)
  for(i = 0; i < 10; i++){
80105113:	c7 45 f8 00 00 00 00 	movl   $0x0,-0x8(%ebp)
8010511a:	eb 32                	jmp    8010514e <getcallerpcs+0x4a>
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
8010511c:	83 7d fc 00          	cmpl   $0x0,-0x4(%ebp)
80105120:	74 47                	je     80105169 <getcallerpcs+0x65>
80105122:	81 7d fc ff ff ff 7f 	cmpl   $0x7fffffff,-0x4(%ebp)
80105129:	76 3e                	jbe    80105169 <getcallerpcs+0x65>
8010512b:	83 7d fc ff          	cmpl   $0xffffffff,-0x4(%ebp)
8010512f:	74 38                	je     80105169 <getcallerpcs+0x65>
      break;
    pcs[i] = ebp[1];     // saved %eip
80105131:	8b 45 f8             	mov    -0x8(%ebp),%eax
80105134:	c1 e0 02             	shl    $0x2,%eax
80105137:	03 45 0c             	add    0xc(%ebp),%eax
8010513a:	8b 55 fc             	mov    -0x4(%ebp),%edx
8010513d:	8b 52 04             	mov    0x4(%edx),%edx
80105140:	89 10                	mov    %edx,(%eax)
    ebp = (uint*)ebp[0]; // saved %ebp
80105142:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105145:	8b 00                	mov    (%eax),%eax
80105147:	89 45 fc             	mov    %eax,-0x4(%ebp)
{
  uint *ebp;
  int i;
  
  ebp = (uint*)v - 2;
  for(i = 0; i < 10; i++){
8010514a:	83 45 f8 01          	addl   $0x1,-0x8(%ebp)
8010514e:	83 7d f8 09          	cmpl   $0x9,-0x8(%ebp)
80105152:	7e c8                	jle    8010511c <getcallerpcs+0x18>
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
      break;
    pcs[i] = ebp[1];     // saved %eip
    ebp = (uint*)ebp[0]; // saved %ebp
  }
  for(; i < 10; i++)
80105154:	eb 13                	jmp    80105169 <getcallerpcs+0x65>
    pcs[i] = 0;
80105156:	8b 45 f8             	mov    -0x8(%ebp),%eax
80105159:	c1 e0 02             	shl    $0x2,%eax
8010515c:	03 45 0c             	add    0xc(%ebp),%eax
8010515f:	c7 00 00 00 00 00    	movl   $0x0,(%eax)
    if(ebp == 0 || ebp < (uint*)KERNBASE || ebp == (uint*)0xffffffff)
      break;
    pcs[i] = ebp[1];     // saved %eip
    ebp = (uint*)ebp[0]; // saved %ebp
  }
  for(; i < 10; i++)
80105165:	83 45 f8 01          	addl   $0x1,-0x8(%ebp)
80105169:	83 7d f8 09          	cmpl   $0x9,-0x8(%ebp)
8010516d:	7e e7                	jle    80105156 <getcallerpcs+0x52>
    pcs[i] = 0;
}
8010516f:	c9                   	leave  
80105170:	c3                   	ret    

80105171 <holding>:

// Check whether this cpu is holding the lock.
int
holding(struct spinlock *lock)
{
80105171:	55                   	push   %ebp
80105172:	89 e5                	mov    %esp,%ebp
  return lock->locked && lock->cpu == cpu;
80105174:	8b 45 08             	mov    0x8(%ebp),%eax
80105177:	8b 00                	mov    (%eax),%eax
80105179:	85 c0                	test   %eax,%eax
8010517b:	74 17                	je     80105194 <holding+0x23>
8010517d:	8b 45 08             	mov    0x8(%ebp),%eax
80105180:	8b 50 08             	mov    0x8(%eax),%edx
80105183:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80105189:	39 c2                	cmp    %eax,%edx
8010518b:	75 07                	jne    80105194 <holding+0x23>
8010518d:	b8 01 00 00 00       	mov    $0x1,%eax
80105192:	eb 05                	jmp    80105199 <holding+0x28>
80105194:	b8 00 00 00 00       	mov    $0x0,%eax
}
80105199:	5d                   	pop    %ebp
8010519a:	c3                   	ret    

8010519b <pushcli>:
// it takes two popcli to undo two pushcli.  Also, if interrupts
// are off, then pushcli, popcli leaves them off.

void
pushcli(void)
{
8010519b:	55                   	push   %ebp
8010519c:	89 e5                	mov    %esp,%ebp
8010519e:	83 ec 10             	sub    $0x10,%esp
  int eflags;
  
  eflags = readeflags();
801051a1:	e8 46 fe ff ff       	call   80104fec <readeflags>
801051a6:	89 45 fc             	mov    %eax,-0x4(%ebp)
  cli();
801051a9:	e8 53 fe ff ff       	call   80105001 <cli>
  if(cpu->ncli++ == 0)
801051ae:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801051b4:	8b 90 ac 00 00 00    	mov    0xac(%eax),%edx
801051ba:	85 d2                	test   %edx,%edx
801051bc:	0f 94 c1             	sete   %cl
801051bf:	83 c2 01             	add    $0x1,%edx
801051c2:	89 90 ac 00 00 00    	mov    %edx,0xac(%eax)
801051c8:	84 c9                	test   %cl,%cl
801051ca:	74 15                	je     801051e1 <pushcli+0x46>
    cpu->intena = eflags & FL_IF;
801051cc:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
801051d2:	8b 55 fc             	mov    -0x4(%ebp),%edx
801051d5:	81 e2 00 02 00 00    	and    $0x200,%edx
801051db:	89 90 b0 00 00 00    	mov    %edx,0xb0(%eax)
}
801051e1:	c9                   	leave  
801051e2:	c3                   	ret    

801051e3 <popcli>:

void
popcli(void)
{
801051e3:	55                   	push   %ebp
801051e4:	89 e5                	mov    %esp,%ebp
801051e6:	83 ec 18             	sub    $0x18,%esp
  if(readeflags()&FL_IF)
801051e9:	e8 fe fd ff ff       	call   80104fec <readeflags>
801051ee:	25 00 02 00 00       	and    $0x200,%eax
801051f3:	85 c0                	test   %eax,%eax
801051f5:	74 0c                	je     80105203 <popcli+0x20>
    panic("popcli - interruptible");
801051f7:	c7 04 24 d8 8a 10 80 	movl   $0x80108ad8,(%esp)
801051fe:	e8 3a b3 ff ff       	call   8010053d <panic>
  if(--cpu->ncli < 0)
80105203:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80105209:	8b 90 ac 00 00 00    	mov    0xac(%eax),%edx
8010520f:	83 ea 01             	sub    $0x1,%edx
80105212:	89 90 ac 00 00 00    	mov    %edx,0xac(%eax)
80105218:	8b 80 ac 00 00 00    	mov    0xac(%eax),%eax
8010521e:	85 c0                	test   %eax,%eax
80105220:	79 0c                	jns    8010522e <popcli+0x4b>
    panic("popcli");
80105222:	c7 04 24 ef 8a 10 80 	movl   $0x80108aef,(%esp)
80105229:	e8 0f b3 ff ff       	call   8010053d <panic>
  if(cpu->ncli == 0 && cpu->intena)
8010522e:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80105234:	8b 80 ac 00 00 00    	mov    0xac(%eax),%eax
8010523a:	85 c0                	test   %eax,%eax
8010523c:	75 15                	jne    80105253 <popcli+0x70>
8010523e:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80105244:	8b 80 b0 00 00 00    	mov    0xb0(%eax),%eax
8010524a:	85 c0                	test   %eax,%eax
8010524c:	74 05                	je     80105253 <popcli+0x70>
    sti();
8010524e:	e8 b4 fd ff ff       	call   80105007 <sti>
}
80105253:	c9                   	leave  
80105254:	c3                   	ret    
80105255:	00 00                	add    %al,(%eax)
	...

80105258 <stosb>:
80105258:	55                   	push   %ebp
80105259:	89 e5                	mov    %esp,%ebp
8010525b:	57                   	push   %edi
8010525c:	53                   	push   %ebx
8010525d:	8b 4d 08             	mov    0x8(%ebp),%ecx
80105260:	8b 55 10             	mov    0x10(%ebp),%edx
80105263:	8b 45 0c             	mov    0xc(%ebp),%eax
80105266:	89 cb                	mov    %ecx,%ebx
80105268:	89 df                	mov    %ebx,%edi
8010526a:	89 d1                	mov    %edx,%ecx
8010526c:	fc                   	cld    
8010526d:	f3 aa                	rep stos %al,%es:(%edi)
8010526f:	89 ca                	mov    %ecx,%edx
80105271:	89 fb                	mov    %edi,%ebx
80105273:	89 5d 08             	mov    %ebx,0x8(%ebp)
80105276:	89 55 10             	mov    %edx,0x10(%ebp)
80105279:	5b                   	pop    %ebx
8010527a:	5f                   	pop    %edi
8010527b:	5d                   	pop    %ebp
8010527c:	c3                   	ret    

8010527d <stosl>:
8010527d:	55                   	push   %ebp
8010527e:	89 e5                	mov    %esp,%ebp
80105280:	57                   	push   %edi
80105281:	53                   	push   %ebx
80105282:	8b 4d 08             	mov    0x8(%ebp),%ecx
80105285:	8b 55 10             	mov    0x10(%ebp),%edx
80105288:	8b 45 0c             	mov    0xc(%ebp),%eax
8010528b:	89 cb                	mov    %ecx,%ebx
8010528d:	89 df                	mov    %ebx,%edi
8010528f:	89 d1                	mov    %edx,%ecx
80105291:	fc                   	cld    
80105292:	f3 ab                	rep stos %eax,%es:(%edi)
80105294:	89 ca                	mov    %ecx,%edx
80105296:	89 fb                	mov    %edi,%ebx
80105298:	89 5d 08             	mov    %ebx,0x8(%ebp)
8010529b:	89 55 10             	mov    %edx,0x10(%ebp)
8010529e:	5b                   	pop    %ebx
8010529f:	5f                   	pop    %edi
801052a0:	5d                   	pop    %ebp
801052a1:	c3                   	ret    

801052a2 <memset>:
801052a2:	55                   	push   %ebp
801052a3:	89 e5                	mov    %esp,%ebp
801052a5:	83 ec 0c             	sub    $0xc,%esp
801052a8:	8b 45 08             	mov    0x8(%ebp),%eax
801052ab:	83 e0 03             	and    $0x3,%eax
801052ae:	85 c0                	test   %eax,%eax
801052b0:	75 49                	jne    801052fb <memset+0x59>
801052b2:	8b 45 10             	mov    0x10(%ebp),%eax
801052b5:	83 e0 03             	and    $0x3,%eax
801052b8:	85 c0                	test   %eax,%eax
801052ba:	75 3f                	jne    801052fb <memset+0x59>
801052bc:	81 65 0c ff 00 00 00 	andl   $0xff,0xc(%ebp)
801052c3:	8b 45 10             	mov    0x10(%ebp),%eax
801052c6:	c1 e8 02             	shr    $0x2,%eax
801052c9:	89 c2                	mov    %eax,%edx
801052cb:	8b 45 0c             	mov    0xc(%ebp),%eax
801052ce:	89 c1                	mov    %eax,%ecx
801052d0:	c1 e1 18             	shl    $0x18,%ecx
801052d3:	8b 45 0c             	mov    0xc(%ebp),%eax
801052d6:	c1 e0 10             	shl    $0x10,%eax
801052d9:	09 c1                	or     %eax,%ecx
801052db:	8b 45 0c             	mov    0xc(%ebp),%eax
801052de:	c1 e0 08             	shl    $0x8,%eax
801052e1:	09 c8                	or     %ecx,%eax
801052e3:	0b 45 0c             	or     0xc(%ebp),%eax
801052e6:	89 54 24 08          	mov    %edx,0x8(%esp)
801052ea:	89 44 24 04          	mov    %eax,0x4(%esp)
801052ee:	8b 45 08             	mov    0x8(%ebp),%eax
801052f1:	89 04 24             	mov    %eax,(%esp)
801052f4:	e8 84 ff ff ff       	call   8010527d <stosl>
801052f9:	eb 19                	jmp    80105314 <memset+0x72>
801052fb:	8b 45 10             	mov    0x10(%ebp),%eax
801052fe:	89 44 24 08          	mov    %eax,0x8(%esp)
80105302:	8b 45 0c             	mov    0xc(%ebp),%eax
80105305:	89 44 24 04          	mov    %eax,0x4(%esp)
80105309:	8b 45 08             	mov    0x8(%ebp),%eax
8010530c:	89 04 24             	mov    %eax,(%esp)
8010530f:	e8 44 ff ff ff       	call   80105258 <stosb>
80105314:	8b 45 08             	mov    0x8(%ebp),%eax
80105317:	c9                   	leave  
80105318:	c3                   	ret    

80105319 <memcmp>:
80105319:	55                   	push   %ebp
8010531a:	89 e5                	mov    %esp,%ebp
8010531c:	83 ec 10             	sub    $0x10,%esp
8010531f:	8b 45 08             	mov    0x8(%ebp),%eax
80105322:	89 45 fc             	mov    %eax,-0x4(%ebp)
80105325:	8b 45 0c             	mov    0xc(%ebp),%eax
80105328:	89 45 f8             	mov    %eax,-0x8(%ebp)
8010532b:	eb 32                	jmp    8010535f <memcmp+0x46>
8010532d:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105330:	0f b6 10             	movzbl (%eax),%edx
80105333:	8b 45 f8             	mov    -0x8(%ebp),%eax
80105336:	0f b6 00             	movzbl (%eax),%eax
80105339:	38 c2                	cmp    %al,%dl
8010533b:	74 1a                	je     80105357 <memcmp+0x3e>
8010533d:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105340:	0f b6 00             	movzbl (%eax),%eax
80105343:	0f b6 d0             	movzbl %al,%edx
80105346:	8b 45 f8             	mov    -0x8(%ebp),%eax
80105349:	0f b6 00             	movzbl (%eax),%eax
8010534c:	0f b6 c0             	movzbl %al,%eax
8010534f:	89 d1                	mov    %edx,%ecx
80105351:	29 c1                	sub    %eax,%ecx
80105353:	89 c8                	mov    %ecx,%eax
80105355:	eb 1c                	jmp    80105373 <memcmp+0x5a>
80105357:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
8010535b:	83 45 f8 01          	addl   $0x1,-0x8(%ebp)
8010535f:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80105363:	0f 95 c0             	setne  %al
80105366:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
8010536a:	84 c0                	test   %al,%al
8010536c:	75 bf                	jne    8010532d <memcmp+0x14>
8010536e:	b8 00 00 00 00       	mov    $0x0,%eax
80105373:	c9                   	leave  
80105374:	c3                   	ret    

80105375 <memmove>:
80105375:	55                   	push   %ebp
80105376:	89 e5                	mov    %esp,%ebp
80105378:	83 ec 10             	sub    $0x10,%esp
8010537b:	8b 45 0c             	mov    0xc(%ebp),%eax
8010537e:	89 45 fc             	mov    %eax,-0x4(%ebp)
80105381:	8b 45 08             	mov    0x8(%ebp),%eax
80105384:	89 45 f8             	mov    %eax,-0x8(%ebp)
80105387:	8b 45 fc             	mov    -0x4(%ebp),%eax
8010538a:	3b 45 f8             	cmp    -0x8(%ebp),%eax
8010538d:	73 54                	jae    801053e3 <memmove+0x6e>
8010538f:	8b 45 10             	mov    0x10(%ebp),%eax
80105392:	8b 55 fc             	mov    -0x4(%ebp),%edx
80105395:	01 d0                	add    %edx,%eax
80105397:	3b 45 f8             	cmp    -0x8(%ebp),%eax
8010539a:	76 47                	jbe    801053e3 <memmove+0x6e>
8010539c:	8b 45 10             	mov    0x10(%ebp),%eax
8010539f:	01 45 fc             	add    %eax,-0x4(%ebp)
801053a2:	8b 45 10             	mov    0x10(%ebp),%eax
801053a5:	01 45 f8             	add    %eax,-0x8(%ebp)
801053a8:	eb 13                	jmp    801053bd <memmove+0x48>
801053aa:	83 6d f8 01          	subl   $0x1,-0x8(%ebp)
801053ae:	83 6d fc 01          	subl   $0x1,-0x4(%ebp)
801053b2:	8b 45 fc             	mov    -0x4(%ebp),%eax
801053b5:	0f b6 10             	movzbl (%eax),%edx
801053b8:	8b 45 f8             	mov    -0x8(%ebp),%eax
801053bb:	88 10                	mov    %dl,(%eax)
801053bd:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801053c1:	0f 95 c0             	setne  %al
801053c4:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
801053c8:	84 c0                	test   %al,%al
801053ca:	75 de                	jne    801053aa <memmove+0x35>
801053cc:	eb 25                	jmp    801053f3 <memmove+0x7e>
801053ce:	8b 45 fc             	mov    -0x4(%ebp),%eax
801053d1:	0f b6 10             	movzbl (%eax),%edx
801053d4:	8b 45 f8             	mov    -0x8(%ebp),%eax
801053d7:	88 10                	mov    %dl,(%eax)
801053d9:	83 45 f8 01          	addl   $0x1,-0x8(%ebp)
801053dd:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
801053e1:	eb 01                	jmp    801053e4 <memmove+0x6f>
801053e3:	90                   	nop
801053e4:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801053e8:	0f 95 c0             	setne  %al
801053eb:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
801053ef:	84 c0                	test   %al,%al
801053f1:	75 db                	jne    801053ce <memmove+0x59>
801053f3:	8b 45 08             	mov    0x8(%ebp),%eax
801053f6:	c9                   	leave  
801053f7:	c3                   	ret    

801053f8 <memcpy>:
801053f8:	55                   	push   %ebp
801053f9:	89 e5                	mov    %esp,%ebp
801053fb:	83 ec 0c             	sub    $0xc,%esp
801053fe:	8b 45 10             	mov    0x10(%ebp),%eax
80105401:	89 44 24 08          	mov    %eax,0x8(%esp)
80105405:	8b 45 0c             	mov    0xc(%ebp),%eax
80105408:	89 44 24 04          	mov    %eax,0x4(%esp)
8010540c:	8b 45 08             	mov    0x8(%ebp),%eax
8010540f:	89 04 24             	mov    %eax,(%esp)
80105412:	e8 5e ff ff ff       	call   80105375 <memmove>
80105417:	c9                   	leave  
80105418:	c3                   	ret    

80105419 <strncmp>:
80105419:	55                   	push   %ebp
8010541a:	89 e5                	mov    %esp,%ebp
8010541c:	eb 0c                	jmp    8010542a <strncmp+0x11>
8010541e:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
80105422:	83 45 08 01          	addl   $0x1,0x8(%ebp)
80105426:	83 45 0c 01          	addl   $0x1,0xc(%ebp)
8010542a:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
8010542e:	74 1a                	je     8010544a <strncmp+0x31>
80105430:	8b 45 08             	mov    0x8(%ebp),%eax
80105433:	0f b6 00             	movzbl (%eax),%eax
80105436:	84 c0                	test   %al,%al
80105438:	74 10                	je     8010544a <strncmp+0x31>
8010543a:	8b 45 08             	mov    0x8(%ebp),%eax
8010543d:	0f b6 10             	movzbl (%eax),%edx
80105440:	8b 45 0c             	mov    0xc(%ebp),%eax
80105443:	0f b6 00             	movzbl (%eax),%eax
80105446:	38 c2                	cmp    %al,%dl
80105448:	74 d4                	je     8010541e <strncmp+0x5>
8010544a:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
8010544e:	75 07                	jne    80105457 <strncmp+0x3e>
80105450:	b8 00 00 00 00       	mov    $0x0,%eax
80105455:	eb 18                	jmp    8010546f <strncmp+0x56>
80105457:	8b 45 08             	mov    0x8(%ebp),%eax
8010545a:	0f b6 00             	movzbl (%eax),%eax
8010545d:	0f b6 d0             	movzbl %al,%edx
80105460:	8b 45 0c             	mov    0xc(%ebp),%eax
80105463:	0f b6 00             	movzbl (%eax),%eax
80105466:	0f b6 c0             	movzbl %al,%eax
80105469:	89 d1                	mov    %edx,%ecx
8010546b:	29 c1                	sub    %eax,%ecx
8010546d:	89 c8                	mov    %ecx,%eax
8010546f:	5d                   	pop    %ebp
80105470:	c3                   	ret    

80105471 <strncpy>:
80105471:	55                   	push   %ebp
80105472:	89 e5                	mov    %esp,%ebp
80105474:	83 ec 10             	sub    $0x10,%esp
80105477:	8b 45 08             	mov    0x8(%ebp),%eax
8010547a:	89 45 fc             	mov    %eax,-0x4(%ebp)
8010547d:	90                   	nop
8010547e:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80105482:	0f 9f c0             	setg   %al
80105485:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
80105489:	84 c0                	test   %al,%al
8010548b:	74 30                	je     801054bd <strncpy+0x4c>
8010548d:	8b 45 0c             	mov    0xc(%ebp),%eax
80105490:	0f b6 10             	movzbl (%eax),%edx
80105493:	8b 45 08             	mov    0x8(%ebp),%eax
80105496:	88 10                	mov    %dl,(%eax)
80105498:	8b 45 08             	mov    0x8(%ebp),%eax
8010549b:	0f b6 00             	movzbl (%eax),%eax
8010549e:	84 c0                	test   %al,%al
801054a0:	0f 95 c0             	setne  %al
801054a3:	83 45 08 01          	addl   $0x1,0x8(%ebp)
801054a7:	83 45 0c 01          	addl   $0x1,0xc(%ebp)
801054ab:	84 c0                	test   %al,%al
801054ad:	75 cf                	jne    8010547e <strncpy+0xd>
801054af:	eb 0c                	jmp    801054bd <strncpy+0x4c>
801054b1:	8b 45 08             	mov    0x8(%ebp),%eax
801054b4:	c6 00 00             	movb   $0x0,(%eax)
801054b7:	83 45 08 01          	addl   $0x1,0x8(%ebp)
801054bb:	eb 01                	jmp    801054be <strncpy+0x4d>
801054bd:	90                   	nop
801054be:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801054c2:	0f 9f c0             	setg   %al
801054c5:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
801054c9:	84 c0                	test   %al,%al
801054cb:	75 e4                	jne    801054b1 <strncpy+0x40>
801054cd:	8b 45 fc             	mov    -0x4(%ebp),%eax
801054d0:	c9                   	leave  
801054d1:	c3                   	ret    

801054d2 <safestrcpy>:
801054d2:	55                   	push   %ebp
801054d3:	89 e5                	mov    %esp,%ebp
801054d5:	83 ec 10             	sub    $0x10,%esp
801054d8:	8b 45 08             	mov    0x8(%ebp),%eax
801054db:	89 45 fc             	mov    %eax,-0x4(%ebp)
801054de:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801054e2:	7f 05                	jg     801054e9 <safestrcpy+0x17>
801054e4:	8b 45 fc             	mov    -0x4(%ebp),%eax
801054e7:	eb 35                	jmp    8010551e <safestrcpy+0x4c>
801054e9:	83 6d 10 01          	subl   $0x1,0x10(%ebp)
801054ed:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801054f1:	7e 22                	jle    80105515 <safestrcpy+0x43>
801054f3:	8b 45 0c             	mov    0xc(%ebp),%eax
801054f6:	0f b6 10             	movzbl (%eax),%edx
801054f9:	8b 45 08             	mov    0x8(%ebp),%eax
801054fc:	88 10                	mov    %dl,(%eax)
801054fe:	8b 45 08             	mov    0x8(%ebp),%eax
80105501:	0f b6 00             	movzbl (%eax),%eax
80105504:	84 c0                	test   %al,%al
80105506:	0f 95 c0             	setne  %al
80105509:	83 45 08 01          	addl   $0x1,0x8(%ebp)
8010550d:	83 45 0c 01          	addl   $0x1,0xc(%ebp)
80105511:	84 c0                	test   %al,%al
80105513:	75 d4                	jne    801054e9 <safestrcpy+0x17>
80105515:	8b 45 08             	mov    0x8(%ebp),%eax
80105518:	c6 00 00             	movb   $0x0,(%eax)
8010551b:	8b 45 fc             	mov    -0x4(%ebp),%eax
8010551e:	c9                   	leave  
8010551f:	c3                   	ret    

80105520 <strlen>:
80105520:	55                   	push   %ebp
80105521:	89 e5                	mov    %esp,%ebp
80105523:	83 ec 10             	sub    $0x10,%esp
80105526:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
8010552d:	eb 04                	jmp    80105533 <strlen+0x13>
8010552f:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80105533:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105536:	03 45 08             	add    0x8(%ebp),%eax
80105539:	0f b6 00             	movzbl (%eax),%eax
8010553c:	84 c0                	test   %al,%al
8010553e:	75 ef                	jne    8010552f <strlen+0xf>
80105540:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105543:	c9                   	leave  
80105544:	c3                   	ret    
80105545:	00 00                	add    %al,(%eax)
	...

80105548 <swtch>:
80105548:	8b 44 24 04          	mov    0x4(%esp),%eax
8010554c:	8b 54 24 08          	mov    0x8(%esp),%edx
80105550:	55                   	push   %ebp
80105551:	53                   	push   %ebx
80105552:	56                   	push   %esi
80105553:	57                   	push   %edi
80105554:	89 20                	mov    %esp,(%eax)
80105556:	89 d4                	mov    %edx,%esp
80105558:	5f                   	pop    %edi
80105559:	5e                   	pop    %esi
8010555a:	5b                   	pop    %ebx
8010555b:	5d                   	pop    %ebp
8010555c:	c3                   	ret    
8010555d:	00 00                	add    %al,(%eax)
	...

80105560 <fetchint>:
// to a saved program counter, and then the first argument.

// Fetch the int at addr from the current process.
int
fetchint(uint addr, int *ip)
{
80105560:	55                   	push   %ebp
80105561:	89 e5                	mov    %esp,%ebp
  if(addr >= proc->sz || addr+4 > proc->sz)
80105563:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105569:	8b 00                	mov    (%eax),%eax
8010556b:	3b 45 08             	cmp    0x8(%ebp),%eax
8010556e:	76 12                	jbe    80105582 <fetchint+0x22>
80105570:	8b 45 08             	mov    0x8(%ebp),%eax
80105573:	8d 50 04             	lea    0x4(%eax),%edx
80105576:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010557c:	8b 00                	mov    (%eax),%eax
8010557e:	39 c2                	cmp    %eax,%edx
80105580:	76 07                	jbe    80105589 <fetchint+0x29>
    return -1;
80105582:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105587:	eb 0f                	jmp    80105598 <fetchint+0x38>
  *ip = *(int*)(addr);
80105589:	8b 45 08             	mov    0x8(%ebp),%eax
8010558c:	8b 10                	mov    (%eax),%edx
8010558e:	8b 45 0c             	mov    0xc(%ebp),%eax
80105591:	89 10                	mov    %edx,(%eax)
  return 0;
80105593:	b8 00 00 00 00       	mov    $0x0,%eax
}
80105598:	5d                   	pop    %ebp
80105599:	c3                   	ret    

8010559a <fetchstr>:
// Fetch the nul-terminated string at addr from the current process.
// Doesn't actually copy the string - just sets *pp to point at it.
// Returns length of string, not including nul.
int
fetchstr(uint addr, char **pp)
{
8010559a:	55                   	push   %ebp
8010559b:	89 e5                	mov    %esp,%ebp
8010559d:	83 ec 10             	sub    $0x10,%esp
  char *s, *ep;

  if(addr >= proc->sz)
801055a0:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801055a6:	8b 00                	mov    (%eax),%eax
801055a8:	3b 45 08             	cmp    0x8(%ebp),%eax
801055ab:	77 07                	ja     801055b4 <fetchstr+0x1a>
    return -1;
801055ad:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801055b2:	eb 48                	jmp    801055fc <fetchstr+0x62>
  *pp = (char*)addr;
801055b4:	8b 55 08             	mov    0x8(%ebp),%edx
801055b7:	8b 45 0c             	mov    0xc(%ebp),%eax
801055ba:	89 10                	mov    %edx,(%eax)
  ep = (char*)proc->sz;
801055bc:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801055c2:	8b 00                	mov    (%eax),%eax
801055c4:	89 45 f8             	mov    %eax,-0x8(%ebp)
  for(s = *pp; s < ep; s++)
801055c7:	8b 45 0c             	mov    0xc(%ebp),%eax
801055ca:	8b 00                	mov    (%eax),%eax
801055cc:	89 45 fc             	mov    %eax,-0x4(%ebp)
801055cf:	eb 1e                	jmp    801055ef <fetchstr+0x55>
    if(*s == 0)
801055d1:	8b 45 fc             	mov    -0x4(%ebp),%eax
801055d4:	0f b6 00             	movzbl (%eax),%eax
801055d7:	84 c0                	test   %al,%al
801055d9:	75 10                	jne    801055eb <fetchstr+0x51>
      return s - *pp;
801055db:	8b 55 fc             	mov    -0x4(%ebp),%edx
801055de:	8b 45 0c             	mov    0xc(%ebp),%eax
801055e1:	8b 00                	mov    (%eax),%eax
801055e3:	89 d1                	mov    %edx,%ecx
801055e5:	29 c1                	sub    %eax,%ecx
801055e7:	89 c8                	mov    %ecx,%eax
801055e9:	eb 11                	jmp    801055fc <fetchstr+0x62>

  if(addr >= proc->sz)
    return -1;
  *pp = (char*)addr;
  ep = (char*)proc->sz;
  for(s = *pp; s < ep; s++)
801055eb:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
801055ef:	8b 45 fc             	mov    -0x4(%ebp),%eax
801055f2:	3b 45 f8             	cmp    -0x8(%ebp),%eax
801055f5:	72 da                	jb     801055d1 <fetchstr+0x37>
    if(*s == 0)
      return s - *pp;
  return -1;
801055f7:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
801055fc:	c9                   	leave  
801055fd:	c3                   	ret    

801055fe <argint>:

// Fetch the nth 32-bit system call argument.
int
argint(int n, int *ip)
{
801055fe:	55                   	push   %ebp
801055ff:	89 e5                	mov    %esp,%ebp
80105601:	83 ec 08             	sub    $0x8,%esp
  return fetchint(proc->tf->esp + 4 + 4*n, ip);
80105604:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010560a:	8b 40 18             	mov    0x18(%eax),%eax
8010560d:	8b 50 44             	mov    0x44(%eax),%edx
80105610:	8b 45 08             	mov    0x8(%ebp),%eax
80105613:	c1 e0 02             	shl    $0x2,%eax
80105616:	01 d0                	add    %edx,%eax
80105618:	8d 50 04             	lea    0x4(%eax),%edx
8010561b:	8b 45 0c             	mov    0xc(%ebp),%eax
8010561e:	89 44 24 04          	mov    %eax,0x4(%esp)
80105622:	89 14 24             	mov    %edx,(%esp)
80105625:	e8 36 ff ff ff       	call   80105560 <fetchint>
}
8010562a:	c9                   	leave  
8010562b:	c3                   	ret    

8010562c <argptr>:
// Fetch the nth word-sized system call argument as a pointer
// to a block of memory of size n bytes.  Check that the pointer
// lies within the process address space.
int
argptr(int n, char **pp, int size)
{
8010562c:	55                   	push   %ebp
8010562d:	89 e5                	mov    %esp,%ebp
8010562f:	83 ec 18             	sub    $0x18,%esp
  int i;
  
  if(argint(n, &i) < 0)
80105632:	8d 45 fc             	lea    -0x4(%ebp),%eax
80105635:	89 44 24 04          	mov    %eax,0x4(%esp)
80105639:	8b 45 08             	mov    0x8(%ebp),%eax
8010563c:	89 04 24             	mov    %eax,(%esp)
8010563f:	e8 ba ff ff ff       	call   801055fe <argint>
80105644:	85 c0                	test   %eax,%eax
80105646:	79 07                	jns    8010564f <argptr+0x23>
    return -1;
80105648:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010564d:	eb 3d                	jmp    8010568c <argptr+0x60>
  if((uint)i >= proc->sz || (uint)i+size > proc->sz)
8010564f:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105652:	89 c2                	mov    %eax,%edx
80105654:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010565a:	8b 00                	mov    (%eax),%eax
8010565c:	39 c2                	cmp    %eax,%edx
8010565e:	73 16                	jae    80105676 <argptr+0x4a>
80105660:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105663:	89 c2                	mov    %eax,%edx
80105665:	8b 45 10             	mov    0x10(%ebp),%eax
80105668:	01 c2                	add    %eax,%edx
8010566a:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105670:	8b 00                	mov    (%eax),%eax
80105672:	39 c2                	cmp    %eax,%edx
80105674:	76 07                	jbe    8010567d <argptr+0x51>
    return -1;
80105676:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010567b:	eb 0f                	jmp    8010568c <argptr+0x60>
  *pp = (char*)i;
8010567d:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105680:	89 c2                	mov    %eax,%edx
80105682:	8b 45 0c             	mov    0xc(%ebp),%eax
80105685:	89 10                	mov    %edx,(%eax)
  return 0;
80105687:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010568c:	c9                   	leave  
8010568d:	c3                   	ret    

8010568e <argstr>:
// Check that the pointer is valid and the string is nul-terminated.
// (There is no shared writable memory, so the string can't change
// between this check and being used by the kernel.)
int
argstr(int n, char **pp)
{
8010568e:	55                   	push   %ebp
8010568f:	89 e5                	mov    %esp,%ebp
80105691:	83 ec 18             	sub    $0x18,%esp
  int addr;
  if(argint(n, &addr) < 0)
80105694:	8d 45 fc             	lea    -0x4(%ebp),%eax
80105697:	89 44 24 04          	mov    %eax,0x4(%esp)
8010569b:	8b 45 08             	mov    0x8(%ebp),%eax
8010569e:	89 04 24             	mov    %eax,(%esp)
801056a1:	e8 58 ff ff ff       	call   801055fe <argint>
801056a6:	85 c0                	test   %eax,%eax
801056a8:	79 07                	jns    801056b1 <argstr+0x23>
    return -1;
801056aa:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801056af:	eb 12                	jmp    801056c3 <argstr+0x35>
  return fetchstr(addr, pp);
801056b1:	8b 45 fc             	mov    -0x4(%ebp),%eax
801056b4:	8b 55 0c             	mov    0xc(%ebp),%edx
801056b7:	89 54 24 04          	mov    %edx,0x4(%esp)
801056bb:	89 04 24             	mov    %eax,(%esp)
801056be:	e8 d7 fe ff ff       	call   8010559a <fetchstr>
}
801056c3:	c9                   	leave  
801056c4:	c3                   	ret    

801056c5 <syscall>:
[SYS_ps]      sys_ps,
};

void
syscall(void)
{
801056c5:	55                   	push   %ebp
801056c6:	89 e5                	mov    %esp,%ebp
801056c8:	53                   	push   %ebx
801056c9:	83 ec 24             	sub    $0x24,%esp
  int num;

  num = proc->tf->eax;
801056cc:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801056d2:	8b 40 18             	mov    0x18(%eax),%eax
801056d5:	8b 40 1c             	mov    0x1c(%eax),%eax
801056d8:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
801056db:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801056df:	7e 30                	jle    80105711 <syscall+0x4c>
801056e1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801056e4:	83 f8 16             	cmp    $0x16,%eax
801056e7:	77 28                	ja     80105711 <syscall+0x4c>
801056e9:	8b 45 f4             	mov    -0xc(%ebp),%eax
801056ec:	8b 04 85 40 b0 10 80 	mov    -0x7fef4fc0(,%eax,4),%eax
801056f3:	85 c0                	test   %eax,%eax
801056f5:	74 1a                	je     80105711 <syscall+0x4c>
    proc->tf->eax = syscalls[num]();
801056f7:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801056fd:	8b 58 18             	mov    0x18(%eax),%ebx
80105700:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105703:	8b 04 85 40 b0 10 80 	mov    -0x7fef4fc0(,%eax,4),%eax
8010570a:	ff d0                	call   *%eax
8010570c:	89 43 1c             	mov    %eax,0x1c(%ebx)
8010570f:	eb 3d                	jmp    8010574e <syscall+0x89>
  } else {
    cprintf("%d %s: unknown sys call %d\n",
            proc->pid, proc->name, num);
80105711:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105717:	8d 48 6c             	lea    0x6c(%eax),%ecx
8010571a:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax

  num = proc->tf->eax;
  if(num > 0 && num < NELEM(syscalls) && syscalls[num]) {
    proc->tf->eax = syscalls[num]();
  } else {
    cprintf("%d %s: unknown sys call %d\n",
80105720:	8b 40 10             	mov    0x10(%eax),%eax
80105723:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105726:	89 54 24 0c          	mov    %edx,0xc(%esp)
8010572a:	89 4c 24 08          	mov    %ecx,0x8(%esp)
8010572e:	89 44 24 04          	mov    %eax,0x4(%esp)
80105732:	c7 04 24 f6 8a 10 80 	movl   $0x80108af6,(%esp)
80105739:	e8 63 ac ff ff       	call   801003a1 <cprintf>
            proc->pid, proc->name, num);
    proc->tf->eax = -1;
8010573e:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105744:	8b 40 18             	mov    0x18(%eax),%eax
80105747:	c7 40 1c ff ff ff ff 	movl   $0xffffffff,0x1c(%eax)
  }
}
8010574e:	83 c4 24             	add    $0x24,%esp
80105751:	5b                   	pop    %ebx
80105752:	5d                   	pop    %ebp
80105753:	c3                   	ret    

80105754 <argfd>:

// Fetch the nth word-sized system call argument as a file descriptor
// and return both the descriptor and the corresponding struct file.
static int
argfd(int n, int *pfd, struct file **pf)
{
80105754:	55                   	push   %ebp
80105755:	89 e5                	mov    %esp,%ebp
80105757:	83 ec 28             	sub    $0x28,%esp
  int fd;
  struct file *f;

  if(argint(n, &fd) < 0)
8010575a:	8d 45 f0             	lea    -0x10(%ebp),%eax
8010575d:	89 44 24 04          	mov    %eax,0x4(%esp)
80105761:	8b 45 08             	mov    0x8(%ebp),%eax
80105764:	89 04 24             	mov    %eax,(%esp)
80105767:	e8 92 fe ff ff       	call   801055fe <argint>
8010576c:	85 c0                	test   %eax,%eax
8010576e:	79 07                	jns    80105777 <argfd+0x23>
    return -1;
80105770:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105775:	eb 50                	jmp    801057c7 <argfd+0x73>
  if(fd < 0 || fd >= NOFILE || (f=proc->ofile[fd]) == 0)
80105777:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010577a:	85 c0                	test   %eax,%eax
8010577c:	78 21                	js     8010579f <argfd+0x4b>
8010577e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105781:	83 f8 0f             	cmp    $0xf,%eax
80105784:	7f 19                	jg     8010579f <argfd+0x4b>
80105786:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010578c:	8b 55 f0             	mov    -0x10(%ebp),%edx
8010578f:	83 c2 08             	add    $0x8,%edx
80105792:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
80105796:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105799:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010579d:	75 07                	jne    801057a6 <argfd+0x52>
    return -1;
8010579f:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801057a4:	eb 21                	jmp    801057c7 <argfd+0x73>
  if(pfd)
801057a6:	83 7d 0c 00          	cmpl   $0x0,0xc(%ebp)
801057aa:	74 08                	je     801057b4 <argfd+0x60>
    *pfd = fd;
801057ac:	8b 55 f0             	mov    -0x10(%ebp),%edx
801057af:	8b 45 0c             	mov    0xc(%ebp),%eax
801057b2:	89 10                	mov    %edx,(%eax)
  if(pf)
801057b4:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
801057b8:	74 08                	je     801057c2 <argfd+0x6e>
    *pf = f;
801057ba:	8b 45 10             	mov    0x10(%ebp),%eax
801057bd:	8b 55 f4             	mov    -0xc(%ebp),%edx
801057c0:	89 10                	mov    %edx,(%eax)
  return 0;
801057c2:	b8 00 00 00 00       	mov    $0x0,%eax
}
801057c7:	c9                   	leave  
801057c8:	c3                   	ret    

801057c9 <fdalloc>:

// Allocate a file descriptor for the given file.
// Takes over file reference from caller on success.
static int
fdalloc(struct file *f)
{
801057c9:	55                   	push   %ebp
801057ca:	89 e5                	mov    %esp,%ebp
801057cc:	83 ec 10             	sub    $0x10,%esp
  int fd;

  for(fd = 0; fd < NOFILE; fd++){
801057cf:	c7 45 fc 00 00 00 00 	movl   $0x0,-0x4(%ebp)
801057d6:	eb 30                	jmp    80105808 <fdalloc+0x3f>
    if(proc->ofile[fd] == 0){
801057d8:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801057de:	8b 55 fc             	mov    -0x4(%ebp),%edx
801057e1:	83 c2 08             	add    $0x8,%edx
801057e4:	8b 44 90 08          	mov    0x8(%eax,%edx,4),%eax
801057e8:	85 c0                	test   %eax,%eax
801057ea:	75 18                	jne    80105804 <fdalloc+0x3b>
      proc->ofile[fd] = f;
801057ec:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801057f2:	8b 55 fc             	mov    -0x4(%ebp),%edx
801057f5:	8d 4a 08             	lea    0x8(%edx),%ecx
801057f8:	8b 55 08             	mov    0x8(%ebp),%edx
801057fb:	89 54 88 08          	mov    %edx,0x8(%eax,%ecx,4)
      return fd;
801057ff:	8b 45 fc             	mov    -0x4(%ebp),%eax
80105802:	eb 0f                	jmp    80105813 <fdalloc+0x4a>
static int
fdalloc(struct file *f)
{
  int fd;

  for(fd = 0; fd < NOFILE; fd++){
80105804:	83 45 fc 01          	addl   $0x1,-0x4(%ebp)
80105808:	83 7d fc 0f          	cmpl   $0xf,-0x4(%ebp)
8010580c:	7e ca                	jle    801057d8 <fdalloc+0xf>
    if(proc->ofile[fd] == 0){
      proc->ofile[fd] = f;
      return fd;
    }
  }
  return -1;
8010580e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80105813:	c9                   	leave  
80105814:	c3                   	ret    

80105815 <sys_dup>:

int
sys_dup(void)
{
80105815:	55                   	push   %ebp
80105816:	89 e5                	mov    %esp,%ebp
80105818:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  int fd;
  
  if(argfd(0, 0, &f) < 0)
8010581b:	8d 45 f0             	lea    -0x10(%ebp),%eax
8010581e:	89 44 24 08          	mov    %eax,0x8(%esp)
80105822:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80105829:	00 
8010582a:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105831:	e8 1e ff ff ff       	call   80105754 <argfd>
80105836:	85 c0                	test   %eax,%eax
80105838:	79 07                	jns    80105841 <sys_dup+0x2c>
    return -1;
8010583a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010583f:	eb 29                	jmp    8010586a <sys_dup+0x55>
  if((fd=fdalloc(f)) < 0)
80105841:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105844:	89 04 24             	mov    %eax,(%esp)
80105847:	e8 7d ff ff ff       	call   801057c9 <fdalloc>
8010584c:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010584f:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105853:	79 07                	jns    8010585c <sys_dup+0x47>
    return -1;
80105855:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010585a:	eb 0e                	jmp    8010586a <sys_dup+0x55>
  filedup(f);
8010585c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010585f:	89 04 24             	mov    %eax,(%esp)
80105862:	e8 0d b7 ff ff       	call   80100f74 <filedup>
  return fd;
80105867:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
8010586a:	c9                   	leave  
8010586b:	c3                   	ret    

8010586c <sys_read>:

int
sys_read(void)
{
8010586c:	55                   	push   %ebp
8010586d:	89 e5                	mov    %esp,%ebp
8010586f:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  int n;
  char *p;

  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argptr(1, &p, n) < 0)
80105872:	8d 45 f4             	lea    -0xc(%ebp),%eax
80105875:	89 44 24 08          	mov    %eax,0x8(%esp)
80105879:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80105880:	00 
80105881:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105888:	e8 c7 fe ff ff       	call   80105754 <argfd>
8010588d:	85 c0                	test   %eax,%eax
8010588f:	78 35                	js     801058c6 <sys_read+0x5a>
80105891:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105894:	89 44 24 04          	mov    %eax,0x4(%esp)
80105898:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
8010589f:	e8 5a fd ff ff       	call   801055fe <argint>
801058a4:	85 c0                	test   %eax,%eax
801058a6:	78 1e                	js     801058c6 <sys_read+0x5a>
801058a8:	8b 45 f0             	mov    -0x10(%ebp),%eax
801058ab:	89 44 24 08          	mov    %eax,0x8(%esp)
801058af:	8d 45 ec             	lea    -0x14(%ebp),%eax
801058b2:	89 44 24 04          	mov    %eax,0x4(%esp)
801058b6:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801058bd:	e8 6a fd ff ff       	call   8010562c <argptr>
801058c2:	85 c0                	test   %eax,%eax
801058c4:	79 07                	jns    801058cd <sys_read+0x61>
    return -1;
801058c6:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801058cb:	eb 19                	jmp    801058e6 <sys_read+0x7a>
  return fileread(f, p, n);
801058cd:	8b 4d f0             	mov    -0x10(%ebp),%ecx
801058d0:	8b 55 ec             	mov    -0x14(%ebp),%edx
801058d3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801058d6:	89 4c 24 08          	mov    %ecx,0x8(%esp)
801058da:	89 54 24 04          	mov    %edx,0x4(%esp)
801058de:	89 04 24             	mov    %eax,(%esp)
801058e1:	e8 fb b7 ff ff       	call   801010e1 <fileread>
}
801058e6:	c9                   	leave  
801058e7:	c3                   	ret    

801058e8 <sys_write>:

int
sys_write(void)
{
801058e8:	55                   	push   %ebp
801058e9:	89 e5                	mov    %esp,%ebp
801058eb:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  int n;
  char *p;

  if(argfd(0, 0, &f) < 0 || argint(2, &n) < 0 || argptr(1, &p, n) < 0)
801058ee:	8d 45 f4             	lea    -0xc(%ebp),%eax
801058f1:	89 44 24 08          	mov    %eax,0x8(%esp)
801058f5:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801058fc:	00 
801058fd:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105904:	e8 4b fe ff ff       	call   80105754 <argfd>
80105909:	85 c0                	test   %eax,%eax
8010590b:	78 35                	js     80105942 <sys_write+0x5a>
8010590d:	8d 45 f0             	lea    -0x10(%ebp),%eax
80105910:	89 44 24 04          	mov    %eax,0x4(%esp)
80105914:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
8010591b:	e8 de fc ff ff       	call   801055fe <argint>
80105920:	85 c0                	test   %eax,%eax
80105922:	78 1e                	js     80105942 <sys_write+0x5a>
80105924:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105927:	89 44 24 08          	mov    %eax,0x8(%esp)
8010592b:	8d 45 ec             	lea    -0x14(%ebp),%eax
8010592e:	89 44 24 04          	mov    %eax,0x4(%esp)
80105932:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80105939:	e8 ee fc ff ff       	call   8010562c <argptr>
8010593e:	85 c0                	test   %eax,%eax
80105940:	79 07                	jns    80105949 <sys_write+0x61>
    return -1;
80105942:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105947:	eb 19                	jmp    80105962 <sys_write+0x7a>
  return filewrite(f, p, n);
80105949:	8b 4d f0             	mov    -0x10(%ebp),%ecx
8010594c:	8b 55 ec             	mov    -0x14(%ebp),%edx
8010594f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105952:	89 4c 24 08          	mov    %ecx,0x8(%esp)
80105956:	89 54 24 04          	mov    %edx,0x4(%esp)
8010595a:	89 04 24             	mov    %eax,(%esp)
8010595d:	e8 3b b8 ff ff       	call   8010119d <filewrite>
}
80105962:	c9                   	leave  
80105963:	c3                   	ret    

80105964 <sys_close>:

int
sys_close(void)
{
80105964:	55                   	push   %ebp
80105965:	89 e5                	mov    %esp,%ebp
80105967:	83 ec 28             	sub    $0x28,%esp
  int fd;
  struct file *f;
  
  if(argfd(0, &fd, &f) < 0)
8010596a:	8d 45 f0             	lea    -0x10(%ebp),%eax
8010596d:	89 44 24 08          	mov    %eax,0x8(%esp)
80105971:	8d 45 f4             	lea    -0xc(%ebp),%eax
80105974:	89 44 24 04          	mov    %eax,0x4(%esp)
80105978:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
8010597f:	e8 d0 fd ff ff       	call   80105754 <argfd>
80105984:	85 c0                	test   %eax,%eax
80105986:	79 07                	jns    8010598f <sys_close+0x2b>
    return -1;
80105988:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010598d:	eb 24                	jmp    801059b3 <sys_close+0x4f>
  proc->ofile[fd] = 0;
8010598f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80105995:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105998:	83 c2 08             	add    $0x8,%edx
8010599b:	c7 44 90 08 00 00 00 	movl   $0x0,0x8(%eax,%edx,4)
801059a2:	00 
  fileclose(f);
801059a3:	8b 45 f0             	mov    -0x10(%ebp),%eax
801059a6:	89 04 24             	mov    %eax,(%esp)
801059a9:	e8 0e b6 ff ff       	call   80100fbc <fileclose>
  return 0;
801059ae:	b8 00 00 00 00       	mov    $0x0,%eax
}
801059b3:	c9                   	leave  
801059b4:	c3                   	ret    

801059b5 <sys_fstat>:

int
sys_fstat(void)
{
801059b5:	55                   	push   %ebp
801059b6:	89 e5                	mov    %esp,%ebp
801059b8:	83 ec 28             	sub    $0x28,%esp
  struct file *f;
  struct stat *st;
  
  if(argfd(0, 0, &f) < 0 || argptr(1, (void*)&st, sizeof(*st)) < 0)
801059bb:	8d 45 f4             	lea    -0xc(%ebp),%eax
801059be:	89 44 24 08          	mov    %eax,0x8(%esp)
801059c2:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801059c9:	00 
801059ca:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801059d1:	e8 7e fd ff ff       	call   80105754 <argfd>
801059d6:	85 c0                	test   %eax,%eax
801059d8:	78 1f                	js     801059f9 <sys_fstat+0x44>
801059da:	c7 44 24 08 14 00 00 	movl   $0x14,0x8(%esp)
801059e1:	00 
801059e2:	8d 45 f0             	lea    -0x10(%ebp),%eax
801059e5:	89 44 24 04          	mov    %eax,0x4(%esp)
801059e9:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801059f0:	e8 37 fc ff ff       	call   8010562c <argptr>
801059f5:	85 c0                	test   %eax,%eax
801059f7:	79 07                	jns    80105a00 <sys_fstat+0x4b>
    return -1;
801059f9:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801059fe:	eb 12                	jmp    80105a12 <sys_fstat+0x5d>
  return filestat(f, st);
80105a00:	8b 55 f0             	mov    -0x10(%ebp),%edx
80105a03:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a06:	89 54 24 04          	mov    %edx,0x4(%esp)
80105a0a:	89 04 24             	mov    %eax,(%esp)
80105a0d:	e8 80 b6 ff ff       	call   80101092 <filestat>
}
80105a12:	c9                   	leave  
80105a13:	c3                   	ret    

80105a14 <sys_link>:

// Create the path new as a link to the same inode as old.
int
sys_link(void)
{
80105a14:	55                   	push   %ebp
80105a15:	89 e5                	mov    %esp,%ebp
80105a17:	83 ec 38             	sub    $0x38,%esp
  char name[DIRSIZ], *new, *old;
  struct inode *dp, *ip;

  if(argstr(0, &old) < 0 || argstr(1, &new) < 0)
80105a1a:	8d 45 d8             	lea    -0x28(%ebp),%eax
80105a1d:	89 44 24 04          	mov    %eax,0x4(%esp)
80105a21:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105a28:	e8 61 fc ff ff       	call   8010568e <argstr>
80105a2d:	85 c0                	test   %eax,%eax
80105a2f:	78 17                	js     80105a48 <sys_link+0x34>
80105a31:	8d 45 dc             	lea    -0x24(%ebp),%eax
80105a34:	89 44 24 04          	mov    %eax,0x4(%esp)
80105a38:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80105a3f:	e8 4a fc ff ff       	call   8010568e <argstr>
80105a44:	85 c0                	test   %eax,%eax
80105a46:	79 0a                	jns    80105a52 <sys_link+0x3e>
    return -1;
80105a48:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105a4d:	e9 3c 01 00 00       	jmp    80105b8e <sys_link+0x17a>
  if((ip = namei(old)) == 0)
80105a52:	8b 45 d8             	mov    -0x28(%ebp),%eax
80105a55:	89 04 24             	mov    %eax,(%esp)
80105a58:	e8 a5 c9 ff ff       	call   80102402 <namei>
80105a5d:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105a60:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105a64:	75 0a                	jne    80105a70 <sys_link+0x5c>
    return -1;
80105a66:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105a6b:	e9 1e 01 00 00       	jmp    80105b8e <sys_link+0x17a>

  begin_trans();
80105a70:	e8 a0 d7 ff ff       	call   80103215 <begin_trans>

  ilock(ip);
80105a75:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a78:	89 04 24             	mov    %eax,(%esp)
80105a7b:	e8 e0 bd ff ff       	call   80101860 <ilock>
  if(ip->type == T_DIR){
80105a80:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a83:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80105a87:	66 83 f8 01          	cmp    $0x1,%ax
80105a8b:	75 1a                	jne    80105aa7 <sys_link+0x93>
    iunlockput(ip);
80105a8d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105a90:	89 04 24             	mov    %eax,(%esp)
80105a93:	e8 4c c0 ff ff       	call   80101ae4 <iunlockput>
    commit_trans();
80105a98:	e8 c1 d7 ff ff       	call   8010325e <commit_trans>
    return -1;
80105a9d:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105aa2:	e9 e7 00 00 00       	jmp    80105b8e <sys_link+0x17a>
  }

  ip->nlink++;
80105aa7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105aaa:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105aae:	8d 50 01             	lea    0x1(%eax),%edx
80105ab1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105ab4:	66 89 50 16          	mov    %dx,0x16(%eax)
  iupdate(ip);
80105ab8:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105abb:	89 04 24             	mov    %eax,(%esp)
80105abe:	e8 e1 bb ff ff       	call   801016a4 <iupdate>
  iunlock(ip);
80105ac3:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105ac6:	89 04 24             	mov    %eax,(%esp)
80105ac9:	e8 e0 be ff ff       	call   801019ae <iunlock>

  if((dp = nameiparent(new, name)) == 0)
80105ace:	8b 45 dc             	mov    -0x24(%ebp),%eax
80105ad1:	8d 55 e2             	lea    -0x1e(%ebp),%edx
80105ad4:	89 54 24 04          	mov    %edx,0x4(%esp)
80105ad8:	89 04 24             	mov    %eax,(%esp)
80105adb:	e8 44 c9 ff ff       	call   80102424 <nameiparent>
80105ae0:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105ae3:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105ae7:	74 68                	je     80105b51 <sys_link+0x13d>
    goto bad;
  ilock(dp);
80105ae9:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105aec:	89 04 24             	mov    %eax,(%esp)
80105aef:	e8 6c bd ff ff       	call   80101860 <ilock>
  if(dp->dev != ip->dev || dirlink(dp, name, ip->inum) < 0){
80105af4:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105af7:	8b 10                	mov    (%eax),%edx
80105af9:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105afc:	8b 00                	mov    (%eax),%eax
80105afe:	39 c2                	cmp    %eax,%edx
80105b00:	75 20                	jne    80105b22 <sys_link+0x10e>
80105b02:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105b05:	8b 40 04             	mov    0x4(%eax),%eax
80105b08:	89 44 24 08          	mov    %eax,0x8(%esp)
80105b0c:	8d 45 e2             	lea    -0x1e(%ebp),%eax
80105b0f:	89 44 24 04          	mov    %eax,0x4(%esp)
80105b13:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105b16:	89 04 24             	mov    %eax,(%esp)
80105b19:	e8 23 c6 ff ff       	call   80102141 <dirlink>
80105b1e:	85 c0                	test   %eax,%eax
80105b20:	79 0d                	jns    80105b2f <sys_link+0x11b>
    iunlockput(dp);
80105b22:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105b25:	89 04 24             	mov    %eax,(%esp)
80105b28:	e8 b7 bf ff ff       	call   80101ae4 <iunlockput>
    goto bad;
80105b2d:	eb 23                	jmp    80105b52 <sys_link+0x13e>
  }
  iunlockput(dp);
80105b2f:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105b32:	89 04 24             	mov    %eax,(%esp)
80105b35:	e8 aa bf ff ff       	call   80101ae4 <iunlockput>
  iput(ip);
80105b3a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105b3d:	89 04 24             	mov    %eax,(%esp)
80105b40:	e8 ce be ff ff       	call   80101a13 <iput>

  commit_trans();
80105b45:	e8 14 d7 ff ff       	call   8010325e <commit_trans>

  return 0;
80105b4a:	b8 00 00 00 00       	mov    $0x0,%eax
80105b4f:	eb 3d                	jmp    80105b8e <sys_link+0x17a>
  ip->nlink++;
  iupdate(ip);
  iunlock(ip);

  if((dp = nameiparent(new, name)) == 0)
    goto bad;
80105b51:	90                   	nop
  commit_trans();

  return 0;

bad:
  ilock(ip);
80105b52:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105b55:	89 04 24             	mov    %eax,(%esp)
80105b58:	e8 03 bd ff ff       	call   80101860 <ilock>
  ip->nlink--;
80105b5d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105b60:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105b64:	8d 50 ff             	lea    -0x1(%eax),%edx
80105b67:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105b6a:	66 89 50 16          	mov    %dx,0x16(%eax)
  iupdate(ip);
80105b6e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105b71:	89 04 24             	mov    %eax,(%esp)
80105b74:	e8 2b bb ff ff       	call   801016a4 <iupdate>
  iunlockput(ip);
80105b79:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105b7c:	89 04 24             	mov    %eax,(%esp)
80105b7f:	e8 60 bf ff ff       	call   80101ae4 <iunlockput>
  commit_trans();
80105b84:	e8 d5 d6 ff ff       	call   8010325e <commit_trans>
  return -1;
80105b89:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80105b8e:	c9                   	leave  
80105b8f:	c3                   	ret    

80105b90 <isdirempty>:

// Is the directory dp empty except for "." and ".." ?
static int
isdirempty(struct inode *dp)
{
80105b90:	55                   	push   %ebp
80105b91:	89 e5                	mov    %esp,%ebp
80105b93:	83 ec 38             	sub    $0x38,%esp
  int off;
  struct dirent de;

  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
80105b96:	c7 45 f4 20 00 00 00 	movl   $0x20,-0xc(%ebp)
80105b9d:	eb 4b                	jmp    80105bea <isdirempty+0x5a>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80105b9f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105ba2:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
80105ba9:	00 
80105baa:	89 44 24 08          	mov    %eax,0x8(%esp)
80105bae:	8d 45 e4             	lea    -0x1c(%ebp),%eax
80105bb1:	89 44 24 04          	mov    %eax,0x4(%esp)
80105bb5:	8b 45 08             	mov    0x8(%ebp),%eax
80105bb8:	89 04 24             	mov    %eax,(%esp)
80105bbb:	e8 96 c1 ff ff       	call   80101d56 <readi>
80105bc0:	83 f8 10             	cmp    $0x10,%eax
80105bc3:	74 0c                	je     80105bd1 <isdirempty+0x41>
      panic("isdirempty: readi");
80105bc5:	c7 04 24 12 8b 10 80 	movl   $0x80108b12,(%esp)
80105bcc:	e8 6c a9 ff ff       	call   8010053d <panic>
    if(de.inum != 0)
80105bd1:	0f b7 45 e4          	movzwl -0x1c(%ebp),%eax
80105bd5:	66 85 c0             	test   %ax,%ax
80105bd8:	74 07                	je     80105be1 <isdirempty+0x51>
      return 0;
80105bda:	b8 00 00 00 00       	mov    $0x0,%eax
80105bdf:	eb 1b                	jmp    80105bfc <isdirempty+0x6c>
isdirempty(struct inode *dp)
{
  int off;
  struct dirent de;

  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
80105be1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105be4:	83 c0 10             	add    $0x10,%eax
80105be7:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105bea:	8b 55 f4             	mov    -0xc(%ebp),%edx
80105bed:	8b 45 08             	mov    0x8(%ebp),%eax
80105bf0:	8b 40 18             	mov    0x18(%eax),%eax
80105bf3:	39 c2                	cmp    %eax,%edx
80105bf5:	72 a8                	jb     80105b9f <isdirempty+0xf>
    if(readi(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
      panic("isdirempty: readi");
    if(de.inum != 0)
      return 0;
  }
  return 1;
80105bf7:	b8 01 00 00 00       	mov    $0x1,%eax
}
80105bfc:	c9                   	leave  
80105bfd:	c3                   	ret    

80105bfe <sys_unlink>:

//PAGEBREAK!
int
sys_unlink(void)
{
80105bfe:	55                   	push   %ebp
80105bff:	89 e5                	mov    %esp,%ebp
80105c01:	83 ec 48             	sub    $0x48,%esp
  struct inode *ip, *dp;
  struct dirent de;
  char name[DIRSIZ], *path;
  uint off;

  if(argstr(0, &path) < 0)
80105c04:	8d 45 cc             	lea    -0x34(%ebp),%eax
80105c07:	89 44 24 04          	mov    %eax,0x4(%esp)
80105c0b:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105c12:	e8 77 fa ff ff       	call   8010568e <argstr>
80105c17:	85 c0                	test   %eax,%eax
80105c19:	79 0a                	jns    80105c25 <sys_unlink+0x27>
    return -1;
80105c1b:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105c20:	e9 aa 01 00 00       	jmp    80105dcf <sys_unlink+0x1d1>
  if((dp = nameiparent(path, name)) == 0)
80105c25:	8b 45 cc             	mov    -0x34(%ebp),%eax
80105c28:	8d 55 d2             	lea    -0x2e(%ebp),%edx
80105c2b:	89 54 24 04          	mov    %edx,0x4(%esp)
80105c2f:	89 04 24             	mov    %eax,(%esp)
80105c32:	e8 ed c7 ff ff       	call   80102424 <nameiparent>
80105c37:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105c3a:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105c3e:	75 0a                	jne    80105c4a <sys_unlink+0x4c>
    return -1;
80105c40:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105c45:	e9 85 01 00 00       	jmp    80105dcf <sys_unlink+0x1d1>

  begin_trans();
80105c4a:	e8 c6 d5 ff ff       	call   80103215 <begin_trans>

  ilock(dp);
80105c4f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105c52:	89 04 24             	mov    %eax,(%esp)
80105c55:	e8 06 bc ff ff       	call   80101860 <ilock>

  // Cannot unlink "." or "..".
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
80105c5a:	c7 44 24 04 24 8b 10 	movl   $0x80108b24,0x4(%esp)
80105c61:	80 
80105c62:	8d 45 d2             	lea    -0x2e(%ebp),%eax
80105c65:	89 04 24             	mov    %eax,(%esp)
80105c68:	e8 ea c3 ff ff       	call   80102057 <namecmp>
80105c6d:	85 c0                	test   %eax,%eax
80105c6f:	0f 84 45 01 00 00    	je     80105dba <sys_unlink+0x1bc>
80105c75:	c7 44 24 04 26 8b 10 	movl   $0x80108b26,0x4(%esp)
80105c7c:	80 
80105c7d:	8d 45 d2             	lea    -0x2e(%ebp),%eax
80105c80:	89 04 24             	mov    %eax,(%esp)
80105c83:	e8 cf c3 ff ff       	call   80102057 <namecmp>
80105c88:	85 c0                	test   %eax,%eax
80105c8a:	0f 84 2a 01 00 00    	je     80105dba <sys_unlink+0x1bc>
    goto bad;

  if((ip = dirlookup(dp, name, &off)) == 0)
80105c90:	8d 45 c8             	lea    -0x38(%ebp),%eax
80105c93:	89 44 24 08          	mov    %eax,0x8(%esp)
80105c97:	8d 45 d2             	lea    -0x2e(%ebp),%eax
80105c9a:	89 44 24 04          	mov    %eax,0x4(%esp)
80105c9e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105ca1:	89 04 24             	mov    %eax,(%esp)
80105ca4:	e8 d0 c3 ff ff       	call   80102079 <dirlookup>
80105ca9:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105cac:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105cb0:	0f 84 03 01 00 00    	je     80105db9 <sys_unlink+0x1bb>
    goto bad;
  ilock(ip);
80105cb6:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105cb9:	89 04 24             	mov    %eax,(%esp)
80105cbc:	e8 9f bb ff ff       	call   80101860 <ilock>

  if(ip->nlink < 1)
80105cc1:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105cc4:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105cc8:	66 85 c0             	test   %ax,%ax
80105ccb:	7f 0c                	jg     80105cd9 <sys_unlink+0xdb>
    panic("unlink: nlink < 1");
80105ccd:	c7 04 24 29 8b 10 80 	movl   $0x80108b29,(%esp)
80105cd4:	e8 64 a8 ff ff       	call   8010053d <panic>
  if(ip->type == T_DIR && !isdirempty(ip)){
80105cd9:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105cdc:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80105ce0:	66 83 f8 01          	cmp    $0x1,%ax
80105ce4:	75 1f                	jne    80105d05 <sys_unlink+0x107>
80105ce6:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105ce9:	89 04 24             	mov    %eax,(%esp)
80105cec:	e8 9f fe ff ff       	call   80105b90 <isdirempty>
80105cf1:	85 c0                	test   %eax,%eax
80105cf3:	75 10                	jne    80105d05 <sys_unlink+0x107>
    iunlockput(ip);
80105cf5:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105cf8:	89 04 24             	mov    %eax,(%esp)
80105cfb:	e8 e4 bd ff ff       	call   80101ae4 <iunlockput>
    goto bad;
80105d00:	e9 b5 00 00 00       	jmp    80105dba <sys_unlink+0x1bc>
  }

  memset(&de, 0, sizeof(de));
80105d05:	c7 44 24 08 10 00 00 	movl   $0x10,0x8(%esp)
80105d0c:	00 
80105d0d:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80105d14:	00 
80105d15:	8d 45 e0             	lea    -0x20(%ebp),%eax
80105d18:	89 04 24             	mov    %eax,(%esp)
80105d1b:	e8 82 f5 ff ff       	call   801052a2 <memset>
  if(writei(dp, (char*)&de, off, sizeof(de)) != sizeof(de))
80105d20:	8b 45 c8             	mov    -0x38(%ebp),%eax
80105d23:	c7 44 24 0c 10 00 00 	movl   $0x10,0xc(%esp)
80105d2a:	00 
80105d2b:	89 44 24 08          	mov    %eax,0x8(%esp)
80105d2f:	8d 45 e0             	lea    -0x20(%ebp),%eax
80105d32:	89 44 24 04          	mov    %eax,0x4(%esp)
80105d36:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105d39:	89 04 24             	mov    %eax,(%esp)
80105d3c:	e8 80 c1 ff ff       	call   80101ec1 <writei>
80105d41:	83 f8 10             	cmp    $0x10,%eax
80105d44:	74 0c                	je     80105d52 <sys_unlink+0x154>
    panic("unlink: writei");
80105d46:	c7 04 24 3b 8b 10 80 	movl   $0x80108b3b,(%esp)
80105d4d:	e8 eb a7 ff ff       	call   8010053d <panic>
  if(ip->type == T_DIR){
80105d52:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105d55:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80105d59:	66 83 f8 01          	cmp    $0x1,%ax
80105d5d:	75 1c                	jne    80105d7b <sys_unlink+0x17d>
    dp->nlink--;
80105d5f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105d62:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105d66:	8d 50 ff             	lea    -0x1(%eax),%edx
80105d69:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105d6c:	66 89 50 16          	mov    %dx,0x16(%eax)
    iupdate(dp);
80105d70:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105d73:	89 04 24             	mov    %eax,(%esp)
80105d76:	e8 29 b9 ff ff       	call   801016a4 <iupdate>
  }
  iunlockput(dp);
80105d7b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105d7e:	89 04 24             	mov    %eax,(%esp)
80105d81:	e8 5e bd ff ff       	call   80101ae4 <iunlockput>

  ip->nlink--;
80105d86:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105d89:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105d8d:	8d 50 ff             	lea    -0x1(%eax),%edx
80105d90:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105d93:	66 89 50 16          	mov    %dx,0x16(%eax)
  iupdate(ip);
80105d97:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105d9a:	89 04 24             	mov    %eax,(%esp)
80105d9d:	e8 02 b9 ff ff       	call   801016a4 <iupdate>
  iunlockput(ip);
80105da2:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105da5:	89 04 24             	mov    %eax,(%esp)
80105da8:	e8 37 bd ff ff       	call   80101ae4 <iunlockput>

  commit_trans();
80105dad:	e8 ac d4 ff ff       	call   8010325e <commit_trans>

  return 0;
80105db2:	b8 00 00 00 00       	mov    $0x0,%eax
80105db7:	eb 16                	jmp    80105dcf <sys_unlink+0x1d1>
  // Cannot unlink "." or "..".
  if(namecmp(name, ".") == 0 || namecmp(name, "..") == 0)
    goto bad;

  if((ip = dirlookup(dp, name, &off)) == 0)
    goto bad;
80105db9:	90                   	nop
  commit_trans();

  return 0;

bad:
  iunlockput(dp);
80105dba:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105dbd:	89 04 24             	mov    %eax,(%esp)
80105dc0:	e8 1f bd ff ff       	call   80101ae4 <iunlockput>
  commit_trans();
80105dc5:	e8 94 d4 ff ff       	call   8010325e <commit_trans>
  return -1;
80105dca:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
}
80105dcf:	c9                   	leave  
80105dd0:	c3                   	ret    

80105dd1 <create>:

static struct inode*
create(char *path, short type, short major, short minor)
{
80105dd1:	55                   	push   %ebp
80105dd2:	89 e5                	mov    %esp,%ebp
80105dd4:	83 ec 48             	sub    $0x48,%esp
80105dd7:	8b 4d 0c             	mov    0xc(%ebp),%ecx
80105dda:	8b 55 10             	mov    0x10(%ebp),%edx
80105ddd:	8b 45 14             	mov    0x14(%ebp),%eax
80105de0:	66 89 4d d4          	mov    %cx,-0x2c(%ebp)
80105de4:	66 89 55 d0          	mov    %dx,-0x30(%ebp)
80105de8:	66 89 45 cc          	mov    %ax,-0x34(%ebp)
  uint off;
  struct inode *ip, *dp;
  char name[DIRSIZ];

  if((dp = nameiparent(path, name)) == 0)
80105dec:	8d 45 de             	lea    -0x22(%ebp),%eax
80105def:	89 44 24 04          	mov    %eax,0x4(%esp)
80105df3:	8b 45 08             	mov    0x8(%ebp),%eax
80105df6:	89 04 24             	mov    %eax,(%esp)
80105df9:	e8 26 c6 ff ff       	call   80102424 <nameiparent>
80105dfe:	89 45 f4             	mov    %eax,-0xc(%ebp)
80105e01:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80105e05:	75 0a                	jne    80105e11 <create+0x40>
    return 0;
80105e07:	b8 00 00 00 00       	mov    $0x0,%eax
80105e0c:	e9 7e 01 00 00       	jmp    80105f8f <create+0x1be>
  ilock(dp);
80105e11:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105e14:	89 04 24             	mov    %eax,(%esp)
80105e17:	e8 44 ba ff ff       	call   80101860 <ilock>

  if((ip = dirlookup(dp, name, &off)) != 0){
80105e1c:	8d 45 ec             	lea    -0x14(%ebp),%eax
80105e1f:	89 44 24 08          	mov    %eax,0x8(%esp)
80105e23:	8d 45 de             	lea    -0x22(%ebp),%eax
80105e26:	89 44 24 04          	mov    %eax,0x4(%esp)
80105e2a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105e2d:	89 04 24             	mov    %eax,(%esp)
80105e30:	e8 44 c2 ff ff       	call   80102079 <dirlookup>
80105e35:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105e38:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105e3c:	74 47                	je     80105e85 <create+0xb4>
    iunlockput(dp);
80105e3e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105e41:	89 04 24             	mov    %eax,(%esp)
80105e44:	e8 9b bc ff ff       	call   80101ae4 <iunlockput>
    ilock(ip);
80105e49:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105e4c:	89 04 24             	mov    %eax,(%esp)
80105e4f:	e8 0c ba ff ff       	call   80101860 <ilock>
    if(type == T_FILE && ip->type == T_FILE)
80105e54:	66 83 7d d4 02       	cmpw   $0x2,-0x2c(%ebp)
80105e59:	75 15                	jne    80105e70 <create+0x9f>
80105e5b:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105e5e:	0f b7 40 10          	movzwl 0x10(%eax),%eax
80105e62:	66 83 f8 02          	cmp    $0x2,%ax
80105e66:	75 08                	jne    80105e70 <create+0x9f>
      return ip;
80105e68:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105e6b:	e9 1f 01 00 00       	jmp    80105f8f <create+0x1be>
    iunlockput(ip);
80105e70:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105e73:	89 04 24             	mov    %eax,(%esp)
80105e76:	e8 69 bc ff ff       	call   80101ae4 <iunlockput>
    return 0;
80105e7b:	b8 00 00 00 00       	mov    $0x0,%eax
80105e80:	e9 0a 01 00 00       	jmp    80105f8f <create+0x1be>
  }

  if((ip = ialloc(dp->dev, type)) == 0)
80105e85:	0f bf 55 d4          	movswl -0x2c(%ebp),%edx
80105e89:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105e8c:	8b 00                	mov    (%eax),%eax
80105e8e:	89 54 24 04          	mov    %edx,0x4(%esp)
80105e92:	89 04 24             	mov    %eax,(%esp)
80105e95:	e8 2d b7 ff ff       	call   801015c7 <ialloc>
80105e9a:	89 45 f0             	mov    %eax,-0x10(%ebp)
80105e9d:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80105ea1:	75 0c                	jne    80105eaf <create+0xde>
    panic("create: ialloc");
80105ea3:	c7 04 24 4a 8b 10 80 	movl   $0x80108b4a,(%esp)
80105eaa:	e8 8e a6 ff ff       	call   8010053d <panic>

  ilock(ip);
80105eaf:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105eb2:	89 04 24             	mov    %eax,(%esp)
80105eb5:	e8 a6 b9 ff ff       	call   80101860 <ilock>
  ip->major = major;
80105eba:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105ebd:	0f b7 55 d0          	movzwl -0x30(%ebp),%edx
80105ec1:	66 89 50 12          	mov    %dx,0x12(%eax)
  ip->minor = minor;
80105ec5:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105ec8:	0f b7 55 cc          	movzwl -0x34(%ebp),%edx
80105ecc:	66 89 50 14          	mov    %dx,0x14(%eax)
  ip->nlink = 1;
80105ed0:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105ed3:	66 c7 40 16 01 00    	movw   $0x1,0x16(%eax)
  iupdate(ip);
80105ed9:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105edc:	89 04 24             	mov    %eax,(%esp)
80105edf:	e8 c0 b7 ff ff       	call   801016a4 <iupdate>

  if(type == T_DIR){  // Create . and .. entries.
80105ee4:	66 83 7d d4 01       	cmpw   $0x1,-0x2c(%ebp)
80105ee9:	75 6a                	jne    80105f55 <create+0x184>
    dp->nlink++;  // for ".."
80105eeb:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105eee:	0f b7 40 16          	movzwl 0x16(%eax),%eax
80105ef2:	8d 50 01             	lea    0x1(%eax),%edx
80105ef5:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105ef8:	66 89 50 16          	mov    %dx,0x16(%eax)
    iupdate(dp);
80105efc:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105eff:	89 04 24             	mov    %eax,(%esp)
80105f02:	e8 9d b7 ff ff       	call   801016a4 <iupdate>
    // No ip->nlink++ for ".": avoid cyclic ref count.
    if(dirlink(ip, ".", ip->inum) < 0 || dirlink(ip, "..", dp->inum) < 0)
80105f07:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105f0a:	8b 40 04             	mov    0x4(%eax),%eax
80105f0d:	89 44 24 08          	mov    %eax,0x8(%esp)
80105f11:	c7 44 24 04 24 8b 10 	movl   $0x80108b24,0x4(%esp)
80105f18:	80 
80105f19:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105f1c:	89 04 24             	mov    %eax,(%esp)
80105f1f:	e8 1d c2 ff ff       	call   80102141 <dirlink>
80105f24:	85 c0                	test   %eax,%eax
80105f26:	78 21                	js     80105f49 <create+0x178>
80105f28:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105f2b:	8b 40 04             	mov    0x4(%eax),%eax
80105f2e:	89 44 24 08          	mov    %eax,0x8(%esp)
80105f32:	c7 44 24 04 26 8b 10 	movl   $0x80108b26,0x4(%esp)
80105f39:	80 
80105f3a:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105f3d:	89 04 24             	mov    %eax,(%esp)
80105f40:	e8 fc c1 ff ff       	call   80102141 <dirlink>
80105f45:	85 c0                	test   %eax,%eax
80105f47:	79 0c                	jns    80105f55 <create+0x184>
      panic("create dots");
80105f49:	c7 04 24 59 8b 10 80 	movl   $0x80108b59,(%esp)
80105f50:	e8 e8 a5 ff ff       	call   8010053d <panic>
  }

  if(dirlink(dp, name, ip->inum) < 0)
80105f55:	8b 45 f0             	mov    -0x10(%ebp),%eax
80105f58:	8b 40 04             	mov    0x4(%eax),%eax
80105f5b:	89 44 24 08          	mov    %eax,0x8(%esp)
80105f5f:	8d 45 de             	lea    -0x22(%ebp),%eax
80105f62:	89 44 24 04          	mov    %eax,0x4(%esp)
80105f66:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105f69:	89 04 24             	mov    %eax,(%esp)
80105f6c:	e8 d0 c1 ff ff       	call   80102141 <dirlink>
80105f71:	85 c0                	test   %eax,%eax
80105f73:	79 0c                	jns    80105f81 <create+0x1b0>
    panic("create: dirlink");
80105f75:	c7 04 24 65 8b 10 80 	movl   $0x80108b65,(%esp)
80105f7c:	e8 bc a5 ff ff       	call   8010053d <panic>

  iunlockput(dp);
80105f81:	8b 45 f4             	mov    -0xc(%ebp),%eax
80105f84:	89 04 24             	mov    %eax,(%esp)
80105f87:	e8 58 bb ff ff       	call   80101ae4 <iunlockput>

  return ip;
80105f8c:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
80105f8f:	c9                   	leave  
80105f90:	c3                   	ret    

80105f91 <sys_open>:

int
sys_open(void)
{
80105f91:	55                   	push   %ebp
80105f92:	89 e5                	mov    %esp,%ebp
80105f94:	83 ec 38             	sub    $0x38,%esp
  char *path;
  int fd, omode;
  struct file *f;
  struct inode *ip;

  if(argstr(0, &path) < 0 || argint(1, &omode) < 0)
80105f97:	8d 45 e8             	lea    -0x18(%ebp),%eax
80105f9a:	89 44 24 04          	mov    %eax,0x4(%esp)
80105f9e:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80105fa5:	e8 e4 f6 ff ff       	call   8010568e <argstr>
80105faa:	85 c0                	test   %eax,%eax
80105fac:	78 17                	js     80105fc5 <sys_open+0x34>
80105fae:	8d 45 e4             	lea    -0x1c(%ebp),%eax
80105fb1:	89 44 24 04          	mov    %eax,0x4(%esp)
80105fb5:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
80105fbc:	e8 3d f6 ff ff       	call   801055fe <argint>
80105fc1:	85 c0                	test   %eax,%eax
80105fc3:	79 0a                	jns    80105fcf <sys_open+0x3e>
    return -1;
80105fc5:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80105fca:	e9 46 01 00 00       	jmp    80106115 <sys_open+0x184>
  if(omode & O_CREATE){
80105fcf:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80105fd2:	25 00 02 00 00       	and    $0x200,%eax
80105fd7:	85 c0                	test   %eax,%eax
80105fd9:	74 40                	je     8010601b <sys_open+0x8a>
    begin_trans();
80105fdb:	e8 35 d2 ff ff       	call   80103215 <begin_trans>
    ip = create(path, T_FILE, 0, 0);
80105fe0:	8b 45 e8             	mov    -0x18(%ebp),%eax
80105fe3:	c7 44 24 0c 00 00 00 	movl   $0x0,0xc(%esp)
80105fea:	00 
80105feb:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80105ff2:	00 
80105ff3:	c7 44 24 04 02 00 00 	movl   $0x2,0x4(%esp)
80105ffa:	00 
80105ffb:	89 04 24             	mov    %eax,(%esp)
80105ffe:	e8 ce fd ff ff       	call   80105dd1 <create>
80106003:	89 45 f4             	mov    %eax,-0xc(%ebp)
    commit_trans();
80106006:	e8 53 d2 ff ff       	call   8010325e <commit_trans>
    if(ip == 0)
8010600b:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010600f:	75 5c                	jne    8010606d <sys_open+0xdc>
      return -1;
80106011:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106016:	e9 fa 00 00 00       	jmp    80106115 <sys_open+0x184>
  } else {
    if((ip = namei(path)) == 0)
8010601b:	8b 45 e8             	mov    -0x18(%ebp),%eax
8010601e:	89 04 24             	mov    %eax,(%esp)
80106021:	e8 dc c3 ff ff       	call   80102402 <namei>
80106026:	89 45 f4             	mov    %eax,-0xc(%ebp)
80106029:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010602d:	75 0a                	jne    80106039 <sys_open+0xa8>
      return -1;
8010602f:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106034:	e9 dc 00 00 00       	jmp    80106115 <sys_open+0x184>
    ilock(ip);
80106039:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010603c:	89 04 24             	mov    %eax,(%esp)
8010603f:	e8 1c b8 ff ff       	call   80101860 <ilock>
    if(ip->type == T_DIR && omode != O_RDONLY){
80106044:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106047:	0f b7 40 10          	movzwl 0x10(%eax),%eax
8010604b:	66 83 f8 01          	cmp    $0x1,%ax
8010604f:	75 1c                	jne    8010606d <sys_open+0xdc>
80106051:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80106054:	85 c0                	test   %eax,%eax
80106056:	74 15                	je     8010606d <sys_open+0xdc>
      iunlockput(ip);
80106058:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010605b:	89 04 24             	mov    %eax,(%esp)
8010605e:	e8 81 ba ff ff       	call   80101ae4 <iunlockput>
      return -1;
80106063:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106068:	e9 a8 00 00 00       	jmp    80106115 <sys_open+0x184>
    }
  }

  if((f = filealloc()) == 0 || (fd = fdalloc(f)) < 0){
8010606d:	e8 a2 ae ff ff       	call   80100f14 <filealloc>
80106072:	89 45 f0             	mov    %eax,-0x10(%ebp)
80106075:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80106079:	74 14                	je     8010608f <sys_open+0xfe>
8010607b:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010607e:	89 04 24             	mov    %eax,(%esp)
80106081:	e8 43 f7 ff ff       	call   801057c9 <fdalloc>
80106086:	89 45 ec             	mov    %eax,-0x14(%ebp)
80106089:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
8010608d:	79 23                	jns    801060b2 <sys_open+0x121>
    if(f)
8010608f:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80106093:	74 0b                	je     801060a0 <sys_open+0x10f>
      fileclose(f);
80106095:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106098:	89 04 24             	mov    %eax,(%esp)
8010609b:	e8 1c af ff ff       	call   80100fbc <fileclose>
    iunlockput(ip);
801060a0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801060a3:	89 04 24             	mov    %eax,(%esp)
801060a6:	e8 39 ba ff ff       	call   80101ae4 <iunlockput>
    return -1;
801060ab:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801060b0:	eb 63                	jmp    80106115 <sys_open+0x184>
  }
  iunlock(ip);
801060b2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801060b5:	89 04 24             	mov    %eax,(%esp)
801060b8:	e8 f1 b8 ff ff       	call   801019ae <iunlock>

  f->type = FD_INODE;
801060bd:	8b 45 f0             	mov    -0x10(%ebp),%eax
801060c0:	c7 00 02 00 00 00    	movl   $0x2,(%eax)
  f->ip = ip;
801060c6:	8b 45 f0             	mov    -0x10(%ebp),%eax
801060c9:	8b 55 f4             	mov    -0xc(%ebp),%edx
801060cc:	89 50 10             	mov    %edx,0x10(%eax)
  f->off = 0;
801060cf:	8b 45 f0             	mov    -0x10(%ebp),%eax
801060d2:	c7 40 14 00 00 00 00 	movl   $0x0,0x14(%eax)
  f->readable = !(omode & O_WRONLY);
801060d9:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801060dc:	83 e0 01             	and    $0x1,%eax
801060df:	85 c0                	test   %eax,%eax
801060e1:	0f 94 c2             	sete   %dl
801060e4:	8b 45 f0             	mov    -0x10(%ebp),%eax
801060e7:	88 50 08             	mov    %dl,0x8(%eax)
  f->writable = (omode & O_WRONLY) || (omode & O_RDWR);
801060ea:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801060ed:	83 e0 01             	and    $0x1,%eax
801060f0:	84 c0                	test   %al,%al
801060f2:	75 0a                	jne    801060fe <sys_open+0x16d>
801060f4:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801060f7:	83 e0 02             	and    $0x2,%eax
801060fa:	85 c0                	test   %eax,%eax
801060fc:	74 07                	je     80106105 <sys_open+0x174>
801060fe:	b8 01 00 00 00       	mov    $0x1,%eax
80106103:	eb 05                	jmp    8010610a <sys_open+0x179>
80106105:	b8 00 00 00 00       	mov    $0x0,%eax
8010610a:	89 c2                	mov    %eax,%edx
8010610c:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010610f:	88 50 09             	mov    %dl,0x9(%eax)
  return fd;
80106112:	8b 45 ec             	mov    -0x14(%ebp),%eax
}
80106115:	c9                   	leave  
80106116:	c3                   	ret    

80106117 <sys_mkdir>:

int
sys_mkdir(void)
{
80106117:	55                   	push   %ebp
80106118:	89 e5                	mov    %esp,%ebp
8010611a:	83 ec 28             	sub    $0x28,%esp
  char *path;
  struct inode *ip;

  begin_trans();
8010611d:	e8 f3 d0 ff ff       	call   80103215 <begin_trans>
  if(argstr(0, &path) < 0 || (ip = create(path, T_DIR, 0, 0)) == 0){
80106122:	8d 45 f0             	lea    -0x10(%ebp),%eax
80106125:	89 44 24 04          	mov    %eax,0x4(%esp)
80106129:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106130:	e8 59 f5 ff ff       	call   8010568e <argstr>
80106135:	85 c0                	test   %eax,%eax
80106137:	78 2c                	js     80106165 <sys_mkdir+0x4e>
80106139:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010613c:	c7 44 24 0c 00 00 00 	movl   $0x0,0xc(%esp)
80106143:	00 
80106144:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
8010614b:	00 
8010614c:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80106153:	00 
80106154:	89 04 24             	mov    %eax,(%esp)
80106157:	e8 75 fc ff ff       	call   80105dd1 <create>
8010615c:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010615f:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80106163:	75 0c                	jne    80106171 <sys_mkdir+0x5a>
    commit_trans();
80106165:	e8 f4 d0 ff ff       	call   8010325e <commit_trans>
    return -1;
8010616a:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010616f:	eb 15                	jmp    80106186 <sys_mkdir+0x6f>
  }
  iunlockput(ip);
80106171:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106174:	89 04 24             	mov    %eax,(%esp)
80106177:	e8 68 b9 ff ff       	call   80101ae4 <iunlockput>
  commit_trans();
8010617c:	e8 dd d0 ff ff       	call   8010325e <commit_trans>
  return 0;
80106181:	b8 00 00 00 00       	mov    $0x0,%eax
}
80106186:	c9                   	leave  
80106187:	c3                   	ret    

80106188 <sys_mknod>:

int
sys_mknod(void)
{
80106188:	55                   	push   %ebp
80106189:	89 e5                	mov    %esp,%ebp
8010618b:	83 ec 38             	sub    $0x38,%esp
  struct inode *ip;
  char *path;
  int len;
  int major, minor;
  
  begin_trans();
8010618e:	e8 82 d0 ff ff       	call   80103215 <begin_trans>
  if((len=argstr(0, &path)) < 0 ||
80106193:	8d 45 ec             	lea    -0x14(%ebp),%eax
80106196:	89 44 24 04          	mov    %eax,0x4(%esp)
8010619a:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801061a1:	e8 e8 f4 ff ff       	call   8010568e <argstr>
801061a6:	89 45 f4             	mov    %eax,-0xc(%ebp)
801061a9:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
801061ad:	78 5e                	js     8010620d <sys_mknod+0x85>
     argint(1, &major) < 0 ||
801061af:	8d 45 e8             	lea    -0x18(%ebp),%eax
801061b2:	89 44 24 04          	mov    %eax,0x4(%esp)
801061b6:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801061bd:	e8 3c f4 ff ff       	call   801055fe <argint>
  char *path;
  int len;
  int major, minor;
  
  begin_trans();
  if((len=argstr(0, &path)) < 0 ||
801061c2:	85 c0                	test   %eax,%eax
801061c4:	78 47                	js     8010620d <sys_mknod+0x85>
     argint(1, &major) < 0 ||
     argint(2, &minor) < 0 ||
801061c6:	8d 45 e4             	lea    -0x1c(%ebp),%eax
801061c9:	89 44 24 04          	mov    %eax,0x4(%esp)
801061cd:	c7 04 24 02 00 00 00 	movl   $0x2,(%esp)
801061d4:	e8 25 f4 ff ff       	call   801055fe <argint>
  int len;
  int major, minor;
  
  begin_trans();
  if((len=argstr(0, &path)) < 0 ||
     argint(1, &major) < 0 ||
801061d9:	85 c0                	test   %eax,%eax
801061db:	78 30                	js     8010620d <sys_mknod+0x85>
     argint(2, &minor) < 0 ||
     (ip = create(path, T_DEV, major, minor)) == 0){
801061dd:	8b 45 e4             	mov    -0x1c(%ebp),%eax
801061e0:	0f bf c8             	movswl %ax,%ecx
801061e3:	8b 45 e8             	mov    -0x18(%ebp),%eax
801061e6:	0f bf d0             	movswl %ax,%edx
801061e9:	8b 45 ec             	mov    -0x14(%ebp),%eax
  int major, minor;
  
  begin_trans();
  if((len=argstr(0, &path)) < 0 ||
     argint(1, &major) < 0 ||
     argint(2, &minor) < 0 ||
801061ec:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
801061f0:	89 54 24 08          	mov    %edx,0x8(%esp)
801061f4:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
801061fb:	00 
801061fc:	89 04 24             	mov    %eax,(%esp)
801061ff:	e8 cd fb ff ff       	call   80105dd1 <create>
80106204:	89 45 f0             	mov    %eax,-0x10(%ebp)
80106207:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
8010620b:	75 0c                	jne    80106219 <sys_mknod+0x91>
     (ip = create(path, T_DEV, major, minor)) == 0){
    commit_trans();
8010620d:	e8 4c d0 ff ff       	call   8010325e <commit_trans>
    return -1;
80106212:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106217:	eb 15                	jmp    8010622e <sys_mknod+0xa6>
  }
  iunlockput(ip);
80106219:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010621c:	89 04 24             	mov    %eax,(%esp)
8010621f:	e8 c0 b8 ff ff       	call   80101ae4 <iunlockput>
  commit_trans();
80106224:	e8 35 d0 ff ff       	call   8010325e <commit_trans>
  return 0;
80106229:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010622e:	c9                   	leave  
8010622f:	c3                   	ret    

80106230 <sys_chdir>:

int
sys_chdir(void)
{
80106230:	55                   	push   %ebp
80106231:	89 e5                	mov    %esp,%ebp
80106233:	83 ec 28             	sub    $0x28,%esp
  char *path;
  struct inode *ip;

  if(argstr(0, &path) < 0 || (ip = namei(path)) == 0)
80106236:	8d 45 f0             	lea    -0x10(%ebp),%eax
80106239:	89 44 24 04          	mov    %eax,0x4(%esp)
8010623d:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106244:	e8 45 f4 ff ff       	call   8010568e <argstr>
80106249:	85 c0                	test   %eax,%eax
8010624b:	78 14                	js     80106261 <sys_chdir+0x31>
8010624d:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106250:	89 04 24             	mov    %eax,(%esp)
80106253:	e8 aa c1 ff ff       	call   80102402 <namei>
80106258:	89 45 f4             	mov    %eax,-0xc(%ebp)
8010625b:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010625f:	75 07                	jne    80106268 <sys_chdir+0x38>
    return -1;
80106261:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106266:	eb 57                	jmp    801062bf <sys_chdir+0x8f>
  ilock(ip);
80106268:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010626b:	89 04 24             	mov    %eax,(%esp)
8010626e:	e8 ed b5 ff ff       	call   80101860 <ilock>
  if(ip->type != T_DIR){
80106273:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106276:	0f b7 40 10          	movzwl 0x10(%eax),%eax
8010627a:	66 83 f8 01          	cmp    $0x1,%ax
8010627e:	74 12                	je     80106292 <sys_chdir+0x62>
    iunlockput(ip);
80106280:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106283:	89 04 24             	mov    %eax,(%esp)
80106286:	e8 59 b8 ff ff       	call   80101ae4 <iunlockput>
    return -1;
8010628b:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106290:	eb 2d                	jmp    801062bf <sys_chdir+0x8f>
  }
  iunlock(ip);
80106292:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106295:	89 04 24             	mov    %eax,(%esp)
80106298:	e8 11 b7 ff ff       	call   801019ae <iunlock>
  iput(proc->cwd);
8010629d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801062a3:	8b 40 68             	mov    0x68(%eax),%eax
801062a6:	89 04 24             	mov    %eax,(%esp)
801062a9:	e8 65 b7 ff ff       	call   80101a13 <iput>
  proc->cwd = ip;
801062ae:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
801062b4:	8b 55 f4             	mov    -0xc(%ebp),%edx
801062b7:	89 50 68             	mov    %edx,0x68(%eax)
  return 0;
801062ba:	b8 00 00 00 00       	mov    $0x0,%eax
}
801062bf:	c9                   	leave  
801062c0:	c3                   	ret    

801062c1 <sys_exec>:

int
sys_exec(void)
{
801062c1:	55                   	push   %ebp
801062c2:	89 e5                	mov    %esp,%ebp
801062c4:	81 ec a8 00 00 00    	sub    $0xa8,%esp
  char *path, *argv[MAXARG];
  int i;
  uint uargv, uarg;

  if(argstr(0, &path) < 0 || argint(1, (int*)&uargv) < 0){
801062ca:	8d 45 f0             	lea    -0x10(%ebp),%eax
801062cd:	89 44 24 04          	mov    %eax,0x4(%esp)
801062d1:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801062d8:	e8 b1 f3 ff ff       	call   8010568e <argstr>
801062dd:	85 c0                	test   %eax,%eax
801062df:	78 1a                	js     801062fb <sys_exec+0x3a>
801062e1:	8d 85 6c ff ff ff    	lea    -0x94(%ebp),%eax
801062e7:	89 44 24 04          	mov    %eax,0x4(%esp)
801062eb:	c7 04 24 01 00 00 00 	movl   $0x1,(%esp)
801062f2:	e8 07 f3 ff ff       	call   801055fe <argint>
801062f7:	85 c0                	test   %eax,%eax
801062f9:	79 0a                	jns    80106305 <sys_exec+0x44>
    return -1;
801062fb:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106300:	e9 cc 00 00 00       	jmp    801063d1 <sys_exec+0x110>
  }
  memset(argv, 0, sizeof(argv));
80106305:	c7 44 24 08 80 00 00 	movl   $0x80,0x8(%esp)
8010630c:	00 
8010630d:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106314:	00 
80106315:	8d 85 70 ff ff ff    	lea    -0x90(%ebp),%eax
8010631b:	89 04 24             	mov    %eax,(%esp)
8010631e:	e8 7f ef ff ff       	call   801052a2 <memset>
  for(i=0;; i++){
80106323:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
    if(i >= NELEM(argv))
8010632a:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010632d:	83 f8 1f             	cmp    $0x1f,%eax
80106330:	76 0a                	jbe    8010633c <sys_exec+0x7b>
      return -1;
80106332:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106337:	e9 95 00 00 00       	jmp    801063d1 <sys_exec+0x110>
    if(fetchint(uargv+4*i, (int*)&uarg) < 0)
8010633c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010633f:	c1 e0 02             	shl    $0x2,%eax
80106342:	89 c2                	mov    %eax,%edx
80106344:	8b 85 6c ff ff ff    	mov    -0x94(%ebp),%eax
8010634a:	01 c2                	add    %eax,%edx
8010634c:	8d 85 68 ff ff ff    	lea    -0x98(%ebp),%eax
80106352:	89 44 24 04          	mov    %eax,0x4(%esp)
80106356:	89 14 24             	mov    %edx,(%esp)
80106359:	e8 02 f2 ff ff       	call   80105560 <fetchint>
8010635e:	85 c0                	test   %eax,%eax
80106360:	79 07                	jns    80106369 <sys_exec+0xa8>
      return -1;
80106362:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106367:	eb 68                	jmp    801063d1 <sys_exec+0x110>
    if(uarg == 0){
80106369:	8b 85 68 ff ff ff    	mov    -0x98(%ebp),%eax
8010636f:	85 c0                	test   %eax,%eax
80106371:	75 26                	jne    80106399 <sys_exec+0xd8>
      argv[i] = 0;
80106373:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106376:	c7 84 85 70 ff ff ff 	movl   $0x0,-0x90(%ebp,%eax,4)
8010637d:	00 00 00 00 
      break;
80106381:	90                   	nop
    }
    if(fetchstr(uarg, &argv[i]) < 0)
      return -1;
  }
  return exec(path, argv);
80106382:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106385:	8d 95 70 ff ff ff    	lea    -0x90(%ebp),%edx
8010638b:	89 54 24 04          	mov    %edx,0x4(%esp)
8010638f:	89 04 24             	mov    %eax,(%esp)
80106392:	e8 65 a7 ff ff       	call   80100afc <exec>
80106397:	eb 38                	jmp    801063d1 <sys_exec+0x110>
      return -1;
    if(uarg == 0){
      argv[i] = 0;
      break;
    }
    if(fetchstr(uarg, &argv[i]) < 0)
80106399:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010639c:	8d 14 85 00 00 00 00 	lea    0x0(,%eax,4),%edx
801063a3:	8d 85 70 ff ff ff    	lea    -0x90(%ebp),%eax
801063a9:	01 c2                	add    %eax,%edx
801063ab:	8b 85 68 ff ff ff    	mov    -0x98(%ebp),%eax
801063b1:	89 54 24 04          	mov    %edx,0x4(%esp)
801063b5:	89 04 24             	mov    %eax,(%esp)
801063b8:	e8 dd f1 ff ff       	call   8010559a <fetchstr>
801063bd:	85 c0                	test   %eax,%eax
801063bf:	79 07                	jns    801063c8 <sys_exec+0x107>
      return -1;
801063c1:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801063c6:	eb 09                	jmp    801063d1 <sys_exec+0x110>

  if(argstr(0, &path) < 0 || argint(1, (int*)&uargv) < 0){
    return -1;
  }
  memset(argv, 0, sizeof(argv));
  for(i=0;; i++){
801063c8:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
      argv[i] = 0;
      break;
    }
    if(fetchstr(uarg, &argv[i]) < 0)
      return -1;
  }
801063cc:	e9 59 ff ff ff       	jmp    8010632a <sys_exec+0x69>
  return exec(path, argv);
}
801063d1:	c9                   	leave  
801063d2:	c3                   	ret    

801063d3 <sys_pipe>:

int
sys_pipe(void)
{
801063d3:	55                   	push   %ebp
801063d4:	89 e5                	mov    %esp,%ebp
801063d6:	83 ec 38             	sub    $0x38,%esp
  int *fd;
  struct file *rf, *wf;
  int fd0, fd1;

  if(argptr(0, (void*)&fd, 2*sizeof(fd[0])) < 0)
801063d9:	c7 44 24 08 08 00 00 	movl   $0x8,0x8(%esp)
801063e0:	00 
801063e1:	8d 45 ec             	lea    -0x14(%ebp),%eax
801063e4:	89 44 24 04          	mov    %eax,0x4(%esp)
801063e8:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801063ef:	e8 38 f2 ff ff       	call   8010562c <argptr>
801063f4:	85 c0                	test   %eax,%eax
801063f6:	79 0a                	jns    80106402 <sys_pipe+0x2f>
    return -1;
801063f8:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801063fd:	e9 9b 00 00 00       	jmp    8010649d <sys_pipe+0xca>
  if(pipealloc(&rf, &wf) < 0)
80106402:	8d 45 e4             	lea    -0x1c(%ebp),%eax
80106405:	89 44 24 04          	mov    %eax,0x4(%esp)
80106409:	8d 45 e8             	lea    -0x18(%ebp),%eax
8010640c:	89 04 24             	mov    %eax,(%esp)
8010640f:	e8 10 d8 ff ff       	call   80103c24 <pipealloc>
80106414:	85 c0                	test   %eax,%eax
80106416:	79 07                	jns    8010641f <sys_pipe+0x4c>
    return -1;
80106418:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010641d:	eb 7e                	jmp    8010649d <sys_pipe+0xca>
  fd0 = -1;
8010641f:	c7 45 f4 ff ff ff ff 	movl   $0xffffffff,-0xc(%ebp)
  if((fd0 = fdalloc(rf)) < 0 || (fd1 = fdalloc(wf)) < 0){
80106426:	8b 45 e8             	mov    -0x18(%ebp),%eax
80106429:	89 04 24             	mov    %eax,(%esp)
8010642c:	e8 98 f3 ff ff       	call   801057c9 <fdalloc>
80106431:	89 45 f4             	mov    %eax,-0xc(%ebp)
80106434:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80106438:	78 14                	js     8010644e <sys_pipe+0x7b>
8010643a:	8b 45 e4             	mov    -0x1c(%ebp),%eax
8010643d:	89 04 24             	mov    %eax,(%esp)
80106440:	e8 84 f3 ff ff       	call   801057c9 <fdalloc>
80106445:	89 45 f0             	mov    %eax,-0x10(%ebp)
80106448:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
8010644c:	79 37                	jns    80106485 <sys_pipe+0xb2>
    if(fd0 >= 0)
8010644e:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80106452:	78 14                	js     80106468 <sys_pipe+0x95>
      proc->ofile[fd0] = 0;
80106454:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010645a:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010645d:	83 c2 08             	add    $0x8,%edx
80106460:	c7 44 90 08 00 00 00 	movl   $0x0,0x8(%eax,%edx,4)
80106467:	00 
    fileclose(rf);
80106468:	8b 45 e8             	mov    -0x18(%ebp),%eax
8010646b:	89 04 24             	mov    %eax,(%esp)
8010646e:	e8 49 ab ff ff       	call   80100fbc <fileclose>
    fileclose(wf);
80106473:	8b 45 e4             	mov    -0x1c(%ebp),%eax
80106476:	89 04 24             	mov    %eax,(%esp)
80106479:	e8 3e ab ff ff       	call   80100fbc <fileclose>
    return -1;
8010647e:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106483:	eb 18                	jmp    8010649d <sys_pipe+0xca>
  }
  fd[0] = fd0;
80106485:	8b 45 ec             	mov    -0x14(%ebp),%eax
80106488:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010648b:	89 10                	mov    %edx,(%eax)
  fd[1] = fd1;
8010648d:	8b 45 ec             	mov    -0x14(%ebp),%eax
80106490:	8d 50 04             	lea    0x4(%eax),%edx
80106493:	8b 45 f0             	mov    -0x10(%ebp),%eax
80106496:	89 02                	mov    %eax,(%edx)
  return 0;
80106498:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010649d:	c9                   	leave  
8010649e:	c3                   	ret    
	...

801064a0 <sys_fork>:
#include "mmu.h"
#include "proc.h"

int
sys_fork(void)
{
801064a0:	55                   	push   %ebp
801064a1:	89 e5                	mov    %esp,%ebp
801064a3:	83 ec 08             	sub    $0x8,%esp
  return fork();
801064a6:	e8 2d de ff ff       	call   801042d8 <fork>
}
801064ab:	c9                   	leave  
801064ac:	c3                   	ret    

801064ad <sys_exit>:

int
sys_exit(void)
{
801064ad:	55                   	push   %ebp
801064ae:	89 e5                	mov    %esp,%ebp
801064b0:	83 ec 08             	sub    $0x8,%esp
  exit();
801064b3:	e8 84 df ff ff       	call   8010443c <exit>
  return 0;  // not reached
801064b8:	b8 00 00 00 00       	mov    $0x0,%eax
}
801064bd:	c9                   	leave  
801064be:	c3                   	ret    

801064bf <sys_wait>:

int
sys_wait(void)
{
801064bf:	55                   	push   %ebp
801064c0:	89 e5                	mov    %esp,%ebp
801064c2:	83 ec 08             	sub    $0x8,%esp
  return wait();
801064c5:	e8 8a e0 ff ff       	call   80104554 <wait>
}
801064ca:	c9                   	leave  
801064cb:	c3                   	ret    

801064cc <sys_kill>:

int
sys_kill(void)
{
801064cc:	55                   	push   %ebp
801064cd:	89 e5                	mov    %esp,%ebp
801064cf:	83 ec 28             	sub    $0x28,%esp
  int pid;

  if(argint(0, &pid) < 0)
801064d2:	8d 45 f4             	lea    -0xc(%ebp),%eax
801064d5:	89 44 24 04          	mov    %eax,0x4(%esp)
801064d9:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
801064e0:	e8 19 f1 ff ff       	call   801055fe <argint>
801064e5:	85 c0                	test   %eax,%eax
801064e7:	79 07                	jns    801064f0 <sys_kill+0x24>
    return -1;
801064e9:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801064ee:	eb 0b                	jmp    801064fb <sys_kill+0x2f>
  return kill(pid);
801064f0:	8b 45 f4             	mov    -0xc(%ebp),%eax
801064f3:	89 04 24             	mov    %eax,(%esp)
801064f6:	e8 1c e4 ff ff       	call   80104917 <kill>
}
801064fb:	c9                   	leave  
801064fc:	c3                   	ret    

801064fd <sys_getpid>:

int
sys_getpid(void)
{
801064fd:	55                   	push   %ebp
801064fe:	89 e5                	mov    %esp,%ebp
  return proc->pid;
80106500:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106506:	8b 40 10             	mov    0x10(%eax),%eax
}
80106509:	5d                   	pop    %ebp
8010650a:	c3                   	ret    

8010650b <sys_sbrk>:

int
sys_sbrk(void)
{
8010650b:	55                   	push   %ebp
8010650c:	89 e5                	mov    %esp,%ebp
8010650e:	83 ec 28             	sub    $0x28,%esp
  int addr;
  int n;

  if(argint(0, &n) < 0)
80106511:	8d 45 f0             	lea    -0x10(%ebp),%eax
80106514:	89 44 24 04          	mov    %eax,0x4(%esp)
80106518:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
8010651f:	e8 da f0 ff ff       	call   801055fe <argint>
80106524:	85 c0                	test   %eax,%eax
80106526:	79 07                	jns    8010652f <sys_sbrk+0x24>
    return -1;
80106528:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010652d:	eb 24                	jmp    80106553 <sys_sbrk+0x48>
  addr = proc->sz;
8010652f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106535:	8b 00                	mov    (%eax),%eax
80106537:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(growproc(n) < 0)
8010653a:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010653d:	89 04 24             	mov    %eax,(%esp)
80106540:	e8 ee dc ff ff       	call   80104233 <growproc>
80106545:	85 c0                	test   %eax,%eax
80106547:	79 07                	jns    80106550 <sys_sbrk+0x45>
    return -1;
80106549:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
8010654e:	eb 03                	jmp    80106553 <sys_sbrk+0x48>
  return addr;
80106550:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80106553:	c9                   	leave  
80106554:	c3                   	ret    

80106555 <sys_sleep>:

int
sys_sleep(void)
{
80106555:	55                   	push   %ebp
80106556:	89 e5                	mov    %esp,%ebp
80106558:	83 ec 28             	sub    $0x28,%esp
  int n;
  uint ticks0;
  
  if(argint(0, &n) < 0)
8010655b:	8d 45 f0             	lea    -0x10(%ebp),%eax
8010655e:	89 44 24 04          	mov    %eax,0x4(%esp)
80106562:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
80106569:	e8 90 f0 ff ff       	call   801055fe <argint>
8010656e:	85 c0                	test   %eax,%eax
80106570:	79 07                	jns    80106579 <sys_sleep+0x24>
    return -1;
80106572:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106577:	eb 6c                	jmp    801065e5 <sys_sleep+0x90>
  acquire(&tickslock);
80106579:	c7 04 24 c0 24 11 80 	movl   $0x801124c0,(%esp)
80106580:	e8 ce ea ff ff       	call   80105053 <acquire>
  ticks0 = ticks;
80106585:	a1 00 2d 11 80       	mov    0x80112d00,%eax
8010658a:	89 45 f4             	mov    %eax,-0xc(%ebp)
  while(ticks - ticks0 < n){
8010658d:	eb 34                	jmp    801065c3 <sys_sleep+0x6e>
    if(proc->killed){
8010658f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106595:	8b 40 24             	mov    0x24(%eax),%eax
80106598:	85 c0                	test   %eax,%eax
8010659a:	74 13                	je     801065af <sys_sleep+0x5a>
      release(&tickslock);
8010659c:	c7 04 24 c0 24 11 80 	movl   $0x801124c0,(%esp)
801065a3:	e8 0d eb ff ff       	call   801050b5 <release>
      return -1;
801065a8:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801065ad:	eb 36                	jmp    801065e5 <sys_sleep+0x90>
    }
    sleep(&ticks, &tickslock);
801065af:	c7 44 24 04 c0 24 11 	movl   $0x801124c0,0x4(%esp)
801065b6:	80 
801065b7:	c7 04 24 00 2d 11 80 	movl   $0x80112d00,(%esp)
801065be:	e8 4f e2 ff ff       	call   80104812 <sleep>
  
  if(argint(0, &n) < 0)
    return -1;
  acquire(&tickslock);
  ticks0 = ticks;
  while(ticks - ticks0 < n){
801065c3:	a1 00 2d 11 80       	mov    0x80112d00,%eax
801065c8:	89 c2                	mov    %eax,%edx
801065ca:	2b 55 f4             	sub    -0xc(%ebp),%edx
801065cd:	8b 45 f0             	mov    -0x10(%ebp),%eax
801065d0:	39 c2                	cmp    %eax,%edx
801065d2:	72 bb                	jb     8010658f <sys_sleep+0x3a>
      release(&tickslock);
      return -1;
    }
    sleep(&ticks, &tickslock);
  }
  release(&tickslock);
801065d4:	c7 04 24 c0 24 11 80 	movl   $0x801124c0,(%esp)
801065db:	e8 d5 ea ff ff       	call   801050b5 <release>
  return 0;
801065e0:	b8 00 00 00 00       	mov    $0x0,%eax
}
801065e5:	c9                   	leave  
801065e6:	c3                   	ret    

801065e7 <sys_uptime>:

// return how many clock tick interrupts have occurred
// since start.
int
sys_uptime(void)
{
801065e7:	55                   	push   %ebp
801065e8:	89 e5                	mov    %esp,%ebp
801065ea:	83 ec 28             	sub    $0x28,%esp
  uint xticks;
  
  acquire(&tickslock);
801065ed:	c7 04 24 c0 24 11 80 	movl   $0x801124c0,(%esp)
801065f4:	e8 5a ea ff ff       	call   80105053 <acquire>
  xticks = ticks;
801065f9:	a1 00 2d 11 80       	mov    0x80112d00,%eax
801065fe:	89 45 f4             	mov    %eax,-0xc(%ebp)
  release(&tickslock);
80106601:	c7 04 24 c0 24 11 80 	movl   $0x801124c0,(%esp)
80106608:	e8 a8 ea ff ff       	call   801050b5 <release>
  return xticks;
8010660d:	8b 45 f4             	mov    -0xc(%ebp),%eax
}
80106610:	c9                   	leave  
80106611:	c3                   	ret    

80106612 <sys_ps>:
//changes
int
sys_ps(void)
{
80106612:	55                   	push   %ebp
80106613:	89 e5                	mov    %esp,%ebp
80106615:	83 ec 08             	sub    $0x8,%esp
	ps();
80106618:	e8 d0 e4 ff ff       	call   80104aed <ps>
	return 0;
8010661d:	b8 00 00 00 00       	mov    $0x0,%eax
}
80106622:	c9                   	leave  
80106623:	c3                   	ret    

80106624 <sys_getpriority>:
//------------nice, setpriority,getpriority
int
sys_getpriority(int pid)
{
80106624:	55                   	push   %ebp
80106625:	89 e5                	mov    %esp,%ebp
	return proc->priority;
80106627:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010662d:	8b 40 7c             	mov    0x7c(%eax),%eax
}
80106630:	5d                   	pop    %ebp
80106631:	c3                   	ret    

80106632 <sys_setpriority>:

int
sys_setpriority(int pid,int priority)
{
80106632:	55                   	push   %ebp
80106633:	89 e5                	mov    %esp,%ebp
80106635:	83 ec 18             	sub    $0x18,%esp
	return setpriority(pid,priority);
80106638:	8b 45 0c             	mov    0xc(%ebp),%eax
8010663b:	89 44 24 04          	mov    %eax,0x4(%esp)
8010663f:	8b 45 08             	mov    0x8(%ebp),%eax
80106642:	89 04 24             	mov    %eax,(%esp)
80106645:	e8 22 e8 ff ff       	call   80104e6c <setpriority>
}
8010664a:	c9                   	leave  
8010664b:	c3                   	ret    

8010664c <sys_nice>:

int
sys_nice(int value)
{
8010664c:	55                   	push   %ebp
8010664d:	89 e5                	mov    %esp,%ebp
	if(value==1)
8010664f:	83 7d 08 01          	cmpl   $0x1,0x8(%ebp)
80106653:	75 32                	jne    80106687 <sys_nice+0x3b>
	{
		proc->priority+=1;
80106655:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010665b:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
80106662:	8b 52 7c             	mov    0x7c(%edx),%edx
80106665:	83 c2 01             	add    $0x1,%edx
80106668:	89 50 7c             	mov    %edx,0x7c(%eax)
		if(proc->priority<=PRIORITYLEVELS)
8010666b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106671:	8b 40 7c             	mov    0x7c(%eax),%eax
80106674:	83 f8 05             	cmp    $0x5,%eax
80106677:	77 07                	ja     80106680 <sys_nice+0x34>
		{
			return 0;
80106679:	b8 00 00 00 00       	mov    $0x0,%eax
8010667e:	eb 2f                	jmp    801066af <sys_nice+0x63>
		}
		else
		{
			return -1;
80106680:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106685:	eb 28                	jmp    801066af <sys_nice+0x63>
		}
	}
	if(value==-1)
80106687:	83 7d 08 ff          	cmpl   $0xffffffff,0x8(%ebp)
8010668b:	75 1d                	jne    801066aa <sys_nice+0x5e>
	{
		proc->priority-=1;
8010668d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106693:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
8010669a:	8b 52 7c             	mov    0x7c(%edx),%edx
8010669d:	83 ea 01             	sub    $0x1,%edx
801066a0:	89 50 7c             	mov    %edx,0x7c(%eax)
				{
					return -1;
				}
				else
				{
					return 0;
801066a3:	b8 00 00 00 00       	mov    $0x0,%eax
801066a8:	eb 05                	jmp    801066af <sys_nice+0x63>
				}
	}
	else
	{
		return -1;
801066aa:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
	}
}
801066af:	5d                   	pop    %ebp
801066b0:	c3                   	ret    
801066b1:	00 00                	add    %al,(%eax)
	...

801066b4 <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
801066b4:	55                   	push   %ebp
801066b5:	89 e5                	mov    %esp,%ebp
801066b7:	83 ec 08             	sub    $0x8,%esp
801066ba:	8b 55 08             	mov    0x8(%ebp),%edx
801066bd:	8b 45 0c             	mov    0xc(%ebp),%eax
801066c0:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
801066c4:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
801066c7:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
801066cb:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
801066cf:	ee                   	out    %al,(%dx)
}
801066d0:	c9                   	leave  
801066d1:	c3                   	ret    

801066d2 <timerinit>:
#define TIMER_RATEGEN   0x04    // mode 2, rate generator
#define TIMER_16BIT     0x30    // r/w counter 16 bits, LSB first

void
timerinit(void)
{
801066d2:	55                   	push   %ebp
801066d3:	89 e5                	mov    %esp,%ebp
801066d5:	83 ec 18             	sub    $0x18,%esp
  // Interrupt 100 times/sec.
  outb(TIMER_MODE, TIMER_SEL0 | TIMER_RATEGEN | TIMER_16BIT);
801066d8:	c7 44 24 04 34 00 00 	movl   $0x34,0x4(%esp)
801066df:	00 
801066e0:	c7 04 24 43 00 00 00 	movl   $0x43,(%esp)
801066e7:	e8 c8 ff ff ff       	call   801066b4 <outb>
  outb(IO_TIMER1, TIMER_DIV(100) % 256);
801066ec:	c7 44 24 04 9c 00 00 	movl   $0x9c,0x4(%esp)
801066f3:	00 
801066f4:	c7 04 24 40 00 00 00 	movl   $0x40,(%esp)
801066fb:	e8 b4 ff ff ff       	call   801066b4 <outb>
  outb(IO_TIMER1, TIMER_DIV(100) / 256);
80106700:	c7 44 24 04 2e 00 00 	movl   $0x2e,0x4(%esp)
80106707:	00 
80106708:	c7 04 24 40 00 00 00 	movl   $0x40,(%esp)
8010670f:	e8 a0 ff ff ff       	call   801066b4 <outb>
  picenable(IRQ_TIMER);
80106714:	c7 04 24 00 00 00 00 	movl   $0x0,(%esp)
8010671b:	e8 8d d3 ff ff       	call   80103aad <picenable>
}
80106720:	c9                   	leave  
80106721:	c3                   	ret    
	...

80106724 <alltraps>:
80106724:	1e                   	push   %ds
80106725:	06                   	push   %es
80106726:	0f a0                	push   %fs
80106728:	0f a8                	push   %gs
8010672a:	60                   	pusha  
8010672b:	66 b8 10 00          	mov    $0x10,%ax
8010672f:	8e d8                	mov    %eax,%ds
80106731:	8e c0                	mov    %eax,%es
80106733:	66 b8 18 00          	mov    $0x18,%ax
80106737:	8e e0                	mov    %eax,%fs
80106739:	8e e8                	mov    %eax,%gs
8010673b:	54                   	push   %esp
8010673c:	e8 de 01 00 00       	call   8010691f <trap>
80106741:	83 c4 04             	add    $0x4,%esp

80106744 <trapret>:
80106744:	61                   	popa   
80106745:	0f a9                	pop    %gs
80106747:	0f a1                	pop    %fs
80106749:	07                   	pop    %es
8010674a:	1f                   	pop    %ds
8010674b:	83 c4 08             	add    $0x8,%esp
8010674e:	cf                   	iret   
	...

80106750 <lidt>:

struct gatedesc;

static inline void
lidt(struct gatedesc *p, int size)
{
80106750:	55                   	push   %ebp
80106751:	89 e5                	mov    %esp,%ebp
80106753:	83 ec 10             	sub    $0x10,%esp
  volatile ushort pd[3];

  pd[0] = size-1;
80106756:	8b 45 0c             	mov    0xc(%ebp),%eax
80106759:	83 e8 01             	sub    $0x1,%eax
8010675c:	66 89 45 fa          	mov    %ax,-0x6(%ebp)
  pd[1] = (uint)p;
80106760:	8b 45 08             	mov    0x8(%ebp),%eax
80106763:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  pd[2] = (uint)p >> 16;
80106767:	8b 45 08             	mov    0x8(%ebp),%eax
8010676a:	c1 e8 10             	shr    $0x10,%eax
8010676d:	66 89 45 fe          	mov    %ax,-0x2(%ebp)

  asm volatile("lidt (%0)" : : "r" (pd));
80106771:	8d 45 fa             	lea    -0x6(%ebp),%eax
80106774:	0f 01 18             	lidtl  (%eax)
}
80106777:	c9                   	leave  
80106778:	c3                   	ret    

80106779 <rcr2>:
  return result;
}

static inline uint
rcr2(void)
{
80106779:	55                   	push   %ebp
8010677a:	89 e5                	mov    %esp,%ebp
8010677c:	53                   	push   %ebx
8010677d:	83 ec 10             	sub    $0x10,%esp
  uint val;
  asm volatile("movl %%cr2,%0" : "=r" (val));
80106780:	0f 20 d3             	mov    %cr2,%ebx
80106783:	89 5d f8             	mov    %ebx,-0x8(%ebp)
  return val;
80106786:	8b 45 f8             	mov    -0x8(%ebp),%eax
}
80106789:	83 c4 10             	add    $0x10,%esp
8010678c:	5b                   	pop    %ebx
8010678d:	5d                   	pop    %ebp
8010678e:	c3                   	ret    

8010678f <tvinit>:
struct spinlock tickslock;
uint ticks;

void
tvinit(void)
{
8010678f:	55                   	push   %ebp
80106790:	89 e5                	mov    %esp,%ebp
80106792:	83 ec 28             	sub    $0x28,%esp
  int i;

  for(i = 0; i < 256; i++)
80106795:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010679c:	e9 c3 00 00 00       	jmp    80106864 <tvinit+0xd5>
    SETGATE(idt[i], 0, SEG_KCODE<<3, vectors[i], 0);
801067a1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801067a4:	8b 04 85 9c b0 10 80 	mov    -0x7fef4f64(,%eax,4),%eax
801067ab:	89 c2                	mov    %eax,%edx
801067ad:	8b 45 f4             	mov    -0xc(%ebp),%eax
801067b0:	66 89 14 c5 00 25 11 	mov    %dx,-0x7feedb00(,%eax,8)
801067b7:	80 
801067b8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801067bb:	66 c7 04 c5 02 25 11 	movw   $0x8,-0x7feedafe(,%eax,8)
801067c2:	80 08 00 
801067c5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801067c8:	0f b6 14 c5 04 25 11 	movzbl -0x7feedafc(,%eax,8),%edx
801067cf:	80 
801067d0:	83 e2 e0             	and    $0xffffffe0,%edx
801067d3:	88 14 c5 04 25 11 80 	mov    %dl,-0x7feedafc(,%eax,8)
801067da:	8b 45 f4             	mov    -0xc(%ebp),%eax
801067dd:	0f b6 14 c5 04 25 11 	movzbl -0x7feedafc(,%eax,8),%edx
801067e4:	80 
801067e5:	83 e2 1f             	and    $0x1f,%edx
801067e8:	88 14 c5 04 25 11 80 	mov    %dl,-0x7feedafc(,%eax,8)
801067ef:	8b 45 f4             	mov    -0xc(%ebp),%eax
801067f2:	0f b6 14 c5 05 25 11 	movzbl -0x7feedafb(,%eax,8),%edx
801067f9:	80 
801067fa:	83 e2 f0             	and    $0xfffffff0,%edx
801067fd:	83 ca 0e             	or     $0xe,%edx
80106800:	88 14 c5 05 25 11 80 	mov    %dl,-0x7feedafb(,%eax,8)
80106807:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010680a:	0f b6 14 c5 05 25 11 	movzbl -0x7feedafb(,%eax,8),%edx
80106811:	80 
80106812:	83 e2 ef             	and    $0xffffffef,%edx
80106815:	88 14 c5 05 25 11 80 	mov    %dl,-0x7feedafb(,%eax,8)
8010681c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010681f:	0f b6 14 c5 05 25 11 	movzbl -0x7feedafb(,%eax,8),%edx
80106826:	80 
80106827:	83 e2 9f             	and    $0xffffff9f,%edx
8010682a:	88 14 c5 05 25 11 80 	mov    %dl,-0x7feedafb(,%eax,8)
80106831:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106834:	0f b6 14 c5 05 25 11 	movzbl -0x7feedafb(,%eax,8),%edx
8010683b:	80 
8010683c:	83 ca 80             	or     $0xffffff80,%edx
8010683f:	88 14 c5 05 25 11 80 	mov    %dl,-0x7feedafb(,%eax,8)
80106846:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106849:	8b 04 85 9c b0 10 80 	mov    -0x7fef4f64(,%eax,4),%eax
80106850:	c1 e8 10             	shr    $0x10,%eax
80106853:	89 c2                	mov    %eax,%edx
80106855:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106858:	66 89 14 c5 06 25 11 	mov    %dx,-0x7feedafa(,%eax,8)
8010685f:	80 
void
tvinit(void)
{
  int i;

  for(i = 0; i < 256; i++)
80106860:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80106864:	81 7d f4 ff 00 00 00 	cmpl   $0xff,-0xc(%ebp)
8010686b:	0f 8e 30 ff ff ff    	jle    801067a1 <tvinit+0x12>
    SETGATE(idt[i], 0, SEG_KCODE<<3, vectors[i], 0);
  SETGATE(idt[T_SYSCALL], 1, SEG_KCODE<<3, vectors[T_SYSCALL], DPL_USER);
80106871:	a1 9c b1 10 80       	mov    0x8010b19c,%eax
80106876:	66 a3 00 27 11 80    	mov    %ax,0x80112700
8010687c:	66 c7 05 02 27 11 80 	movw   $0x8,0x80112702
80106883:	08 00 
80106885:	0f b6 05 04 27 11 80 	movzbl 0x80112704,%eax
8010688c:	83 e0 e0             	and    $0xffffffe0,%eax
8010688f:	a2 04 27 11 80       	mov    %al,0x80112704
80106894:	0f b6 05 04 27 11 80 	movzbl 0x80112704,%eax
8010689b:	83 e0 1f             	and    $0x1f,%eax
8010689e:	a2 04 27 11 80       	mov    %al,0x80112704
801068a3:	0f b6 05 05 27 11 80 	movzbl 0x80112705,%eax
801068aa:	83 c8 0f             	or     $0xf,%eax
801068ad:	a2 05 27 11 80       	mov    %al,0x80112705
801068b2:	0f b6 05 05 27 11 80 	movzbl 0x80112705,%eax
801068b9:	83 e0 ef             	and    $0xffffffef,%eax
801068bc:	a2 05 27 11 80       	mov    %al,0x80112705
801068c1:	0f b6 05 05 27 11 80 	movzbl 0x80112705,%eax
801068c8:	83 c8 60             	or     $0x60,%eax
801068cb:	a2 05 27 11 80       	mov    %al,0x80112705
801068d0:	0f b6 05 05 27 11 80 	movzbl 0x80112705,%eax
801068d7:	83 c8 80             	or     $0xffffff80,%eax
801068da:	a2 05 27 11 80       	mov    %al,0x80112705
801068df:	a1 9c b1 10 80       	mov    0x8010b19c,%eax
801068e4:	c1 e8 10             	shr    $0x10,%eax
801068e7:	66 a3 06 27 11 80    	mov    %ax,0x80112706
  
  initlock(&tickslock, "time");
801068ed:	c7 44 24 04 78 8b 10 	movl   $0x80108b78,0x4(%esp)
801068f4:	80 
801068f5:	c7 04 24 c0 24 11 80 	movl   $0x801124c0,(%esp)
801068fc:	e8 31 e7 ff ff       	call   80105032 <initlock>
}
80106901:	c9                   	leave  
80106902:	c3                   	ret    

80106903 <idtinit>:

void
idtinit(void)
{
80106903:	55                   	push   %ebp
80106904:	89 e5                	mov    %esp,%ebp
80106906:	83 ec 08             	sub    $0x8,%esp
  lidt(idt, sizeof(idt));
80106909:	c7 44 24 04 00 08 00 	movl   $0x800,0x4(%esp)
80106910:	00 
80106911:	c7 04 24 00 25 11 80 	movl   $0x80112500,(%esp)
80106918:	e8 33 fe ff ff       	call   80106750 <lidt>
}
8010691d:	c9                   	leave  
8010691e:	c3                   	ret    

8010691f <trap>:

//PAGEBREAK: 41
void
trap(struct trapframe *tf)
{
8010691f:	55                   	push   %ebp
80106920:	89 e5                	mov    %esp,%ebp
80106922:	57                   	push   %edi
80106923:	56                   	push   %esi
80106924:	53                   	push   %ebx
80106925:	83 ec 3c             	sub    $0x3c,%esp
  if(tf->trapno == T_SYSCALL){
80106928:	8b 45 08             	mov    0x8(%ebp),%eax
8010692b:	8b 40 30             	mov    0x30(%eax),%eax
8010692e:	83 f8 40             	cmp    $0x40,%eax
80106931:	75 3e                	jne    80106971 <trap+0x52>
    if(proc->killed)
80106933:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106939:	8b 40 24             	mov    0x24(%eax),%eax
8010693c:	85 c0                	test   %eax,%eax
8010693e:	74 05                	je     80106945 <trap+0x26>
      exit();
80106940:	e8 f7 da ff ff       	call   8010443c <exit>
    proc->tf = tf;
80106945:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010694b:	8b 55 08             	mov    0x8(%ebp),%edx
8010694e:	89 50 18             	mov    %edx,0x18(%eax)
    syscall();
80106951:	e8 6f ed ff ff       	call   801056c5 <syscall>
    if(proc->killed)
80106956:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
8010695c:	8b 40 24             	mov    0x24(%eax),%eax
8010695f:	85 c0                	test   %eax,%eax
80106961:	0f 84 34 02 00 00    	je     80106b9b <trap+0x27c>
      exit();
80106967:	e8 d0 da ff ff       	call   8010443c <exit>
    return;
8010696c:	e9 2a 02 00 00       	jmp    80106b9b <trap+0x27c>
  }

  switch(tf->trapno){
80106971:	8b 45 08             	mov    0x8(%ebp),%eax
80106974:	8b 40 30             	mov    0x30(%eax),%eax
80106977:	83 e8 20             	sub    $0x20,%eax
8010697a:	83 f8 1f             	cmp    $0x1f,%eax
8010697d:	0f 87 bc 00 00 00    	ja     80106a3f <trap+0x120>
80106983:	8b 04 85 20 8c 10 80 	mov    -0x7fef73e0(,%eax,4),%eax
8010698a:	ff e0                	jmp    *%eax
  case T_IRQ0 + IRQ_TIMER:
    if(cpu->id == 0){
8010698c:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80106992:	0f b6 00             	movzbl (%eax),%eax
80106995:	84 c0                	test   %al,%al
80106997:	75 31                	jne    801069ca <trap+0xab>
      acquire(&tickslock);
80106999:	c7 04 24 c0 24 11 80 	movl   $0x801124c0,(%esp)
801069a0:	e8 ae e6 ff ff       	call   80105053 <acquire>
      ticks++;
801069a5:	a1 00 2d 11 80       	mov    0x80112d00,%eax
801069aa:	83 c0 01             	add    $0x1,%eax
801069ad:	a3 00 2d 11 80       	mov    %eax,0x80112d00
      wakeup(&ticks);
801069b2:	c7 04 24 00 2d 11 80 	movl   $0x80112d00,(%esp)
801069b9:	e8 2e df ff ff       	call   801048ec <wakeup>
      release(&tickslock);
801069be:	c7 04 24 c0 24 11 80 	movl   $0x801124c0,(%esp)
801069c5:	e8 eb e6 ff ff       	call   801050b5 <release>
    }
    lapiceoi();
801069ca:	e8 12 c5 ff ff       	call   80102ee1 <lapiceoi>
    break;
801069cf:	e9 41 01 00 00       	jmp    80106b15 <trap+0x1f6>
  case T_IRQ0 + IRQ_IDE:
    ideintr();
801069d4:	e8 10 bd ff ff       	call   801026e9 <ideintr>
    lapiceoi();
801069d9:	e8 03 c5 ff ff       	call   80102ee1 <lapiceoi>
    break;
801069de:	e9 32 01 00 00       	jmp    80106b15 <trap+0x1f6>
  case T_IRQ0 + IRQ_IDE+1:
    // Bochs generates spurious IDE1 interrupts.
    break;
  case T_IRQ0 + IRQ_KBD:
    kbdintr();
801069e3:	e8 d7 c2 ff ff       	call   80102cbf <kbdintr>
    lapiceoi();
801069e8:	e8 f4 c4 ff ff       	call   80102ee1 <lapiceoi>
    break;
801069ed:	e9 23 01 00 00       	jmp    80106b15 <trap+0x1f6>
  case T_IRQ0 + IRQ_COM1:
    uartintr();
801069f2:	e8 a9 03 00 00       	call   80106da0 <uartintr>
    lapiceoi();
801069f7:	e8 e5 c4 ff ff       	call   80102ee1 <lapiceoi>
    break;
801069fc:	e9 14 01 00 00       	jmp    80106b15 <trap+0x1f6>
  case T_IRQ0 + 7:
  case T_IRQ0 + IRQ_SPURIOUS:
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
            cpu->id, tf->cs, tf->eip);
80106a01:	8b 45 08             	mov    0x8(%ebp),%eax
    uartintr();
    lapiceoi();
    break;
  case T_IRQ0 + 7:
  case T_IRQ0 + IRQ_SPURIOUS:
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
80106a04:	8b 48 38             	mov    0x38(%eax),%ecx
            cpu->id, tf->cs, tf->eip);
80106a07:	8b 45 08             	mov    0x8(%ebp),%eax
80106a0a:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
    uartintr();
    lapiceoi();
    break;
  case T_IRQ0 + 7:
  case T_IRQ0 + IRQ_SPURIOUS:
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
80106a0e:	0f b7 d0             	movzwl %ax,%edx
            cpu->id, tf->cs, tf->eip);
80106a11:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80106a17:	0f b6 00             	movzbl (%eax),%eax
    uartintr();
    lapiceoi();
    break;
  case T_IRQ0 + 7:
  case T_IRQ0 + IRQ_SPURIOUS:
    cprintf("cpu%d: spurious interrupt at %x:%x\n",
80106a1a:	0f b6 c0             	movzbl %al,%eax
80106a1d:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
80106a21:	89 54 24 08          	mov    %edx,0x8(%esp)
80106a25:	89 44 24 04          	mov    %eax,0x4(%esp)
80106a29:	c7 04 24 80 8b 10 80 	movl   $0x80108b80,(%esp)
80106a30:	e8 6c 99 ff ff       	call   801003a1 <cprintf>
            cpu->id, tf->cs, tf->eip);
    lapiceoi();
80106a35:	e8 a7 c4 ff ff       	call   80102ee1 <lapiceoi>
    break;
80106a3a:	e9 d6 00 00 00       	jmp    80106b15 <trap+0x1f6>
   
  //PAGEBREAK: 13
  default:
    if(proc == 0 || (tf->cs&3) == 0){
80106a3f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106a45:	85 c0                	test   %eax,%eax
80106a47:	74 11                	je     80106a5a <trap+0x13b>
80106a49:	8b 45 08             	mov    0x8(%ebp),%eax
80106a4c:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
80106a50:	0f b7 c0             	movzwl %ax,%eax
80106a53:	83 e0 03             	and    $0x3,%eax
80106a56:	85 c0                	test   %eax,%eax
80106a58:	75 46                	jne    80106aa0 <trap+0x181>
      // In kernel, it must be our mistake.
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
80106a5a:	e8 1a fd ff ff       	call   80106779 <rcr2>
              tf->trapno, cpu->id, tf->eip, rcr2());
80106a5f:	8b 55 08             	mov    0x8(%ebp),%edx
   
  //PAGEBREAK: 13
  default:
    if(proc == 0 || (tf->cs&3) == 0){
      // In kernel, it must be our mistake.
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
80106a62:	8b 5a 38             	mov    0x38(%edx),%ebx
              tf->trapno, cpu->id, tf->eip, rcr2());
80106a65:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80106a6c:	0f b6 12             	movzbl (%edx),%edx
   
  //PAGEBREAK: 13
  default:
    if(proc == 0 || (tf->cs&3) == 0){
      // In kernel, it must be our mistake.
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
80106a6f:	0f b6 ca             	movzbl %dl,%ecx
              tf->trapno, cpu->id, tf->eip, rcr2());
80106a72:	8b 55 08             	mov    0x8(%ebp),%edx
   
  //PAGEBREAK: 13
  default:
    if(proc == 0 || (tf->cs&3) == 0){
      // In kernel, it must be our mistake.
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
80106a75:	8b 52 30             	mov    0x30(%edx),%edx
80106a78:	89 44 24 10          	mov    %eax,0x10(%esp)
80106a7c:	89 5c 24 0c          	mov    %ebx,0xc(%esp)
80106a80:	89 4c 24 08          	mov    %ecx,0x8(%esp)
80106a84:	89 54 24 04          	mov    %edx,0x4(%esp)
80106a88:	c7 04 24 a4 8b 10 80 	movl   $0x80108ba4,(%esp)
80106a8f:	e8 0d 99 ff ff       	call   801003a1 <cprintf>
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
80106a94:	c7 04 24 d6 8b 10 80 	movl   $0x80108bd6,(%esp)
80106a9b:	e8 9d 9a ff ff       	call   8010053d <panic>
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106aa0:	e8 d4 fc ff ff       	call   80106779 <rcr2>
80106aa5:	89 c2                	mov    %eax,%edx
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
80106aa7:	8b 45 08             	mov    0x8(%ebp),%eax
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106aaa:	8b 78 38             	mov    0x38(%eax),%edi
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
80106aad:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80106ab3:	0f b6 00             	movzbl (%eax),%eax
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106ab6:	0f b6 f0             	movzbl %al,%esi
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
80106ab9:	8b 45 08             	mov    0x8(%ebp),%eax
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106abc:	8b 58 34             	mov    0x34(%eax),%ebx
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
80106abf:	8b 45 08             	mov    0x8(%ebp),%eax
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106ac2:	8b 48 30             	mov    0x30(%eax),%ecx
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
80106ac5:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106acb:	83 c0 6c             	add    $0x6c,%eax
80106ace:	89 45 e4             	mov    %eax,-0x1c(%ebp)
80106ad1:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
      cprintf("unexpected trap %d from cpu %d eip %x (cr2=0x%x)\n",
              tf->trapno, cpu->id, tf->eip, rcr2());
      panic("trap");
    }
    // In user space, assume process misbehaved.
    cprintf("pid %d %s: trap %d err %d on cpu %d "
80106ad7:	8b 40 10             	mov    0x10(%eax),%eax
80106ada:	89 54 24 1c          	mov    %edx,0x1c(%esp)
80106ade:	89 7c 24 18          	mov    %edi,0x18(%esp)
80106ae2:	89 74 24 14          	mov    %esi,0x14(%esp)
80106ae6:	89 5c 24 10          	mov    %ebx,0x10(%esp)
80106aea:	89 4c 24 0c          	mov    %ecx,0xc(%esp)
80106aee:	8b 55 e4             	mov    -0x1c(%ebp),%edx
80106af1:	89 54 24 08          	mov    %edx,0x8(%esp)
80106af5:	89 44 24 04          	mov    %eax,0x4(%esp)
80106af9:	c7 04 24 dc 8b 10 80 	movl   $0x80108bdc,(%esp)
80106b00:	e8 9c 98 ff ff       	call   801003a1 <cprintf>
            "eip 0x%x addr 0x%x--kill proc\n",
            proc->pid, proc->name, tf->trapno, tf->err, cpu->id, tf->eip, 
            rcr2());
    proc->killed = 1;
80106b05:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106b0b:	c7 40 24 01 00 00 00 	movl   $0x1,0x24(%eax)
80106b12:	eb 01                	jmp    80106b15 <trap+0x1f6>
    ideintr();
    lapiceoi();
    break;
  case T_IRQ0 + IRQ_IDE+1:
    // Bochs generates spurious IDE1 interrupts.
    break;
80106b14:	90                   	nop
  }

  // Force process exit if it has been killed and is in user space.
  // (If it is still executing in the kernel, let it keep running 
  // until it gets to the regular system call return.)
  if(proc && proc->killed && (tf->cs&3) == DPL_USER)
80106b15:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106b1b:	85 c0                	test   %eax,%eax
80106b1d:	74 24                	je     80106b43 <trap+0x224>
80106b1f:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106b25:	8b 40 24             	mov    0x24(%eax),%eax
80106b28:	85 c0                	test   %eax,%eax
80106b2a:	74 17                	je     80106b43 <trap+0x224>
80106b2c:	8b 45 08             	mov    0x8(%ebp),%eax
80106b2f:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
80106b33:	0f b7 c0             	movzwl %ax,%eax
80106b36:	83 e0 03             	and    $0x3,%eax
80106b39:	83 f8 03             	cmp    $0x3,%eax
80106b3c:	75 05                	jne    80106b43 <trap+0x224>
    exit();
80106b3e:	e8 f9 d8 ff ff       	call   8010443c <exit>

  // Force process to give up CPU on clock tick.
  // If interrupts were on while locks held, would need to check nlock.
  if(proc && proc->state == RUNNING && tf->trapno == T_IRQ0+IRQ_TIMER)
80106b43:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106b49:	85 c0                	test   %eax,%eax
80106b4b:	74 1e                	je     80106b6b <trap+0x24c>
80106b4d:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106b53:	8b 40 0c             	mov    0xc(%eax),%eax
80106b56:	83 f8 04             	cmp    $0x4,%eax
80106b59:	75 10                	jne    80106b6b <trap+0x24c>
80106b5b:	8b 45 08             	mov    0x8(%ebp),%eax
80106b5e:	8b 40 30             	mov    0x30(%eax),%eax
80106b61:	83 f8 20             	cmp    $0x20,%eax
80106b64:	75 05                	jne    80106b6b <trap+0x24c>
    yield();
80106b66:	e8 48 dc ff ff       	call   801047b3 <yield>

  // Check if the process has been killed since we yielded
  if(proc && proc->killed && (tf->cs&3) == DPL_USER)
80106b6b:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106b71:	85 c0                	test   %eax,%eax
80106b73:	74 27                	je     80106b9c <trap+0x27d>
80106b75:	65 a1 04 00 00 00    	mov    %gs:0x4,%eax
80106b7b:	8b 40 24             	mov    0x24(%eax),%eax
80106b7e:	85 c0                	test   %eax,%eax
80106b80:	74 1a                	je     80106b9c <trap+0x27d>
80106b82:	8b 45 08             	mov    0x8(%ebp),%eax
80106b85:	0f b7 40 3c          	movzwl 0x3c(%eax),%eax
80106b89:	0f b7 c0             	movzwl %ax,%eax
80106b8c:	83 e0 03             	and    $0x3,%eax
80106b8f:	83 f8 03             	cmp    $0x3,%eax
80106b92:	75 08                	jne    80106b9c <trap+0x27d>
    exit();
80106b94:	e8 a3 d8 ff ff       	call   8010443c <exit>
80106b99:	eb 01                	jmp    80106b9c <trap+0x27d>
      exit();
    proc->tf = tf;
    syscall();
    if(proc->killed)
      exit();
    return;
80106b9b:	90                   	nop
    yield();

  // Check if the process has been killed since we yielded
  if(proc && proc->killed && (tf->cs&3) == DPL_USER)
    exit();
}
80106b9c:	83 c4 3c             	add    $0x3c,%esp
80106b9f:	5b                   	pop    %ebx
80106ba0:	5e                   	pop    %esi
80106ba1:	5f                   	pop    %edi
80106ba2:	5d                   	pop    %ebp
80106ba3:	c3                   	ret    

80106ba4 <inb>:
// Routines to let C code use special x86 instructions.

static inline uchar
inb(ushort port)
{
80106ba4:	55                   	push   %ebp
80106ba5:	89 e5                	mov    %esp,%ebp
80106ba7:	53                   	push   %ebx
80106ba8:	83 ec 14             	sub    $0x14,%esp
80106bab:	8b 45 08             	mov    0x8(%ebp),%eax
80106bae:	66 89 45 e8          	mov    %ax,-0x18(%ebp)
  uchar data;

  asm volatile("in %1,%0" : "=a" (data) : "d" (port));
80106bb2:	0f b7 55 e8          	movzwl -0x18(%ebp),%edx
80106bb6:	66 89 55 ea          	mov    %dx,-0x16(%ebp)
80106bba:	0f b7 55 ea          	movzwl -0x16(%ebp),%edx
80106bbe:	ec                   	in     (%dx),%al
80106bbf:	89 c3                	mov    %eax,%ebx
80106bc1:	88 5d fb             	mov    %bl,-0x5(%ebp)
  return data;
80106bc4:	0f b6 45 fb          	movzbl -0x5(%ebp),%eax
}
80106bc8:	83 c4 14             	add    $0x14,%esp
80106bcb:	5b                   	pop    %ebx
80106bcc:	5d                   	pop    %ebp
80106bcd:	c3                   	ret    

80106bce <outb>:
               "memory", "cc");
}

static inline void
outb(ushort port, uchar data)
{
80106bce:	55                   	push   %ebp
80106bcf:	89 e5                	mov    %esp,%ebp
80106bd1:	83 ec 08             	sub    $0x8,%esp
80106bd4:	8b 55 08             	mov    0x8(%ebp),%edx
80106bd7:	8b 45 0c             	mov    0xc(%ebp),%eax
80106bda:	66 89 55 fc          	mov    %dx,-0x4(%ebp)
80106bde:	88 45 f8             	mov    %al,-0x8(%ebp)
  asm volatile("out %0,%1" : : "a" (data), "d" (port));
80106be1:	0f b6 45 f8          	movzbl -0x8(%ebp),%eax
80106be5:	0f b7 55 fc          	movzwl -0x4(%ebp),%edx
80106be9:	ee                   	out    %al,(%dx)
}
80106bea:	c9                   	leave  
80106beb:	c3                   	ret    

80106bec <uartinit>:

static int uart;    // is there a uart?

void
uartinit(void)
{
80106bec:	55                   	push   %ebp
80106bed:	89 e5                	mov    %esp,%ebp
80106bef:	83 ec 28             	sub    $0x28,%esp
  char *p;

  // Turn off the FIFO
  outb(COM1+2, 0);
80106bf2:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106bf9:	00 
80106bfa:	c7 04 24 fa 03 00 00 	movl   $0x3fa,(%esp)
80106c01:	e8 c8 ff ff ff       	call   80106bce <outb>
  
  // 9600 baud, 8 data bits, 1 stop bit, parity off.
  outb(COM1+3, 0x80);    // Unlock divisor
80106c06:	c7 44 24 04 80 00 00 	movl   $0x80,0x4(%esp)
80106c0d:	00 
80106c0e:	c7 04 24 fb 03 00 00 	movl   $0x3fb,(%esp)
80106c15:	e8 b4 ff ff ff       	call   80106bce <outb>
  outb(COM1+0, 115200/9600);
80106c1a:	c7 44 24 04 0c 00 00 	movl   $0xc,0x4(%esp)
80106c21:	00 
80106c22:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
80106c29:	e8 a0 ff ff ff       	call   80106bce <outb>
  outb(COM1+1, 0);
80106c2e:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106c35:	00 
80106c36:	c7 04 24 f9 03 00 00 	movl   $0x3f9,(%esp)
80106c3d:	e8 8c ff ff ff       	call   80106bce <outb>
  outb(COM1+3, 0x03);    // Lock divisor, 8 data bits.
80106c42:	c7 44 24 04 03 00 00 	movl   $0x3,0x4(%esp)
80106c49:	00 
80106c4a:	c7 04 24 fb 03 00 00 	movl   $0x3fb,(%esp)
80106c51:	e8 78 ff ff ff       	call   80106bce <outb>
  outb(COM1+4, 0);
80106c56:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106c5d:	00 
80106c5e:	c7 04 24 fc 03 00 00 	movl   $0x3fc,(%esp)
80106c65:	e8 64 ff ff ff       	call   80106bce <outb>
  outb(COM1+1, 0x01);    // Enable receive interrupts.
80106c6a:	c7 44 24 04 01 00 00 	movl   $0x1,0x4(%esp)
80106c71:	00 
80106c72:	c7 04 24 f9 03 00 00 	movl   $0x3f9,(%esp)
80106c79:	e8 50 ff ff ff       	call   80106bce <outb>

  // If status is 0xFF, no serial port.
  if(inb(COM1+5) == 0xFF)
80106c7e:	c7 04 24 fd 03 00 00 	movl   $0x3fd,(%esp)
80106c85:	e8 1a ff ff ff       	call   80106ba4 <inb>
80106c8a:	3c ff                	cmp    $0xff,%al
80106c8c:	74 6c                	je     80106cfa <uartinit+0x10e>
    return;
  uart = 1;
80106c8e:	c7 05 4c b6 10 80 01 	movl   $0x1,0x8010b64c
80106c95:	00 00 00 

  // Acknowledge pre-existing interrupt conditions;
  // enable interrupts.
  inb(COM1+2);
80106c98:	c7 04 24 fa 03 00 00 	movl   $0x3fa,(%esp)
80106c9f:	e8 00 ff ff ff       	call   80106ba4 <inb>
  inb(COM1+0);
80106ca4:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
80106cab:	e8 f4 fe ff ff       	call   80106ba4 <inb>
  picenable(IRQ_COM1);
80106cb0:	c7 04 24 04 00 00 00 	movl   $0x4,(%esp)
80106cb7:	e8 f1 cd ff ff       	call   80103aad <picenable>
  ioapicenable(IRQ_COM1, 0);
80106cbc:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80106cc3:	00 
80106cc4:	c7 04 24 04 00 00 00 	movl   $0x4,(%esp)
80106ccb:	e8 9e bc ff ff       	call   8010296e <ioapicenable>
  
  // Announce that we're here.
  for(p="xv6...\n"; *p; p++)
80106cd0:	c7 45 f4 a0 8c 10 80 	movl   $0x80108ca0,-0xc(%ebp)
80106cd7:	eb 15                	jmp    80106cee <uartinit+0x102>
    uartputc(*p);
80106cd9:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106cdc:	0f b6 00             	movzbl (%eax),%eax
80106cdf:	0f be c0             	movsbl %al,%eax
80106ce2:	89 04 24             	mov    %eax,(%esp)
80106ce5:	e8 13 00 00 00       	call   80106cfd <uartputc>
  inb(COM1+0);
  picenable(IRQ_COM1);
  ioapicenable(IRQ_COM1, 0);
  
  // Announce that we're here.
  for(p="xv6...\n"; *p; p++)
80106cea:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80106cee:	8b 45 f4             	mov    -0xc(%ebp),%eax
80106cf1:	0f b6 00             	movzbl (%eax),%eax
80106cf4:	84 c0                	test   %al,%al
80106cf6:	75 e1                	jne    80106cd9 <uartinit+0xed>
80106cf8:	eb 01                	jmp    80106cfb <uartinit+0x10f>
  outb(COM1+4, 0);
  outb(COM1+1, 0x01);    // Enable receive interrupts.

  // If status is 0xFF, no serial port.
  if(inb(COM1+5) == 0xFF)
    return;
80106cfa:	90                   	nop
  ioapicenable(IRQ_COM1, 0);
  
  // Announce that we're here.
  for(p="xv6...\n"; *p; p++)
    uartputc(*p);
}
80106cfb:	c9                   	leave  
80106cfc:	c3                   	ret    

80106cfd <uartputc>:

void
uartputc(int c)
{
80106cfd:	55                   	push   %ebp
80106cfe:	89 e5                	mov    %esp,%ebp
80106d00:	83 ec 28             	sub    $0x28,%esp
  int i;

  if(!uart)
80106d03:	a1 4c b6 10 80       	mov    0x8010b64c,%eax
80106d08:	85 c0                	test   %eax,%eax
80106d0a:	74 4d                	je     80106d59 <uartputc+0x5c>
    return;
  for(i = 0; i < 128 && !(inb(COM1+5) & 0x20); i++)
80106d0c:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
80106d13:	eb 10                	jmp    80106d25 <uartputc+0x28>
    microdelay(10);
80106d15:	c7 04 24 0a 00 00 00 	movl   $0xa,(%esp)
80106d1c:	e8 e5 c1 ff ff       	call   80102f06 <microdelay>
{
  int i;

  if(!uart)
    return;
  for(i = 0; i < 128 && !(inb(COM1+5) & 0x20); i++)
80106d21:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80106d25:	83 7d f4 7f          	cmpl   $0x7f,-0xc(%ebp)
80106d29:	7f 16                	jg     80106d41 <uartputc+0x44>
80106d2b:	c7 04 24 fd 03 00 00 	movl   $0x3fd,(%esp)
80106d32:	e8 6d fe ff ff       	call   80106ba4 <inb>
80106d37:	0f b6 c0             	movzbl %al,%eax
80106d3a:	83 e0 20             	and    $0x20,%eax
80106d3d:	85 c0                	test   %eax,%eax
80106d3f:	74 d4                	je     80106d15 <uartputc+0x18>
    microdelay(10);
  outb(COM1+0, c);
80106d41:	8b 45 08             	mov    0x8(%ebp),%eax
80106d44:	0f b6 c0             	movzbl %al,%eax
80106d47:	89 44 24 04          	mov    %eax,0x4(%esp)
80106d4b:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
80106d52:	e8 77 fe ff ff       	call   80106bce <outb>
80106d57:	eb 01                	jmp    80106d5a <uartputc+0x5d>
uartputc(int c)
{
  int i;

  if(!uart)
    return;
80106d59:	90                   	nop
  for(i = 0; i < 128 && !(inb(COM1+5) & 0x20); i++)
    microdelay(10);
  outb(COM1+0, c);
}
80106d5a:	c9                   	leave  
80106d5b:	c3                   	ret    

80106d5c <uartgetc>:

static int
uartgetc(void)
{
80106d5c:	55                   	push   %ebp
80106d5d:	89 e5                	mov    %esp,%ebp
80106d5f:	83 ec 04             	sub    $0x4,%esp
  if(!uart)
80106d62:	a1 4c b6 10 80       	mov    0x8010b64c,%eax
80106d67:	85 c0                	test   %eax,%eax
80106d69:	75 07                	jne    80106d72 <uartgetc+0x16>
    return -1;
80106d6b:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106d70:	eb 2c                	jmp    80106d9e <uartgetc+0x42>
  if(!(inb(COM1+5) & 0x01))
80106d72:	c7 04 24 fd 03 00 00 	movl   $0x3fd,(%esp)
80106d79:	e8 26 fe ff ff       	call   80106ba4 <inb>
80106d7e:	0f b6 c0             	movzbl %al,%eax
80106d81:	83 e0 01             	and    $0x1,%eax
80106d84:	85 c0                	test   %eax,%eax
80106d86:	75 07                	jne    80106d8f <uartgetc+0x33>
    return -1;
80106d88:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80106d8d:	eb 0f                	jmp    80106d9e <uartgetc+0x42>
  return inb(COM1+0);
80106d8f:	c7 04 24 f8 03 00 00 	movl   $0x3f8,(%esp)
80106d96:	e8 09 fe ff ff       	call   80106ba4 <inb>
80106d9b:	0f b6 c0             	movzbl %al,%eax
}
80106d9e:	c9                   	leave  
80106d9f:	c3                   	ret    

80106da0 <uartintr>:

void
uartintr(void)
{
80106da0:	55                   	push   %ebp
80106da1:	89 e5                	mov    %esp,%ebp
80106da3:	83 ec 18             	sub    $0x18,%esp
  consoleintr(uartgetc);
80106da6:	c7 04 24 5c 6d 10 80 	movl   $0x80106d5c,(%esp)
80106dad:	e8 fb 99 ff ff       	call   801007ad <consoleintr>
}
80106db2:	c9                   	leave  
80106db3:	c3                   	ret    

80106db4 <vector0>:
80106db4:	6a 00                	push   $0x0
80106db6:	6a 00                	push   $0x0
80106db8:	e9 67 f9 ff ff       	jmp    80106724 <alltraps>

80106dbd <vector1>:
80106dbd:	6a 00                	push   $0x0
80106dbf:	6a 01                	push   $0x1
80106dc1:	e9 5e f9 ff ff       	jmp    80106724 <alltraps>

80106dc6 <vector2>:
80106dc6:	6a 00                	push   $0x0
80106dc8:	6a 02                	push   $0x2
80106dca:	e9 55 f9 ff ff       	jmp    80106724 <alltraps>

80106dcf <vector3>:
80106dcf:	6a 00                	push   $0x0
80106dd1:	6a 03                	push   $0x3
80106dd3:	e9 4c f9 ff ff       	jmp    80106724 <alltraps>

80106dd8 <vector4>:
80106dd8:	6a 00                	push   $0x0
80106dda:	6a 04                	push   $0x4
80106ddc:	e9 43 f9 ff ff       	jmp    80106724 <alltraps>

80106de1 <vector5>:
80106de1:	6a 00                	push   $0x0
80106de3:	6a 05                	push   $0x5
80106de5:	e9 3a f9 ff ff       	jmp    80106724 <alltraps>

80106dea <vector6>:
80106dea:	6a 00                	push   $0x0
80106dec:	6a 06                	push   $0x6
80106dee:	e9 31 f9 ff ff       	jmp    80106724 <alltraps>

80106df3 <vector7>:
80106df3:	6a 00                	push   $0x0
80106df5:	6a 07                	push   $0x7
80106df7:	e9 28 f9 ff ff       	jmp    80106724 <alltraps>

80106dfc <vector8>:
80106dfc:	6a 08                	push   $0x8
80106dfe:	e9 21 f9 ff ff       	jmp    80106724 <alltraps>

80106e03 <vector9>:
80106e03:	6a 00                	push   $0x0
80106e05:	6a 09                	push   $0x9
80106e07:	e9 18 f9 ff ff       	jmp    80106724 <alltraps>

80106e0c <vector10>:
80106e0c:	6a 0a                	push   $0xa
80106e0e:	e9 11 f9 ff ff       	jmp    80106724 <alltraps>

80106e13 <vector11>:
80106e13:	6a 0b                	push   $0xb
80106e15:	e9 0a f9 ff ff       	jmp    80106724 <alltraps>

80106e1a <vector12>:
80106e1a:	6a 0c                	push   $0xc
80106e1c:	e9 03 f9 ff ff       	jmp    80106724 <alltraps>

80106e21 <vector13>:
80106e21:	6a 0d                	push   $0xd
80106e23:	e9 fc f8 ff ff       	jmp    80106724 <alltraps>

80106e28 <vector14>:
80106e28:	6a 0e                	push   $0xe
80106e2a:	e9 f5 f8 ff ff       	jmp    80106724 <alltraps>

80106e2f <vector15>:
80106e2f:	6a 00                	push   $0x0
80106e31:	6a 0f                	push   $0xf
80106e33:	e9 ec f8 ff ff       	jmp    80106724 <alltraps>

80106e38 <vector16>:
80106e38:	6a 00                	push   $0x0
80106e3a:	6a 10                	push   $0x10
80106e3c:	e9 e3 f8 ff ff       	jmp    80106724 <alltraps>

80106e41 <vector17>:
80106e41:	6a 11                	push   $0x11
80106e43:	e9 dc f8 ff ff       	jmp    80106724 <alltraps>

80106e48 <vector18>:
80106e48:	6a 00                	push   $0x0
80106e4a:	6a 12                	push   $0x12
80106e4c:	e9 d3 f8 ff ff       	jmp    80106724 <alltraps>

80106e51 <vector19>:
80106e51:	6a 00                	push   $0x0
80106e53:	6a 13                	push   $0x13
80106e55:	e9 ca f8 ff ff       	jmp    80106724 <alltraps>

80106e5a <vector20>:
80106e5a:	6a 00                	push   $0x0
80106e5c:	6a 14                	push   $0x14
80106e5e:	e9 c1 f8 ff ff       	jmp    80106724 <alltraps>

80106e63 <vector21>:
80106e63:	6a 00                	push   $0x0
80106e65:	6a 15                	push   $0x15
80106e67:	e9 b8 f8 ff ff       	jmp    80106724 <alltraps>

80106e6c <vector22>:
80106e6c:	6a 00                	push   $0x0
80106e6e:	6a 16                	push   $0x16
80106e70:	e9 af f8 ff ff       	jmp    80106724 <alltraps>

80106e75 <vector23>:
80106e75:	6a 00                	push   $0x0
80106e77:	6a 17                	push   $0x17
80106e79:	e9 a6 f8 ff ff       	jmp    80106724 <alltraps>

80106e7e <vector24>:
80106e7e:	6a 00                	push   $0x0
80106e80:	6a 18                	push   $0x18
80106e82:	e9 9d f8 ff ff       	jmp    80106724 <alltraps>

80106e87 <vector25>:
80106e87:	6a 00                	push   $0x0
80106e89:	6a 19                	push   $0x19
80106e8b:	e9 94 f8 ff ff       	jmp    80106724 <alltraps>

80106e90 <vector26>:
80106e90:	6a 00                	push   $0x0
80106e92:	6a 1a                	push   $0x1a
80106e94:	e9 8b f8 ff ff       	jmp    80106724 <alltraps>

80106e99 <vector27>:
80106e99:	6a 00                	push   $0x0
80106e9b:	6a 1b                	push   $0x1b
80106e9d:	e9 82 f8 ff ff       	jmp    80106724 <alltraps>

80106ea2 <vector28>:
80106ea2:	6a 00                	push   $0x0
80106ea4:	6a 1c                	push   $0x1c
80106ea6:	e9 79 f8 ff ff       	jmp    80106724 <alltraps>

80106eab <vector29>:
80106eab:	6a 00                	push   $0x0
80106ead:	6a 1d                	push   $0x1d
80106eaf:	e9 70 f8 ff ff       	jmp    80106724 <alltraps>

80106eb4 <vector30>:
80106eb4:	6a 00                	push   $0x0
80106eb6:	6a 1e                	push   $0x1e
80106eb8:	e9 67 f8 ff ff       	jmp    80106724 <alltraps>

80106ebd <vector31>:
80106ebd:	6a 00                	push   $0x0
80106ebf:	6a 1f                	push   $0x1f
80106ec1:	e9 5e f8 ff ff       	jmp    80106724 <alltraps>

80106ec6 <vector32>:
80106ec6:	6a 00                	push   $0x0
80106ec8:	6a 20                	push   $0x20
80106eca:	e9 55 f8 ff ff       	jmp    80106724 <alltraps>

80106ecf <vector33>:
80106ecf:	6a 00                	push   $0x0
80106ed1:	6a 21                	push   $0x21
80106ed3:	e9 4c f8 ff ff       	jmp    80106724 <alltraps>

80106ed8 <vector34>:
80106ed8:	6a 00                	push   $0x0
80106eda:	6a 22                	push   $0x22
80106edc:	e9 43 f8 ff ff       	jmp    80106724 <alltraps>

80106ee1 <vector35>:
80106ee1:	6a 00                	push   $0x0
80106ee3:	6a 23                	push   $0x23
80106ee5:	e9 3a f8 ff ff       	jmp    80106724 <alltraps>

80106eea <vector36>:
80106eea:	6a 00                	push   $0x0
80106eec:	6a 24                	push   $0x24
80106eee:	e9 31 f8 ff ff       	jmp    80106724 <alltraps>

80106ef3 <vector37>:
80106ef3:	6a 00                	push   $0x0
80106ef5:	6a 25                	push   $0x25
80106ef7:	e9 28 f8 ff ff       	jmp    80106724 <alltraps>

80106efc <vector38>:
80106efc:	6a 00                	push   $0x0
80106efe:	6a 26                	push   $0x26
80106f00:	e9 1f f8 ff ff       	jmp    80106724 <alltraps>

80106f05 <vector39>:
80106f05:	6a 00                	push   $0x0
80106f07:	6a 27                	push   $0x27
80106f09:	e9 16 f8 ff ff       	jmp    80106724 <alltraps>

80106f0e <vector40>:
80106f0e:	6a 00                	push   $0x0
80106f10:	6a 28                	push   $0x28
80106f12:	e9 0d f8 ff ff       	jmp    80106724 <alltraps>

80106f17 <vector41>:
80106f17:	6a 00                	push   $0x0
80106f19:	6a 29                	push   $0x29
80106f1b:	e9 04 f8 ff ff       	jmp    80106724 <alltraps>

80106f20 <vector42>:
80106f20:	6a 00                	push   $0x0
80106f22:	6a 2a                	push   $0x2a
80106f24:	e9 fb f7 ff ff       	jmp    80106724 <alltraps>

80106f29 <vector43>:
80106f29:	6a 00                	push   $0x0
80106f2b:	6a 2b                	push   $0x2b
80106f2d:	e9 f2 f7 ff ff       	jmp    80106724 <alltraps>

80106f32 <vector44>:
80106f32:	6a 00                	push   $0x0
80106f34:	6a 2c                	push   $0x2c
80106f36:	e9 e9 f7 ff ff       	jmp    80106724 <alltraps>

80106f3b <vector45>:
80106f3b:	6a 00                	push   $0x0
80106f3d:	6a 2d                	push   $0x2d
80106f3f:	e9 e0 f7 ff ff       	jmp    80106724 <alltraps>

80106f44 <vector46>:
80106f44:	6a 00                	push   $0x0
80106f46:	6a 2e                	push   $0x2e
80106f48:	e9 d7 f7 ff ff       	jmp    80106724 <alltraps>

80106f4d <vector47>:
80106f4d:	6a 00                	push   $0x0
80106f4f:	6a 2f                	push   $0x2f
80106f51:	e9 ce f7 ff ff       	jmp    80106724 <alltraps>

80106f56 <vector48>:
80106f56:	6a 00                	push   $0x0
80106f58:	6a 30                	push   $0x30
80106f5a:	e9 c5 f7 ff ff       	jmp    80106724 <alltraps>

80106f5f <vector49>:
80106f5f:	6a 00                	push   $0x0
80106f61:	6a 31                	push   $0x31
80106f63:	e9 bc f7 ff ff       	jmp    80106724 <alltraps>

80106f68 <vector50>:
80106f68:	6a 00                	push   $0x0
80106f6a:	6a 32                	push   $0x32
80106f6c:	e9 b3 f7 ff ff       	jmp    80106724 <alltraps>

80106f71 <vector51>:
80106f71:	6a 00                	push   $0x0
80106f73:	6a 33                	push   $0x33
80106f75:	e9 aa f7 ff ff       	jmp    80106724 <alltraps>

80106f7a <vector52>:
80106f7a:	6a 00                	push   $0x0
80106f7c:	6a 34                	push   $0x34
80106f7e:	e9 a1 f7 ff ff       	jmp    80106724 <alltraps>

80106f83 <vector53>:
80106f83:	6a 00                	push   $0x0
80106f85:	6a 35                	push   $0x35
80106f87:	e9 98 f7 ff ff       	jmp    80106724 <alltraps>

80106f8c <vector54>:
80106f8c:	6a 00                	push   $0x0
80106f8e:	6a 36                	push   $0x36
80106f90:	e9 8f f7 ff ff       	jmp    80106724 <alltraps>

80106f95 <vector55>:
80106f95:	6a 00                	push   $0x0
80106f97:	6a 37                	push   $0x37
80106f99:	e9 86 f7 ff ff       	jmp    80106724 <alltraps>

80106f9e <vector56>:
80106f9e:	6a 00                	push   $0x0
80106fa0:	6a 38                	push   $0x38
80106fa2:	e9 7d f7 ff ff       	jmp    80106724 <alltraps>

80106fa7 <vector57>:
80106fa7:	6a 00                	push   $0x0
80106fa9:	6a 39                	push   $0x39
80106fab:	e9 74 f7 ff ff       	jmp    80106724 <alltraps>

80106fb0 <vector58>:
80106fb0:	6a 00                	push   $0x0
80106fb2:	6a 3a                	push   $0x3a
80106fb4:	e9 6b f7 ff ff       	jmp    80106724 <alltraps>

80106fb9 <vector59>:
80106fb9:	6a 00                	push   $0x0
80106fbb:	6a 3b                	push   $0x3b
80106fbd:	e9 62 f7 ff ff       	jmp    80106724 <alltraps>

80106fc2 <vector60>:
80106fc2:	6a 00                	push   $0x0
80106fc4:	6a 3c                	push   $0x3c
80106fc6:	e9 59 f7 ff ff       	jmp    80106724 <alltraps>

80106fcb <vector61>:
80106fcb:	6a 00                	push   $0x0
80106fcd:	6a 3d                	push   $0x3d
80106fcf:	e9 50 f7 ff ff       	jmp    80106724 <alltraps>

80106fd4 <vector62>:
80106fd4:	6a 00                	push   $0x0
80106fd6:	6a 3e                	push   $0x3e
80106fd8:	e9 47 f7 ff ff       	jmp    80106724 <alltraps>

80106fdd <vector63>:
80106fdd:	6a 00                	push   $0x0
80106fdf:	6a 3f                	push   $0x3f
80106fe1:	e9 3e f7 ff ff       	jmp    80106724 <alltraps>

80106fe6 <vector64>:
80106fe6:	6a 00                	push   $0x0
80106fe8:	6a 40                	push   $0x40
80106fea:	e9 35 f7 ff ff       	jmp    80106724 <alltraps>

80106fef <vector65>:
80106fef:	6a 00                	push   $0x0
80106ff1:	6a 41                	push   $0x41
80106ff3:	e9 2c f7 ff ff       	jmp    80106724 <alltraps>

80106ff8 <vector66>:
80106ff8:	6a 00                	push   $0x0
80106ffa:	6a 42                	push   $0x42
80106ffc:	e9 23 f7 ff ff       	jmp    80106724 <alltraps>

80107001 <vector67>:
80107001:	6a 00                	push   $0x0
80107003:	6a 43                	push   $0x43
80107005:	e9 1a f7 ff ff       	jmp    80106724 <alltraps>

8010700a <vector68>:
8010700a:	6a 00                	push   $0x0
8010700c:	6a 44                	push   $0x44
8010700e:	e9 11 f7 ff ff       	jmp    80106724 <alltraps>

80107013 <vector69>:
80107013:	6a 00                	push   $0x0
80107015:	6a 45                	push   $0x45
80107017:	e9 08 f7 ff ff       	jmp    80106724 <alltraps>

8010701c <vector70>:
8010701c:	6a 00                	push   $0x0
8010701e:	6a 46                	push   $0x46
80107020:	e9 ff f6 ff ff       	jmp    80106724 <alltraps>

80107025 <vector71>:
80107025:	6a 00                	push   $0x0
80107027:	6a 47                	push   $0x47
80107029:	e9 f6 f6 ff ff       	jmp    80106724 <alltraps>

8010702e <vector72>:
8010702e:	6a 00                	push   $0x0
80107030:	6a 48                	push   $0x48
80107032:	e9 ed f6 ff ff       	jmp    80106724 <alltraps>

80107037 <vector73>:
80107037:	6a 00                	push   $0x0
80107039:	6a 49                	push   $0x49
8010703b:	e9 e4 f6 ff ff       	jmp    80106724 <alltraps>

80107040 <vector74>:
80107040:	6a 00                	push   $0x0
80107042:	6a 4a                	push   $0x4a
80107044:	e9 db f6 ff ff       	jmp    80106724 <alltraps>

80107049 <vector75>:
80107049:	6a 00                	push   $0x0
8010704b:	6a 4b                	push   $0x4b
8010704d:	e9 d2 f6 ff ff       	jmp    80106724 <alltraps>

80107052 <vector76>:
80107052:	6a 00                	push   $0x0
80107054:	6a 4c                	push   $0x4c
80107056:	e9 c9 f6 ff ff       	jmp    80106724 <alltraps>

8010705b <vector77>:
8010705b:	6a 00                	push   $0x0
8010705d:	6a 4d                	push   $0x4d
8010705f:	e9 c0 f6 ff ff       	jmp    80106724 <alltraps>

80107064 <vector78>:
80107064:	6a 00                	push   $0x0
80107066:	6a 4e                	push   $0x4e
80107068:	e9 b7 f6 ff ff       	jmp    80106724 <alltraps>

8010706d <vector79>:
8010706d:	6a 00                	push   $0x0
8010706f:	6a 4f                	push   $0x4f
80107071:	e9 ae f6 ff ff       	jmp    80106724 <alltraps>

80107076 <vector80>:
80107076:	6a 00                	push   $0x0
80107078:	6a 50                	push   $0x50
8010707a:	e9 a5 f6 ff ff       	jmp    80106724 <alltraps>

8010707f <vector81>:
8010707f:	6a 00                	push   $0x0
80107081:	6a 51                	push   $0x51
80107083:	e9 9c f6 ff ff       	jmp    80106724 <alltraps>

80107088 <vector82>:
80107088:	6a 00                	push   $0x0
8010708a:	6a 52                	push   $0x52
8010708c:	e9 93 f6 ff ff       	jmp    80106724 <alltraps>

80107091 <vector83>:
80107091:	6a 00                	push   $0x0
80107093:	6a 53                	push   $0x53
80107095:	e9 8a f6 ff ff       	jmp    80106724 <alltraps>

8010709a <vector84>:
8010709a:	6a 00                	push   $0x0
8010709c:	6a 54                	push   $0x54
8010709e:	e9 81 f6 ff ff       	jmp    80106724 <alltraps>

801070a3 <vector85>:
801070a3:	6a 00                	push   $0x0
801070a5:	6a 55                	push   $0x55
801070a7:	e9 78 f6 ff ff       	jmp    80106724 <alltraps>

801070ac <vector86>:
801070ac:	6a 00                	push   $0x0
801070ae:	6a 56                	push   $0x56
801070b0:	e9 6f f6 ff ff       	jmp    80106724 <alltraps>

801070b5 <vector87>:
801070b5:	6a 00                	push   $0x0
801070b7:	6a 57                	push   $0x57
801070b9:	e9 66 f6 ff ff       	jmp    80106724 <alltraps>

801070be <vector88>:
801070be:	6a 00                	push   $0x0
801070c0:	6a 58                	push   $0x58
801070c2:	e9 5d f6 ff ff       	jmp    80106724 <alltraps>

801070c7 <vector89>:
801070c7:	6a 00                	push   $0x0
801070c9:	6a 59                	push   $0x59
801070cb:	e9 54 f6 ff ff       	jmp    80106724 <alltraps>

801070d0 <vector90>:
801070d0:	6a 00                	push   $0x0
801070d2:	6a 5a                	push   $0x5a
801070d4:	e9 4b f6 ff ff       	jmp    80106724 <alltraps>

801070d9 <vector91>:
801070d9:	6a 00                	push   $0x0
801070db:	6a 5b                	push   $0x5b
801070dd:	e9 42 f6 ff ff       	jmp    80106724 <alltraps>

801070e2 <vector92>:
801070e2:	6a 00                	push   $0x0
801070e4:	6a 5c                	push   $0x5c
801070e6:	e9 39 f6 ff ff       	jmp    80106724 <alltraps>

801070eb <vector93>:
801070eb:	6a 00                	push   $0x0
801070ed:	6a 5d                	push   $0x5d
801070ef:	e9 30 f6 ff ff       	jmp    80106724 <alltraps>

801070f4 <vector94>:
801070f4:	6a 00                	push   $0x0
801070f6:	6a 5e                	push   $0x5e
801070f8:	e9 27 f6 ff ff       	jmp    80106724 <alltraps>

801070fd <vector95>:
801070fd:	6a 00                	push   $0x0
801070ff:	6a 5f                	push   $0x5f
80107101:	e9 1e f6 ff ff       	jmp    80106724 <alltraps>

80107106 <vector96>:
80107106:	6a 00                	push   $0x0
80107108:	6a 60                	push   $0x60
8010710a:	e9 15 f6 ff ff       	jmp    80106724 <alltraps>

8010710f <vector97>:
8010710f:	6a 00                	push   $0x0
80107111:	6a 61                	push   $0x61
80107113:	e9 0c f6 ff ff       	jmp    80106724 <alltraps>

80107118 <vector98>:
80107118:	6a 00                	push   $0x0
8010711a:	6a 62                	push   $0x62
8010711c:	e9 03 f6 ff ff       	jmp    80106724 <alltraps>

80107121 <vector99>:
80107121:	6a 00                	push   $0x0
80107123:	6a 63                	push   $0x63
80107125:	e9 fa f5 ff ff       	jmp    80106724 <alltraps>

8010712a <vector100>:
8010712a:	6a 00                	push   $0x0
8010712c:	6a 64                	push   $0x64
8010712e:	e9 f1 f5 ff ff       	jmp    80106724 <alltraps>

80107133 <vector101>:
80107133:	6a 00                	push   $0x0
80107135:	6a 65                	push   $0x65
80107137:	e9 e8 f5 ff ff       	jmp    80106724 <alltraps>

8010713c <vector102>:
8010713c:	6a 00                	push   $0x0
8010713e:	6a 66                	push   $0x66
80107140:	e9 df f5 ff ff       	jmp    80106724 <alltraps>

80107145 <vector103>:
80107145:	6a 00                	push   $0x0
80107147:	6a 67                	push   $0x67
80107149:	e9 d6 f5 ff ff       	jmp    80106724 <alltraps>

8010714e <vector104>:
8010714e:	6a 00                	push   $0x0
80107150:	6a 68                	push   $0x68
80107152:	e9 cd f5 ff ff       	jmp    80106724 <alltraps>

80107157 <vector105>:
80107157:	6a 00                	push   $0x0
80107159:	6a 69                	push   $0x69
8010715b:	e9 c4 f5 ff ff       	jmp    80106724 <alltraps>

80107160 <vector106>:
80107160:	6a 00                	push   $0x0
80107162:	6a 6a                	push   $0x6a
80107164:	e9 bb f5 ff ff       	jmp    80106724 <alltraps>

80107169 <vector107>:
80107169:	6a 00                	push   $0x0
8010716b:	6a 6b                	push   $0x6b
8010716d:	e9 b2 f5 ff ff       	jmp    80106724 <alltraps>

80107172 <vector108>:
80107172:	6a 00                	push   $0x0
80107174:	6a 6c                	push   $0x6c
80107176:	e9 a9 f5 ff ff       	jmp    80106724 <alltraps>

8010717b <vector109>:
8010717b:	6a 00                	push   $0x0
8010717d:	6a 6d                	push   $0x6d
8010717f:	e9 a0 f5 ff ff       	jmp    80106724 <alltraps>

80107184 <vector110>:
80107184:	6a 00                	push   $0x0
80107186:	6a 6e                	push   $0x6e
80107188:	e9 97 f5 ff ff       	jmp    80106724 <alltraps>

8010718d <vector111>:
8010718d:	6a 00                	push   $0x0
8010718f:	6a 6f                	push   $0x6f
80107191:	e9 8e f5 ff ff       	jmp    80106724 <alltraps>

80107196 <vector112>:
80107196:	6a 00                	push   $0x0
80107198:	6a 70                	push   $0x70
8010719a:	e9 85 f5 ff ff       	jmp    80106724 <alltraps>

8010719f <vector113>:
8010719f:	6a 00                	push   $0x0
801071a1:	6a 71                	push   $0x71
801071a3:	e9 7c f5 ff ff       	jmp    80106724 <alltraps>

801071a8 <vector114>:
801071a8:	6a 00                	push   $0x0
801071aa:	6a 72                	push   $0x72
801071ac:	e9 73 f5 ff ff       	jmp    80106724 <alltraps>

801071b1 <vector115>:
801071b1:	6a 00                	push   $0x0
801071b3:	6a 73                	push   $0x73
801071b5:	e9 6a f5 ff ff       	jmp    80106724 <alltraps>

801071ba <vector116>:
801071ba:	6a 00                	push   $0x0
801071bc:	6a 74                	push   $0x74
801071be:	e9 61 f5 ff ff       	jmp    80106724 <alltraps>

801071c3 <vector117>:
801071c3:	6a 00                	push   $0x0
801071c5:	6a 75                	push   $0x75
801071c7:	e9 58 f5 ff ff       	jmp    80106724 <alltraps>

801071cc <vector118>:
801071cc:	6a 00                	push   $0x0
801071ce:	6a 76                	push   $0x76
801071d0:	e9 4f f5 ff ff       	jmp    80106724 <alltraps>

801071d5 <vector119>:
801071d5:	6a 00                	push   $0x0
801071d7:	6a 77                	push   $0x77
801071d9:	e9 46 f5 ff ff       	jmp    80106724 <alltraps>

801071de <vector120>:
801071de:	6a 00                	push   $0x0
801071e0:	6a 78                	push   $0x78
801071e2:	e9 3d f5 ff ff       	jmp    80106724 <alltraps>

801071e7 <vector121>:
801071e7:	6a 00                	push   $0x0
801071e9:	6a 79                	push   $0x79
801071eb:	e9 34 f5 ff ff       	jmp    80106724 <alltraps>

801071f0 <vector122>:
801071f0:	6a 00                	push   $0x0
801071f2:	6a 7a                	push   $0x7a
801071f4:	e9 2b f5 ff ff       	jmp    80106724 <alltraps>

801071f9 <vector123>:
801071f9:	6a 00                	push   $0x0
801071fb:	6a 7b                	push   $0x7b
801071fd:	e9 22 f5 ff ff       	jmp    80106724 <alltraps>

80107202 <vector124>:
80107202:	6a 00                	push   $0x0
80107204:	6a 7c                	push   $0x7c
80107206:	e9 19 f5 ff ff       	jmp    80106724 <alltraps>

8010720b <vector125>:
8010720b:	6a 00                	push   $0x0
8010720d:	6a 7d                	push   $0x7d
8010720f:	e9 10 f5 ff ff       	jmp    80106724 <alltraps>

80107214 <vector126>:
80107214:	6a 00                	push   $0x0
80107216:	6a 7e                	push   $0x7e
80107218:	e9 07 f5 ff ff       	jmp    80106724 <alltraps>

8010721d <vector127>:
8010721d:	6a 00                	push   $0x0
8010721f:	6a 7f                	push   $0x7f
80107221:	e9 fe f4 ff ff       	jmp    80106724 <alltraps>

80107226 <vector128>:
80107226:	6a 00                	push   $0x0
80107228:	68 80 00 00 00       	push   $0x80
8010722d:	e9 f2 f4 ff ff       	jmp    80106724 <alltraps>

80107232 <vector129>:
80107232:	6a 00                	push   $0x0
80107234:	68 81 00 00 00       	push   $0x81
80107239:	e9 e6 f4 ff ff       	jmp    80106724 <alltraps>

8010723e <vector130>:
8010723e:	6a 00                	push   $0x0
80107240:	68 82 00 00 00       	push   $0x82
80107245:	e9 da f4 ff ff       	jmp    80106724 <alltraps>

8010724a <vector131>:
8010724a:	6a 00                	push   $0x0
8010724c:	68 83 00 00 00       	push   $0x83
80107251:	e9 ce f4 ff ff       	jmp    80106724 <alltraps>

80107256 <vector132>:
80107256:	6a 00                	push   $0x0
80107258:	68 84 00 00 00       	push   $0x84
8010725d:	e9 c2 f4 ff ff       	jmp    80106724 <alltraps>

80107262 <vector133>:
80107262:	6a 00                	push   $0x0
80107264:	68 85 00 00 00       	push   $0x85
80107269:	e9 b6 f4 ff ff       	jmp    80106724 <alltraps>

8010726e <vector134>:
8010726e:	6a 00                	push   $0x0
80107270:	68 86 00 00 00       	push   $0x86
80107275:	e9 aa f4 ff ff       	jmp    80106724 <alltraps>

8010727a <vector135>:
8010727a:	6a 00                	push   $0x0
8010727c:	68 87 00 00 00       	push   $0x87
80107281:	e9 9e f4 ff ff       	jmp    80106724 <alltraps>

80107286 <vector136>:
80107286:	6a 00                	push   $0x0
80107288:	68 88 00 00 00       	push   $0x88
8010728d:	e9 92 f4 ff ff       	jmp    80106724 <alltraps>

80107292 <vector137>:
80107292:	6a 00                	push   $0x0
80107294:	68 89 00 00 00       	push   $0x89
80107299:	e9 86 f4 ff ff       	jmp    80106724 <alltraps>

8010729e <vector138>:
8010729e:	6a 00                	push   $0x0
801072a0:	68 8a 00 00 00       	push   $0x8a
801072a5:	e9 7a f4 ff ff       	jmp    80106724 <alltraps>

801072aa <vector139>:
801072aa:	6a 00                	push   $0x0
801072ac:	68 8b 00 00 00       	push   $0x8b
801072b1:	e9 6e f4 ff ff       	jmp    80106724 <alltraps>

801072b6 <vector140>:
801072b6:	6a 00                	push   $0x0
801072b8:	68 8c 00 00 00       	push   $0x8c
801072bd:	e9 62 f4 ff ff       	jmp    80106724 <alltraps>

801072c2 <vector141>:
801072c2:	6a 00                	push   $0x0
801072c4:	68 8d 00 00 00       	push   $0x8d
801072c9:	e9 56 f4 ff ff       	jmp    80106724 <alltraps>

801072ce <vector142>:
801072ce:	6a 00                	push   $0x0
801072d0:	68 8e 00 00 00       	push   $0x8e
801072d5:	e9 4a f4 ff ff       	jmp    80106724 <alltraps>

801072da <vector143>:
801072da:	6a 00                	push   $0x0
801072dc:	68 8f 00 00 00       	push   $0x8f
801072e1:	e9 3e f4 ff ff       	jmp    80106724 <alltraps>

801072e6 <vector144>:
801072e6:	6a 00                	push   $0x0
801072e8:	68 90 00 00 00       	push   $0x90
801072ed:	e9 32 f4 ff ff       	jmp    80106724 <alltraps>

801072f2 <vector145>:
801072f2:	6a 00                	push   $0x0
801072f4:	68 91 00 00 00       	push   $0x91
801072f9:	e9 26 f4 ff ff       	jmp    80106724 <alltraps>

801072fe <vector146>:
801072fe:	6a 00                	push   $0x0
80107300:	68 92 00 00 00       	push   $0x92
80107305:	e9 1a f4 ff ff       	jmp    80106724 <alltraps>

8010730a <vector147>:
8010730a:	6a 00                	push   $0x0
8010730c:	68 93 00 00 00       	push   $0x93
80107311:	e9 0e f4 ff ff       	jmp    80106724 <alltraps>

80107316 <vector148>:
80107316:	6a 00                	push   $0x0
80107318:	68 94 00 00 00       	push   $0x94
8010731d:	e9 02 f4 ff ff       	jmp    80106724 <alltraps>

80107322 <vector149>:
80107322:	6a 00                	push   $0x0
80107324:	68 95 00 00 00       	push   $0x95
80107329:	e9 f6 f3 ff ff       	jmp    80106724 <alltraps>

8010732e <vector150>:
8010732e:	6a 00                	push   $0x0
80107330:	68 96 00 00 00       	push   $0x96
80107335:	e9 ea f3 ff ff       	jmp    80106724 <alltraps>

8010733a <vector151>:
8010733a:	6a 00                	push   $0x0
8010733c:	68 97 00 00 00       	push   $0x97
80107341:	e9 de f3 ff ff       	jmp    80106724 <alltraps>

80107346 <vector152>:
80107346:	6a 00                	push   $0x0
80107348:	68 98 00 00 00       	push   $0x98
8010734d:	e9 d2 f3 ff ff       	jmp    80106724 <alltraps>

80107352 <vector153>:
80107352:	6a 00                	push   $0x0
80107354:	68 99 00 00 00       	push   $0x99
80107359:	e9 c6 f3 ff ff       	jmp    80106724 <alltraps>

8010735e <vector154>:
8010735e:	6a 00                	push   $0x0
80107360:	68 9a 00 00 00       	push   $0x9a
80107365:	e9 ba f3 ff ff       	jmp    80106724 <alltraps>

8010736a <vector155>:
8010736a:	6a 00                	push   $0x0
8010736c:	68 9b 00 00 00       	push   $0x9b
80107371:	e9 ae f3 ff ff       	jmp    80106724 <alltraps>

80107376 <vector156>:
80107376:	6a 00                	push   $0x0
80107378:	68 9c 00 00 00       	push   $0x9c
8010737d:	e9 a2 f3 ff ff       	jmp    80106724 <alltraps>

80107382 <vector157>:
80107382:	6a 00                	push   $0x0
80107384:	68 9d 00 00 00       	push   $0x9d
80107389:	e9 96 f3 ff ff       	jmp    80106724 <alltraps>

8010738e <vector158>:
8010738e:	6a 00                	push   $0x0
80107390:	68 9e 00 00 00       	push   $0x9e
80107395:	e9 8a f3 ff ff       	jmp    80106724 <alltraps>

8010739a <vector159>:
8010739a:	6a 00                	push   $0x0
8010739c:	68 9f 00 00 00       	push   $0x9f
801073a1:	e9 7e f3 ff ff       	jmp    80106724 <alltraps>

801073a6 <vector160>:
801073a6:	6a 00                	push   $0x0
801073a8:	68 a0 00 00 00       	push   $0xa0
801073ad:	e9 72 f3 ff ff       	jmp    80106724 <alltraps>

801073b2 <vector161>:
801073b2:	6a 00                	push   $0x0
801073b4:	68 a1 00 00 00       	push   $0xa1
801073b9:	e9 66 f3 ff ff       	jmp    80106724 <alltraps>

801073be <vector162>:
801073be:	6a 00                	push   $0x0
801073c0:	68 a2 00 00 00       	push   $0xa2
801073c5:	e9 5a f3 ff ff       	jmp    80106724 <alltraps>

801073ca <vector163>:
801073ca:	6a 00                	push   $0x0
801073cc:	68 a3 00 00 00       	push   $0xa3
801073d1:	e9 4e f3 ff ff       	jmp    80106724 <alltraps>

801073d6 <vector164>:
801073d6:	6a 00                	push   $0x0
801073d8:	68 a4 00 00 00       	push   $0xa4
801073dd:	e9 42 f3 ff ff       	jmp    80106724 <alltraps>

801073e2 <vector165>:
801073e2:	6a 00                	push   $0x0
801073e4:	68 a5 00 00 00       	push   $0xa5
801073e9:	e9 36 f3 ff ff       	jmp    80106724 <alltraps>

801073ee <vector166>:
801073ee:	6a 00                	push   $0x0
801073f0:	68 a6 00 00 00       	push   $0xa6
801073f5:	e9 2a f3 ff ff       	jmp    80106724 <alltraps>

801073fa <vector167>:
801073fa:	6a 00                	push   $0x0
801073fc:	68 a7 00 00 00       	push   $0xa7
80107401:	e9 1e f3 ff ff       	jmp    80106724 <alltraps>

80107406 <vector168>:
80107406:	6a 00                	push   $0x0
80107408:	68 a8 00 00 00       	push   $0xa8
8010740d:	e9 12 f3 ff ff       	jmp    80106724 <alltraps>

80107412 <vector169>:
80107412:	6a 00                	push   $0x0
80107414:	68 a9 00 00 00       	push   $0xa9
80107419:	e9 06 f3 ff ff       	jmp    80106724 <alltraps>

8010741e <vector170>:
8010741e:	6a 00                	push   $0x0
80107420:	68 aa 00 00 00       	push   $0xaa
80107425:	e9 fa f2 ff ff       	jmp    80106724 <alltraps>

8010742a <vector171>:
8010742a:	6a 00                	push   $0x0
8010742c:	68 ab 00 00 00       	push   $0xab
80107431:	e9 ee f2 ff ff       	jmp    80106724 <alltraps>

80107436 <vector172>:
80107436:	6a 00                	push   $0x0
80107438:	68 ac 00 00 00       	push   $0xac
8010743d:	e9 e2 f2 ff ff       	jmp    80106724 <alltraps>

80107442 <vector173>:
80107442:	6a 00                	push   $0x0
80107444:	68 ad 00 00 00       	push   $0xad
80107449:	e9 d6 f2 ff ff       	jmp    80106724 <alltraps>

8010744e <vector174>:
8010744e:	6a 00                	push   $0x0
80107450:	68 ae 00 00 00       	push   $0xae
80107455:	e9 ca f2 ff ff       	jmp    80106724 <alltraps>

8010745a <vector175>:
8010745a:	6a 00                	push   $0x0
8010745c:	68 af 00 00 00       	push   $0xaf
80107461:	e9 be f2 ff ff       	jmp    80106724 <alltraps>

80107466 <vector176>:
80107466:	6a 00                	push   $0x0
80107468:	68 b0 00 00 00       	push   $0xb0
8010746d:	e9 b2 f2 ff ff       	jmp    80106724 <alltraps>

80107472 <vector177>:
80107472:	6a 00                	push   $0x0
80107474:	68 b1 00 00 00       	push   $0xb1
80107479:	e9 a6 f2 ff ff       	jmp    80106724 <alltraps>

8010747e <vector178>:
8010747e:	6a 00                	push   $0x0
80107480:	68 b2 00 00 00       	push   $0xb2
80107485:	e9 9a f2 ff ff       	jmp    80106724 <alltraps>

8010748a <vector179>:
8010748a:	6a 00                	push   $0x0
8010748c:	68 b3 00 00 00       	push   $0xb3
80107491:	e9 8e f2 ff ff       	jmp    80106724 <alltraps>

80107496 <vector180>:
80107496:	6a 00                	push   $0x0
80107498:	68 b4 00 00 00       	push   $0xb4
8010749d:	e9 82 f2 ff ff       	jmp    80106724 <alltraps>

801074a2 <vector181>:
801074a2:	6a 00                	push   $0x0
801074a4:	68 b5 00 00 00       	push   $0xb5
801074a9:	e9 76 f2 ff ff       	jmp    80106724 <alltraps>

801074ae <vector182>:
801074ae:	6a 00                	push   $0x0
801074b0:	68 b6 00 00 00       	push   $0xb6
801074b5:	e9 6a f2 ff ff       	jmp    80106724 <alltraps>

801074ba <vector183>:
801074ba:	6a 00                	push   $0x0
801074bc:	68 b7 00 00 00       	push   $0xb7
801074c1:	e9 5e f2 ff ff       	jmp    80106724 <alltraps>

801074c6 <vector184>:
801074c6:	6a 00                	push   $0x0
801074c8:	68 b8 00 00 00       	push   $0xb8
801074cd:	e9 52 f2 ff ff       	jmp    80106724 <alltraps>

801074d2 <vector185>:
801074d2:	6a 00                	push   $0x0
801074d4:	68 b9 00 00 00       	push   $0xb9
801074d9:	e9 46 f2 ff ff       	jmp    80106724 <alltraps>

801074de <vector186>:
801074de:	6a 00                	push   $0x0
801074e0:	68 ba 00 00 00       	push   $0xba
801074e5:	e9 3a f2 ff ff       	jmp    80106724 <alltraps>

801074ea <vector187>:
801074ea:	6a 00                	push   $0x0
801074ec:	68 bb 00 00 00       	push   $0xbb
801074f1:	e9 2e f2 ff ff       	jmp    80106724 <alltraps>

801074f6 <vector188>:
801074f6:	6a 00                	push   $0x0
801074f8:	68 bc 00 00 00       	push   $0xbc
801074fd:	e9 22 f2 ff ff       	jmp    80106724 <alltraps>

80107502 <vector189>:
80107502:	6a 00                	push   $0x0
80107504:	68 bd 00 00 00       	push   $0xbd
80107509:	e9 16 f2 ff ff       	jmp    80106724 <alltraps>

8010750e <vector190>:
8010750e:	6a 00                	push   $0x0
80107510:	68 be 00 00 00       	push   $0xbe
80107515:	e9 0a f2 ff ff       	jmp    80106724 <alltraps>

8010751a <vector191>:
8010751a:	6a 00                	push   $0x0
8010751c:	68 bf 00 00 00       	push   $0xbf
80107521:	e9 fe f1 ff ff       	jmp    80106724 <alltraps>

80107526 <vector192>:
80107526:	6a 00                	push   $0x0
80107528:	68 c0 00 00 00       	push   $0xc0
8010752d:	e9 f2 f1 ff ff       	jmp    80106724 <alltraps>

80107532 <vector193>:
80107532:	6a 00                	push   $0x0
80107534:	68 c1 00 00 00       	push   $0xc1
80107539:	e9 e6 f1 ff ff       	jmp    80106724 <alltraps>

8010753e <vector194>:
8010753e:	6a 00                	push   $0x0
80107540:	68 c2 00 00 00       	push   $0xc2
80107545:	e9 da f1 ff ff       	jmp    80106724 <alltraps>

8010754a <vector195>:
8010754a:	6a 00                	push   $0x0
8010754c:	68 c3 00 00 00       	push   $0xc3
80107551:	e9 ce f1 ff ff       	jmp    80106724 <alltraps>

80107556 <vector196>:
80107556:	6a 00                	push   $0x0
80107558:	68 c4 00 00 00       	push   $0xc4
8010755d:	e9 c2 f1 ff ff       	jmp    80106724 <alltraps>

80107562 <vector197>:
80107562:	6a 00                	push   $0x0
80107564:	68 c5 00 00 00       	push   $0xc5
80107569:	e9 b6 f1 ff ff       	jmp    80106724 <alltraps>

8010756e <vector198>:
8010756e:	6a 00                	push   $0x0
80107570:	68 c6 00 00 00       	push   $0xc6
80107575:	e9 aa f1 ff ff       	jmp    80106724 <alltraps>

8010757a <vector199>:
8010757a:	6a 00                	push   $0x0
8010757c:	68 c7 00 00 00       	push   $0xc7
80107581:	e9 9e f1 ff ff       	jmp    80106724 <alltraps>

80107586 <vector200>:
80107586:	6a 00                	push   $0x0
80107588:	68 c8 00 00 00       	push   $0xc8
8010758d:	e9 92 f1 ff ff       	jmp    80106724 <alltraps>

80107592 <vector201>:
80107592:	6a 00                	push   $0x0
80107594:	68 c9 00 00 00       	push   $0xc9
80107599:	e9 86 f1 ff ff       	jmp    80106724 <alltraps>

8010759e <vector202>:
8010759e:	6a 00                	push   $0x0
801075a0:	68 ca 00 00 00       	push   $0xca
801075a5:	e9 7a f1 ff ff       	jmp    80106724 <alltraps>

801075aa <vector203>:
801075aa:	6a 00                	push   $0x0
801075ac:	68 cb 00 00 00       	push   $0xcb
801075b1:	e9 6e f1 ff ff       	jmp    80106724 <alltraps>

801075b6 <vector204>:
801075b6:	6a 00                	push   $0x0
801075b8:	68 cc 00 00 00       	push   $0xcc
801075bd:	e9 62 f1 ff ff       	jmp    80106724 <alltraps>

801075c2 <vector205>:
801075c2:	6a 00                	push   $0x0
801075c4:	68 cd 00 00 00       	push   $0xcd
801075c9:	e9 56 f1 ff ff       	jmp    80106724 <alltraps>

801075ce <vector206>:
801075ce:	6a 00                	push   $0x0
801075d0:	68 ce 00 00 00       	push   $0xce
801075d5:	e9 4a f1 ff ff       	jmp    80106724 <alltraps>

801075da <vector207>:
801075da:	6a 00                	push   $0x0
801075dc:	68 cf 00 00 00       	push   $0xcf
801075e1:	e9 3e f1 ff ff       	jmp    80106724 <alltraps>

801075e6 <vector208>:
801075e6:	6a 00                	push   $0x0
801075e8:	68 d0 00 00 00       	push   $0xd0
801075ed:	e9 32 f1 ff ff       	jmp    80106724 <alltraps>

801075f2 <vector209>:
801075f2:	6a 00                	push   $0x0
801075f4:	68 d1 00 00 00       	push   $0xd1
801075f9:	e9 26 f1 ff ff       	jmp    80106724 <alltraps>

801075fe <vector210>:
801075fe:	6a 00                	push   $0x0
80107600:	68 d2 00 00 00       	push   $0xd2
80107605:	e9 1a f1 ff ff       	jmp    80106724 <alltraps>

8010760a <vector211>:
8010760a:	6a 00                	push   $0x0
8010760c:	68 d3 00 00 00       	push   $0xd3
80107611:	e9 0e f1 ff ff       	jmp    80106724 <alltraps>

80107616 <vector212>:
80107616:	6a 00                	push   $0x0
80107618:	68 d4 00 00 00       	push   $0xd4
8010761d:	e9 02 f1 ff ff       	jmp    80106724 <alltraps>

80107622 <vector213>:
80107622:	6a 00                	push   $0x0
80107624:	68 d5 00 00 00       	push   $0xd5
80107629:	e9 f6 f0 ff ff       	jmp    80106724 <alltraps>

8010762e <vector214>:
8010762e:	6a 00                	push   $0x0
80107630:	68 d6 00 00 00       	push   $0xd6
80107635:	e9 ea f0 ff ff       	jmp    80106724 <alltraps>

8010763a <vector215>:
8010763a:	6a 00                	push   $0x0
8010763c:	68 d7 00 00 00       	push   $0xd7
80107641:	e9 de f0 ff ff       	jmp    80106724 <alltraps>

80107646 <vector216>:
80107646:	6a 00                	push   $0x0
80107648:	68 d8 00 00 00       	push   $0xd8
8010764d:	e9 d2 f0 ff ff       	jmp    80106724 <alltraps>

80107652 <vector217>:
80107652:	6a 00                	push   $0x0
80107654:	68 d9 00 00 00       	push   $0xd9
80107659:	e9 c6 f0 ff ff       	jmp    80106724 <alltraps>

8010765e <vector218>:
8010765e:	6a 00                	push   $0x0
80107660:	68 da 00 00 00       	push   $0xda
80107665:	e9 ba f0 ff ff       	jmp    80106724 <alltraps>

8010766a <vector219>:
8010766a:	6a 00                	push   $0x0
8010766c:	68 db 00 00 00       	push   $0xdb
80107671:	e9 ae f0 ff ff       	jmp    80106724 <alltraps>

80107676 <vector220>:
80107676:	6a 00                	push   $0x0
80107678:	68 dc 00 00 00       	push   $0xdc
8010767d:	e9 a2 f0 ff ff       	jmp    80106724 <alltraps>

80107682 <vector221>:
80107682:	6a 00                	push   $0x0
80107684:	68 dd 00 00 00       	push   $0xdd
80107689:	e9 96 f0 ff ff       	jmp    80106724 <alltraps>

8010768e <vector222>:
8010768e:	6a 00                	push   $0x0
80107690:	68 de 00 00 00       	push   $0xde
80107695:	e9 8a f0 ff ff       	jmp    80106724 <alltraps>

8010769a <vector223>:
8010769a:	6a 00                	push   $0x0
8010769c:	68 df 00 00 00       	push   $0xdf
801076a1:	e9 7e f0 ff ff       	jmp    80106724 <alltraps>

801076a6 <vector224>:
801076a6:	6a 00                	push   $0x0
801076a8:	68 e0 00 00 00       	push   $0xe0
801076ad:	e9 72 f0 ff ff       	jmp    80106724 <alltraps>

801076b2 <vector225>:
801076b2:	6a 00                	push   $0x0
801076b4:	68 e1 00 00 00       	push   $0xe1
801076b9:	e9 66 f0 ff ff       	jmp    80106724 <alltraps>

801076be <vector226>:
801076be:	6a 00                	push   $0x0
801076c0:	68 e2 00 00 00       	push   $0xe2
801076c5:	e9 5a f0 ff ff       	jmp    80106724 <alltraps>

801076ca <vector227>:
801076ca:	6a 00                	push   $0x0
801076cc:	68 e3 00 00 00       	push   $0xe3
801076d1:	e9 4e f0 ff ff       	jmp    80106724 <alltraps>

801076d6 <vector228>:
801076d6:	6a 00                	push   $0x0
801076d8:	68 e4 00 00 00       	push   $0xe4
801076dd:	e9 42 f0 ff ff       	jmp    80106724 <alltraps>

801076e2 <vector229>:
801076e2:	6a 00                	push   $0x0
801076e4:	68 e5 00 00 00       	push   $0xe5
801076e9:	e9 36 f0 ff ff       	jmp    80106724 <alltraps>

801076ee <vector230>:
801076ee:	6a 00                	push   $0x0
801076f0:	68 e6 00 00 00       	push   $0xe6
801076f5:	e9 2a f0 ff ff       	jmp    80106724 <alltraps>

801076fa <vector231>:
801076fa:	6a 00                	push   $0x0
801076fc:	68 e7 00 00 00       	push   $0xe7
80107701:	e9 1e f0 ff ff       	jmp    80106724 <alltraps>

80107706 <vector232>:
80107706:	6a 00                	push   $0x0
80107708:	68 e8 00 00 00       	push   $0xe8
8010770d:	e9 12 f0 ff ff       	jmp    80106724 <alltraps>

80107712 <vector233>:
80107712:	6a 00                	push   $0x0
80107714:	68 e9 00 00 00       	push   $0xe9
80107719:	e9 06 f0 ff ff       	jmp    80106724 <alltraps>

8010771e <vector234>:
8010771e:	6a 00                	push   $0x0
80107720:	68 ea 00 00 00       	push   $0xea
80107725:	e9 fa ef ff ff       	jmp    80106724 <alltraps>

8010772a <vector235>:
8010772a:	6a 00                	push   $0x0
8010772c:	68 eb 00 00 00       	push   $0xeb
80107731:	e9 ee ef ff ff       	jmp    80106724 <alltraps>

80107736 <vector236>:
80107736:	6a 00                	push   $0x0
80107738:	68 ec 00 00 00       	push   $0xec
8010773d:	e9 e2 ef ff ff       	jmp    80106724 <alltraps>

80107742 <vector237>:
80107742:	6a 00                	push   $0x0
80107744:	68 ed 00 00 00       	push   $0xed
80107749:	e9 d6 ef ff ff       	jmp    80106724 <alltraps>

8010774e <vector238>:
8010774e:	6a 00                	push   $0x0
80107750:	68 ee 00 00 00       	push   $0xee
80107755:	e9 ca ef ff ff       	jmp    80106724 <alltraps>

8010775a <vector239>:
8010775a:	6a 00                	push   $0x0
8010775c:	68 ef 00 00 00       	push   $0xef
80107761:	e9 be ef ff ff       	jmp    80106724 <alltraps>

80107766 <vector240>:
80107766:	6a 00                	push   $0x0
80107768:	68 f0 00 00 00       	push   $0xf0
8010776d:	e9 b2 ef ff ff       	jmp    80106724 <alltraps>

80107772 <vector241>:
80107772:	6a 00                	push   $0x0
80107774:	68 f1 00 00 00       	push   $0xf1
80107779:	e9 a6 ef ff ff       	jmp    80106724 <alltraps>

8010777e <vector242>:
8010777e:	6a 00                	push   $0x0
80107780:	68 f2 00 00 00       	push   $0xf2
80107785:	e9 9a ef ff ff       	jmp    80106724 <alltraps>

8010778a <vector243>:
8010778a:	6a 00                	push   $0x0
8010778c:	68 f3 00 00 00       	push   $0xf3
80107791:	e9 8e ef ff ff       	jmp    80106724 <alltraps>

80107796 <vector244>:
80107796:	6a 00                	push   $0x0
80107798:	68 f4 00 00 00       	push   $0xf4
8010779d:	e9 82 ef ff ff       	jmp    80106724 <alltraps>

801077a2 <vector245>:
801077a2:	6a 00                	push   $0x0
801077a4:	68 f5 00 00 00       	push   $0xf5
801077a9:	e9 76 ef ff ff       	jmp    80106724 <alltraps>

801077ae <vector246>:
801077ae:	6a 00                	push   $0x0
801077b0:	68 f6 00 00 00       	push   $0xf6
801077b5:	e9 6a ef ff ff       	jmp    80106724 <alltraps>

801077ba <vector247>:
801077ba:	6a 00                	push   $0x0
801077bc:	68 f7 00 00 00       	push   $0xf7
801077c1:	e9 5e ef ff ff       	jmp    80106724 <alltraps>

801077c6 <vector248>:
801077c6:	6a 00                	push   $0x0
801077c8:	68 f8 00 00 00       	push   $0xf8
801077cd:	e9 52 ef ff ff       	jmp    80106724 <alltraps>

801077d2 <vector249>:
801077d2:	6a 00                	push   $0x0
801077d4:	68 f9 00 00 00       	push   $0xf9
801077d9:	e9 46 ef ff ff       	jmp    80106724 <alltraps>

801077de <vector250>:
801077de:	6a 00                	push   $0x0
801077e0:	68 fa 00 00 00       	push   $0xfa
801077e5:	e9 3a ef ff ff       	jmp    80106724 <alltraps>

801077ea <vector251>:
801077ea:	6a 00                	push   $0x0
801077ec:	68 fb 00 00 00       	push   $0xfb
801077f1:	e9 2e ef ff ff       	jmp    80106724 <alltraps>

801077f6 <vector252>:
801077f6:	6a 00                	push   $0x0
801077f8:	68 fc 00 00 00       	push   $0xfc
801077fd:	e9 22 ef ff ff       	jmp    80106724 <alltraps>

80107802 <vector253>:
80107802:	6a 00                	push   $0x0
80107804:	68 fd 00 00 00       	push   $0xfd
80107809:	e9 16 ef ff ff       	jmp    80106724 <alltraps>

8010780e <vector254>:
8010780e:	6a 00                	push   $0x0
80107810:	68 fe 00 00 00       	push   $0xfe
80107815:	e9 0a ef ff ff       	jmp    80106724 <alltraps>

8010781a <vector255>:
8010781a:	6a 00                	push   $0x0
8010781c:	68 ff 00 00 00       	push   $0xff
80107821:	e9 fe ee ff ff       	jmp    80106724 <alltraps>
	...

80107828 <lgdt>:

struct segdesc;

static inline void
lgdt(struct segdesc *p, int size)
{
80107828:	55                   	push   %ebp
80107829:	89 e5                	mov    %esp,%ebp
8010782b:	83 ec 10             	sub    $0x10,%esp
  volatile ushort pd[3];

  pd[0] = size-1;
8010782e:	8b 45 0c             	mov    0xc(%ebp),%eax
80107831:	83 e8 01             	sub    $0x1,%eax
80107834:	66 89 45 fa          	mov    %ax,-0x6(%ebp)
  pd[1] = (uint)p;
80107838:	8b 45 08             	mov    0x8(%ebp),%eax
8010783b:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  pd[2] = (uint)p >> 16;
8010783f:	8b 45 08             	mov    0x8(%ebp),%eax
80107842:	c1 e8 10             	shr    $0x10,%eax
80107845:	66 89 45 fe          	mov    %ax,-0x2(%ebp)

  asm volatile("lgdt (%0)" : : "r" (pd));
80107849:	8d 45 fa             	lea    -0x6(%ebp),%eax
8010784c:	0f 01 10             	lgdtl  (%eax)
}
8010784f:	c9                   	leave  
80107850:	c3                   	ret    

80107851 <ltr>:
  asm volatile("lidt (%0)" : : "r" (pd));
}

static inline void
ltr(ushort sel)
{
80107851:	55                   	push   %ebp
80107852:	89 e5                	mov    %esp,%ebp
80107854:	83 ec 04             	sub    $0x4,%esp
80107857:	8b 45 08             	mov    0x8(%ebp),%eax
8010785a:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  asm volatile("ltr %0" : : "r" (sel));
8010785e:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80107862:	0f 00 d8             	ltr    %ax
}
80107865:	c9                   	leave  
80107866:	c3                   	ret    

80107867 <loadgs>:
  return eflags;
}

static inline void
loadgs(ushort v)
{
80107867:	55                   	push   %ebp
80107868:	89 e5                	mov    %esp,%ebp
8010786a:	83 ec 04             	sub    $0x4,%esp
8010786d:	8b 45 08             	mov    0x8(%ebp),%eax
80107870:	66 89 45 fc          	mov    %ax,-0x4(%ebp)
  asm volatile("movw %0, %%gs" : : "r" (v));
80107874:	0f b7 45 fc          	movzwl -0x4(%ebp),%eax
80107878:	8e e8                	mov    %eax,%gs
}
8010787a:	c9                   	leave  
8010787b:	c3                   	ret    

8010787c <lcr3>:
  return val;
}

static inline void
lcr3(uint val) 
{
8010787c:	55                   	push   %ebp
8010787d:	89 e5                	mov    %esp,%ebp
  asm volatile("movl %0,%%cr3" : : "r" (val));
8010787f:	8b 45 08             	mov    0x8(%ebp),%eax
80107882:	0f 22 d8             	mov    %eax,%cr3
}
80107885:	5d                   	pop    %ebp
80107886:	c3                   	ret    

80107887 <v2p>:
#define KERNBASE 0x80000000         // First kernel virtual address
#define KERNLINK (KERNBASE+EXTMEM)  // Address where kernel is linked

#ifndef __ASSEMBLER__

static inline uint v2p(void *a) { return ((uint) (a))  - KERNBASE; }
80107887:	55                   	push   %ebp
80107888:	89 e5                	mov    %esp,%ebp
8010788a:	8b 45 08             	mov    0x8(%ebp),%eax
8010788d:	05 00 00 00 80       	add    $0x80000000,%eax
80107892:	5d                   	pop    %ebp
80107893:	c3                   	ret    

80107894 <p2v>:
static inline void *p2v(uint a) { return (void *) ((a) + KERNBASE); }
80107894:	55                   	push   %ebp
80107895:	89 e5                	mov    %esp,%ebp
80107897:	8b 45 08             	mov    0x8(%ebp),%eax
8010789a:	05 00 00 00 80       	add    $0x80000000,%eax
8010789f:	5d                   	pop    %ebp
801078a0:	c3                   	ret    

801078a1 <seginit>:

// Set up CPU's kernel segment descriptors.
// Run once on entry on each CPU.
void
seginit(void)
{
801078a1:	55                   	push   %ebp
801078a2:	89 e5                	mov    %esp,%ebp
801078a4:	53                   	push   %ebx
801078a5:	83 ec 24             	sub    $0x24,%esp

  // Map "logical" addresses to virtual addresses using identity map.
  // Cannot share a CODE descriptor for both kernel and user
  // because it would have to have DPL_USR, but the CPU forbids
  // an interrupt from CPL=0 to DPL=3.
  c = &cpus[cpunum()];
801078a8:	e8 d8 b5 ff ff       	call   80102e85 <cpunum>
801078ad:	69 c0 bc 00 00 00    	imul   $0xbc,%eax,%eax
801078b3:	05 20 f9 10 80       	add    $0x8010f920,%eax
801078b8:	89 45 f4             	mov    %eax,-0xc(%ebp)
  c->gdt[SEG_KCODE] = SEG(STA_X|STA_R, 0, 0xffffffff, 0);
801078bb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801078be:	66 c7 40 78 ff ff    	movw   $0xffff,0x78(%eax)
801078c4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801078c7:	66 c7 40 7a 00 00    	movw   $0x0,0x7a(%eax)
801078cd:	8b 45 f4             	mov    -0xc(%ebp),%eax
801078d0:	c6 40 7c 00          	movb   $0x0,0x7c(%eax)
801078d4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801078d7:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
801078db:	83 e2 f0             	and    $0xfffffff0,%edx
801078de:	83 ca 0a             	or     $0xa,%edx
801078e1:	88 50 7d             	mov    %dl,0x7d(%eax)
801078e4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801078e7:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
801078eb:	83 ca 10             	or     $0x10,%edx
801078ee:	88 50 7d             	mov    %dl,0x7d(%eax)
801078f1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801078f4:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
801078f8:	83 e2 9f             	and    $0xffffff9f,%edx
801078fb:	88 50 7d             	mov    %dl,0x7d(%eax)
801078fe:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107901:	0f b6 50 7d          	movzbl 0x7d(%eax),%edx
80107905:	83 ca 80             	or     $0xffffff80,%edx
80107908:	88 50 7d             	mov    %dl,0x7d(%eax)
8010790b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010790e:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
80107912:	83 ca 0f             	or     $0xf,%edx
80107915:	88 50 7e             	mov    %dl,0x7e(%eax)
80107918:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010791b:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
8010791f:	83 e2 ef             	and    $0xffffffef,%edx
80107922:	88 50 7e             	mov    %dl,0x7e(%eax)
80107925:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107928:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
8010792c:	83 e2 df             	and    $0xffffffdf,%edx
8010792f:	88 50 7e             	mov    %dl,0x7e(%eax)
80107932:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107935:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
80107939:	83 ca 40             	or     $0x40,%edx
8010793c:	88 50 7e             	mov    %dl,0x7e(%eax)
8010793f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107942:	0f b6 50 7e          	movzbl 0x7e(%eax),%edx
80107946:	83 ca 80             	or     $0xffffff80,%edx
80107949:	88 50 7e             	mov    %dl,0x7e(%eax)
8010794c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010794f:	c6 40 7f 00          	movb   $0x0,0x7f(%eax)
  c->gdt[SEG_KDATA] = SEG(STA_W, 0, 0xffffffff, 0);
80107953:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107956:	66 c7 80 80 00 00 00 	movw   $0xffff,0x80(%eax)
8010795d:	ff ff 
8010795f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107962:	66 c7 80 82 00 00 00 	movw   $0x0,0x82(%eax)
80107969:	00 00 
8010796b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010796e:	c6 80 84 00 00 00 00 	movb   $0x0,0x84(%eax)
80107975:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107978:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
8010797f:	83 e2 f0             	and    $0xfffffff0,%edx
80107982:	83 ca 02             	or     $0x2,%edx
80107985:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
8010798b:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010798e:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
80107995:	83 ca 10             	or     $0x10,%edx
80107998:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
8010799e:	8b 45 f4             	mov    -0xc(%ebp),%eax
801079a1:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
801079a8:	83 e2 9f             	and    $0xffffff9f,%edx
801079ab:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
801079b1:	8b 45 f4             	mov    -0xc(%ebp),%eax
801079b4:	0f b6 90 85 00 00 00 	movzbl 0x85(%eax),%edx
801079bb:	83 ca 80             	or     $0xffffff80,%edx
801079be:	88 90 85 00 00 00    	mov    %dl,0x85(%eax)
801079c4:	8b 45 f4             	mov    -0xc(%ebp),%eax
801079c7:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
801079ce:	83 ca 0f             	or     $0xf,%edx
801079d1:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
801079d7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801079da:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
801079e1:	83 e2 ef             	and    $0xffffffef,%edx
801079e4:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
801079ea:	8b 45 f4             	mov    -0xc(%ebp),%eax
801079ed:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
801079f4:	83 e2 df             	and    $0xffffffdf,%edx
801079f7:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
801079fd:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a00:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
80107a07:	83 ca 40             	or     $0x40,%edx
80107a0a:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80107a10:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a13:	0f b6 90 86 00 00 00 	movzbl 0x86(%eax),%edx
80107a1a:	83 ca 80             	or     $0xffffff80,%edx
80107a1d:	88 90 86 00 00 00    	mov    %dl,0x86(%eax)
80107a23:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a26:	c6 80 87 00 00 00 00 	movb   $0x0,0x87(%eax)
  c->gdt[SEG_UCODE] = SEG(STA_X|STA_R, 0, 0xffffffff, DPL_USER);
80107a2d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a30:	66 c7 80 90 00 00 00 	movw   $0xffff,0x90(%eax)
80107a37:	ff ff 
80107a39:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a3c:	66 c7 80 92 00 00 00 	movw   $0x0,0x92(%eax)
80107a43:	00 00 
80107a45:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a48:	c6 80 94 00 00 00 00 	movb   $0x0,0x94(%eax)
80107a4f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a52:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
80107a59:	83 e2 f0             	and    $0xfffffff0,%edx
80107a5c:	83 ca 0a             	or     $0xa,%edx
80107a5f:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
80107a65:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a68:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
80107a6f:	83 ca 10             	or     $0x10,%edx
80107a72:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
80107a78:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a7b:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
80107a82:	83 ca 60             	or     $0x60,%edx
80107a85:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
80107a8b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107a8e:	0f b6 90 95 00 00 00 	movzbl 0x95(%eax),%edx
80107a95:	83 ca 80             	or     $0xffffff80,%edx
80107a98:	88 90 95 00 00 00    	mov    %dl,0x95(%eax)
80107a9e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107aa1:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107aa8:	83 ca 0f             	or     $0xf,%edx
80107aab:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80107ab1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ab4:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107abb:	83 e2 ef             	and    $0xffffffef,%edx
80107abe:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80107ac4:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ac7:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107ace:	83 e2 df             	and    $0xffffffdf,%edx
80107ad1:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80107ad7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ada:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107ae1:	83 ca 40             	or     $0x40,%edx
80107ae4:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80107aea:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107aed:	0f b6 90 96 00 00 00 	movzbl 0x96(%eax),%edx
80107af4:	83 ca 80             	or     $0xffffff80,%edx
80107af7:	88 90 96 00 00 00    	mov    %dl,0x96(%eax)
80107afd:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b00:	c6 80 97 00 00 00 00 	movb   $0x0,0x97(%eax)
  c->gdt[SEG_UDATA] = SEG(STA_W, 0, 0xffffffff, DPL_USER);
80107b07:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b0a:	66 c7 80 98 00 00 00 	movw   $0xffff,0x98(%eax)
80107b11:	ff ff 
80107b13:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b16:	66 c7 80 9a 00 00 00 	movw   $0x0,0x9a(%eax)
80107b1d:	00 00 
80107b1f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b22:	c6 80 9c 00 00 00 00 	movb   $0x0,0x9c(%eax)
80107b29:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b2c:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
80107b33:	83 e2 f0             	and    $0xfffffff0,%edx
80107b36:	83 ca 02             	or     $0x2,%edx
80107b39:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
80107b3f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b42:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
80107b49:	83 ca 10             	or     $0x10,%edx
80107b4c:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
80107b52:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b55:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
80107b5c:	83 ca 60             	or     $0x60,%edx
80107b5f:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
80107b65:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b68:	0f b6 90 9d 00 00 00 	movzbl 0x9d(%eax),%edx
80107b6f:	83 ca 80             	or     $0xffffff80,%edx
80107b72:	88 90 9d 00 00 00    	mov    %dl,0x9d(%eax)
80107b78:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b7b:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107b82:	83 ca 0f             	or     $0xf,%edx
80107b85:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107b8b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107b8e:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107b95:	83 e2 ef             	and    $0xffffffef,%edx
80107b98:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107b9e:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ba1:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107ba8:	83 e2 df             	and    $0xffffffdf,%edx
80107bab:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107bb1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107bb4:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107bbb:	83 ca 40             	or     $0x40,%edx
80107bbe:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107bc4:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107bc7:	0f b6 90 9e 00 00 00 	movzbl 0x9e(%eax),%edx
80107bce:	83 ca 80             	or     $0xffffff80,%edx
80107bd1:	88 90 9e 00 00 00    	mov    %dl,0x9e(%eax)
80107bd7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107bda:	c6 80 9f 00 00 00 00 	movb   $0x0,0x9f(%eax)

  // Map cpu, and curproc
  c->gdt[SEG_KCPU] = SEG(STA_W, &c->cpu, 8, 0);
80107be1:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107be4:	05 b4 00 00 00       	add    $0xb4,%eax
80107be9:	89 c3                	mov    %eax,%ebx
80107beb:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107bee:	05 b4 00 00 00       	add    $0xb4,%eax
80107bf3:	c1 e8 10             	shr    $0x10,%eax
80107bf6:	89 c1                	mov    %eax,%ecx
80107bf8:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107bfb:	05 b4 00 00 00       	add    $0xb4,%eax
80107c00:	c1 e8 18             	shr    $0x18,%eax
80107c03:	89 c2                	mov    %eax,%edx
80107c05:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c08:	66 c7 80 88 00 00 00 	movw   $0x0,0x88(%eax)
80107c0f:	00 00 
80107c11:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c14:	66 89 98 8a 00 00 00 	mov    %bx,0x8a(%eax)
80107c1b:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c1e:	88 88 8c 00 00 00    	mov    %cl,0x8c(%eax)
80107c24:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c27:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
80107c2e:	83 e1 f0             	and    $0xfffffff0,%ecx
80107c31:	83 c9 02             	or     $0x2,%ecx
80107c34:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
80107c3a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c3d:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
80107c44:	83 c9 10             	or     $0x10,%ecx
80107c47:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
80107c4d:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c50:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
80107c57:	83 e1 9f             	and    $0xffffff9f,%ecx
80107c5a:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
80107c60:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c63:	0f b6 88 8d 00 00 00 	movzbl 0x8d(%eax),%ecx
80107c6a:	83 c9 80             	or     $0xffffff80,%ecx
80107c6d:	88 88 8d 00 00 00    	mov    %cl,0x8d(%eax)
80107c73:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c76:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107c7d:	83 e1 f0             	and    $0xfffffff0,%ecx
80107c80:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107c86:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c89:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107c90:	83 e1 ef             	and    $0xffffffef,%ecx
80107c93:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107c99:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107c9c:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107ca3:	83 e1 df             	and    $0xffffffdf,%ecx
80107ca6:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107cac:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107caf:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107cb6:	83 c9 40             	or     $0x40,%ecx
80107cb9:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107cbf:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107cc2:	0f b6 88 8e 00 00 00 	movzbl 0x8e(%eax),%ecx
80107cc9:	83 c9 80             	or     $0xffffff80,%ecx
80107ccc:	88 88 8e 00 00 00    	mov    %cl,0x8e(%eax)
80107cd2:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107cd5:	88 90 8f 00 00 00    	mov    %dl,0x8f(%eax)

  lgdt(c->gdt, sizeof(c->gdt));
80107cdb:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107cde:	83 c0 70             	add    $0x70,%eax
80107ce1:	c7 44 24 04 38 00 00 	movl   $0x38,0x4(%esp)
80107ce8:	00 
80107ce9:	89 04 24             	mov    %eax,(%esp)
80107cec:	e8 37 fb ff ff       	call   80107828 <lgdt>
  loadgs(SEG_KCPU << 3);
80107cf1:	c7 04 24 18 00 00 00 	movl   $0x18,(%esp)
80107cf8:	e8 6a fb ff ff       	call   80107867 <loadgs>
  
  // Initialize cpu-local storage.
  cpu = c;
80107cfd:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d00:	65 a3 00 00 00 00    	mov    %eax,%gs:0x0
  proc = 0;
80107d06:	65 c7 05 04 00 00 00 	movl   $0x0,%gs:0x4
80107d0d:	00 00 00 00 
}
80107d11:	83 c4 24             	add    $0x24,%esp
80107d14:	5b                   	pop    %ebx
80107d15:	5d                   	pop    %ebp
80107d16:	c3                   	ret    

80107d17 <walkpgdir>:
// Return the address of the PTE in page table pgdir
// that corresponds to virtual address va.  If alloc!=0,
// create any required page table pages.
static pte_t *
walkpgdir(pde_t *pgdir, const void *va, int alloc)
{
80107d17:	55                   	push   %ebp
80107d18:	89 e5                	mov    %esp,%ebp
80107d1a:	83 ec 28             	sub    $0x28,%esp
  pde_t *pde;
  pte_t *pgtab;

  pde = &pgdir[PDX(va)];
80107d1d:	8b 45 0c             	mov    0xc(%ebp),%eax
80107d20:	c1 e8 16             	shr    $0x16,%eax
80107d23:	c1 e0 02             	shl    $0x2,%eax
80107d26:	03 45 08             	add    0x8(%ebp),%eax
80107d29:	89 45 f0             	mov    %eax,-0x10(%ebp)
  if(*pde & PTE_P){
80107d2c:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107d2f:	8b 00                	mov    (%eax),%eax
80107d31:	83 e0 01             	and    $0x1,%eax
80107d34:	84 c0                	test   %al,%al
80107d36:	74 17                	je     80107d4f <walkpgdir+0x38>
    pgtab = (pte_t*)p2v(PTE_ADDR(*pde));
80107d38:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107d3b:	8b 00                	mov    (%eax),%eax
80107d3d:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80107d42:	89 04 24             	mov    %eax,(%esp)
80107d45:	e8 4a fb ff ff       	call   80107894 <p2v>
80107d4a:	89 45 f4             	mov    %eax,-0xc(%ebp)
80107d4d:	eb 4b                	jmp    80107d9a <walkpgdir+0x83>
  } else {
    if(!alloc || (pgtab = (pte_t*)kalloc()) == 0)
80107d4f:	83 7d 10 00          	cmpl   $0x0,0x10(%ebp)
80107d53:	74 0e                	je     80107d63 <walkpgdir+0x4c>
80107d55:	e8 9d ad ff ff       	call   80102af7 <kalloc>
80107d5a:	89 45 f4             	mov    %eax,-0xc(%ebp)
80107d5d:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
80107d61:	75 07                	jne    80107d6a <walkpgdir+0x53>
      return 0;
80107d63:	b8 00 00 00 00       	mov    $0x0,%eax
80107d68:	eb 41                	jmp    80107dab <walkpgdir+0x94>
    // Make sure all those PTE_P bits are zero.
    memset(pgtab, 0, PGSIZE);
80107d6a:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80107d71:	00 
80107d72:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80107d79:	00 
80107d7a:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d7d:	89 04 24             	mov    %eax,(%esp)
80107d80:	e8 1d d5 ff ff       	call   801052a2 <memset>
    // The permissions here are overly generous, but they can
    // be further restricted by the permissions in the page table 
    // entries, if necessary.
    *pde = v2p(pgtab) | PTE_P | PTE_W | PTE_U;
80107d85:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107d88:	89 04 24             	mov    %eax,(%esp)
80107d8b:	e8 f7 fa ff ff       	call   80107887 <v2p>
80107d90:	89 c2                	mov    %eax,%edx
80107d92:	83 ca 07             	or     $0x7,%edx
80107d95:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107d98:	89 10                	mov    %edx,(%eax)
  }
  return &pgtab[PTX(va)];
80107d9a:	8b 45 0c             	mov    0xc(%ebp),%eax
80107d9d:	c1 e8 0c             	shr    $0xc,%eax
80107da0:	25 ff 03 00 00       	and    $0x3ff,%eax
80107da5:	c1 e0 02             	shl    $0x2,%eax
80107da8:	03 45 f4             	add    -0xc(%ebp),%eax
}
80107dab:	c9                   	leave  
80107dac:	c3                   	ret    

80107dad <mappages>:
// Create PTEs for virtual addresses starting at va that refer to
// physical addresses starting at pa. va and size might not
// be page-aligned.
static int
mappages(pde_t *pgdir, void *va, uint size, uint pa, int perm)
{
80107dad:	55                   	push   %ebp
80107dae:	89 e5                	mov    %esp,%ebp
80107db0:	83 ec 28             	sub    $0x28,%esp
  char *a, *last;
  pte_t *pte;
  
  a = (char*)PGROUNDDOWN((uint)va);
80107db3:	8b 45 0c             	mov    0xc(%ebp),%eax
80107db6:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80107dbb:	89 45 f4             	mov    %eax,-0xc(%ebp)
  last = (char*)PGROUNDDOWN(((uint)va) + size - 1);
80107dbe:	8b 45 0c             	mov    0xc(%ebp),%eax
80107dc1:	03 45 10             	add    0x10(%ebp),%eax
80107dc4:	83 e8 01             	sub    $0x1,%eax
80107dc7:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80107dcc:	89 45 f0             	mov    %eax,-0x10(%ebp)
  for(;;){
    if((pte = walkpgdir(pgdir, a, 1)) == 0)
80107dcf:	c7 44 24 08 01 00 00 	movl   $0x1,0x8(%esp)
80107dd6:	00 
80107dd7:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107dda:	89 44 24 04          	mov    %eax,0x4(%esp)
80107dde:	8b 45 08             	mov    0x8(%ebp),%eax
80107de1:	89 04 24             	mov    %eax,(%esp)
80107de4:	e8 2e ff ff ff       	call   80107d17 <walkpgdir>
80107de9:	89 45 ec             	mov    %eax,-0x14(%ebp)
80107dec:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80107df0:	75 07                	jne    80107df9 <mappages+0x4c>
      return -1;
80107df2:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80107df7:	eb 46                	jmp    80107e3f <mappages+0x92>
    if(*pte & PTE_P)
80107df9:	8b 45 ec             	mov    -0x14(%ebp),%eax
80107dfc:	8b 00                	mov    (%eax),%eax
80107dfe:	83 e0 01             	and    $0x1,%eax
80107e01:	84 c0                	test   %al,%al
80107e03:	74 0c                	je     80107e11 <mappages+0x64>
      panic("remap");
80107e05:	c7 04 24 a8 8c 10 80 	movl   $0x80108ca8,(%esp)
80107e0c:	e8 2c 87 ff ff       	call   8010053d <panic>
    *pte = pa | perm | PTE_P;
80107e11:	8b 45 18             	mov    0x18(%ebp),%eax
80107e14:	0b 45 14             	or     0x14(%ebp),%eax
80107e17:	89 c2                	mov    %eax,%edx
80107e19:	83 ca 01             	or     $0x1,%edx
80107e1c:	8b 45 ec             	mov    -0x14(%ebp),%eax
80107e1f:	89 10                	mov    %edx,(%eax)
    if(a == last)
80107e21:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107e24:	3b 45 f0             	cmp    -0x10(%ebp),%eax
80107e27:	74 10                	je     80107e39 <mappages+0x8c>
      break;
    a += PGSIZE;
80107e29:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
    pa += PGSIZE;
80107e30:	81 45 14 00 10 00 00 	addl   $0x1000,0x14(%ebp)
  }
80107e37:	eb 96                	jmp    80107dcf <mappages+0x22>
      return -1;
    if(*pte & PTE_P)
      panic("remap");
    *pte = pa | perm | PTE_P;
    if(a == last)
      break;
80107e39:	90                   	nop
    a += PGSIZE;
    pa += PGSIZE;
  }
  return 0;
80107e3a:	b8 00 00 00 00       	mov    $0x0,%eax
}
80107e3f:	c9                   	leave  
80107e40:	c3                   	ret    

80107e41 <setupkvm>:
};

// Set up kernel part of a page table.
pde_t*
setupkvm(void)
{
80107e41:	55                   	push   %ebp
80107e42:	89 e5                	mov    %esp,%ebp
80107e44:	53                   	push   %ebx
80107e45:	83 ec 34             	sub    $0x34,%esp
  pde_t *pgdir;
  struct kmap *k;

  if((pgdir = (pde_t*)kalloc()) == 0)
80107e48:	e8 aa ac ff ff       	call   80102af7 <kalloc>
80107e4d:	89 45 f0             	mov    %eax,-0x10(%ebp)
80107e50:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80107e54:	75 0a                	jne    80107e60 <setupkvm+0x1f>
    return 0;
80107e56:	b8 00 00 00 00       	mov    $0x0,%eax
80107e5b:	e9 98 00 00 00       	jmp    80107ef8 <setupkvm+0xb7>
  memset(pgdir, 0, PGSIZE);
80107e60:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80107e67:	00 
80107e68:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80107e6f:	00 
80107e70:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107e73:	89 04 24             	mov    %eax,(%esp)
80107e76:	e8 27 d4 ff ff       	call   801052a2 <memset>
  if (p2v(PHYSTOP) > (void*)DEVSPACE)
80107e7b:	c7 04 24 00 00 00 0e 	movl   $0xe000000,(%esp)
80107e82:	e8 0d fa ff ff       	call   80107894 <p2v>
80107e87:	3d 00 00 00 fe       	cmp    $0xfe000000,%eax
80107e8c:	76 0c                	jbe    80107e9a <setupkvm+0x59>
    panic("PHYSTOP too high");
80107e8e:	c7 04 24 ae 8c 10 80 	movl   $0x80108cae,(%esp)
80107e95:	e8 a3 86 ff ff       	call   8010053d <panic>
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
80107e9a:	c7 45 f4 a0 b4 10 80 	movl   $0x8010b4a0,-0xc(%ebp)
80107ea1:	eb 49                	jmp    80107eec <setupkvm+0xab>
    if(mappages(pgdir, k->virt, k->phys_end - k->phys_start, 
                (uint)k->phys_start, k->perm) < 0)
80107ea3:	8b 45 f4             	mov    -0xc(%ebp),%eax
    return 0;
  memset(pgdir, 0, PGSIZE);
  if (p2v(PHYSTOP) > (void*)DEVSPACE)
    panic("PHYSTOP too high");
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
    if(mappages(pgdir, k->virt, k->phys_end - k->phys_start, 
80107ea6:	8b 48 0c             	mov    0xc(%eax),%ecx
                (uint)k->phys_start, k->perm) < 0)
80107ea9:	8b 45 f4             	mov    -0xc(%ebp),%eax
    return 0;
  memset(pgdir, 0, PGSIZE);
  if (p2v(PHYSTOP) > (void*)DEVSPACE)
    panic("PHYSTOP too high");
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
    if(mappages(pgdir, k->virt, k->phys_end - k->phys_start, 
80107eac:	8b 50 04             	mov    0x4(%eax),%edx
80107eaf:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107eb2:	8b 58 08             	mov    0x8(%eax),%ebx
80107eb5:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107eb8:	8b 40 04             	mov    0x4(%eax),%eax
80107ebb:	29 c3                	sub    %eax,%ebx
80107ebd:	8b 45 f4             	mov    -0xc(%ebp),%eax
80107ec0:	8b 00                	mov    (%eax),%eax
80107ec2:	89 4c 24 10          	mov    %ecx,0x10(%esp)
80107ec6:	89 54 24 0c          	mov    %edx,0xc(%esp)
80107eca:	89 5c 24 08          	mov    %ebx,0x8(%esp)
80107ece:	89 44 24 04          	mov    %eax,0x4(%esp)
80107ed2:	8b 45 f0             	mov    -0x10(%ebp),%eax
80107ed5:	89 04 24             	mov    %eax,(%esp)
80107ed8:	e8 d0 fe ff ff       	call   80107dad <mappages>
80107edd:	85 c0                	test   %eax,%eax
80107edf:	79 07                	jns    80107ee8 <setupkvm+0xa7>
                (uint)k->phys_start, k->perm) < 0)
      return 0;
80107ee1:	b8 00 00 00 00       	mov    $0x0,%eax
80107ee6:	eb 10                	jmp    80107ef8 <setupkvm+0xb7>
  if((pgdir = (pde_t*)kalloc()) == 0)
    return 0;
  memset(pgdir, 0, PGSIZE);
  if (p2v(PHYSTOP) > (void*)DEVSPACE)
    panic("PHYSTOP too high");
  for(k = kmap; k < &kmap[NELEM(kmap)]; k++)
80107ee8:	83 45 f4 10          	addl   $0x10,-0xc(%ebp)
80107eec:	81 7d f4 e0 b4 10 80 	cmpl   $0x8010b4e0,-0xc(%ebp)
80107ef3:	72 ae                	jb     80107ea3 <setupkvm+0x62>
    if(mappages(pgdir, k->virt, k->phys_end - k->phys_start, 
                (uint)k->phys_start, k->perm) < 0)
      return 0;
  return pgdir;
80107ef5:	8b 45 f0             	mov    -0x10(%ebp),%eax
}
80107ef8:	83 c4 34             	add    $0x34,%esp
80107efb:	5b                   	pop    %ebx
80107efc:	5d                   	pop    %ebp
80107efd:	c3                   	ret    

80107efe <kvmalloc>:

// Allocate one page table for the machine for the kernel address
// space for scheduler processes.
void
kvmalloc(void)
{
80107efe:	55                   	push   %ebp
80107eff:	89 e5                	mov    %esp,%ebp
80107f01:	83 ec 08             	sub    $0x8,%esp
  kpgdir = setupkvm();
80107f04:	e8 38 ff ff ff       	call   80107e41 <setupkvm>
80107f09:	a3 58 2d 11 80       	mov    %eax,0x80112d58
  switchkvm();
80107f0e:	e8 02 00 00 00       	call   80107f15 <switchkvm>
}
80107f13:	c9                   	leave  
80107f14:	c3                   	ret    

80107f15 <switchkvm>:

// Switch h/w page table register to the kernel-only page table,
// for when no process is running.
void
switchkvm(void)
{
80107f15:	55                   	push   %ebp
80107f16:	89 e5                	mov    %esp,%ebp
80107f18:	83 ec 04             	sub    $0x4,%esp
  lcr3(v2p(kpgdir));   // switch to the kernel page table
80107f1b:	a1 58 2d 11 80       	mov    0x80112d58,%eax
80107f20:	89 04 24             	mov    %eax,(%esp)
80107f23:	e8 5f f9 ff ff       	call   80107887 <v2p>
80107f28:	89 04 24             	mov    %eax,(%esp)
80107f2b:	e8 4c f9 ff ff       	call   8010787c <lcr3>
}
80107f30:	c9                   	leave  
80107f31:	c3                   	ret    

80107f32 <switchuvm>:

// Switch TSS and h/w page table to correspond to process p.
void
switchuvm(struct proc *p)
{
80107f32:	55                   	push   %ebp
80107f33:	89 e5                	mov    %esp,%ebp
80107f35:	53                   	push   %ebx
80107f36:	83 ec 14             	sub    $0x14,%esp
  pushcli();
80107f39:	e8 5d d2 ff ff       	call   8010519b <pushcli>
  cpu->gdt[SEG_TSS] = SEG16(STS_T32A, &cpu->ts, sizeof(cpu->ts)-1, 0);
80107f3e:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80107f44:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80107f4b:	83 c2 08             	add    $0x8,%edx
80107f4e:	89 d3                	mov    %edx,%ebx
80107f50:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80107f57:	83 c2 08             	add    $0x8,%edx
80107f5a:	c1 ea 10             	shr    $0x10,%edx
80107f5d:	89 d1                	mov    %edx,%ecx
80107f5f:	65 8b 15 00 00 00 00 	mov    %gs:0x0,%edx
80107f66:	83 c2 08             	add    $0x8,%edx
80107f69:	c1 ea 18             	shr    $0x18,%edx
80107f6c:	66 c7 80 a0 00 00 00 	movw   $0x67,0xa0(%eax)
80107f73:	67 00 
80107f75:	66 89 98 a2 00 00 00 	mov    %bx,0xa2(%eax)
80107f7c:	88 88 a4 00 00 00    	mov    %cl,0xa4(%eax)
80107f82:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
80107f89:	83 e1 f0             	and    $0xfffffff0,%ecx
80107f8c:	83 c9 09             	or     $0x9,%ecx
80107f8f:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
80107f95:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
80107f9c:	83 c9 10             	or     $0x10,%ecx
80107f9f:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
80107fa5:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
80107fac:	83 e1 9f             	and    $0xffffff9f,%ecx
80107faf:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
80107fb5:	0f b6 88 a5 00 00 00 	movzbl 0xa5(%eax),%ecx
80107fbc:	83 c9 80             	or     $0xffffff80,%ecx
80107fbf:	88 88 a5 00 00 00    	mov    %cl,0xa5(%eax)
80107fc5:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
80107fcc:	83 e1 f0             	and    $0xfffffff0,%ecx
80107fcf:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
80107fd5:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
80107fdc:	83 e1 ef             	and    $0xffffffef,%ecx
80107fdf:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
80107fe5:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
80107fec:	83 e1 df             	and    $0xffffffdf,%ecx
80107fef:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
80107ff5:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
80107ffc:	83 c9 40             	or     $0x40,%ecx
80107fff:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
80108005:	0f b6 88 a6 00 00 00 	movzbl 0xa6(%eax),%ecx
8010800c:	83 e1 7f             	and    $0x7f,%ecx
8010800f:	88 88 a6 00 00 00    	mov    %cl,0xa6(%eax)
80108015:	88 90 a7 00 00 00    	mov    %dl,0xa7(%eax)
  cpu->gdt[SEG_TSS].s = 0;
8010801b:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80108021:	0f b6 90 a5 00 00 00 	movzbl 0xa5(%eax),%edx
80108028:	83 e2 ef             	and    $0xffffffef,%edx
8010802b:	88 90 a5 00 00 00    	mov    %dl,0xa5(%eax)
  cpu->ts.ss0 = SEG_KDATA << 3;
80108031:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80108037:	66 c7 40 10 10 00    	movw   $0x10,0x10(%eax)
  cpu->ts.esp0 = (uint)proc->kstack + KSTACKSIZE;
8010803d:	65 a1 00 00 00 00    	mov    %gs:0x0,%eax
80108043:	65 8b 15 04 00 00 00 	mov    %gs:0x4,%edx
8010804a:	8b 52 08             	mov    0x8(%edx),%edx
8010804d:	81 c2 00 10 00 00    	add    $0x1000,%edx
80108053:	89 50 0c             	mov    %edx,0xc(%eax)
  ltr(SEG_TSS << 3);
80108056:	c7 04 24 30 00 00 00 	movl   $0x30,(%esp)
8010805d:	e8 ef f7 ff ff       	call   80107851 <ltr>
  if(p->pgdir == 0)
80108062:	8b 45 08             	mov    0x8(%ebp),%eax
80108065:	8b 40 04             	mov    0x4(%eax),%eax
80108068:	85 c0                	test   %eax,%eax
8010806a:	75 0c                	jne    80108078 <switchuvm+0x146>
    panic("switchuvm: no pgdir");
8010806c:	c7 04 24 bf 8c 10 80 	movl   $0x80108cbf,(%esp)
80108073:	e8 c5 84 ff ff       	call   8010053d <panic>
  lcr3(v2p(p->pgdir));  // switch to new address space
80108078:	8b 45 08             	mov    0x8(%ebp),%eax
8010807b:	8b 40 04             	mov    0x4(%eax),%eax
8010807e:	89 04 24             	mov    %eax,(%esp)
80108081:	e8 01 f8 ff ff       	call   80107887 <v2p>
80108086:	89 04 24             	mov    %eax,(%esp)
80108089:	e8 ee f7 ff ff       	call   8010787c <lcr3>
  popcli();
8010808e:	e8 50 d1 ff ff       	call   801051e3 <popcli>
}
80108093:	83 c4 14             	add    $0x14,%esp
80108096:	5b                   	pop    %ebx
80108097:	5d                   	pop    %ebp
80108098:	c3                   	ret    

80108099 <inituvm>:

// Load the initcode into address 0 of pgdir.
// sz must be less than a page.
void
inituvm(pde_t *pgdir, char *init, uint sz)
{
80108099:	55                   	push   %ebp
8010809a:	89 e5                	mov    %esp,%ebp
8010809c:	83 ec 38             	sub    $0x38,%esp
  char *mem;
  
  if(sz >= PGSIZE)
8010809f:	81 7d 10 ff 0f 00 00 	cmpl   $0xfff,0x10(%ebp)
801080a6:	76 0c                	jbe    801080b4 <inituvm+0x1b>
    panic("inituvm: more than a page");
801080a8:	c7 04 24 d3 8c 10 80 	movl   $0x80108cd3,(%esp)
801080af:	e8 89 84 ff ff       	call   8010053d <panic>
  mem = kalloc();
801080b4:	e8 3e aa ff ff       	call   80102af7 <kalloc>
801080b9:	89 45 f4             	mov    %eax,-0xc(%ebp)
  memset(mem, 0, PGSIZE);
801080bc:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
801080c3:	00 
801080c4:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801080cb:	00 
801080cc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801080cf:	89 04 24             	mov    %eax,(%esp)
801080d2:	e8 cb d1 ff ff       	call   801052a2 <memset>
  mappages(pgdir, 0, PGSIZE, v2p(mem), PTE_W|PTE_U);
801080d7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801080da:	89 04 24             	mov    %eax,(%esp)
801080dd:	e8 a5 f7 ff ff       	call   80107887 <v2p>
801080e2:	c7 44 24 10 06 00 00 	movl   $0x6,0x10(%esp)
801080e9:	00 
801080ea:	89 44 24 0c          	mov    %eax,0xc(%esp)
801080ee:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
801080f5:	00 
801080f6:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
801080fd:	00 
801080fe:	8b 45 08             	mov    0x8(%ebp),%eax
80108101:	89 04 24             	mov    %eax,(%esp)
80108104:	e8 a4 fc ff ff       	call   80107dad <mappages>
  memmove(mem, init, sz);
80108109:	8b 45 10             	mov    0x10(%ebp),%eax
8010810c:	89 44 24 08          	mov    %eax,0x8(%esp)
80108110:	8b 45 0c             	mov    0xc(%ebp),%eax
80108113:	89 44 24 04          	mov    %eax,0x4(%esp)
80108117:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010811a:	89 04 24             	mov    %eax,(%esp)
8010811d:	e8 53 d2 ff ff       	call   80105375 <memmove>
}
80108122:	c9                   	leave  
80108123:	c3                   	ret    

80108124 <loaduvm>:

// Load a program segment into pgdir.  addr must be page-aligned
// and the pages from addr to addr+sz must already be mapped.
int
loaduvm(pde_t *pgdir, char *addr, struct inode *ip, uint offset, uint sz)
{
80108124:	55                   	push   %ebp
80108125:	89 e5                	mov    %esp,%ebp
80108127:	53                   	push   %ebx
80108128:	83 ec 24             	sub    $0x24,%esp
  uint i, pa, n;
  pte_t *pte;

  if((uint) addr % PGSIZE != 0)
8010812b:	8b 45 0c             	mov    0xc(%ebp),%eax
8010812e:	25 ff 0f 00 00       	and    $0xfff,%eax
80108133:	85 c0                	test   %eax,%eax
80108135:	74 0c                	je     80108143 <loaduvm+0x1f>
    panic("loaduvm: addr must be page aligned");
80108137:	c7 04 24 f0 8c 10 80 	movl   $0x80108cf0,(%esp)
8010813e:	e8 fa 83 ff ff       	call   8010053d <panic>
  for(i = 0; i < sz; i += PGSIZE){
80108143:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
8010814a:	e9 ad 00 00 00       	jmp    801081fc <loaduvm+0xd8>
    if((pte = walkpgdir(pgdir, addr+i, 0)) == 0)
8010814f:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108152:	8b 55 0c             	mov    0xc(%ebp),%edx
80108155:	01 d0                	add    %edx,%eax
80108157:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
8010815e:	00 
8010815f:	89 44 24 04          	mov    %eax,0x4(%esp)
80108163:	8b 45 08             	mov    0x8(%ebp),%eax
80108166:	89 04 24             	mov    %eax,(%esp)
80108169:	e8 a9 fb ff ff       	call   80107d17 <walkpgdir>
8010816e:	89 45 ec             	mov    %eax,-0x14(%ebp)
80108171:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80108175:	75 0c                	jne    80108183 <loaduvm+0x5f>
      panic("loaduvm: address should exist");
80108177:	c7 04 24 13 8d 10 80 	movl   $0x80108d13,(%esp)
8010817e:	e8 ba 83 ff ff       	call   8010053d <panic>
    pa = PTE_ADDR(*pte);
80108183:	8b 45 ec             	mov    -0x14(%ebp),%eax
80108186:	8b 00                	mov    (%eax),%eax
80108188:	25 00 f0 ff ff       	and    $0xfffff000,%eax
8010818d:	89 45 e8             	mov    %eax,-0x18(%ebp)
    if(sz - i < PGSIZE)
80108190:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108193:	8b 55 18             	mov    0x18(%ebp),%edx
80108196:	89 d1                	mov    %edx,%ecx
80108198:	29 c1                	sub    %eax,%ecx
8010819a:	89 c8                	mov    %ecx,%eax
8010819c:	3d ff 0f 00 00       	cmp    $0xfff,%eax
801081a1:	77 11                	ja     801081b4 <loaduvm+0x90>
      n = sz - i;
801081a3:	8b 45 f4             	mov    -0xc(%ebp),%eax
801081a6:	8b 55 18             	mov    0x18(%ebp),%edx
801081a9:	89 d1                	mov    %edx,%ecx
801081ab:	29 c1                	sub    %eax,%ecx
801081ad:	89 c8                	mov    %ecx,%eax
801081af:	89 45 f0             	mov    %eax,-0x10(%ebp)
801081b2:	eb 07                	jmp    801081bb <loaduvm+0x97>
    else
      n = PGSIZE;
801081b4:	c7 45 f0 00 10 00 00 	movl   $0x1000,-0x10(%ebp)
    if(readi(ip, p2v(pa), offset+i, n) != n)
801081bb:	8b 45 f4             	mov    -0xc(%ebp),%eax
801081be:	8b 55 14             	mov    0x14(%ebp),%edx
801081c1:	8d 1c 02             	lea    (%edx,%eax,1),%ebx
801081c4:	8b 45 e8             	mov    -0x18(%ebp),%eax
801081c7:	89 04 24             	mov    %eax,(%esp)
801081ca:	e8 c5 f6 ff ff       	call   80107894 <p2v>
801081cf:	8b 55 f0             	mov    -0x10(%ebp),%edx
801081d2:	89 54 24 0c          	mov    %edx,0xc(%esp)
801081d6:	89 5c 24 08          	mov    %ebx,0x8(%esp)
801081da:	89 44 24 04          	mov    %eax,0x4(%esp)
801081de:	8b 45 10             	mov    0x10(%ebp),%eax
801081e1:	89 04 24             	mov    %eax,(%esp)
801081e4:	e8 6d 9b ff ff       	call   80101d56 <readi>
801081e9:	3b 45 f0             	cmp    -0x10(%ebp),%eax
801081ec:	74 07                	je     801081f5 <loaduvm+0xd1>
      return -1;
801081ee:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
801081f3:	eb 18                	jmp    8010820d <loaduvm+0xe9>
  uint i, pa, n;
  pte_t *pte;

  if((uint) addr % PGSIZE != 0)
    panic("loaduvm: addr must be page aligned");
  for(i = 0; i < sz; i += PGSIZE){
801081f5:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
801081fc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801081ff:	3b 45 18             	cmp    0x18(%ebp),%eax
80108202:	0f 82 47 ff ff ff    	jb     8010814f <loaduvm+0x2b>
    else
      n = PGSIZE;
    if(readi(ip, p2v(pa), offset+i, n) != n)
      return -1;
  }
  return 0;
80108208:	b8 00 00 00 00       	mov    $0x0,%eax
}
8010820d:	83 c4 24             	add    $0x24,%esp
80108210:	5b                   	pop    %ebx
80108211:	5d                   	pop    %ebp
80108212:	c3                   	ret    

80108213 <allocuvm>:

// Allocate page tables and physical memory to grow process from oldsz to
// newsz, which need not be page aligned.  Returns new size or 0 on error.
int
allocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
80108213:	55                   	push   %ebp
80108214:	89 e5                	mov    %esp,%ebp
80108216:	83 ec 38             	sub    $0x38,%esp
  char *mem;
  uint a;

  if(newsz >= KERNBASE)
80108219:	8b 45 10             	mov    0x10(%ebp),%eax
8010821c:	85 c0                	test   %eax,%eax
8010821e:	79 0a                	jns    8010822a <allocuvm+0x17>
    return 0;
80108220:	b8 00 00 00 00       	mov    $0x0,%eax
80108225:	e9 c1 00 00 00       	jmp    801082eb <allocuvm+0xd8>
  if(newsz < oldsz)
8010822a:	8b 45 10             	mov    0x10(%ebp),%eax
8010822d:	3b 45 0c             	cmp    0xc(%ebp),%eax
80108230:	73 08                	jae    8010823a <allocuvm+0x27>
    return oldsz;
80108232:	8b 45 0c             	mov    0xc(%ebp),%eax
80108235:	e9 b1 00 00 00       	jmp    801082eb <allocuvm+0xd8>

  a = PGROUNDUP(oldsz);
8010823a:	8b 45 0c             	mov    0xc(%ebp),%eax
8010823d:	05 ff 0f 00 00       	add    $0xfff,%eax
80108242:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80108247:	89 45 f4             	mov    %eax,-0xc(%ebp)
  for(; a < newsz; a += PGSIZE){
8010824a:	e9 8d 00 00 00       	jmp    801082dc <allocuvm+0xc9>
    mem = kalloc();
8010824f:	e8 a3 a8 ff ff       	call   80102af7 <kalloc>
80108254:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(mem == 0){
80108257:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
8010825b:	75 2c                	jne    80108289 <allocuvm+0x76>
      cprintf("allocuvm out of memory\n");
8010825d:	c7 04 24 31 8d 10 80 	movl   $0x80108d31,(%esp)
80108264:	e8 38 81 ff ff       	call   801003a1 <cprintf>
      deallocuvm(pgdir, newsz, oldsz);
80108269:	8b 45 0c             	mov    0xc(%ebp),%eax
8010826c:	89 44 24 08          	mov    %eax,0x8(%esp)
80108270:	8b 45 10             	mov    0x10(%ebp),%eax
80108273:	89 44 24 04          	mov    %eax,0x4(%esp)
80108277:	8b 45 08             	mov    0x8(%ebp),%eax
8010827a:	89 04 24             	mov    %eax,(%esp)
8010827d:	e8 6b 00 00 00       	call   801082ed <deallocuvm>
      return 0;
80108282:	b8 00 00 00 00       	mov    $0x0,%eax
80108287:	eb 62                	jmp    801082eb <allocuvm+0xd8>
    }
    memset(mem, 0, PGSIZE);
80108289:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80108290:	00 
80108291:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
80108298:	00 
80108299:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010829c:	89 04 24             	mov    %eax,(%esp)
8010829f:	e8 fe cf ff ff       	call   801052a2 <memset>
    mappages(pgdir, (char*)a, PGSIZE, v2p(mem), PTE_W|PTE_U);
801082a4:	8b 45 f0             	mov    -0x10(%ebp),%eax
801082a7:	89 04 24             	mov    %eax,(%esp)
801082aa:	e8 d8 f5 ff ff       	call   80107887 <v2p>
801082af:	8b 55 f4             	mov    -0xc(%ebp),%edx
801082b2:	c7 44 24 10 06 00 00 	movl   $0x6,0x10(%esp)
801082b9:	00 
801082ba:	89 44 24 0c          	mov    %eax,0xc(%esp)
801082be:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
801082c5:	00 
801082c6:	89 54 24 04          	mov    %edx,0x4(%esp)
801082ca:	8b 45 08             	mov    0x8(%ebp),%eax
801082cd:	89 04 24             	mov    %eax,(%esp)
801082d0:	e8 d8 fa ff ff       	call   80107dad <mappages>
    return 0;
  if(newsz < oldsz)
    return oldsz;

  a = PGROUNDUP(oldsz);
  for(; a < newsz; a += PGSIZE){
801082d5:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
801082dc:	8b 45 f4             	mov    -0xc(%ebp),%eax
801082df:	3b 45 10             	cmp    0x10(%ebp),%eax
801082e2:	0f 82 67 ff ff ff    	jb     8010824f <allocuvm+0x3c>
      return 0;
    }
    memset(mem, 0, PGSIZE);
    mappages(pgdir, (char*)a, PGSIZE, v2p(mem), PTE_W|PTE_U);
  }
  return newsz;
801082e8:	8b 45 10             	mov    0x10(%ebp),%eax
}
801082eb:	c9                   	leave  
801082ec:	c3                   	ret    

801082ed <deallocuvm>:
// newsz.  oldsz and newsz need not be page-aligned, nor does newsz
// need to be less than oldsz.  oldsz can be larger than the actual
// process size.  Returns the new process size.
int
deallocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
801082ed:	55                   	push   %ebp
801082ee:	89 e5                	mov    %esp,%ebp
801082f0:	83 ec 28             	sub    $0x28,%esp
  pte_t *pte;
  uint a, pa;

  if(newsz >= oldsz)
801082f3:	8b 45 10             	mov    0x10(%ebp),%eax
801082f6:	3b 45 0c             	cmp    0xc(%ebp),%eax
801082f9:	72 08                	jb     80108303 <deallocuvm+0x16>
    return oldsz;
801082fb:	8b 45 0c             	mov    0xc(%ebp),%eax
801082fe:	e9 a4 00 00 00       	jmp    801083a7 <deallocuvm+0xba>

  a = PGROUNDUP(newsz);
80108303:	8b 45 10             	mov    0x10(%ebp),%eax
80108306:	05 ff 0f 00 00       	add    $0xfff,%eax
8010830b:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80108310:	89 45 f4             	mov    %eax,-0xc(%ebp)
  for(; a  < oldsz; a += PGSIZE){
80108313:	e9 80 00 00 00       	jmp    80108398 <deallocuvm+0xab>
    pte = walkpgdir(pgdir, (char*)a, 0);
80108318:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010831b:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80108322:	00 
80108323:	89 44 24 04          	mov    %eax,0x4(%esp)
80108327:	8b 45 08             	mov    0x8(%ebp),%eax
8010832a:	89 04 24             	mov    %eax,(%esp)
8010832d:	e8 e5 f9 ff ff       	call   80107d17 <walkpgdir>
80108332:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(!pte)
80108335:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80108339:	75 09                	jne    80108344 <deallocuvm+0x57>
      a += (NPTENTRIES - 1) * PGSIZE;
8010833b:	81 45 f4 00 f0 3f 00 	addl   $0x3ff000,-0xc(%ebp)
80108342:	eb 4d                	jmp    80108391 <deallocuvm+0xa4>
    else if((*pte & PTE_P) != 0){
80108344:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108347:	8b 00                	mov    (%eax),%eax
80108349:	83 e0 01             	and    $0x1,%eax
8010834c:	84 c0                	test   %al,%al
8010834e:	74 41                	je     80108391 <deallocuvm+0xa4>
      pa = PTE_ADDR(*pte);
80108350:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108353:	8b 00                	mov    (%eax),%eax
80108355:	25 00 f0 ff ff       	and    $0xfffff000,%eax
8010835a:	89 45 ec             	mov    %eax,-0x14(%ebp)
      if(pa == 0)
8010835d:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
80108361:	75 0c                	jne    8010836f <deallocuvm+0x82>
        panic("kfree");
80108363:	c7 04 24 49 8d 10 80 	movl   $0x80108d49,(%esp)
8010836a:	e8 ce 81 ff ff       	call   8010053d <panic>
      char *v = p2v(pa);
8010836f:	8b 45 ec             	mov    -0x14(%ebp),%eax
80108372:	89 04 24             	mov    %eax,(%esp)
80108375:	e8 1a f5 ff ff       	call   80107894 <p2v>
8010837a:	89 45 e8             	mov    %eax,-0x18(%ebp)
      kfree(v);
8010837d:	8b 45 e8             	mov    -0x18(%ebp),%eax
80108380:	89 04 24             	mov    %eax,(%esp)
80108383:	e8 d6 a6 ff ff       	call   80102a5e <kfree>
      *pte = 0;
80108388:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010838b:	c7 00 00 00 00 00    	movl   $0x0,(%eax)

  if(newsz >= oldsz)
    return oldsz;

  a = PGROUNDUP(newsz);
  for(; a  < oldsz; a += PGSIZE){
80108391:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
80108398:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010839b:	3b 45 0c             	cmp    0xc(%ebp),%eax
8010839e:	0f 82 74 ff ff ff    	jb     80108318 <deallocuvm+0x2b>
      char *v = p2v(pa);
      kfree(v);
      *pte = 0;
    }
  }
  return newsz;
801083a4:	8b 45 10             	mov    0x10(%ebp),%eax
}
801083a7:	c9                   	leave  
801083a8:	c3                   	ret    

801083a9 <freevm>:

// Free a page table and all the physical memory pages
// in the user part.
void
freevm(pde_t *pgdir)
{
801083a9:	55                   	push   %ebp
801083aa:	89 e5                	mov    %esp,%ebp
801083ac:	83 ec 28             	sub    $0x28,%esp
  uint i;

  if(pgdir == 0)
801083af:	83 7d 08 00          	cmpl   $0x0,0x8(%ebp)
801083b3:	75 0c                	jne    801083c1 <freevm+0x18>
    panic("freevm: no pgdir");
801083b5:	c7 04 24 4f 8d 10 80 	movl   $0x80108d4f,(%esp)
801083bc:	e8 7c 81 ff ff       	call   8010053d <panic>
  deallocuvm(pgdir, KERNBASE, 0);
801083c1:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
801083c8:	00 
801083c9:	c7 44 24 04 00 00 00 	movl   $0x80000000,0x4(%esp)
801083d0:	80 
801083d1:	8b 45 08             	mov    0x8(%ebp),%eax
801083d4:	89 04 24             	mov    %eax,(%esp)
801083d7:	e8 11 ff ff ff       	call   801082ed <deallocuvm>
  for(i = 0; i < NPDENTRIES; i++){
801083dc:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801083e3:	eb 3c                	jmp    80108421 <freevm+0x78>
    if(pgdir[i] & PTE_P){
801083e5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801083e8:	c1 e0 02             	shl    $0x2,%eax
801083eb:	03 45 08             	add    0x8(%ebp),%eax
801083ee:	8b 00                	mov    (%eax),%eax
801083f0:	83 e0 01             	and    $0x1,%eax
801083f3:	84 c0                	test   %al,%al
801083f5:	74 26                	je     8010841d <freevm+0x74>
      char * v = p2v(PTE_ADDR(pgdir[i]));
801083f7:	8b 45 f4             	mov    -0xc(%ebp),%eax
801083fa:	c1 e0 02             	shl    $0x2,%eax
801083fd:	03 45 08             	add    0x8(%ebp),%eax
80108400:	8b 00                	mov    (%eax),%eax
80108402:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80108407:	89 04 24             	mov    %eax,(%esp)
8010840a:	e8 85 f4 ff ff       	call   80107894 <p2v>
8010840f:	89 45 f0             	mov    %eax,-0x10(%ebp)
      kfree(v);
80108412:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108415:	89 04 24             	mov    %eax,(%esp)
80108418:	e8 41 a6 ff ff       	call   80102a5e <kfree>
  uint i;

  if(pgdir == 0)
    panic("freevm: no pgdir");
  deallocuvm(pgdir, KERNBASE, 0);
  for(i = 0; i < NPDENTRIES; i++){
8010841d:	83 45 f4 01          	addl   $0x1,-0xc(%ebp)
80108421:	81 7d f4 ff 03 00 00 	cmpl   $0x3ff,-0xc(%ebp)
80108428:	76 bb                	jbe    801083e5 <freevm+0x3c>
    if(pgdir[i] & PTE_P){
      char * v = p2v(PTE_ADDR(pgdir[i]));
      kfree(v);
    }
  }
  kfree((char*)pgdir);
8010842a:	8b 45 08             	mov    0x8(%ebp),%eax
8010842d:	89 04 24             	mov    %eax,(%esp)
80108430:	e8 29 a6 ff ff       	call   80102a5e <kfree>
}
80108435:	c9                   	leave  
80108436:	c3                   	ret    

80108437 <clearpteu>:

// Clear PTE_U on a page. Used to create an inaccessible
// page beneath the user stack.
void
clearpteu(pde_t *pgdir, char *uva)
{
80108437:	55                   	push   %ebp
80108438:	89 e5                	mov    %esp,%ebp
8010843a:	83 ec 28             	sub    $0x28,%esp
  pte_t *pte;

  pte = walkpgdir(pgdir, uva, 0);
8010843d:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
80108444:	00 
80108445:	8b 45 0c             	mov    0xc(%ebp),%eax
80108448:	89 44 24 04          	mov    %eax,0x4(%esp)
8010844c:	8b 45 08             	mov    0x8(%ebp),%eax
8010844f:	89 04 24             	mov    %eax,(%esp)
80108452:	e8 c0 f8 ff ff       	call   80107d17 <walkpgdir>
80108457:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if(pte == 0)
8010845a:	83 7d f4 00          	cmpl   $0x0,-0xc(%ebp)
8010845e:	75 0c                	jne    8010846c <clearpteu+0x35>
    panic("clearpteu");
80108460:	c7 04 24 60 8d 10 80 	movl   $0x80108d60,(%esp)
80108467:	e8 d1 80 ff ff       	call   8010053d <panic>
  *pte &= ~PTE_U;
8010846c:	8b 45 f4             	mov    -0xc(%ebp),%eax
8010846f:	8b 00                	mov    (%eax),%eax
80108471:	89 c2                	mov    %eax,%edx
80108473:	83 e2 fb             	and    $0xfffffffb,%edx
80108476:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108479:	89 10                	mov    %edx,(%eax)
}
8010847b:	c9                   	leave  
8010847c:	c3                   	ret    

8010847d <copyuvm>:

// Given a parent process's page table, create a copy
// of it for a child.
pde_t*
copyuvm(pde_t *pgdir, uint sz)
{
8010847d:	55                   	push   %ebp
8010847e:	89 e5                	mov    %esp,%ebp
80108480:	53                   	push   %ebx
80108481:	83 ec 44             	sub    $0x44,%esp
  pde_t *d;
  pte_t *pte;
  uint pa, i, flags;
  char *mem;

  if((d = setupkvm()) == 0)
80108484:	e8 b8 f9 ff ff       	call   80107e41 <setupkvm>
80108489:	89 45 f0             	mov    %eax,-0x10(%ebp)
8010848c:	83 7d f0 00          	cmpl   $0x0,-0x10(%ebp)
80108490:	75 0a                	jne    8010849c <copyuvm+0x1f>
    return 0;
80108492:	b8 00 00 00 00       	mov    $0x0,%eax
80108497:	e9 fd 00 00 00       	jmp    80108599 <copyuvm+0x11c>
  for(i = 0; i < sz; i += PGSIZE){
8010849c:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%ebp)
801084a3:	e9 cc 00 00 00       	jmp    80108574 <copyuvm+0xf7>
    if((pte = walkpgdir(pgdir, (void *) i, 0)) == 0)
801084a8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801084ab:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
801084b2:	00 
801084b3:	89 44 24 04          	mov    %eax,0x4(%esp)
801084b7:	8b 45 08             	mov    0x8(%ebp),%eax
801084ba:	89 04 24             	mov    %eax,(%esp)
801084bd:	e8 55 f8 ff ff       	call   80107d17 <walkpgdir>
801084c2:	89 45 ec             	mov    %eax,-0x14(%ebp)
801084c5:	83 7d ec 00          	cmpl   $0x0,-0x14(%ebp)
801084c9:	75 0c                	jne    801084d7 <copyuvm+0x5a>
      panic("copyuvm: pte should exist");
801084cb:	c7 04 24 6a 8d 10 80 	movl   $0x80108d6a,(%esp)
801084d2:	e8 66 80 ff ff       	call   8010053d <panic>
    if(!(*pte & PTE_P))
801084d7:	8b 45 ec             	mov    -0x14(%ebp),%eax
801084da:	8b 00                	mov    (%eax),%eax
801084dc:	83 e0 01             	and    $0x1,%eax
801084df:	85 c0                	test   %eax,%eax
801084e1:	75 0c                	jne    801084ef <copyuvm+0x72>
      panic("copyuvm: page not present");
801084e3:	c7 04 24 84 8d 10 80 	movl   $0x80108d84,(%esp)
801084ea:	e8 4e 80 ff ff       	call   8010053d <panic>
    pa = PTE_ADDR(*pte);
801084ef:	8b 45 ec             	mov    -0x14(%ebp),%eax
801084f2:	8b 00                	mov    (%eax),%eax
801084f4:	25 00 f0 ff ff       	and    $0xfffff000,%eax
801084f9:	89 45 e8             	mov    %eax,-0x18(%ebp)
    flags = PTE_FLAGS(*pte);
801084fc:	8b 45 ec             	mov    -0x14(%ebp),%eax
801084ff:	8b 00                	mov    (%eax),%eax
80108501:	25 ff 0f 00 00       	and    $0xfff,%eax
80108506:	89 45 e4             	mov    %eax,-0x1c(%ebp)
    if((mem = kalloc()) == 0)
80108509:	e8 e9 a5 ff ff       	call   80102af7 <kalloc>
8010850e:	89 45 e0             	mov    %eax,-0x20(%ebp)
80108511:	83 7d e0 00          	cmpl   $0x0,-0x20(%ebp)
80108515:	74 6e                	je     80108585 <copyuvm+0x108>
      goto bad;
    memmove(mem, (char*)p2v(pa), PGSIZE);
80108517:	8b 45 e8             	mov    -0x18(%ebp),%eax
8010851a:	89 04 24             	mov    %eax,(%esp)
8010851d:	e8 72 f3 ff ff       	call   80107894 <p2v>
80108522:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80108529:	00 
8010852a:	89 44 24 04          	mov    %eax,0x4(%esp)
8010852e:	8b 45 e0             	mov    -0x20(%ebp),%eax
80108531:	89 04 24             	mov    %eax,(%esp)
80108534:	e8 3c ce ff ff       	call   80105375 <memmove>
    if(mappages(d, (void*)i, PGSIZE, v2p(mem), flags) < 0)
80108539:	8b 5d e4             	mov    -0x1c(%ebp),%ebx
8010853c:	8b 45 e0             	mov    -0x20(%ebp),%eax
8010853f:	89 04 24             	mov    %eax,(%esp)
80108542:	e8 40 f3 ff ff       	call   80107887 <v2p>
80108547:	8b 55 f4             	mov    -0xc(%ebp),%edx
8010854a:	89 5c 24 10          	mov    %ebx,0x10(%esp)
8010854e:	89 44 24 0c          	mov    %eax,0xc(%esp)
80108552:	c7 44 24 08 00 10 00 	movl   $0x1000,0x8(%esp)
80108559:	00 
8010855a:	89 54 24 04          	mov    %edx,0x4(%esp)
8010855e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108561:	89 04 24             	mov    %eax,(%esp)
80108564:	e8 44 f8 ff ff       	call   80107dad <mappages>
80108569:	85 c0                	test   %eax,%eax
8010856b:	78 1b                	js     80108588 <copyuvm+0x10b>
  uint pa, i, flags;
  char *mem;

  if((d = setupkvm()) == 0)
    return 0;
  for(i = 0; i < sz; i += PGSIZE){
8010856d:	81 45 f4 00 10 00 00 	addl   $0x1000,-0xc(%ebp)
80108574:	8b 45 f4             	mov    -0xc(%ebp),%eax
80108577:	3b 45 0c             	cmp    0xc(%ebp),%eax
8010857a:	0f 82 28 ff ff ff    	jb     801084a8 <copyuvm+0x2b>
      goto bad;
    memmove(mem, (char*)p2v(pa), PGSIZE);
    if(mappages(d, (void*)i, PGSIZE, v2p(mem), flags) < 0)
      goto bad;
  }
  return d;
80108580:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108583:	eb 14                	jmp    80108599 <copyuvm+0x11c>
    if(!(*pte & PTE_P))
      panic("copyuvm: page not present");
    pa = PTE_ADDR(*pte);
    flags = PTE_FLAGS(*pte);
    if((mem = kalloc()) == 0)
      goto bad;
80108585:	90                   	nop
80108586:	eb 01                	jmp    80108589 <copyuvm+0x10c>
    memmove(mem, (char*)p2v(pa), PGSIZE);
    if(mappages(d, (void*)i, PGSIZE, v2p(mem), flags) < 0)
      goto bad;
80108588:	90                   	nop
  }
  return d;

bad:
  freevm(d);
80108589:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010858c:	89 04 24             	mov    %eax,(%esp)
8010858f:	e8 15 fe ff ff       	call   801083a9 <freevm>
  return 0;
80108594:	b8 00 00 00 00       	mov    $0x0,%eax
}
80108599:	83 c4 44             	add    $0x44,%esp
8010859c:	5b                   	pop    %ebx
8010859d:	5d                   	pop    %ebp
8010859e:	c3                   	ret    

8010859f <uva2ka>:

//PAGEBREAK!
// Map user virtual address to kernel address.
char*
uva2ka(pde_t *pgdir, char *uva)
{
8010859f:	55                   	push   %ebp
801085a0:	89 e5                	mov    %esp,%ebp
801085a2:	83 ec 28             	sub    $0x28,%esp
  pte_t *pte;

  pte = walkpgdir(pgdir, uva, 0);
801085a5:	c7 44 24 08 00 00 00 	movl   $0x0,0x8(%esp)
801085ac:	00 
801085ad:	8b 45 0c             	mov    0xc(%ebp),%eax
801085b0:	89 44 24 04          	mov    %eax,0x4(%esp)
801085b4:	8b 45 08             	mov    0x8(%ebp),%eax
801085b7:	89 04 24             	mov    %eax,(%esp)
801085ba:	e8 58 f7 ff ff       	call   80107d17 <walkpgdir>
801085bf:	89 45 f4             	mov    %eax,-0xc(%ebp)
  if((*pte & PTE_P) == 0)
801085c2:	8b 45 f4             	mov    -0xc(%ebp),%eax
801085c5:	8b 00                	mov    (%eax),%eax
801085c7:	83 e0 01             	and    $0x1,%eax
801085ca:	85 c0                	test   %eax,%eax
801085cc:	75 07                	jne    801085d5 <uva2ka+0x36>
    return 0;
801085ce:	b8 00 00 00 00       	mov    $0x0,%eax
801085d3:	eb 25                	jmp    801085fa <uva2ka+0x5b>
  if((*pte & PTE_U) == 0)
801085d5:	8b 45 f4             	mov    -0xc(%ebp),%eax
801085d8:	8b 00                	mov    (%eax),%eax
801085da:	83 e0 04             	and    $0x4,%eax
801085dd:	85 c0                	test   %eax,%eax
801085df:	75 07                	jne    801085e8 <uva2ka+0x49>
    return 0;
801085e1:	b8 00 00 00 00       	mov    $0x0,%eax
801085e6:	eb 12                	jmp    801085fa <uva2ka+0x5b>
  return (char*)p2v(PTE_ADDR(*pte));
801085e8:	8b 45 f4             	mov    -0xc(%ebp),%eax
801085eb:	8b 00                	mov    (%eax),%eax
801085ed:	25 00 f0 ff ff       	and    $0xfffff000,%eax
801085f2:	89 04 24             	mov    %eax,(%esp)
801085f5:	e8 9a f2 ff ff       	call   80107894 <p2v>
}
801085fa:	c9                   	leave  
801085fb:	c3                   	ret    

801085fc <copyout>:
// Copy len bytes from p to user address va in page table pgdir.
// Most useful when pgdir is not the current page table.
// uva2ka ensures this only works for PTE_U pages.
int
copyout(pde_t *pgdir, uint va, void *p, uint len)
{
801085fc:	55                   	push   %ebp
801085fd:	89 e5                	mov    %esp,%ebp
801085ff:	83 ec 28             	sub    $0x28,%esp
  char *buf, *pa0;
  uint n, va0;

  buf = (char*)p;
80108602:	8b 45 10             	mov    0x10(%ebp),%eax
80108605:	89 45 f4             	mov    %eax,-0xc(%ebp)
  while(len > 0){
80108608:	e9 8b 00 00 00       	jmp    80108698 <copyout+0x9c>
    va0 = (uint)PGROUNDDOWN(va);
8010860d:	8b 45 0c             	mov    0xc(%ebp),%eax
80108610:	25 00 f0 ff ff       	and    $0xfffff000,%eax
80108615:	89 45 ec             	mov    %eax,-0x14(%ebp)
    pa0 = uva2ka(pgdir, (char*)va0);
80108618:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010861b:	89 44 24 04          	mov    %eax,0x4(%esp)
8010861f:	8b 45 08             	mov    0x8(%ebp),%eax
80108622:	89 04 24             	mov    %eax,(%esp)
80108625:	e8 75 ff ff ff       	call   8010859f <uva2ka>
8010862a:	89 45 e8             	mov    %eax,-0x18(%ebp)
    if(pa0 == 0)
8010862d:	83 7d e8 00          	cmpl   $0x0,-0x18(%ebp)
80108631:	75 07                	jne    8010863a <copyout+0x3e>
      return -1;
80108633:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
80108638:	eb 6d                	jmp    801086a7 <copyout+0xab>
    n = PGSIZE - (va - va0);
8010863a:	8b 45 0c             	mov    0xc(%ebp),%eax
8010863d:	8b 55 ec             	mov    -0x14(%ebp),%edx
80108640:	89 d1                	mov    %edx,%ecx
80108642:	29 c1                	sub    %eax,%ecx
80108644:	89 c8                	mov    %ecx,%eax
80108646:	05 00 10 00 00       	add    $0x1000,%eax
8010864b:	89 45 f0             	mov    %eax,-0x10(%ebp)
    if(n > len)
8010864e:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108651:	3b 45 14             	cmp    0x14(%ebp),%eax
80108654:	76 06                	jbe    8010865c <copyout+0x60>
      n = len;
80108656:	8b 45 14             	mov    0x14(%ebp),%eax
80108659:	89 45 f0             	mov    %eax,-0x10(%ebp)
    memmove(pa0 + (va - va0), buf, n);
8010865c:	8b 45 ec             	mov    -0x14(%ebp),%eax
8010865f:	8b 55 0c             	mov    0xc(%ebp),%edx
80108662:	89 d1                	mov    %edx,%ecx
80108664:	29 c1                	sub    %eax,%ecx
80108666:	89 c8                	mov    %ecx,%eax
80108668:	03 45 e8             	add    -0x18(%ebp),%eax
8010866b:	8b 55 f0             	mov    -0x10(%ebp),%edx
8010866e:	89 54 24 08          	mov    %edx,0x8(%esp)
80108672:	8b 55 f4             	mov    -0xc(%ebp),%edx
80108675:	89 54 24 04          	mov    %edx,0x4(%esp)
80108679:	89 04 24             	mov    %eax,(%esp)
8010867c:	e8 f4 cc ff ff       	call   80105375 <memmove>
    len -= n;
80108681:	8b 45 f0             	mov    -0x10(%ebp),%eax
80108684:	29 45 14             	sub    %eax,0x14(%ebp)
    buf += n;
80108687:	8b 45 f0             	mov    -0x10(%ebp),%eax
8010868a:	01 45 f4             	add    %eax,-0xc(%ebp)
    va = va0 + PGSIZE;
8010868d:	8b 45 ec             	mov    -0x14(%ebp),%eax
80108690:	05 00 10 00 00       	add    $0x1000,%eax
80108695:	89 45 0c             	mov    %eax,0xc(%ebp)
{
  char *buf, *pa0;
  uint n, va0;

  buf = (char*)p;
  while(len > 0){
80108698:	83 7d 14 00          	cmpl   $0x0,0x14(%ebp)
8010869c:	0f 85 6b ff ff ff    	jne    8010860d <copyout+0x11>
    memmove(pa0 + (va - va0), buf, n);
    len -= n;
    buf += n;
    va = va0 + PGSIZE;
  }
  return 0;
801086a2:	b8 00 00 00 00       	mov    $0x0,%eax
}
801086a7:	c9                   	leave  
801086a8:	c3                   	ret    
